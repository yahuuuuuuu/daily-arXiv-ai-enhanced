...
Method: 提出LUMA模型，包含双路径锚定：时间域的MoCLIP对比学习提供语义监督，频率域的低频DCT分量提供补充对齐信号，通过时间调制机制自适应融合。
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA基准测试可能高估了视觉语言模型的性能，特别是在处理密集场景细节时。作者认为当前模型在编码和推理细节方面仍面临挑战，需要更严格的测试来评估其真实能力。

Method: 创建了包含高分辨率公共领域绘画扫描图像的VQA数据集，这些图像具有密集的人物、动作和复杂背景。手动标注了六个任务类别的问题，专门测试模型在视觉过载场景中的理解能力。

Result: 测试了37个模型，表现最好的模型(o3)在最难测试集上仅达到19.6%准确率，所有问题上的总体准确率为69.5%。错误分析揭示了模型在计数、OCR和复杂任务逻辑一致性方面的多种失败模式。
...
<details>
  <summary>Details</summary>
Motivation: 基于摄像头的生理信号估计虽然提供了非接触式监测心率的便利方式，但面部视频中的生命信号会引发严重的隐私问题，可能泄露与个人健康和情绪状态相关的敏感信息。

Method: 使用预训练的3D变分自编码器将输入视频编码到潜在空间，通过冻结的文本编码器嵌入目标心率提示，使用带有自适应层归一化的可训练时空层进行融合，并在解码器中应用特征级线性调制和微调输出层以避免生理信号在重建过程中退化。

Result: 在选定数据集上，方法保持了视觉质量（平均PSNR为38.96 dB，SSIM为0.98），同时使用最先进的rPPG估计器实现了平均心率调制误差为10.00 bpm MAE和10.09% MAPE。

Conclusion: 该设计的可控心率编辑功能可用于匿名化真实视频中的生物特征信号或合成具有所需生命体征的逼真视频。
...
Method: 设计了围绕视角转换核心挑战的基准，包含逐步结构化的诊断类别，从简单的单物体任务到复杂的多物体视角转换场景。评估了37个最先进的专有和开源视觉语言模型。

Result: 评估结果显示VLMs存在系统性弱点，人类受试者准确率达到91.2%，而VLMs的表现与人类反应时间强相关。缩放分析显示平滑改进和新兴能力。

Conclusion: SpinBench为理解VLMs的空间推理能力提供了关键见解，突出了它们在物理空间推理方面的关键差距，并捕捉了人类和VLMs共享的空间推理挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法如ConvLSTM难以建模长期依赖关系，且现有研究局限于单模态数据范式，无法充分利用多源信息。

Method: 开发了联合时空注意力机制，统一处理多模态特征，实现深度多模态融合。

Result: 在EGMS数据集上达到新的最优性能，长期预测RMSE比基线方法降低了一个数量级。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的VLMs（如GPT-5）在从2D输入理解3D方面仍有困难，而专门的视觉模型在度量深度估计等3D理解任务上已达到超人类精度。这促使研究者探索VLMs是否能在不改变架构或损失函数的情况下达到专家级精度。

Method: 采用基于文本的监督微调与稀疏标签，通过视觉提示和内在条件增强解决像素参考和跨数据集相机模糊问题，无需密集预测头或复杂的回归/正则化损失。

Result: DepthLM方法使用更小的模型，在精度上超过了最先进的VLMs超过2倍，首次使VLMs与纯视觉模型相媲美。有趣的是，VLMs自然地避免了过度平滑，在边界区域比纯视觉模型有更少的飞点。

Conclusion: DepthLM的简单性使得单个VLM能够覆盖度量深度之外的各种3D任务，证明了VLMs在3D理解方面的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 泛北极高分辨率海冰制图及其可靠的不确定性估计对业务化海冰浓度制图至关重要，但由于冰特征细微、模型不确定性和数据异质性等挑战而难以实现。

Method: 1) 设计具有全局和局部模块的高分辨率Transformer模型以更好识别海冰模式细微差异；2) 提出贝叶斯扩展，将模型参数视为随机变量以更有效捕捉不确定性；3) 在决策级融合三种不同类型数据（Sentinel-1、RCM和AMSR2）。

Result: 在2021年9月的泛北极数据集上测试，结果表明所提模型相比其他不确定性量化方法，能够同时实现高分辨率海冰浓度制图和稳健的不确定性制图。

Conclusion: 该方法成功解决了海冰制图中的关键挑战，为泛北极海冰监测提供了可靠的高分辨率制图和不确定性量化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于基础设施的感知系统在复杂道路环境（如施工区）中面临的透视畸变、几何复杂、遮挡和成本等实际部署障碍，提高高风险路段的安全性。

Method: 集成路边摄像头和LiDAR传感器到协同仿真环境，开发可扩展的车辆检测和定位框架，采用基于卡尔曼滤波的后期融合策略来增强轨迹一致性和准确性。

Result: 仿真中融合算法相比单个传感器将纵向误差减少达70%，同时保持横向精度在1-3米内；现场验证显示融合轨迹即使在单传感器数据间歇或降级时也能紧密匹配真实车辆路径。

Conclusion: 基于KF的传感器融合能可靠补偿单个传感器的局限性，提供精确鲁棒的车辆跟踪能力，为在复杂交通环境中部署基础设施支持的多传感器系统提供了实用途径。
...
Method: 提出"先观察后推理"范式：首先训练MLLMs感知伪造痕迹，增强其伪迹感知的视觉能力；开发Forensic-Chat系统，保持对话能力的同时实现可泛化、可解释的假图像检测；构建ExplainFake-Bench基准评估五个关键方面的可解释性。

Result: 大量实验表明该方法在泛化性和真实可靠的可解释性方面具有优越性。

Conclusion: 通过强化MLLMs的伪迹感知视觉能力，使其推理基于实际观察，能有效提升AI生成图像检测的性能和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 随着深度伪造技术的发展，恶意行为者开始在视频会议中进行实时网络钓鱼攻击。由于深度伪造无法模仿真实人类对话中的微妙非语言视线交流，这为检测提供了机会。

Method: 基于对二元对话中视线模式研究的可解释特征构建模型，利用恶意行为者视频流中的估计视线来检测深度伪造。

Result: 在自建数据集上测试，准确率达到82%。

Conclusion: 这是首个利用视线点追踪进行深度伪造检测的方法，为实时深度伪造检测提供了新思路。
...
Method: 使用自定义渲染器和专门为边缘域设计的新模板匹配度量，仅需低精度、无纹理的3D模型作为输入。
...
Method: 提出LLM-RG混合管道，结合现成的视觉语言模型进行细粒度属性提取和大语言模型进行符号推理。流程包括：使用LLM提取相关对象类型和属性、检测候选区域、用VLM生成丰富视觉描述符，然后将这些描述符与空间元数据结合成自然语言提示，输入LLM进行思维链推理以识别指称对象的边界框。
...
Result: 在LLaVA-1.5-7B和IDEFICS2-8B等模型上，VISOR++图像在拒绝、奉承和生存本能三个对齐方向上达到与转向向量相当的性能，并在未见模型上实现行为转向，同时保持99.9%的MMLU任务性能。
...
Method: 提出战略自玩框架，让模型在"谁是卧底"式游戏中扮演不同角色进行推理；使用任意图像生成游戏；引入迭代自玩策略优化算法，结合自玩和带验证奖励的强化学习。

Result: 在推理、图表问答和视觉理解任务上达到最先进性能，超越基于标注的方法，且无需标签数据。

Conclusion: Vision-Zero证明了视觉语言模型可以通过游戏化自玩实现持续自我改进，为降低训练成本和提升模型泛化能力提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 脉络膜痣有转化为黑色素瘤的风险，早期检测至关重要。现有AI方法在眼底图像分割中面临挑战，特别是数据集分辨率低、标注不一致的问题，限制了分割模型的有效性。

Method: 提出新颖的混合方法，将数学/聚类分割模型与U-Net的见解相结合，充分利用两种方法的优势，提高准确性并减少对大规模训练数据的需求。

Result: 在1024*1024眼底图像上，混合模型达到89.7%的Dice系数和80.01%的IoU，显著优于Attention U-Net的51.3%和34.2%。在外部数据集上也表现出更好的泛化能力。

Conclusion: 这项工作为开发脉络膜痣诊断决策支持系统奠定了基础，有望通过自动病变标注提高诊断和监测的速度与准确性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在海洋生物学等专业科学领域的应用能力尚未充分探索，特别是在监测受人为压力影响的海洋生态系统时，鱼类物种的精确识别至关重要。

Method: 通过引入FishNet++大规模多模态基准，扩展了现有资源，包括35,133个文本描述用于多模态学习、706,426个关键点标注用于形态学研究以及119,399个边界框用于检测。

Result: 评估显示最先进的开源模型在鱼类物种细粒度识别任务上准确率低于10%，表明现有模型在专业领域存在显著局限性。

Conclusion: FishNet++基准为开发能够推进水生科学的专业视觉语言模型提供了全面的评估工具，有助于解决多模态模型在专业科学领域的知识缺失问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉图神经网络(ViGs)在节点-邻居特征聚合方法上存在局限性，需要一种能够有效捕捉复杂节点-邻居关系且无需架构特定优化的通用聚合方法。

Method: 提出交叉注意力聚合方法，其中查询投影来自节点，键投影来自其邻居，并构建了使用该聚合方案进行非局部消息传递的AttentionViG架构。

Result: 在ImageNet-1K基准测试中达到SOTA性能，在MS COCO 2017的目标检测和实例分割以及ADE20K的语义分割等下游任务中表现出良好的迁移能力。

Conclusion: 所提出的方法不仅实现了强大的性能，而且保持了效率，在计算量相当的情况下提供了与先前视觉GNN架构相竞争的准确率。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分析中标注数据稀缺，需要少样本学习方法。但现有研究主要关注标准少样本学习，而医学应用如胸部X光分类需要同时学习新类别并利用已知类别知识，这更符合广义少样本分类场景。

Method: 构建了包含479,215张胸部X光的大规模数据集MetaChest，设计了专门用于少样本分类的元集划分和多标签情节生成算法。评估了标准迁移学习方法和ProtoNet扩展方法。

Result: 增加每情节的类别数和每类的训练样本数能提高分类性能；迁移学习方法持续优于ProtoNet扩展；高分辨率图像提高精度但增加计算成本；高效模型架构能以更少资源达到可比性能。

Conclusion: 在医学图像少样本多标签分类中，简单的迁移学习方法比专门设计的少样本学习方法更有效，且模型效率和图像分辨率之间存在权衡关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割模型通常针对单一任务、模态或器官进行训练，与临床实践中专家整合多种知识的现实形成鲜明对比。
...
<details>
  <summary>Details</summary>
Motivation: 人类感知具有主动聚焦特性，需要空间变化的分辨率细节。现有3DGS方法输出均匀分辨率导致高计算成本，无法利用原始高分辨率图像重建细节；而逐场景优化方法需要密集观测和长时间离线优化。

Method: 提出GaussianLens前向加密框架，融合初始3DGS和多视图图像的多模态信息，设计像素引导的加密机制，在大分辨率增加下有效捕捉细节。

Result: 实验证明该方法在局部精细细节重建方面表现优越，对高达1024×1024分辨率图像具有强可扩展性。

Conclusion: 该方法成功解决了高分辨率整体重建的高成本与用户对局部精细细节需求之间的差距，实现了高效的局部高分辨率重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决眼科疾病诊断中因医疗资源短缺和专科护理受限导致的及时诊断问题，同时填补眼科领域缺乏适合评估生成模型综合基准数据集的空白。

Method: 构建包含多粒度标注的大规模多模态眼科数据集，支持解剖结构识别、疾病筛查、疾病分期和人口统计学预测等任务，并系统评估24种最先进的多模态大语言模型。

Result: 最佳模型在零样本设置下疾病筛查准确率约58%，但在疾病分期等挑战性任务上表现仍不理想。

Conclusion: 该数据集、处理流程和排行榜的公开发布有望推动眼科AI应用发展，减轻全球视力威胁疾病的负担。
...
Method: AFGeo直接为每个像素预测到真实框的四个方向偏移量（左、右、上、下），使用高斯位置编码建模查询图像中的点击点不确定性，并引入跨视角物体关联模块来关联不同视角下的同一物体及其周围环境。

Result: 该方法在基准数据集上实现了最先进的性能，模型轻量且计算效率高。

Conclusion: AFGeo通过无锚框定位范式，结合高斯位置编码和跨视角物体关联模块，有效解决了跨视角物体地理定位问题，在保持轻量化的同时达到了优越的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨模态检索模型在处理融合图像-文本模态的检索键时性能下降的问题，现有方法需要精心构建新数据集且无法泛化到未见过的模态组合。

Method: 提出GCL损失函数，利用现有图像-字幕配对数据集，在小批量中跨所有模态进行对比学习，学习统一表示空间。

Result: 在M-BEIR、MMEB和CoVR基准测试中，对现有多模态检索模型(VISTA、CLIP、TinyCLIP)实现了持续的性能提升。

Conclusion: GCL是一种有效的多模态检索方法，无需额外数据集构建就能提升性能，具有良好的泛化能力。
...
Method: 创建三个包含真实和合成卡车的数据库，使用三种YOLO架构进行训练和测试，通过召回率、精确率、F1分数和mAP评估结果，并用Mann-Whitney U检验统计显著性。
...
Conclusion: Geo-DLC任务和DescribeEarth模型为遥感图像细粒度描述提供了有效解决方案，显著提升了对象级解释能力，在简单、复杂甚至分布外遥感场景中都表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决工业制造中人工缺陷检测耗时耗力的问题，特别是金属板材图像数据不足导致的检测精度下降。

Method: 使用ConSinGAN生成大量金属板材图像数据增强，比较YOLOv3、v4、v7、v9四个版本模型，将最佳模型集成到制造硬件和SCADA系统中。

Result: YOLOv9模型结合ConSinGAN表现最佳，准确率达91.3%，检测时间146ms，成功建立自动化光学检测系统。

Conclusion: 提出的自动缺陷检测方法有效，可轻松应用于工业制造中的其他组件检测。
...
<details>
  <summary>Details</summary>
Motivation: AI生成图像检测和来源识别面临生成模型结构多样性的挑战，现有检测方法容易过拟合特定伪造特征，而来源识别因缺乏大规模分类数据集而受限。

Method: 提出开放集少样本来源识别新范式，引入OmniDFA框架，同时进行图像真实性评估和少样本来源识别，并构建了包含117万张图像、45个生成模型的OmniFake数据集。

Result: OmniDFA在开放集来源识别方面表现出色，在AI生成图像检测上实现了最先进的泛化性能。

Conclusion: OmniDFA框架为AI生成图像检测和来源识别提供了有效的解决方案，OmniFake数据集为相关研究提供了丰富的资源基础。
...
Conclusion: 通过主动获取信息和动态结构化推理过程，AIMCoT代表了向更鲁棒、有效和类人化多模态推理迈出的关键一步。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型会记忆训练数据，引发隐私和版权问题，但记忆发生的根本原因尚不清楚。本文旨在探究扩散模型如何记忆的机制。

Method: 通过分析扩散和去噪过程，研究潜在空间动态，包括训练损失分析、记忆提示的影响以及中间潜在变量的分解。

Result: 发现记忆不能仅用过拟合解释，记忆提示会将训练图像注入噪声预测，强制潜在轨迹收敛，初始随机性被快速抑制并被记忆内容取代。
...
<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言预训练模型未能直接考虑医学信息获取中的多对多映射关系，无法有效处理多种医学模态之间的复杂关联。

Method: 采用概率对比学习建模嵌入分布，使用InfoNCE损失和Hellinger距离整合模态间分布，引入概率合成采样损失来捕捉模态特定的均值和方差。

Result: 在13个医学数据集上的实验表明，该模型在跨模态检索、零样本和少样本分类任务中优于现有医学视觉语言预训练模型，并展示了多模态整合在预后预测中的鲁棒性。

Conclusion: ProbMED通过概率建模方法有效解决了医学多模态数据中的多对多映射问题，显著提升了模态间和模态内的绑定效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖过度简化的成对比较，通过基本扰动或相似性检索生成单个负图像，无法捕捉多模态偏好的复杂性，导致优化偏差和幻觉。

Method: 在CLIP空间中嵌入提示和候选图像，应用稀疏自编码器揭示语义偏差为可解释因子。基于重建难度、与正样本的语义偏差和相互多样性选择负样本。采用Plackett-Luce目标函数和重要性采样策略。

Result: 在五个不同基准测试上的实验表明，MISP-DPO在多项指标上持续优于先前方法，验证了语义感知多负采样在基于偏好的学习中的有效性。

Conclusion: 语义感知的多负采样能够提供更广泛和更有信息的监督，显著改善多模态对齐性能。
...
Method: 提出轻量级Soft Probing模块学习补丁描述符的残差权重，构建在线地理-视觉图融合地理邻近性和视觉相似性，使用贪婪加权团扩展采样器聚焦信息量最大的地点邻域。

Result: 在8个基准测试中达到SOTA性能，在SPED、Pitts30k-test、MSLS-val和Nordland上分别获得98.9%、95.8%、94.5%和96.0%的Recall@1，在SPED上仅使用4096D全局描述符就获得100%的Recall@10。

Conclusion: SAGE通过动态整合空间和视觉信息，显著提升了视觉地点识别的性能，证明了空间-视觉联合优化的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态人脸编辑方法在精确属性控制和身份保持方面存在困难，特别是当条件地标与源图像差异较大时，刚性几何约束会降低身份保持效果。

Method: 提出LaTo框架，包括：1) 地标标记器将原始地标坐标量化为离散面部标记；2) 位置映射位置编码整合面部和图像标记；3) 利用视觉语言模型推断目标地标的预测器。

Result: 在超过15万真实人脸对的大规模基准测试中，LaTo在身份保持方面比现有方法提升7.8%，在语义一致性方面提升4.6%。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂现实场景下视频目标分割的挑战，包括目标长期消失和重现、小目标和不显眼目标的处理
...
<details>
  <summary>Details</summary>
Motivation: 从单张RGB图像恢复3D人体网格存在固有的模糊性，现有方法要么回归单一确定性输出，要么在概率方法中面临准确性与样本多样性的权衡问题。

Method: 使用SO(3)扩散模型生成3D旋转参数分布，通过条件dropout实现无条件和图像条件生成，利用时间无关的transformer提取关节潜在向量，小型MLP去噪模型学习条件分布。

Result: 实验证明该方法能有效预测准确的姿态概率分布，解决了现有概率方法的局限性。

Conclusion: 提出的SO(3)扩散模型框架能够有效建模与2D观测对齐的3D人体姿态分布，在准确性和多样性方面取得良好平衡。
...
Conclusion: GeoDrag通过几何引导的拖拽编辑，实现了精确、一致且结构保持的图像编辑，特别适用于几何密集型操作。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于体素的室内场景重建方法严重依赖多视图像素反投影射线交点作为几何约束，导致重建质量对输入视图密度敏感，在重叠区域和未观测区域表现不佳。
...
Conclusion: 利用视图内空间信息为实际应用中的视图受限场景提供了鲁棒的解决方案。
...
Method: 使用624个标注的YouTube短视频，测试了转录文本(T)、音频(A)、视频(V)三种模态的七种组合（T、A、V、TA、TV、AV、TAV），评估了五个主题：主要推荐、情感分析、视频目的、视觉分析和金融实体识别。

Result: 视频单独使用在五个主题中的四个表现强劲，强调了其在捕捉视觉上下文和有效线索（如情感、手势和肢体语言）方面的价值。选择性双模态组合如TV或AV通常优于全模态TAV，表明过多模态可能引入噪声。

Conclusion: 这些结果为金融短视频字幕生成建立了首个基准，并说明了在该领域基于复杂视觉线索的潜力和挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决超声成像在AI集成中面临的挑战，包括操作者依赖性、图像噪声和实时扫描问题，以及现有大模型在超声领域的局限性。

Method: 构建200万规模多模态数据集，采用三阶段训练策略：领域专业化预训练、指令驱动对齐和基于强化的精炼，Dolphin R1额外使用超声特定奖励的强化学习增强推理能力。

Result: Dolphin R1在U2-Bench八个超声任务中取得0.5835的U2分数，是第二名模型（0.2968）的两倍多，创下新纪录；Dolphin v1.0也表现优异，验证了统一框架的有效性。

Conclusion: 推理增强训练显著提高了诊断准确性、一致性和可解释性，对于高风险医疗AI至关重要，Dolphin系列为超声AI集成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法在保持非试穿区域时存在边界伪影问题，直接替换策略导致过渡不自然。需要一种能够渐进式保持测量一致性的方法。

Method: 将虚拟试穿重新表述为线性逆问题，采用轨迹对齐求解器，结合基于残差先验的初始化和无伪影测量引导采样（包含数据一致性、频率级校正和周期性标准去噪）。

Result: 在VITON-HD、DressCode和SHHQ-1.0数据集上的实验表明，ART-VITON能有效保持身份和背景，消除边界伪影，在视觉保真度和鲁棒性上优于现有基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于RLHF的方法依赖昂贵的配对图像偏好数据或学习奖励函数，面临可扩展性限制，需要更高效的文本-图像对齐方法。

Method: 使用大语言模型扰动原始标题构建不匹配提示，训练模型偏好匹配提示，扩展DPO和KTO算法为TDPO和TKTO。

Result: 在多个基准测试中定量和定性评估显示，该方法持续优于原始对应方法，提供更好的人类偏好分数和改进的文本-图像对齐。

Conclusion: TPO框架提供了一种无需配对图像偏好数据的有效文本-图像对齐方法，具有良好的通用性和兼容性。
...
TL;DR: 本文提出了PCPO框架来解决文本到图像模型强化学习中的训练不稳定问题，通过比例信用分配机制显著提升了收敛速度和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有策略梯度方法在文本到图像模型对齐中存在训练不稳定和高方差问题，阻碍了收敛速度和图像质量。分析发现主要原因是生成采样器的数学结构导致跨时间步的信用分配不成比例。

Method: 提出了比例信用策略优化(PCPO)框架，通过稳定的目标重构和原则性的时间步重加权来强制实施比例信用分配。

Result: PCPO显著稳定了训练过程，加速了收敛速度，提升了图像质量，并减轻了模型崩溃这一常见故障模式。在所有方面都大幅优于现有策略梯度基线方法，包括最先进的DanceGRPO。

Conclusion: PCPO通过解决不成比例信用分配问题，为文本到图像模型的强化学习对齐提供了更稳定和高效的训练框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的反演方法虽然能够忠实重建源图像，但在噪声图编辑灵活性方面存在限制，导致难以紧密遵循目标文本提示进行有效编辑。

Method: 分析了增强可编辑性的噪声图特性，引入了可编辑噪声精炼技术，通过最小化重建噪声图和编辑噪声图之间的差异来与期望编辑对齐。

Result: 大量实验表明，ENM Inversion在广泛的图像编辑任务中，在内容保持和编辑保真度方面均优于现有方法，并能轻松应用于视频编辑以实现时间一致性和跨帧内容操作。

Conclusion: ENM Inversion通过优化噪声图搜索，成功解决了文本引导图像编辑中内容保持与编辑灵活性之间的平衡问题，为图像和视频编辑提供了有效的解决方案。
...
TL;DR: EvoQuality是一个自监督框架，通过自我一致性原则让视觉语言模型在无真实标签的情况下自主提升图像质量评估能力，在多个基准测试中超越有监督方法。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型的后训练阶段需要昂贵的人工标注数据，而自监督方法在感知领域如图像质量评估中的应用尚未充分探索。

Method: 采用自我一致性原则，通过成对多数投票生成伪标签，建立相对质量共识，然后通过组相对策略优化进行迭代进化。

Result: 在7个IQA基准测试中的5个上超越了最先进的有监督VLM模型，零样本性能提升了31.8%。

Conclusion: EvoQuality证明了自监督方法在图像质量评估领域的有效性，为视觉语言模型的感知能力提升提供了新途径。
...
TL;DR: EchoingECG是一个概率性师生模型，利用不确定性感知的ECG嵌入和ECHO监督来改进基于ECG的心脏功能预测，在零样本、少样本和微调设置中优于现有ECG基础模型。


<details>
  <summary>Details</summary>
Motivation: 心电图(ECG)成本低且易于获取，而超声心动图(ECHO)需要大量医院资源但在临床心脏评估中至关重要。研究旨在利用ECG预测传统上需要更复杂模式(如ECHO)获得的关键结果，使ECG成为预测心脏功能的更易获取方法。

Method: 提出EchoingECG概率师生模型，整合概率跨模态嵌入(PCME++)和ECHO-CLIP(在ECHO-文本对上预训练的视觉语言模型)，将ECHO知识蒸馏到ECG表示中。

Result: 实验和外部验证表明，EchoingECG在基于ECG的ECHO预测的零样本、少样本和微调设置中优于最先进的ECG基础模型。方差估计增强了模型性能理解，识别了ECG中的不确定性区域。

Conclusion: EchoingECG成功地将ECHO知识蒸馏到ECG表示中，提高了ECG在心脏功能预测中的性能，并通过不确定性估计提供了对模型性能的更好理解。
...
TL;DR: 提出了Point-It-Out (PIO)基准测试，通过精确的视觉定位系统评估视觉语言模型的具身推理能力，包含三个层次：目标定位、任务驱动指向和视觉轨迹预测。
...
Method: 构建PIO基准，包含三个评估阶段：S1-目标定位、S2-任务驱动指向、S3-视觉轨迹预测，数据来自室内、厨房、驾驶和机器人操作等关键领域。

Result: 对10多个SOTA VLM的实验显示：GPT-4o等通用模型在精确视觉定位上表现不如某些开源模型；MoLMO在S1和S2表现良好但在S3（需要视觉轨迹规划）上表现不佳。

Conclusion: PIO基准揭示了VLMs在具身推理中的关键差距，特别是视觉轨迹规划能力，为未来模型开发提供了重要指导。
...
TL;DR: 该研究提出了动态相似性图适应(DSGA)模块，结合LoRA方法，在极少量数据条件下高效微调SAM模型，用于农业环境中密集小物体的精确分割。


<details>
  <summary>Details</summary>
Motivation: 农业计算机视觉任务面临训练数据有限和复杂田间条件的挑战，需要参数高效的微调方法来适应基础模型。

Method: 使用动态相似性图构建，结合可学习多项式衰减初始化权重排序机制和自适应局部特征聚合，仅需4.00M可训练参数；与LoRA结合形成互补优化框架。

Result: 在鹰嘴豆荚数据集上，DSGA+LoRA在2、4、8、10样本设置下均表现优异，相比基线SAM微调，Structure-measure提升17.31%，自适应F-measure提升62.36%。

Conclusion: 该方法在极少量数据条件下实现了有效的特征判别，在复杂田间条件下实现了准确的豆荚计数(R²=0.8987)，具有农业自动化监测的实际应用价值。
...
TL;DR: 提出Logo-VGR方法解决开放世界logo识别问题，通过对比式任务和领域特定多模态推理，在仅使用少量品牌监督的情况下实现大规模品牌识别的泛化。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM模型在领域特定场景（如产品审核）中应用不足，传统logo识别方法依赖记忆大量品牌表示，在实际场景中不实用。

Method: 将logo识别重新定义为基于比较的任务，匹配产品图像与候选logo；引入领域特定多模态推理：Logo感知基础和Logo引导的视觉基础推理。

Result: Logo-VGR在OOD设置下比强基线高出近10个点，表现出优越的泛化能力。

Conclusion: Logo-VGR通过新的多模态推理范式有效解决了开放世界logo识别问题，在领域特定场景中具有实用价值。
...
TL;DR: GeoLifeCLEF 2023竞赛旨在利用深度学习和遥感数据推进物种分布建模，通过500万植物观测数据和多种环境变量预测欧洲地区物种组成。


<details>
  <summary>Details</summary>
Motivation: 理解物种时空分布是生态学和保护的基础，通过将物种观测与地理环境预测因子结合，可以建模环境与物种关系。

Method: 使用500万植物物种观测数据，结合高分辨率遥感影像、土地覆盖、高程以及粗分辨率的气候、土壤和人类足迹变量，进行多标签分类任务。

Result: 评估了模型在2.2万个小样地中预测物种组成的能力，发现了基于单阳性标签训练的方法在多标签评估中面临的偏差问题。

Conclusion: 提出了一种结合单标签和多标签数据训练的新学习策略，有效解决了多标签分类中的偏差问题。
...
TL;DR: 提出了VELA评估指标和LongCap-Arena基准，用于评估多模态大语言模型生成的长图像描述，解决了现有指标不适用于长描述和推理速度慢的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述评估指标主要针对短描述设计，不适用于长描述评估；基于LLM的评估方法存在推理速度慢的问题。

Method: 提出VELA评估指标，采用新颖的LLM-Hybrid-as-a-Judge框架；构建LongCap-Arena基准，包含7,805张图像、对应的人类参考描述和候选描述，以及32,246个人类判断。

Result: VELA在LongCap-Arena基准上超越了现有评估指标，并达到了超人类水平的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于奖励引导的方法主要关注图像生成，而如何在图像编辑任务中既保持源图像内容又优化特定奖励仍未被充分探索。

Method: 将扩散模型的反向过程视为可控轨迹，通过迭代更新伴随状态来引导编辑过程，实现无需训练的奖励引导图像编辑。

Result: 在多种编辑任务上的实验表明，该方法显著优于现有的基于反转的无训练引导基线，在奖励最大化和源图像保真度之间取得了更好的平衡。

Conclusion: 该框架为奖励引导的图像编辑提供了一种有效的训练免费解决方案，避免了奖励作弊问题，实现了高质量的编辑效果。
...
TL;DR: 该论文发现多模态推理存在双重性：虽然能增强逻辑推理能力，但会逐渐损害感知基础，导致视觉遗忘。作者提出VAPO方法来解决这个问题，并开发了VAPO-Thinker-7B模型，在多个基准测试中取得新的最优结果。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态推理在视觉任务中取得了进展，但研究发现推理过程会导致视觉遗忘现象，即模型逐渐忽视视觉输入，影响基本视觉问题的识别能力。

Method: 提出Vision-Anchored Policy Optimization (VAPO)方法，通过明确引导推理过程朝向视觉基础轨迹来增强模型对视觉信息的依赖。

Result: VAPO-Thinker-7B模型显著增强了模型对视觉信息的依赖，在多个基准测试中取得了新的最优结果。

Conclusion: 多模态推理具有双重性，需要在增强推理能力的同时保持感知基础，VAPO方法有效解决了视觉遗忘问题。
...
TL;DR: MuSLR是首个基于形式逻辑规则的多模态符号逻辑推理基准，包含1093个实例，涵盖7个领域。当前最先进的视觉语言模型在该基准上表现不佳，最佳模型GPT-4.1仅达到46.8%。提出的LogiCAM框架通过应用形式逻辑规则显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 多模态符号逻辑推理在自动驾驶、医疗诊断等高风险应用中至关重要，其严谨的确定性推理有助于防止严重后果。需要评估当前最先进视觉语言模型在此类推理上的能力。

Method: 构建MuSLR基准，包含7个领域的1093个实例，涵盖35个原子符号逻辑和976个逻辑组合，推理深度2-9。提出LogiCAM模块化框架，将形式逻辑规则应用于多模态输入。

Result: 评估7个最先进视觉语言模型，发现它们都难以处理多模态符号推理，最佳模型GPT-4.1仅达到46.8%。LogiCAM将GPT-4.1的思维链性能提升了14.13%，在一阶逻辑等复杂逻辑上提升更大。

Conclusion: 多模态符号逻辑推理对当前视觉语言模型具有挑战性，约70%的失败源于模态间的逻辑不对齐。LogiCAM框架展示了应用形式逻辑规则的有效性，为未来改进提供了关键见解。
...
<details>
  <summary>Details</summary>
Motivation: 工业异常检测越来越依赖基础模型，但现有研究主要关注文本提示调优，视觉处理部分分散且针对特定模型，缺乏统一框架

Method: 构建了视觉提示技术，包括对齐模块和前景掩码，专注于补丁相似性分析，无需训练即可适配多种基础模型

Result: 实验显示在少样本和批量零样本场景下优于先前工作，即使不使用文本特征也能取得优异性能

Conclusion: 统一的纯补丁框架可以实现快速、轻量校准的部署，无需精心设计的文本提示，为实际视觉检测提供了实用的基础模型选择和配置指导
...
Method: 使用多相机配置与机械LiDAR结合，通过初始校准确定相机内参，自动计算LiDAR与相机间的几何变换，无需专用标定目标，并在融合前进行色彩校正确保一致性。

Result: 使用Velodyne Puck Hi-Res LiDAR和四相机配置测试，优化后的软件实现实时性能，在极低光照下仍能可靠着色，成功恢复原本无法检测的场景细节。
...
TL;DR: MAPLE是一个用于少样本全切片图像分类的分层框架，通过整合多尺度视觉语义和实体级预测来改进病理诊断。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖切片级提示，无法捕捉对癌症诊断至关重要的组织学实体的亚型特异性表型变异。
...
Conclusion: MAPLE通过多尺度属性增强提示学习，能够有效捕捉组织学实体的细粒度特征，提高少样本病理诊断的准确性。
...
TL;DR: DeepSketcher提出了一个包含数据集和模型的完整套件，用于推进视觉语言模型的"图像思维"范式，通过直接在视觉嵌入空间生成中间视觉表示来实现免工具、更灵活的多模态推理。


<details>
  <summary>Details</summary>
Motivation: 现有的"图像思维"范式在数据构建准确性、结构设计和应用场景方面仍有很大探索空间，需要进一步推进多模态推理能力的发展。

Method: 构建了包含31k条思维链推理轨迹的数据集，并设计了直接在视觉嵌入空间生成"视觉思维"的模型，避免了调用外部工具和重复编码生成图像。

Result: 在多模态推理基准测试中表现出色，验证了数据集的有效性和模型设计的优越性。

Conclusion: DeepSketcher通过创新的数据集和模型设计，成功推进了"图像思维"范式的发展，为多模态推理提供了更灵活、免工具的解决方案。
...
TL;DR: mpLLM是一个用于3D脑部多参数MRI视觉问答的提示条件分层混合专家架构，通过模态级和令牌级专家路由融合多个相关3D模态，无需图像-报告预训练即可高效训练。
...
Method: 采用提示条件分层MoE架构，集成模态级和令牌级投影专家，结合合成VQA协议从分割标注生成医学相关问答，并与医学专家合作进行临床验证。

Result: 在多个mpMRI数据集上平均优于强医学VLM基线5.3%，创建了首个临床验证的3D脑部mpMRI VQA数据集。

Conclusion: 研究展示了模态级和令牌级专家以及提示条件路由的重要性，证明了该方法在医学应用中的实用性。
...
Method: 1. 系统性定义多模态多轮对话安全性；2. 基于蒙特卡洛树搜索构建自动化多模态多轮红队框架，生成不安全对话；3. 开发MMDS数据集，包含4,484个标注样本；4. 构建LLaVAShield工具，联合检测用户输入和助手回复的风险。

Result: LLaVAShield在多模态多轮内容审核任务和动态策略配置下始终优于强基线方法，建立了新的最先进结果。MMDS数据集包含细粒度安全评级、策略维度标签和基于证据的推理。

Conclusion: 该研究为多模态多轮对话安全提供了系统性框架和工具，通过MMDS数据集和LLaVAShield模型有效解决了多轮多模态对话中的安全挑战，推动了该领域的研究发展。
...
TL;DR: VLM-FO1是一个新框架，通过将物体感知从坐标生成问题转化为特征检索任务，解决了视觉语言模型在细粒度定位任务上的局限性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型擅长高层场景理解，但在需要精确定位的细粒度感知任务上表现不佳，因为生成精确坐标对语言中心架构具有挑战性。

Method: 采用混合细粒度区域编码器（HFRE）和双视觉编码器生成富含语义和空间细节的区域标记，通过基于标记的引用系统让LLM能够对这些视觉区域进行推理和定位。

Result: 实验显示VLM-FO1在多个基准测试中达到最先进性能，在物体定位、区域生成理解和视觉区域推理方面表现优异。

Conclusion: VLM-FO1建立了一个有效灵活的模式来构建感知感知的视觉语言模型，弥合了高层推理和细粒度视觉定位之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 尽管深度神经网络在架构和训练范式上有所进步，但仍然容易受到对抗样本的攻击。研究旨在探索训练数据特征如何影响对抗鲁棒性，挑战仅靠规模就能驱动鲁棒性的假设。

Method: 评估了36种最先进的视觉模型，涵盖监督、自监督和对比学习方法，在1.2M到22B图像的数据集上训练。在六种黑盒攻击类别下进行评估：随机扰动、两种几何掩码、COCO对象操作、ImageNet-C损坏和ImageNet-R风格偏移。

Result: 鲁棒性随数据量和模型规模呈对数缩放规律：数据量增加10倍，攻击成功率平均降低约3.2%；模型规模增加10倍，攻击成功率平均降低约13.4%。自监督模型在精选数据集上训练的表现优于在更大但较少精选数据集上训练的模型。对抗微调改善了结构变化的泛化能力，但对颜色分布的泛化能力没有改善。
...
<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法将模态和类别视为独立因素，导致解决方案碎片化、内存开销过大，且重建方法在处理跨域大变化时存在边界扭曲、领域干扰和高误报率问题。

Method: 采用混合专家(MoE)驱动的特征解压缩机制，包含特征压缩模块抑制潜在异常和促进跨模态交互，以及稀疏门控交叉MoE动态选择专家路径进行模态和类别特定的解压缩。

Result: 在9个异常检测数据集上实现最先进性能，涵盖3个领域、12种模态和66个类别，同时参数使用减少75%并保持稀疏激活和快速推理。

Conclusion: UniMMAD通过统一框架解决了多模态多类别异常检测中的碎片化和效率问题，证明了其在各种复杂场景下的有效性和高效性。
...
TL;DR: 提出CO3方法，通过校正采样策略改善文本到图像扩散模型中多概念提示的保真度，避免概念缺失或冲突问题


<details>
  <summary>Details</summary>
Motivation: 解决多概念提示（如"猫和狗"）在扩散模型中常出现的概念缺失、模糊或冲突问题，这些问题是模型在采样时偏向训练中学习较强的单一概念导致的

Method: 引入校正采样策略，引导模型远离联合提示行为与任何单一概念重叠过强的区域，转向所有概念能够平衡共存的"纯"联合模式；分析现有多概念引导方案的不稳定权重机制，并调整采样以保持在有利区域内

Result: 在多样化多概念提示上的实验表明，相比标准基线和先前组合方法，该方法在概念覆盖率、平衡性和鲁棒性方面均有改进，减少了概念丢失或扭曲的情况
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单独训练的检测模块，需要大量专家标注，成本高且受数据集病理分布偏差限制泛化能力

Method: 使用自监督解剖一致性学习，构建层次化解剖图，通过递归重建细粒度解剖区域实现空间对齐，并引入基于解剖一致性的区域级对比学习

Result: 在无需专家标注情况下，词汇准确率提升10%，临床效能提升25%，在零样本视觉定位任务中超越领先视觉基础模型8%

Conclusion: SS-ACL框架能够生成准确且视觉可解释的医学报告，为临床工作流程提供了有效的解决方案
...
TL;DR: 提出了一种基于姿态估计的鲑鱼追踪框架，通过提取鲑鱼边界框和身体部位信息来解决水下鲑鱼场景中的跟踪挑战，并用于自动化福利监测。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉方法只能监测单一福利指标，且依赖其他应用领域的检测器和跟踪器，资源需求高，易受水下场景中鲑鱼遮挡、外观相似和运动相似等问题的干扰。

Method: 使用姿态估计网络提取鲑鱼边界框和身体部位，通过专门模块利用身体部位信息解决水下鲑鱼场景特有的挑战，然后利用高细节的身体部位轨迹计算福利指标。

Result: 在两个新的鲑鱼跟踪挑战数据集上，该方法优于当前最先进的行人跟踪器BoostTrack，并创建了鲑鱼尾拍波长计算数据集，证明该方法适用于基于尾拍分析的自动化福利监测。

Conclusion: 提出的灵活跟踪框架能够有效解决水下鲑鱼场景中的跟踪挑战，为自动化鲑鱼福利监测提供了有效解决方案。
...
TL;DR: 本文提出了RH-Partial2Global方法，通过结合折刀共形预测和覆盖设计采样，改进了视觉上下文学习中的示例选择机制，解决了现有方法相似性假设缺乏理论依据和随机采样导致覆盖不完整的问题。


<details>
  <summary>Details</summary>
Motivation: 当前VICL方法基于相似性优先假设缺乏充分理论依据，且Partial2Global依赖随机采样构建全局排序，导致覆盖不完整和冗余采样，影响最终排序质量。

Method: 提出RH-Partial2Global方法：1）使用折刀共形预测构建可靠的替代集；2）采用覆盖设计采样确保成对偏好的全面均匀覆盖。

Result: 大量实验表明RH-Partial2Global在多种视觉任务中表现出色，性能优于Partial2Global。
...
Result: 相比最先进的基于扩散的视频编辑方法，实现最高3.7倍加速和60%内存使用降低，同时在帧一致性和文本对齐方面保持竞争力。
...
TL;DR: 提出NS-iHOS任务，利用自然语言叙述作为弱监督来学习手部物体分割，无需在推理时使用叙述。WISH模型通过从叙述中提取知识实现手部物体分割，性能达到全监督方法的50%以上。


<details>
  <summary>Details</summary>
Motivation: 解决第一视角图像中用户操纵物体像素级识别标注数据稀缺的问题，利用廉价易得的自然语言叙述作为弱监督信号。

Method: 提出WISH端到端模型，从自然语言叙述中提取知识学习合理的手-物体关联，实现无需推理时叙述的手部物体分割。

Result: 在EPIC-Kitchens和Ego4D数据集上，WISH超越所有基线方法，性能达到全监督方法的50%以上，且不使用像素级标注。

Conclusion: 自然语言叙述可作为有效的弱监督信号，显著减少对昂贵像素级标注的依赖，为手部物体分割任务提供了可行的解决方案。
...
TL;DR: 提出了AgenticIQA框架，将视觉语言模型与传统图像质量评估工具结合，通过模块化代理系统实现动态、查询感知的图像质量评估，显著提升了评分准确性和解释性。


<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估方法使用固定模型输出标量分数，难以适应不同失真类型、用户特定查询和可解释性需求，且评分与解释通常分离处理。

Method: 将IQA分解为四个子任务：失真检测、失真分析、工具选择和工具执行，由规划器、执行器和总结器协调完成，利用视觉语言模型与传统IQA工具的动态集成。

Result: 在多个IQA数据集上的广泛实验表明，AgenticIQA在评分准确性和解释对齐方面均优于强基线方法。

Conclusion: AgenticIQA框架通过模块化代理方法有效解决了传统IQA方法的局限性，提供了更准确、可解释的图像质量评估解决方案。
...
Conclusion: 这是首个系统研究异构针孔-鱼眼深度估计的工作，既提供了技术创新，又提供了有价值的实证见解。
...
TL;DR: 提出了一种结合扩散和波动特性的四阶非线性PDE模型，用于抑制乘性噪声，相比传统二阶PDE模型能更好地减少块状伪影并保留细节纹理。
...
Method: 构建四阶非线性PDE模型，结合拉普拉斯算子和强度值指导的扩散过程，以及保持细节纹理的波动部分，并将模型扩展到彩色图像处理。

Result: 在灰度图像上使用PSNR和MSSIM指标评估，在SAR图像上使用SI指标，在彩色图像上同样使用PSNR和MSSIM，所有情况下都比现有模型表现更好。

Conclusion: 所提出的四阶PDE模型在抑制乘性噪声方面优于现有的二阶各向异性扩散方法，能有效减少伪影并保持图像细节。
...
TL;DR: SETR提出了一种两阶段零样本组合图像检索方法，通过交集驱动的粗检索和基于多模态LLM的细粒度重排序，解决了CLIP方法中特征融合不精确和语义关系解析不足的问题，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的零样本组合图像检索方法面临两个核心挑战：联合特征融合会携带不相关的背景细节，稀释了预期修改；CLIP的全局余弦相似度缺乏解析细粒度语义关系的能力。

Method: SETR采用两阶段检索：1）粗检索阶段使用交集驱动策略，仅保留参考图像和相对文本的重叠语义，过滤干扰项；2）重排序阶段使用低秩适配的多模态LLM进行二元语义相关性判断，验证关系和属性级一致性。

Result: 在CIRR、Fashion-IQ和CIRCO数据集上的实验表明，SETR实现了新的最先进性能，在CIRR上的Recall@1提升了高达15.15个百分点。

Conclusion: 两阶段推理为鲁棒且可移植的零样本组合图像检索建立了一个通用范式。
...
TL;DR: GeoLink是一个多模态框架，利用OpenStreetMap数据增强遥感基础模型，通过跨模态空间相关性解决遥感与OSM数据之间的模态差异问题。


<details>
  <summary>Details</summary>
Motivation: 将地面地理空间数据与OpenStreetMap等丰富地理背景整合到遥感基础模型中，对于推进地理空间智能和支持广泛任务至关重要。但遥感与OSM数据在数据结构、内容和空间粒度上的差异使得有效协同极具挑战性。

Method: GeoLink在预训练阶段使用来自OSM数据的多粒度学习信号，通过跨模态空间相关性指导信息交互和协作，并引入图像掩码重建以实现稀疏输入的高效预训练。在下游任务中，生成单模态和多模态细粒度编码以支持从土地覆盖分类到城市功能区映射等应用。

Result: 实验表明，在预训练中融入OSM数据提升了遥感图像编码器的性能，而在下游任务中融合RS和OSM数据提高了基础模型对复杂地理场景的适应性。

Conclusion: 多模态协同在推进高级地理空间人工智能方面具有潜力，空间相关性在多模态地理空间数据整合中起着关键作用。
...
TL;DR: 提出PatchVSR方法，利用预训练视频扩散模型进行基于patch的视频超分辨率，通过双流适配器结合局部patch特征和全局上下文特征，实现高效高保真的4K视频超分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接对全尺寸视频进行超分辨率存在计算量大和输出分辨率固定的问题，需要探索更高效的patch级视频超分辨率方法。

Method: 提出PatchVSR方法，包含双流适配器：patch分支提取输入patch特征保持内容保真度，全局分支从缩放的完整视频提取上下文特征；注入patch位置信息；设计多patch联合调制确保视觉一致性。

Result: 实验证明该方法能在patch级别合成高保真、高分辨率细节，基于512x512基础模型实现极具竞争力的4K视频超分辨率，且效率极高。

Conclusion: 基于patch的视频超分辨率范式具有灵活性，能够高效实现高质量的超分辨率效果，为视频生成模型的应用提供了新思路。
...
TL;DR: 提出CGP框架，通过基于ViT的因果掩码引导空间变化高斯扰动，增强OOD泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决深度学习中OOD泛化挑战，特别是在生物医学图像领域，现有方法可能忽略因果机制

Method: 使用Vision Transformers生成软因果掩码，在背景区域施加强扰动、前景区域施加弱扰动

Result: 在WILDS基准Camelyon17上表现优于现有OOD方法

Conclusion: 因果扰动是提升模型可靠性和可解释泛化的有效工具
...
TL;DR: SeMoBridge通过语义模态桥将图像映射到文本模态，解决了CLIP在小样本分类中的模态内不对齐问题，在低数据场景下表现优异且训练时间短。


<details>
  <summary>Details</summary>
Motivation: CLIP在小样本分类中存在模态内不对齐问题，这是由于模态间隙和仅跨模态训练目标导致的，使得直接图像比较不可靠。现有方法要么改进相似度逻辑，要么进行昂贵的逐样本优化。

Method: SeMoBridge通过语义模态桥将图像映射到文本模态，保持语义内容不变。该方法为闭式解，可选择通过多模态监督训练，结合图像和对齐损失来优化投影。

Result: 训练版本SeMoBridge-T仅需少量训练时间，整体性能优于其他方法，特别是在低数据场景（1、2、4样本）中表现突出。

Conclusion: SeMoBridge有效解决了CLIP在小样本分类中的模态内不对齐问题，提供了一种轻量级且强大的解决方案，在低数据场景下具有显著优势。
...
TL;DR: 扩展HAMMER模型以处理全局场景不一致性（如前景-背景不匹配），通过轻量级分割引导评分(SGS)管道增强多模态操纵检测的鲁棒性


<details>
  <summary>Details</summary>
Motivation: HAMMER在多模态操纵检测方面表现优异，但在处理全局场景不一致性（如主体被错误放置到不合理背景中）时存在明显局限性，这源于标签空间偏差、局部注意力焦点和虚假文本-前景对齐问题

Method: 提出分割引导评分(SGS)管道：使用人物/面部分割掩码分离前景和背景区域，通过联合视觉语言模型提取嵌入，计算区域感知一致性分数，并将这些分数与HAMMER的原始预测融合

Result: SGS显著提高了对全局操纵的鲁棒性，增强了二元检测、定位和token级解释能力，且仅需推理阶段，计算开销可忽略

Conclusion: 这项工作证明了区域感知推理在多模态虚假信息检测中的重要性，为改进现有模型提供了轻量级解决方案
...
TL;DR: 本文扩展了DGM4数据集，新增了5000个包含前景-背景不匹配及其与文本操作混合的高质量样本，创建了DGM4+基准数据集，以测试检测器在局部和全局推理上的能力。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展使得制作具有说服力的多模态虚假信息变得更加容易。现有的DGM4数据集仅限于局部操作，而真实世界伪造中普遍存在全局不一致性问题，如前景与背景不匹配。

Method: 使用OpenAI的gpt-image-1和精心设计的提示生成以人为中心的新闻风格图像，将真实人物置于荒谬或不可能的背景下。在三种文本条件下生成标题：字面、文本属性和文本分割，产生三种新的操作类别。

Result: 创建了包含FG-BG、FG-BG+TA和FG-BG+TS三种操作类别的5000个高质量样本，通过质量控制管道确保数据质量，包括可见人脸数量、感知哈希去重、OCR文本清理和真实标题长度。
...
TL;DR: 提出一种基于主纤维丛截面概念的新方法，用于在分类任务中消除对称性影响，避免数据增强，构建更可持续的算法。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉和医学应用中大多数数据集都存在对称性，传统方法使用数据增强来学习这些对称性，但这种方法不够可持续。

Method: 利用主纤维丛截面概念，通过测量对称群作用下对象轨道之间的差异来消除对称性，并优化截面以最大化类别分离。

Result: 在包含平移、旋转、缩放和重参数化对称性的对象轮廓数据集上验证了该方法，提出了一个包含恒定速度参数化的2参数规范参数化族。

Conclusion: 该方法提供了一个简单有效的几何框架来处理对称性，具有广泛的应用潜力，相关代码已开源。
...
TL;DR: 提出使用基础模型生成3D伪标签的方法，解决自监督语义占据预测中的计算成本高和内存占用大的问题，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督模型使用复杂渲染策略（如新视角合成）来补偿缺乏真实标签的问题，但这些方法计算成本高、内存占用大，特别是新视角合成技术。

Method: 使用Grounded-SAM和Metric3Dv2基础模型生成3D伪标签，并利用时序信息进行标签密集化，提出简化模型EasyOcc仅从伪标签学习。

Result: 在OccNeRF模型中mIoU从9.73提升至14.09（提升45%），EasyOcc模型达到13.86 mIoU，在完整场景评估中达到7.71 mIoU，超越之前最佳模型31%。

Conclusion: 基础模型、时序上下文和损失计算空间的选择在自监督学习的全面场景理解中具有关键重要性。
...
TL;DR: 提出基于多模态深度学习的实时点球方向预测框架，在球接触前预测点球方向（左、中、右），准确率达89%，推理时间22毫秒


<details>
  <summary>Details</summary>
Motivation: 点球经常决定冠军归属，守门员需要在极短时间内从细微的生物力学线索中预判踢球者意图

Method: 双分支架构：MobileNetV2 CNN处理RGB帧，LSTM网络处理2D关键点并采用注意力机制，姿态关键点引导视觉关注任务相关区域，距离阈值方法分割球接触前序列
...
TL;DR: Reason-3D是一个基于大型推理模型的文本到场景生成方法，通过对象检索和空间推理来生成复杂的3D环境，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决当前文本到场景方法在处理复杂几何、对象变换和复杂指令遵循方面的局限性。

Method: 集成基于物理、功能和上下文属性的对象检索，根据隐式和显式布局约束放置对象，并使用碰撞感知的空间推理优化位置。
...
Result: 在CIFAR-10上，EVODiff在10次函数评估时将重构误差降低45.5%（FID从5.10提升至2.78）；在ImageNet-256上，将高质量样本的NFE成本减少25%（从20降至15）；同时改善了文本到图像生成质量并减少了伪影。

Conclusion: EVODiff在理论和实验上都验证了其优越性，显著超越了现有最先进的梯度求解器，为扩散模型的高效推理提供了新的解决方案。
...
TL;DR: EchoGen是一个基于视觉自回归模型(VAR)的主题驱动生成框架，通过双路径注入策略实现高效、高质量的主题生成，解决了现有方法在效率和质量之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前主题驱动生成方法存在明显权衡：要么需要昂贵的逐主题微调，牺牲效率和零样本能力；要么使用基于扩散模型的架构，但推理速度慢。VAR模型具有快速采样和强生成质量，但尚未被充分探索用于解决这一矛盾。

Method: 提出EchoGen框架，采用双路径注入策略：1) 语义编码器提取主题的高层语义身份，通过解耦交叉注意力注入以指导整体构图；2) 内容编码器捕获精细视觉细节，通过多模态注意力机制集成以确保高保真纹理和结构保留。

Result: 定量和定性结果证实了设计有效性，EchoGen在主题保真度和图像质量上可与最先进的基于扩散的方法相媲美，同时显著降低了采样延迟。

Conclusion: EchoGen是首个基于VAR模型的前馈主题驱动框架，成功解决了主题驱动生成中效率与质量之间的权衡问题，为快速高质量的主题生成提供了新解决方案。
...
TL;DR: 提出了EntroPE框架，通过条件熵动态检测时间序列中的过渡点来构建patch边界，解决了传统固定长度patch分割破坏时间连贯性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于patch的时间序列预测方法采用时间无关的patch构建方式，任意起始位置和固定长度会破坏时间连贯性，分割自然过渡点，扰乱短期依赖关系并削弱表示学习。
...
Conclusion: 熵引导的动态patch构建为时间序列建模提供了一个有前景的新范式。
...
TL;DR: 提出了一种基于文本引导的边缘案例自动合成管道，通过微调的大型语言模型生成多样化文本提示，指导文本到图像模型生成困难的视觉场景，在FishEye8K目标检测基准上实现了优越的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络性能严重依赖训练数据质量，但手动筛选具有挑战性的边缘案例存在效率瓶颈，需要自动化解决方案来缓解数据集偏差问题。

Method: 使用偏好学习微调的大型语言模型来重新表述图像描述，生成多样化的文本提示，这些提示指导文本到图像模型生成困难的视觉场景。

Result: 在FishEye8K目标检测基准上，该方法实现了优越的鲁棒性，超越了朴素数据增强和手动设计的提示方法。
...
TL;DR: 提出了Human-MME基准测试，用于评估多模态大语言模型在人类中心场景理解中的能力，包含多样化的场景、渐进式评估维度和高质量标注。
...
Method: 构建了包含4个主要视觉领域、15个次要领域和43个子领域的多样化人类场景数据集，设计了8个评估维度，包含19,945个真实世界图像问答对，并开发了自动化标注流程和人工标注平台。

Result: 在17个最先进的多模态大语言模型上的广泛实验有效暴露了现有模型的局限性，为未来研究提供了指导。

Conclusion: Human-MME基准测试为多模态大语言模型在人类中心图像理解方面的研究提供了更全面的评估框架，有助于推动该领域的发展。
...
TL;DR: 该论文系统评估了行人检测模型在姿态和遮挡方面的公平性问题，发现检测器对平行腿、直肘和侧面视角的行人存在偏见，且下半身关节遮挡对检测率影响更大。


<details>
  <summary>Details</summary>
Motivation: 虽然许多检测模型致力于降低漏检率和处理遮挡、远距离识别等挑战，但公平性仍是一个未被充分探索但同样重要的问题。

Method: 评估了五个行人专用检测器和三个通用模型在ECP-DP数据集上的表现，使用EOD指标量化公平性，并应用Z检验评估统计显著性。

Result: 发现检测器对平行腿、直肘和侧面视角的行人存在偏见，下半身关节遮挡比上半身和头部遮挡对检测率影响更大。Cascade R-CNN表现最佳，漏检率最低且偏见最小。

Conclusion: 这是首个在自动驾驶行人检测中进行的全面姿态和遮挡感知公平性评估，揭示了现有检测模型在公平性方面的重要问题。
...
TL;DR: AttriGen是一个用于细胞显微镜图像自动细粒度多属性标注的新框架，通过双模型架构在细胞类型分类和多属性分类上达到94.62%的准确率。


<details>
  <summary>Details</summary>
Motivation: 在细胞显微镜领域，多属性分类相比传统的细胞类型分类研究不足，需要自动化方法来提高标注效率和模型可解释性。

Method: 使用双模型架构：CNN用于细胞类型分类，Vision Transformer用于多属性分类，基于PBC和WBCAtt两个互补数据集。

Result: 在多属性分类上达到94.62%的新基准准确率，显著提高模型可解释性，相比传统人工标注节省大量时间和成本。

Conclusion: 该框架建立了新的范式，可扩展到其他计算机视觉分类任务，有效自动化多属性标签的扩展。
...
TL;DR: 本文提出了TSV360数据集和TSalV360方法，用于360度视频中的文本驱动显著性检测，通过视觉语言模型和注意力机制实现基于文本描述的定制化显著性检测。
...
Method: 扩展了SOTA的视觉显著性检测方法，开发了TSalV360方法，使用视觉语言模型进行数据表示，集成相似性估计模块和视口时空交叉注意力机制。
...
TL;DR: 提出GSDD方法，使用2D高斯基元的稀疏表示进行数据集蒸馏，相比传统密集像素方法更高效且可扩展
...
TL;DR: 本研究通过集成大型多模态模型生成视频摘要的可解释性文本描述，并提出基于语义重叠的合理性评估方法，探讨了视频摘要中可信解释与合理解释之间的关系。


<details>
  <summary>Details</summary>
Motivation: 研究旨在为视频摘要结果生成合理的文本解释，关注可解释AI中最受期待的特性——解释的合理性，即与人类推理和期望的一致性。

Method: 扩展现有多粒度视频摘要解释框架，集成LLaVA-OneVision模型生成视觉解释的文本描述，使用SBERT和SimCSE两种句子嵌入方法量化文本解释与视频摘要文本描述之间的语义重叠。

Result: 使用CA-SUM方法和SumMe、TVSum数据集进行实验研究，评估不同方法生成文本解释的合理性。

Conclusion: 研究确定了生成合理文本解释的最合适方法，并探讨了可信解释与合理解释之间的关系。
...
TL;DR: 提出了MGCE框架，通过多粒度概念专家解决广义类别发现中的两个关键问题：利用多粒度视觉概念信息和自动估计未知类别数量。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要局限：未能充分利用视觉数据中的多粒度概念信息，以及假设训练时已知未标记类别数量，这在现实场景中不切实际。

Method: MGCE包含两个模块：动态概念对比学习（DCCL）交替进行概念挖掘和双级表示学习；多粒度专家协同学习（MECL）引入不同粒度的专家并使用概念对齐矩阵进行跨专家协作。

Result: 在9个细粒度视觉识别基准测试中达到最先进结果，特别是在新类别准确率方面。即使不知道类别数量，MGCE也优于需要知道确切类别数量的参数化方法，平均提升3.6%。

Conclusion: MGCE框架有效解决了GCD问题中的关键挑战，通过自适应挖掘视觉概念和集成多粒度知识，实现了准确的类别发现，且无需预先知道类别数量。
...
TL;DR: IMG是一种无需额外数据或编辑操作的多模态对齐框架，通过MLLM识别生成图像与提示之间的不对齐，使用隐式对齐器操作扩散条件特征进行重新生成，在SDXL等模型上优于现有对齐方法。


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型生成图像与输入提示之间的精确多模态对齐问题，避免传统方法需要高质量偏好数据或编辑操作带来的限制。

Method: 利用MLLM识别不对齐区域，引入隐式对齐器操作扩散条件特征进行重新生成，制定可训练的迭代更新偏好目标。

Result: 在SDXL、SDXL-DPO和FLUX上的定性和定量评估表明，IMG优于现有对齐方法，并能作为即插即用适配器增强现有微调方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型如Donut在文档视觉问答中表现优异，但计算成本过高，难以在实时或资源受限场景中部署。

Method: 利用机制可解释性分析模型内部计算，识别关键子组件，指导学生模型架构设计，通过知识蒸馏训练紧凑模型。

Result: 开发的Donut-MINT模型在DocVQA基准测试中保持强性能，同时显著减少推理时间和内存使用。
...
TL;DR: 提出Farsighted-LAM框架，通过几何感知空间编码和多尺度时间建模改进潜在动作模型，解决空间理解差和时间感知有限的问题，并构建SSM-VLA框架实现最先进性能。
...
Result: 在模拟和真实世界的多个VLA任务上验证SSM-VLA，实现了最先进的性能。
...
TL;DR: 提出PRPO算法解决多模态大语言模型在深度伪造检测中推理与视觉证据不匹配的问题，显著提升检测准确性和推理质量


<details>
  <summary>Details</summary>
Motivation: 合成媒体的快速发展使深度伪造检测成为在线安全和信任的关键挑战，但现有多模态大语言模型在深度伪造检测中表现不佳，常产生与视觉证据不符或幻觉的解释

Method: 引入推理标注数据集，并提出段落级相对策略优化(PRPO)强化学习算法，在段落级别对齐LLM推理与图像内容

Result: PRPO大幅提升检测准确率，获得4.55/5.0的最高推理分数，消融研究显示PRPO在测试条件下显著优于GRPO
...
TL;DR: 提出了一种基于聚类的仿射变换(CAT)方法，用于改善低比特后训练量化的精度下降问题，通过聚类特定参数对齐量化输出与全精度输出，无需微调即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 后训练量化(PTQ)虽然比量化感知训练(QAT)更高效，但在低比特量化(如2-bit)下容易导致精度显著下降。传统的仿射变换使用统一参数集反而会恶化低比特PTQ的结果。

Method: 提出聚类仿射变换(CAT)框架，使用聚类特定参数来对齐低比特量化输出与全精度输出，仅需少量额外参数，无需微调模型或量化参数。

Result: 在ImageNet-1K上的实验表明，该方法在各种架构和低比特设置下均优于现有PTQ方法，W2A2 ResNet-18上达到53.18% Top-1准确率，作为插件可提升现有基线3%以上。

Conclusion: CAT是一种有效的低比特后训练量化误差减少框架，通过聚类特定的仿射变换显著改善了量化模型的精度，且具有很好的通用性和实用性。
...
Method: 使用AttentiveGatedProjector动态融合来自冻结TimeSformer骨干网络的多视角特征，将其投影到为反馈生成调优的语言模型中。

Result: 在EgoExo4D数据集上训练，ProfVLM在减少20倍参数和60%训练时间的情况下超越了最先进方法，并在多样化活动中实现更高准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的点监督方法存在伪标签利用效率低和质量差的问题，需要一种更有效的弱监督方法来替代昂贵的人工标注。

Method: 1) 渐进式标签分配(PLA)：在不同训练阶段动态估计实例大小，实现标签分配方法的使用；2) 先验引导动态掩码损失(PGDM-Loss)：结合SAM模型和分水岭算法的优势，在稀疏和密集场景中都能取得良好性能。
...
TL;DR: Flower是一个基于预训练流模型的逆问题求解器，通过三步迭代过程生成与观测一致的图像重建，在多种逆问题上实现了最先进的重建质量。


<details>
  <summary>Details</summary>
Motivation: 统一即插即用方法和生成式逆问题求解器的视角，提供一种能够近似贝叶斯后验采样的通用逆问题解决方案。

Method: 采用三步迭代过程：(i)流一致目标估计，(ii)基于前向算子的可行集投影精炼，(iii)沿流轨迹的时间推进重投影。

Result: 在多种逆问题上实现了最先进的重建质量，且几乎使用相同的超参数。

Conclusion: Flower成功地将流模型应用于逆问题求解，提供了理论保证和实际性能的统一框架。
...
TL;DR: 提出了一种连续时空视频超分辨率的新方法，使用3D视频傅里叶场表示视频，能够实现任意时空位置的灵活采样，同时捕捉空间细节和时间动态。


<details>
  <summary>Details</summary>
Motivation: 传统方法将视频表示为分离的空间和时间组件，并依赖脆弱的显式帧扭曲进行运动补偿，存在局限性。

Method: 将视频编码为连续、时空相干的3D视频傅里叶场，使用具有大时空感受野的神经编码器预测傅里叶基系数，并包含高斯点扩散函数进行抗锯齿重建。

Result: 在多个基准测试中达到新的最先进水平，在广泛的放大因子范围内提供更清晰、时间更一致的超分辨率重建，同时计算效率更高。

Conclusion: 联合建模方法显著改善了空间和时间超分辨率性能，证明了连续傅里叶场表示在视频处理中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 零样本组合图像检索方法虽然无需任务特定训练或标注数据，但准确捕捉用户意图仍然具有挑战性，需要提升检索质量。

Method: 采用两阶段方法：1）语义查询增强融合阶段，用MLLM生成目标图像描述来丰富CLIP等VLM的查询嵌入；2）高效批量重排阶段，将候选图像以网格形式呈现给MLLM进行联合视觉语义推理。

Result: 在四个标准CIR基准测试中表现出色，即使使用轻量级预训练模型也能保持高性能。

Conclusion: SQUARE框架简单有效，在零样本组合图像检索中实现了强大的性能，具有广泛的应用潜力。
...
TL;DR: 提出了一个名为\mname的奖励模型，用于解决开源图像编辑模型缺乏高质量训练数据的问题。该模型在人类偏好对齐方面表现优异，能够有效筛选高质量训练数据，显著提升图像编辑模型的性能。
...
Result: \mname在多个基准测试中达到最先进的人类相关性，能够有效筛选ShareGPT-4o-Image数据集中的高质量子集，训练出的Step1X-Edit模型相比全数据集训练有显著提升。

Conclusion: \mname作为奖励模型能够有效扩展高质量图像编辑训练数据，其强大对齐能力为基于强化学习的后训练和测试时扩展等高级应用提供了潜力。
...
TL;DR: 提出了任务导向时序定位(ToTG)新问题，旨在基于任务描述定位长视频中的关键时间区间，并构建了ToTG Bench基准和TimeScope框架来解决该问题。
...
Method: 提出TimeScope框架，采用渐进推理方法：先在长视频中识别粗粒度的时间范围，然后通过细粒度时刻划分来精炼该范围。

Result: 大量实验表明TimeScope在各种设置下均优于现有时序定位方法和流行的MLLMs。
...
TL;DR: ScalingAR是首个专为基于下一个token预测的自回归图像生成设计的测试时缩放框架，无需早期解码或辅助奖励，通过token熵和双层级策略显著提升生成质量并减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法依赖于频繁的部分解码和外部奖励模型，不适用于基于下一个token预测的自回归图像生成，因为中间解码结果不完整。

Method: 提出ScalingAR框架，利用token熵作为视觉token生成的新信号，在配置层级融合内在和条件信号生成校准置信状态，在策略层级自适应终止低置信轨迹并动态调度指导强度。

Result: 在通用和组合基准测试中，ScalingAR将基础模型在GenEval上提升12.5%，在TIIF-Bench上提升15.2%，视觉token消耗减少62.0%，在挑战性场景中性能下降缓解26.0%。

Conclusion: ScalingAR成功将测试时缩放应用于自回归图像生成，显著提升生成质量、效率和鲁棒性，为视觉自回归模型提供了有效的测试时优化方案。
...
TL;DR: 提出了PANDA，一种基于多模态大语言模型的智能代理，用于视频异常检测，无需训练数据或人工干预即可处理任意场景和异常类型。


<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法依赖特定领域训练数据和人工调整，存在高劳动成本和泛化能力有限的问题，因此需要实现无需训练和人工干预的通用异常检测。
...
Conclusion: PANDA展示了在无需训练和人工干预的情况下实现通用视频异常检测的可行性，为智能视频分析提供了新的解决方案。
...
TL;DR: MotionRAG是一个检索增强框架，通过从相关参考视频中提取运动先验来提升图像到视频生成的动态真实性，采用上下文感知运动适应技术，在多个领域和基础模型上取得显著改进，且推理时计算开销可忽略。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的图像到视频生成在运动真实性方面仍面临挑战，难以准确建模涉及物理约束、物体交互和领域特定动态的复杂运动。

Method: 提出MotionRAG框架，包括：(i)基于检索的流程提取高级运动特征；(ii)通过因果Transformer架构实现上下文学习的运动适应；(iii)基于注意力的运动注入适配器将转移的运动特征无缝集成到预训练视频扩散模型中。

Result: 广泛实验表明，该方法在多个领域和各种基础模型上均取得显著改进，推理时计算开销可忽略，且模块化设计支持通过更新检索数据库实现对新领域的零样本泛化。
...
TL;DR: 提出了一种基于图像难度感知的超分辨率模型评估方法，通过高频指数和旋转不变边缘指数来预测模型在困难图像上的性能差异，弥补传统平均分数评估的不足。
...
Method: 提出了两种图像难度度量：高频指数和旋转不变边缘指数，以及一种评估方法，使视觉差异能够在客观指标上体现。

Result: 实验结果表明所提出的图像难度度量和评估方法的有效性。

Conclusion: 提出的难度感知评估方法能够更好地区分在平均性能接近但视觉结果不同的超分辨率模型。
...
TL;DR: 提出PRISM框架，通过三阶段渐进式网络（CENet、SFNet、RNet）结合混合注意力UNet和混合域Mamba模块，实现多尺度特征聚合和空间语义建模，有效去除图像中的雨纹和水滴。


<details>
  <summary>Details</summary>
Motivation: 当前单尺度模型在图像去雨任务中难以同时实现细粒度恢复和全局一致性，需要更有效的多尺度特征处理方法。

Method: 三阶段渐进框架：CENet进行粗提取，SFNet使用混合注意力UNet和HDMamba模块进行频率融合，RNet通过原始分辨率子网络恢复细粒度结构。

Result: 在多个数据集上取得了有竞争力的结果，能够有效学习高频雨特征同时保持结构细节和全局上下文。

Conclusion: PRISM框架通过渐进式多尺度处理和混合域建模，显著提升了图像去雨的质量和效果。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但计算需求高，部署困难。现有的8位量化方法效果良好，但扩展到4位量化仍面临挑战，主要是由于大步长放大了低幅度激活的舍入误差，导致纹理细节丢失。

Method: 提出QuaRTZ方法：使用8位最小-最大量化处理异常值，然后通过前导零抑制压缩到4位以保留最低有效位(LSB)，从而保持纹理细节。该方法通过平衡异常值保留和LSB精度来减少舍入误差并提高量化效率。

Result: 4位QuaRTZ在FLUX.1-schnell上实现了6.98的FID分数，优于需要辅助FP16分支的SVDQuant方法。理论推导和实证评估都证明了QuaRTZ在不同激活分布上的泛化能力。

Conclusion: QuaRTZ方法成功实现了扩散模型的4位量化，在保持图像质量的同时显著降低了计算需求，为扩散模型的部署提供了有效的解决方案。
...
TL;DR: 提出了一个端到端的多视角感知系统AVI，用于汽车生产线上的变种规格验证和缺陷检测，通过11个摄像头实现360度覆盖，结合深度学习检测器和语义规则引擎，在300毫秒内生成可解释的通过/失败报告。


<details>
  <summary>Details</summary>
Motivation: 现代汽车生产线需要确保每辆车都按照正确的变种规格制造且没有可见缺陷，这是一个日益复杂的挑战。

Method: 使用11个同步摄像头捕获车辆360度视图，通过专门的模块进行部件检测（YOLOv8）、ICE/EV分类（EfficientNet）、吉祥物OCR（Gemini-1.5 Flash）和划痕凹陷分割（YOLOv8-Seg），结合视图感知融合层和基于VIN的条件规则引擎。

Result: 在混合数据集上达到93%的验证准确率、86%的缺陷检测召回率，并维持每分钟3.3辆车的处理速度，大幅超过单视角或无分割基线方法。

Conclusion: 这是首个公开报道的在工业可部署汽车环境中统一多摄像头特征验证与缺陷检测的系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有的3D风格迁移方法通常需要预计算姿态或进行逐场景优化，限制了其应用范围。Stylos旨在实现无需姿态估计的零样本3D风格迁移，能够处理未见过的类别、场景和风格。

Method: 采用Transformer架构，包含两个路径：几何预测路径使用自注意力保持几何保真度，风格注入路径通过全局交叉注意力实现跨视角视觉一致性。此外引入基于体素的3D风格损失来对齐场景特征与风格统计信息。

Result: 在多个数据集上的实验表明，Stylos能够实现高质量的零样本风格化，验证了全局风格-内容耦合、3D风格损失以及框架从单视角到大规模多视角设置的可扩展性的有效性。

Conclusion: Stylos通过创新的Transformer架构和3D风格损失，实现了无需姿态估计的视图一致3D风格迁移，在保持几何保真度的同时生成高质量的风格化结果，具有良好的泛化能力和可扩展性。
...
TL;DR: 提出ASGRA框架，使用场景图和图注意力网络进行室内场景分类和敏感内容分析，通过结构化表示实现可解释性和隐私保护。


<details>
  <summary>Details</summary>
Motivation: 室内场景分类在计算机视觉中至关重要，特别是在敏感内容分析（如儿童性虐待图像分类）中。传统方法难以处理复杂对象关系和空间布局。

Method: 将图像转换为场景图，然后使用图注意力网络进行推理，直接建模场景组件之间的交互关系。

Result: 在Places8数据集上达到81.27%的平衡准确率，优于基于图像的方法；在真实世界CSAI评估中获得74.27%的平衡准确率。
...
TL;DR: 提出了一个轻量级的CBAM-CNN模型用于槟榔叶病害分类，该模型仅含213万参数，在包含10,185张图像的数据集上实现了95.58%的准确率。
...
Method: 构建轻量级CBAM-CNN模型，集成卷积块注意力模块(CBAM)增强特征关注能力，使用Grad-CAM进行可视化解释，在包含健康叶片、叶腐病和叶斑病三类图像的数据集上进行训练。

Result: 模型在测试集上达到97%精确率、94%召回率、95% F1分数和95.58%准确率，性能优于传统预训练CNN模型。

Conclusion: 轻量级CBAM-CNN模型能有效识别槟榔叶病害，注意力机制提高了对细微差异的辨别能力，为农业病害诊断提供了高效解决方案。
...
TL;DR: 该论文提出了一种从用户移动轨迹重建空间布局（如房屋平面图）的方法，使用基于扩散的后验采样器，并通过对比学习在嵌入空间中重新定义似然分数来解决路径规划过程不可逆、不可微的问题。


<details>
  <summary>Details</summary>
Motivation: 从移动轨迹重建空间布局是一个不适定问题，因为多个平面图可能产生相同的移动轨迹。现有的生成逆求解器面临路径规划过程不可逆、不可微的挑战，导致优化不稳定。

Method: 采用扩散模型进行后验采样，通过对比学习训练嵌入空间，使兼容的平面图和轨迹在嵌入空间中接近，不匹配的对远离，从而在嵌入空间中定义更平滑的似然分数来指导去噪过程。

Result: 实验表明，CoGuide模型能够从轨迹生成更一致的平面图，比可微分规划器基线和引导扩散方法更鲁棒。

Conclusion: 通过在嵌入空间中重新定义似然分数，可以有效地指导扩散模型生成与观测轨迹一致的空间布局，解决了传统方法中路径规划不可微的问题。
...
TL;DR: 提出了一种系统化的后训练流程，通过课程式监督微调和离线策略知识蒸馏，有效提升小型语言模型在边缘设备上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限的边缘环境中部署困难，而预训练后的小型模型在复杂任务上性能不足，需要高效的后训练方法来提升其性能。

Method: 采用课程式监督微调(SFT)和离线策略知识蒸馏的后训练流程，在Ascend边缘设备上开发高性能语言模型。

Result: 生成的指令调优模型在十亿参数模型中达到最先进性能，在严格硬件约束下展现出强泛化能力，并在多种任务中保持竞争力。

Conclusion: 该工作为在Ascend边缘设备上开发高性能语言模型提供了实用且高效的解决方案。
...
TL;DR: DEPTHOR++是一个深度增强框架，通过合成数据集模拟真实噪声、无参数异常检测机制和专门设计的深度补全网络，显著提升了轻量级dToF传感器在真实场景中的深度感知性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度增强方法假设理想dToF输入和完美dToF-RGB对齐，忽略了校准误差和异常值，限制了在实际应用中的适用性。

Method: 1. 基于合成数据集的模拟方法生成真实训练样本；2. 无学习参数异常检测机制识别并移除错误dToF测量；3. 针对噪声dToF输入设计的深度补全网络，整合RGB图像和预训练单目深度估计先验。

Result: 在ZJU-L5数据集上RMSE和Rel分别平均提升22%和11%；在Mirror3D-NYU数据集上镜面区域性能提升37%；在Hammer数据集上使用低成本dToF数据超越L515测量结果22%。

Conclusion: 该方法显著提升了深度补全模型在真实场景中的鲁棒性，使低成本传感器能够超越高端设备性能，具有广泛的实际应用价值。
...
TL;DR: Ferret-UI Lite是一个紧凑的端到端GUI代理，专为小型设备模型设计，在移动、网络和桌面平台上表现优异，通过数据混合、思维链推理和强化学习等技术实现竞争性性能。
...
Method: 使用针对小型模型优化的技术构建3B参数的Ferret-UI Lite代理，包括从真实和合成来源策划多样化的GUI数据混合、通过思维链推理和视觉工具使用增强推理时性能，以及使用设计奖励的强化学习。

Result: 在GUI基础任务中，Ferret-UI Lite在ScreenSpot-V2、ScreenSpot-Pro和OSWorld-G基准测试中分别获得91.6%、53.3%和61.2%的分数。在GUI导航任务中，在AndroidWorld和OSWorld上分别达到28.0%和19.8%的成功率。

Conclusion: Ferret-UI Lite在小型GUI代理中实现了竞争性性能，并分享了开发紧凑型设备GUI代理的方法和经验教训。
...
Method: 构建了包含四个分类体系的评估框架，开发了自动化的提示分类和问题生成流程，进行了大规模人类研究（10+模型，20K视频，80+电影专业人士标注），并训练了自动评估器。

Result: 研究发现即使最强的现有模型在Events和Camera相关控制方面仍存在显著差距，训练的自动评估器在专家标注对齐方面优于现有零样本基线。

Conclusion: SCINE是首个将专业视频生成置于视频生成模型背景下的方法，通过以电影控制为中心的分类体系和结构化评估流程，为未来研究提供指导。
...
TL;DR: 使用机器学习模型自动化电子显微镜图像连接组的手动校对工作，显著降低人工成本并提高效率


<details>
  <summary>Details</summary>
Motivation: 电子显微镜连接组构建需要大量人工校对，这是扩展连接组学的主要瓶颈。作者希望利用已有的人工标注数据来训练机器学习模型，自动化或优化校对流程。

Method: 利用手动标注生成的地面真值数据训练机器学习模型，应用于引导校对工作流和自动合并分割片段。在果蝇雄性中枢神经系统的完整重建上进行验证。

Result: 方法实现了引导校对工作流90%的价值，同时减少80%的成本；自动合并了20万个分割片段（相当于4年人工工作量），将连接组连通完成率提高了1.3个百分点。

Conclusion: 机器学习方法能有效自动化电子显微镜连接组的校对工作，大幅降低人工成本，为大规模连接组重建和比较连接组学提供了可行方案。
...
TL;DR: DiffCamera是一个基于扩散变换器的图像重聚焦模型，能够根据任意新的焦点和模糊程度重新调整已创建图像的景深效果。


<details>
  <summary>Details</summary>
Motivation: 解决图像创建后景深效果固定难以修改的问题，特别是当应用模糊不理想时（如主体失焦）。

Method: 设计扩散变换器框架进行重聚焦学习，使用模拟数据生成大规模图像对，并提出堆叠约束来增强训练效果。

Result: 模型能够在各种场景下稳定地进行重聚焦，为摄影和生成式AI应用提供前所未有的景深调整控制。

Conclusion: DiffCamera通过物理基础的约束实现了精确的景深操作，支持灵活的焦点和模糊程度调整。
...
TL;DR: 提出SAGANet模型，通过视频对象分割实现可控音频生成，解决了现有多模态音频生成模型缺乏精确用户控制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态音频生成模型缺乏精确的用户控制，无法优先处理场景中的特定对象，导致生成不必要的背景声音或关注错误对象，限制了在专业Foley工作流程中的应用。

Method: 引入视频对象分割感知音频生成任务，提出SAGANet多模态生成模型，利用视觉分割掩码、视频和文本线索进行可控音频生成。

Result: 相比现有最先进方法有显著改进，为可控、高保真Foley合成设定了新标准。

Conclusion: SAGANet通过对象级分割实现了细粒度和视觉定位的音频控制，并发布了Segmented Music Solos基准数据集支持进一步研究。
...
TL;DR: Hy-Facial是一个混合特征提取框架，结合深度学习和传统图像处理技术，通过UMAP降维在面部表情识别任务中达到83.3%的准确率。
...
TL;DR: 提出了DA²（Depth Anything in Any Direction），这是一个准确、零样本泛化能力强且完全端到端的全景深度估计器，通过数据增强和SphereViT架构解决了全景数据稀缺和球形失真的问题。


<details>
  <summary>Details</summary>
Motivation: 全景图像具有完整的视野，但存在数据稀缺和球形失真问题，导致现有方法在零样本泛化能力上表现不佳，且效率低下。

Method: 1) 开发数据增强引擎，从透视图像生成高质量全景深度数据，创建约607K全景RGB-深度对；2) 提出SphereViT，利用球面坐标强制全景图像特征的球面几何一致性。

Result: 在多个数据集上的综合基准测试显示，DA²在AbsRel指标上比最强的零样本基线平均提升38%，甚至优于先前的域内方法，展现出卓越的零样本泛化能力。

Conclusion: DA²作为端到端解决方案，在准确性和效率上均优于融合方法，证明了其在全景深度估计领域的先进性和实用性。
...
TL;DR: HART是一个稀疏视角人体重建的统一框架，能够从少量未标定RGB图像中重建出完整的人体网格、对齐的SMPL-X身体网格和高斯溅射表示，在多个任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么优化参数化模板（忽略宽松衣物和人-物交互），要么在简化的相机假设下训练隐式函数，限制了在真实场景中的应用。

Method: 预测逐像素的3D点图、法线和身体对应关系，采用遮挡感知的泊松重建恢复完整几何，与SMPL-X身体模型对齐，并用人体对齐网格初始化高斯溅射。

Result: 在多个数据集上取得SOTA：衣物网格重建的Chamfer距离改善18-23%，SMPL-X估计的PA-V2V降低6-27%，新视角合成的LPIPS减少15-27%。
...
TL;DR: 提出了第一个SIM(3)-等变形状补全网络，通过架构级别的等变性解决传统方法依赖预对齐导致泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统3D形状补全方法假设扫描数据已预对齐到规范坐标系，这会导致网络利用姿态和尺度线索而非推断内在几何结构，当真实数据缺乏对齐时性能急剧下降。

Method: 设计SIM(3)-等变形状补全网络，采用模块化层逐步规范化特征、在相似性不变几何上进行推理，并恢复原始坐标系。

Result: 在去除隐藏线索的去偏评估协议下，该模型在PCN基准上优于等变和数据增强基线方法，在KITTI和OmniObject3D数据集上分别降低最小匹配距离17%和Chamfer距离14%。

Conclusion: 完整的SIM(3)等变性是实现真正可泛化形状补全的有效途径，即使在更严格的评估协议下仍优于在偏置设置下的竞争对手。
...
TL;DR: 提出了一个新的视觉惯性SLAM数据集和基准测试，专注于解决可穿戴设备在城市场景中面临的挑战，如多样化运动、动态内容和长时间运行问题。


<details>
  <summary>Details</summary>
Motivation: 现有SLAM基准测试无法充分反映可穿戴设备在城市场景中的实际挑战，缺乏准确的地面真实位姿数据，特别是对于极端轨迹（如夜间行走、车辆移动）的评估。

Method: 使用眼镜式设备记录城市中心的多模态数据，利用测量工具获取厘米级精度的控制点作为间接位姿标注，设计不同难度级别的测试轨迹。

Result: 实验表明当前最先进的学术SLAM系统在面对这些挑战时缺乏鲁棒性，并识别出了导致性能下降的关键组件。

Conclusion: 该数据集和基准测试为评估和改进SLAM系统在真实城市场景中的性能提供了重要工具，特别针对可穿戴设备的特定需求。
...
TL;DR: 本文提出Query-Kontext方法，通过多模态"kontext"连接VLM和扩散模型，将多模态生成推理能力委托给VLM，同时保留扩散模型的高质量视觉合成能力。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型中，多模态生成推理能力（指令理解、定位、图像引用）与高保真合成能力内在纠缠，需要解耦。

Method: 提出三阶段渐进训练策略：1) 通过多模态kontext令牌连接VLM和轻量扩散头；2) 扩展到预训练扩散模型增强视觉细节；3) 引入低级图像编码器提高保真度并进行指令调优。

Result: 实验表明该方法与强统一基线相当，在某些情况下甚至优于任务特定的最先进方法。

Conclusion: Query-Kontext成功解耦了多模态生成推理和视觉合成，为统一多模态模型提供了新的设计范式。
...
TL;DR: Stitch是一种无需训练的方法，通过自动生成的边界框将外部位置控制集成到多模态扩散变换器中，解决了T2I生成模型在空间关系理解方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的T2I生成模型在准确捕捉空间关系（如"上方"或"右侧"）方面存在持续挑战，且早期方法与现代模型架构不兼容。

Method: 通过自动生成边界框，在指定边界框内生成单个对象，然后无缝拼接在一起；利用目标注意力头在生成过程中隔离和裁剪单个对象。

Result: 在PosEval基准测试中，Stitch显著提升了基础模型性能，将FLUX在GenEval位置任务上提升了218%，在PosEval上提升了206%；与Qwen-Image结合在PosEval上比先前模型提升了54%。

Conclusion: Stitch实现了无需训练的位置控制集成，在空间准确性和视觉吸引力方面都表现出色，在位置基础的T2I生成任务上达到了最先进水平。
...
TL;DR: TTT3R是一种无需训练的测试时训练方法，通过基于记忆状态与观测对齐置信度的闭式学习率，显著提升3D重建模型在超出训练上下文长度时的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现代循环神经网络在3D重建中表现出色，但在超出训练上下文长度时性能显著下降，显示出有限的长度泛化能力。

Method: 从测试时训练角度重新审视3D重建基础模型，将其设计视为在线学习问题，利用记忆状态与观测对齐置信度推导闭式学习率来平衡历史信息保留与新观测适应。

Result: TTT3R显著改善了长度泛化，全局姿态估计性能比基线提升2倍，以20 FPS速度和仅6 GB GPU内存处理数千张图像。

Conclusion: 测试时训练视角为3D重建模型提供了有效的长度泛化解决方案，无需额外训练即可显著提升性能。
...
### [134] [Computational Design and Single-Wire Sensing of 3D Printed Objects with Integrated Capacitive Touchpoints](https://arxiv.org/abs/2509.25387)
*S. Sandra Bae,Takanori Fujiwara,Danielle Albers Szafir,Ellen Yi-Luen Do,Michael L. Rivera*

Main category: cs.GR

TL;DR: 提出了一种计算设计流程，可将多个电容触摸点嵌入到任何封闭无自相交的3D模型中，通过优化导电路径实现单线或双线连接的多区域触摸检测。


<details>
  <summary>Details</summary>
Motivation: 当前制作交互式3D打印对象需要繁琐的3D设计和后期电子器件安装，多材料3D打印和导电PLA为解决这些问题提供了机会。

Method: 用户定义3D物体表面的触摸区域，系统自动生成连接这些区域的导电路径，优化电阻-电容延迟以实现每个区域被触摸时产生独特的信号。

Result: 通过五个计算和传感性能评估（单线连接平均准确率达到93.35%）和六个应用示例验证了方法的有效性。

Conclusion: 该传感技术支持现有应用（如原型制作），并突显了完全通过3D打印生产交互设备的日益增长的前景。
...
### [135] [Interpolated Adaptive Linear Reduced Order Modeling for Deformation Dynamics](https://arxiv.org/abs/2509.25392)
*Yutian Tao,Maurizio Chiaramonte,Pablo Fernandez*

Main category: cs.GR

TL;DR: 提出了一种自适应线性降阶建模方法，通过动态调整降阶映射来显著提高变形动力学模拟的准确性，同时保持计算成本与传统方法相当。


<details>
  <summary>Details</summary>
Motivation: 传统线性降阶建模(ROM)由于采用固定的线性化降阶映射，在变形动力学模拟中的准确性受到限制。

Method: 采用自适应策略使降阶映射随变形状态动态变化，结合历史位移基和Grassmann插值处理大变形情况。

Result: 通过定量在线误差分析和与基于主成分分析(PCA)的线性ROM模拟的定性比较，证明该方法在保持相当计算成本的同时显著提高了准确性。

Conclusion: 所提出的自适应线性ROM方法能够显著提高变形动力学模拟的准确性，特别是在大变形情况下表现出更好的鲁棒性。
...
### [136] [MoReFlow: Motion Retargeting Learning through Unsupervised Flow Matching](https://arxiv.org/abs/2509.25600)
*Wontaek Kim,Tianyu Li,Sehoon Ha*

Main category: cs.GR

TL;DR: MoReFlow是一个基于流匹配的无监督运动重定向框架，通过学习角色运动嵌入空间之间的对应关系，实现跨形态角色的高质量运动重定向，无需配对数据。


<details>
  <summary>Details</summary>
Motivation: 现有运动重定向方法依赖手工约束或配对数据集，限制了其在人形角色之外的应用范围，且忽视了特定领域目标如动画风格保持或机器人任务空间对齐。

Method: 采用两阶段方法：首先使用VQ-VAE训练各角色的标记化运动嵌入，然后通过条件耦合的流匹配来对齐跨角色的潜在空间，同时学习条件和无条件匹配以实现鲁棒灵活的重定向。

Result: 实验表明MoReFlow能在多样化角色和任务中生成高质量运动，相比基线方法具有更好的可控性、泛化能力和运动真实感。

Conclusion: MoReFlow提供了一个灵活可逆的无监督运动重定向框架，无需配对数据即可实现跨角色运动转换，显著提升了运动重定向的质量和适用性。
...
### [137] [Vector sketch animation generation with differentialable motion trajectories](https://arxiv.org/abs/2509.25857)
*Xinding Zhu,Xinye Yang,Shuyang Zheng,Zhexin Zhang,Fei Gao,Jing Huang,Jiazhou Chen*

Main category: cs.GR

TL;DR: 提出一种端到端的矢量草图动画自动生成方法，通过可微分运动轨迹(DMT)表示解决闪烁问题，实现跨帧的全局语义梯度传播，提升时间一致性和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 基于视频的草图动画生成面临时间一致性要求的挑战，现有方法存在闪烁问题，需要解决帧间运动连贯性的技术难题。

Method: 引入可微分运动轨迹(DMT)表示，使用基于多项式的可微分轨迹描述笔画控制点的帧间运动；采用Bernstein基函数平衡多项式参数敏感性；引入稀疏轨迹点进行显式空间建模。

Result: 在DAVIS和LVOS数据集上的评估显示优于现有SOTA方法；在3D模型和文本到视频数据的跨域验证中表现出鲁棒性和兼容性。

Conclusion: DMT方法能够有效解决草图动画生成中的闪烁问题，实现高帧率输出，支持长时视频处理，具有较好的泛化能力。
...
### [138] [GaussEdit: Adaptive 3D Scene Editing with Text and Image Prompts](https://arxiv.org/abs/2509.26055)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有3D场景编辑方法在编辑精度、视觉真实性和处理速度方面的不足，提供更高效、精确的用户驱动3D编辑工具。

Method: 三阶段流程：1）初始化3D高斯确保高质量编辑；2）自适应全局-局部优化策略平衡场景一致性和局部细节，使用类别引导正则化缓解Janus问题；3）通过图像到图像合成技术增强编辑对象纹理。

Result: 实验结果表明，GaussEdit在编辑精度、视觉保真度和处理速度方面均优于现有方法，能够成功将用户指定概念嵌入3D场景。

Conclusion: GaussEdit是一个强大的3D场景编辑工具，相比传统方法有显著改进，支持详细和用户驱动的3D场景编辑。
...
### [139] [Palace: A Library for Interactive GPU-Accelerated Large Tensor Processing and Visualization](https://arxiv.org/abs/2509.26213)
*Dominik Drees,Benjamin Risse*

Main category: cs.GR

TL;DR: Palace是一个开源、跨平台的通用库，用于交互式和加速的核外张量处理和可视化，通过高性能异步并发架构和简单计算图接口实现工作站硬件上的交互式核外管道开发。


<details>
  <summary>Details</summary>
Motivation: 随着成像和仿真技术的进步，张量数据集规模不断增加，需要核外处理和可视化技术，但目前多为专用解决方案。

Method: 采用高性能异步并发架构和简单计算图接口，支持交互式核外管道开发。

Result: 在体积渲染和分层随机游走分割基准测试中，Palace优于或匹配最先进系统，适用于从2D图像到4D时间序列数据集的张量处理。

Conclusion: Palace提供了一个通用、高效的核外张量处理和可视化解决方案，在多种应用场景中表现出色。
...
### [140] [3DiFACE: Synthesizing and Editing Holistic 3D Facial Animation](https://arxiv.org/abs/2509.26233)
*Balamurugan Thambiraja,Malte Prinzler,Sadegh Aliakbarian,Darren Cosker,Justus Thies*

Main category: cs.GR

TL;DR: 3DiFACE是一种创新的语音驱动3D面部动画方法，能够为单一音频输入生成多样化的唇部和头部运动，并支持通过关键帧和插值进行编辑。


<details>
  <summary>Details</summary>
Motivation: 当前语音驱动的3D面部动画方法在创建个性化动画时面临挑战，特别是在精确控制和编辑方面。现有方法大多专注于控制风格或情感，无法编辑/重新生成部分输入动画，且忽视了同一音频输入可能对应多种合理的唇部和头部运动。

Method: 提出了一种全卷积扩散模型，利用训练语料库中的音素级多样性。采用说话风格个性化方法和新颖的稀疏引导运动扩散技术，实现精确控制和编辑。

Result: 通过定量和定性评估表明，该方法能够在单一音频输入下生成和编辑多样化的整体3D面部动画，并在高保真度和多样性之间实现控制。
...
### [141] [SOLD: SELFIES-based Objective-driven Latent Diffusion](https://arxiv.org/abs/2509.25198)
*Elbert Ho*

Main category: cs.LG

TL;DR: 提出了SOLD模型，一种基于SELFIES字符串的潜在扩散模型，用于生成针对目标蛋白的高亲和力分子，相比现有3D构象空间方法更简单高效。
...
Method: 开发了SOLD模型，使用1D SELFIES字符串的潜在空间，结合潜在扩散模型和创新的SELFIES transformer，并提出了多任务机器学习模型的损失平衡方法。

Result: 模型能够以简单高效的方式为目标蛋白生成高亲和力分子，同时为未来通过添加更多数据实现改进留下了空间。

Conclusion: SOLD模型提供了一种更简单高效的分子生成方法，相比现有3D方法具有明显优势，且具有良好的可扩展性。
...
### [142] [VLHSA: Vision-Language Hierarchical Semantic Alignment for Jigsaw Puzzle Solving with Eroded Gaps](https://arxiv.org/abs/2509.25202)
...
Method: 提出视觉-语言层次语义对齐(VLHSA)模块，通过从局部标记到全局上下文的多层次语义匹配，将视觉块与文本描述对齐。采用结合双视觉编码器和语言特征的多模态架构进行跨模态推理。

Result: 在多个数据集上显著优于最先进模型，实现了包括14.2个百分点碎片准确率提升在内的显著改进。消融研究证实VLHSA模块对视觉方法改进的关键作用。

Conclusion: 通过融入多模态语义洞察，为拼图求解建立了新范式，证明语言上下文能有效增强拼图组装性能。
...
### [143] [Spectral Logit Sculpting: Adaptive Low-Rank Logit Transformation for Controlled Text Generation](https://arxiv.org/abs/2509.25204)
*Jin Li,Zhebo Wang,Tianliang Lu,Mohan Li,Wenpeng Xing,Meng Han*

Main category: cs.LG

TL;DR: 提出了一种轻量级推理时优化方法SLS，通过谱分析和熵特性动态调节token分布，在不确定性高时自适应调整logits，无需更新模型参数即可提升LLM可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵的推理方法计算开销大且未能有效利用历史token上下文，需要更高效的推理时优化方案来提升大语言模型的可靠性。

Method: SLS方法维护top-K logits滑动缓冲区，执行在线奇异值分解识别主导谱方向，基于熵和logit间隙统计自适应重新缩放logits，仅在不确定性高时激活。

Result: 在多个公开基准测试中，SLS在数学、编程和科学推理任务上持续优于现有基线方法，实现了更优的准确率。

Conclusion: SLS是一种有效的轻量级推理时优化方法，能够在不更新模型参数的情况下锐化输出分布并保持上下文一致性，显著提升LLM性能。
...
### [144] [Polynomial Contrastive Learning for Privacy-Preserving Representation Learning on Graphs](https://arxiv.org/abs/2509.25205)
*Daksh Pandey*

Main category: cs.LG

TL;DR: 提出了Poly-GRACE框架，这是一个与同态加密兼容的图自监督学习方法，解决了传统SSL方法无法与隐私保护技术结合的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法（如GRACE）由于依赖非多项式操作，无法与同态加密等隐私保护技术兼容，这限制了在隐私敏感场景下的应用。

Method: 开发了完全多项式友好的图卷积网络编码器和基于多项式的新型对比损失函数，使整个框架与同态加密兼容。

Result: 在Cora、CiteSeer和PubMed三个基准数据集上的实验表明，Poly-GRACE不仅支持私有预训练，而且在性能上与标准非私有基线相当，在CiteSeer数据集上甚至表现更优。
...
### [145] [Hyperbolic Optimization](https://arxiv.org/abs/2509.25206)
*Yanke Wang,Kyriakos Flouris*

Main category: cs.LG

TL;DR: 本文提出了在双曲流形上的优化方法，包括双曲SGD和双曲Adam优化器，并展示了在扩散模型训练中的应用。


<details>
  <summary>Details</summary>
Motivation: 基于黎曼优化原理，将优化方法扩展到双曲流形上，特别是在庞加莱球上的学习，这种表示可以在训练早期阶段加速收敛。

Method: 扩展了双曲随机梯度下降（Riemannian SGD的特化）到双曲Adam优化器，并在扩散模型中使用双曲时间离散化的朗之万动力学进行训练。

Result: 在特定数据集上实现了更快的收敛速度，同时没有牺牲生成质量。

Conclusion: 双曲优化方法在训练早期阶段能够加速收敛，特别是在参数远离最优值时，为庞加莱嵌入学习提供了优势。
...
### [146] [Multi-level Diagnosis and Evaluation for Robust Tabular Feature Engineering with Large Language Models](https://arxiv.org/abs/2509.25207)
*Yebin Lim,Susik Yoon*

Main category: cs.LG

TL;DR: 提出多级诊断评估框架评估LLM在特征工程中的鲁棒性，发现LLM生成的高质量特征可将少样本预测性能提升10.52%


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在表格数据特征工程中展现出潜力，但由于生成输出的变异性，对其可靠性的担忧依然存在

Method: 引入多级诊断和评估框架，从关键变量、关系和决策边界值三个主要因素评估LLM在特征工程中的鲁棒性
...
### [147] [DPSformer: A long-tail-aware model for improving heavy rainfall prediction](https://arxiv.org/abs/2509.25208)
...
<details>
  <summary>Details</summary>
Motivation: 强降雨预测面临数据分布不平衡的挑战——大多数观测记录无雨或小雨，而强降雨事件罕见。这种不平衡分布阻碍深度学习模型有效预测强降雨事件。

Method: 将降雨预测明确视为长尾学习问题，引入DPSformer模型，通过高分辨率分支丰富强降雨事件的表征。

Result: 对于≥50mm/6h的强降雨事件，DPSformer将基线数值天气预报模型的CSI从0.012提升到0.067；对于前1%的强降雨事件，其FSS超过0.45，优于现有方法。

Conclusion: 该研究为强降雨预测建立了有效的长尾学习范式，为增强早期预警系统和减轻极端天气事件的社会影响提供了实用工具。
...
### [148] [STCast: Adaptive Boundary Alignment for Global and Regional Weather Forecasting](https://arxiv.org/abs/2509.25210)
*Hao Chen,Tao Han,Jie Zhang,Song Guo,Lei Bai*

Main category: cs.LG

TL;DR: 提出STCast框架，通过空间对齐注意力机制自适应优化区域边界，并使用时间混合专家模块动态分配月度预测，在区域天气预报中超越现有方法。
...
Method: 使用空间对齐注意力机制对齐全局和区域空间分布来初始化边界，并基于注意力模式自适应优化；设计时间混合专家模块，通过离散高斯分布将不同月份大气变量动态路由到专门专家。

Result: 在全局和区域预测、极端事件预测和集合预测四个任务上均优于现有最先进方法。

Conclusion: STCast框架通过自适应边界优化和动态时间分配，显著提升了区域天气预报的性能和泛化能力。
...
### [149] [LEMs: A Primer On Large Execution Models](https://arxiv.org/abs/2509.25211)
*Remi Genet,Hugo Inzirillo*

Main category: cs.LG

TL;DR: LEMs是一种新型深度学习框架，将基于Transformer的架构扩展到解决具有灵活时间边界和多重执行约束的复杂执行问题，通过解耦市场信息处理和分配决策实现统一模型架构。


<details>
  <summary>Details</summary>
Motivation: 现有神经VWAP执行策略主要针对固定时长订单，无法处理执行时长在最小和最大时间边界之间变化的场景，类似于股票回购合约结构。需要开发能够处理灵活时间约束的统一执行模型。

Method: 提出解耦架构：使用TKANs、VSNs和多头注意力机制处理市场数据提取特征，独立分配网络处理不同场景的执行逻辑。支持固定数量vs固定名义价值、买入vs卖出订单等多种执行目标。

Result: 在日内加密货币市场和多日股票交易（道琼斯成分股）上的实证评估表明，LEMs在灵活时间约束内动态优化执行路径，相比传统基准方法获得更优的执行性能。

Conclusion: LEMs的统一模型架构能够通过单一框架部署到不同执行场景，相比资产特定方法具有显著操作优势，为复杂执行问题提供了有效的深度学习解决方案。
...
### [150] [Six Sigma For Neural Networks: Taguchi-based optimization](https://arxiv.org/abs/2509.25213)
*Sai Varun Kodathala*

Main category: cs.LG

TL;DR: 本研究应用田口实验设计方法优化CNN超参数，用于拳击动作识别。通过L12正交阵列评估8个超参数，开发5种多目标优化方法，其中方法3结合加权精度和对数变换损失函数，获得98.84%训练精度和86.25%验证精度。


<details>
  <summary>Details</summary>
Motivation: CNN超参数优化通常需要大量试错或网格搜索，计算成本高且效率低。本研究旨在将传统质量工程中的田口实验设计方法应用于CNN超参数优化，提高优化效率和系统性。

Method: 使用L12(211)正交阵列系统评估8个超参数：图像尺寸、颜色模式、激活函数、学习率、重缩放、洗牌、垂直翻转和水平翻转。开发5种多目标优化方法，结合信噪比分析，采用对数缩放技术统一冲突指标。

Result: 方法3表现最佳，达到98.84%训练精度和86.25%验证精度，同时保持最小损失值。田口分析显示学习率是最重要参数，其次是图像尺寸和激活函数。

Conclusion: 田口实验设计方法能有效优化CNN超参数，提供清晰的参数优先级指导。该方法为机器学习优化提供了系统化的多目标优化框架，显著提高了优化效率。
...
### [151] [On-the-Fly Adaptation to Quantization: Configuration-Aware LoRA for Efficient Fine-Tuning of Quantized LLMs](https://arxiv.org/abs/2509.25214)
...
<details>
  <summary>Details</summary>
Motivation: 随着大型预训练模型的发布，在边缘设备上部署这些模型需要有效的压缩方法。现有的量化与高精度LoRA适配器微调结合的方法虽然能减少模型大小，但为每个量化配置单独微调计算成本过高，且边缘设备能力异构。

Method: 提出CoA-LoRA方法，通过配置感知模型将每个量化配置映射到其低秩调整，无需重复微调。采用基于Pareto的配置搜索迭代优化训练配置集，生成更精确的低秩调整。

Result: 实验表明，CoA-LoRA在不需要为每个配置单独微调LoRA适配器的情况下，实现了与最先进方法相当甚至更优的性能，且不产生额外时间成本。

Conclusion: CoA-LoRA提供了一种高效的方法，能够在边缘设备上动态适应不同的量化配置，解决了现有方法计算成本高的问题，为隐私保护应用中的模型部署提供了可行方案。
...
### [152] [Anomaly detection by partitioning of multi-variate time series](https://arxiv.org/abs/2509.25215)
*Pierre Lotte,André Péninou,Olivier Teste*

Main category: cs.LG

TL;DR: 提出了一种名为PARADISE的无监督分区异常检测方法，用于多元时间序列异常检测。该方法通过聚类变量间的相关系数来划分变量子集，并在每个子集上执行局部异常检测算法。


<details>
  <summary>Details</summary>
Motivation: 现有的多元时间序列异常检测方法可能无法有效处理变量间复杂的关系，需要一种能够保持变量间关系不变的分区方法。

Method: 基于变量间相关系数的聚类来划分时间序列变量，确保变量间关系不被破坏，然后在每个变量子集上局部执行异常检测算法。

Result: 在合成数据集和真实数据集上的实验表明，该方法显著提高了异常检测性能。

Conclusion: PARADISE方法通过保持变量间关系的分区策略，在多元时间序列异常检测中取得了显著改进。
...
### [153] [Evaluating Double Descent in Machine Learning: Insights from Tree-Based Models Applied to a Genomic Prediction Task](https://arxiv.org/abs/2509.25216)
*Guillermo Comesaña Cimadevila*

Main category: cs.LG

TL;DR: 该论文重新审视了机器学习中的双下降现象，发现在生物分类任务中，只有当模型复杂度在两个正交维度（学习器容量和集成规模）上同时扩展时，才会出现双下降现象。


<details>
  <summary>Details</summary>
Motivation: 研究双下降现象在简单模型（如决策树和梯度提升）中的表现，特别是在生物医学分类任务中的适用性。

Method: 使用全基因组测序数据预测结核分枝杆菌的异烟肼耐药性，系统性地在学习和集成规模两个维度上变化模型复杂度。

Result: 只有当两个复杂度维度同时扩展时才会出现双下降现象；如果固定其中一个维度，泛化行为会回归到经典的U形或L形模式。

Conclusion: 模型复杂度应被视为多维结构，双下降现象源于不同泛化机制在单一复杂度轴上的投影。
...
### [154] [Learning to Condition: A Neural Heuristic for Scalable MPE Inference](https://arxiv.org/abs/2509.25217)
*Brij Malhotra,Shivvrat Arya,Tahrima Rahman,Vibhav Giridhar Gogate*

Main category: cs.LG

TL;DR: L2C是一个可扩展的数据驱动框架，通过训练神经网络来评估变量赋值对条件化的效用，从而加速概率图模型中最可能解释(MPE)推理。


<details>
  <summary>Details</summary>
Motivation: MPE推理在概率图模型中是一个基本难解的问题，需要开发可扩展的方法来加速推理过程。

Method: 开发了可扩展的数据生成管道，从现有MPE求解器的搜索轨迹中提取训练信号，训练神经网络作为启发式函数，与搜索算法集成。

Result: 在涉及高树宽PGM的挑战性MPE查询上，学习到的启发式函数显著减少了搜索空间，同时保持或改进了最先进方法的解质量。

Conclusion: L2C框架成功地将学习技术应用于MPE推理加速，展示了数据驱动方法在解决难解推理问题上的潜力。
...
### [155] [On The Dynamic Ensemble Selection for TinyML-based Systems -- a Preliminary Study](https://arxiv.org/abs/2509.25218)
...
<details>
  <summary>Details</summary>
Motivation: TinyML技术在计算、内存和能源方面的特定约束需要专门的优化技术，以平衡推理时间和分类质量。

Method: 开发了TinyDES-Clustering库，采用动态集成选择聚类方法，针对嵌入式系统限制进行优化。

Result: 实验表明，更大的分类器池能提高分类精度，但会增加TinyML设备的平均推理时间。

Conclusion: DES-Clustering方法能够有效调整TinyML系统中的分类精度，但需要在精度和推理时间之间进行权衡。
...
### [156] [Sensor optimization for urban wind estimation with cluster-based probabilistic framework](https://arxiv.org/abs/2509.25222)
...
TL;DR: 提出了一种基于物理信息的机器学习框架，用于复杂城市环境中无人机轨迹的传感器流量估计，能够处理复杂流动、外推训练数据并优化传感器位置。


<details>
  <summary>Details</summary>
Motivation: 传统流量估计方法难以处理复杂城市环境中的流动问题，且大多数文献中传感器位置是固定的，限制了应用范围。

Method: 采用雷诺数缩放、基于物理的域分解、子域聚类流表示、信息熵关联和多变量概率函数五个关键技术。

Result: 框架在三个建筑群的无人机飞行路径上成功演示，能够估计速度和不确定性，并优化传感器位置。

Conclusion: 该框架可扩展到完整城市规模，并有望整合天气输入，具有广泛的应用前景。
...
### [157] [Enhancing Linear Attention with Residual Learning](https://arxiv.org/abs/2509.25223)
...
Method: 引入RLA框架，维护辅助循环状态来累积残差误差并修正基础预测；实例化残差Delta网络（RDN），包含自适应门控和残差裁剪以增强校正控制和稳定性。
...
### [158] [AMLA: MUL by ADD in FlashAttention Rescaling](https://arxiv.org/abs/2509.25224)
...
TL;DR: AMLA是针对华为昇腾NPU优化的高性能内核，通过FlashAttention算法和预加载流水线策略，显著提升MLA在解码阶段的性能，达到614 TFLOPS和86.8%的理论FLOPS利用率。


<details>
  <summary>Details</summary>
Motivation: 多头潜在注意力(MLA)虽然显著减少了KVCache内存使用，但带来了大量计算开销和中间变量扩展，这对硬件实现效率提出了挑战，特别是在解码阶段。
...
### [159] [MSCoD: An Enhanced Bayesian Updating Framework with Multi-Scale Information Bottleneck and Cooperative Attention for Structure-Based Drug Design](https://arxiv.org/abs/2509.25225)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于结构的药物设计方法难以捕捉蛋白质-配体相互作用的多尺度层次结构和内在不对称性，限制了分子设计的准确性。
...
Conclusion: MSCoD通过有效建模多尺度层次结构和不对称相互作用，为基于结构的药物设计提供了更准确的生成框架。
...
### [160] [Integrated Forecasting of Marine Renewable Power: An Adaptively Bayesian-Optimized MVMD-LSTM Framework for Wind-Solar-Wave Energy](https://arxiv.org/abs/2509.25226)
*Baoyi Xie,Shuiling Shi,Wenqi Liu*

Main category: cs.LG

TL;DR: 提出了贝叶斯优化的多元变分模态分解-长短期记忆网络框架，用于集成风-光-波海洋能源系统的超短期功率预测，显著提升了预测精度和自动化程度。
...
Method: 使用MVMD联合分解风、光、波功率序列以保持跨源耦合；采用贝叶斯优化自动搜索MVMD过程中的模式数量和惩罚参数；最后用LSTM对得到的本征模态函数进行建模，实现集成系统的超短期功率预测。

Result: 基于中国某海上集成能源平台的现场测量数据实验表明，该框架在MAPE、RMSE和MAE指标上显著优于基准模型。

Conclusion: 该方法展示了优越的预测精度、鲁棒性和自动化程度，为集成海洋能源系统的安全运行和优化调度提供了有效支持。
...
### [161] [Simple, Fast and Efficient Injective Manifold Density Estimation with Random Projections](https://arxiv.org/abs/2509.25228)
*Ahmad Ayaz Amin*
...
### [162] [Energy Guided Geometric Flow Matching](https://arxiv.org/abs/2509.25230)
*Aaron Zweig,Mingxuan Zhang,Elham Azizi,David Knowles*

Main category: cs.LG

TL;DR: 提出使用分数匹配和退火能量蒸馏学习度量张量，以更准确地捕捉数据几何结构并指导流生成，解决传统流匹配方法在高维空间中的局限性。


<details>
  <summary>Details</summary>
Motivation: 时间数据的一个重要归纳偏置是轨迹应保持在数据流形附近。传统流匹配方法依赖直线条件路径，而学习测地线的流匹配方法使用RBF核或最近邻图，这些方法在高维空间中面临维度灾难问题。

Method: 使用分数匹配和退火能量蒸馏来学习一个忠实捕捉底层数据几何结构的度量张量，从而指导更准确的流生成。

Result: 在具有解析测地线的合成流形上验证了该策略的有效性，并应用于细胞插值任务。

Conclusion: 所提出的方法能够更准确地建模数据几何结构，为时间数据生成提供更好的流匹配解决方案。
...
### [163] [WDformer: A Wavelet-based Differential Transformer Model for Time Series Forecasting](https://arxiv.org/abs/2509.25231)
*Xiaojian Wang,Chaoli Zhang,Zhonglong Zheng,Yunliang Jiang*

Main category: cs.LG

TL;DR: WDformer是一种基于小波变换的差分Transformer模型，通过多分辨率分析和差分注意力机制提升时间序列预测性能，在多个真实数据集上达到SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测方法仅依赖时域或频域建模，无法充分利用多域信息，且传统注意力机制容易过度关注不相关的历史信息，引入噪声导致预测偏差。

Method: 使用小波变换对时间序列进行多分辨率分析，结合时频域联合表示准确提取关键信息；采用维度反转的注意力机制捕捉多变量关系；引入差分注意力机制，通过两个独立softmax注意力矩阵的差异计算注意力分数。

Result: 在多个具有挑战性的真实世界数据集上取得了最先进的(SOTA)结果，证明了模型的准确性和有效性。

Conclusion: WDformer通过小波变换和差分注意力机制有效解决了时间序列预测中的多域信息利用和噪声抑制问题，在多个应用场景中表现出优越性能。
...
### [164] [Sampling via Gaussian Mixture Approximations](https://arxiv.org/abs/2509.25232)
*Yongchao Huang*

Main category: cs.LG

TL;DR: 提出了一系列高斯混合近似（GMA）采样器，用于对未归一化目标密度进行采样，包括仅权重GMA、拉普拉斯混合近似、期望最大化GMA等变体。该方法采用两阶段范式：从高斯混合提议中采样，然后通过优化组件权重或均值和方差来拟合目标密度。


<details>
  <summary>Details</summary>
Motivation: 开发一种无需梯度、计算高效的采样方法，能够从复杂的未归一化目标密度中生成忠实样本，克服传统采样方法的局限性。

Method: 两阶段方法：1）初始化高斯组件并从混合提议中采样；2）通过基于样本的KL散度目标优化组件权重或均值和方差，然后进行分层重采样。使用投影梯度下降、镜像下降和EM等高效优化方法。

Result: 该方法在温和条件下产生一致的近似，经验验证显示在各种密度函数上具有高精度和快速采样速度。

Conclusion: GMA采样器提供了一种梯度无关、计算高效的采样框架，能够准确快速地近似复杂目标分布，适用于各种未归一化密度函数的采样任务。
...
### [165] [FedCLF -- Towards Efficient Participant Selection for Federated Learning in Heterogeneous IoV Networks](https://arxiv.org/abs/2509.25233)
*Kasun Eranda Wijethilake,Adnan Mahmood,Quan Z. Sheng*

Main category: cs.LG

TL;DR: 提出FedCLF方法，在联邦学习中引入校准损失和反馈控制机制，解决车辆物联网中数据异构性高的问题，提升模型准确性和资源利用效率。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在车辆物联网等高动态、异构环境中面临数据异构性挑战，需要提升模型准确性和资源利用效率。

Method: FedCLF方法：1) 在校验损失作为效用函数进行参与者选择；2) 反馈控制机制动态调整客户端采样频率。

Result: 在CIFAR-10数据集上测试，FedCLF相比FedAvg、Newt、Oort基线模型，在高数据异构性场景下准确率提升高达16%，同时通过降低采样频率提高了效率。

Conclusion: FedCLF能有效应对联邦学习中的数据异构性挑战，在车辆物联网等资源受限环境中实现更好的性能和效率平衡。
...
### [166] [Machine Learning for Pattern Detection in Printhead Nozzle Logging](https://arxiv.org/abs/2509.25235)
*Nikola Prianikov,Evelyne Janssen-van Dam,Marcin Pietrasik,Charalampos S. Kouzinopoulos*

Main category: cs.LG

TL;DR: 提出了一种基于机器学习的打印头故障分类方法，通过提取喷嘴日志数据的时间和空间特征，使用一对多随机森林分类器优于基于规则的基准方法。


<details>
  <summary>Details</summary>
Motivation: 准确识别打印头故障机制对制造商确保产品质量至关重要，佳能生产打印的打印头故障可以通过单个喷嘴行为模式来识别。

Method: 采用基于特征的时间序列分类框架，在领域专家指导下选择时间和空间特征，评估多种传统ML分类器，最终选择一对多随机森林。
...
### [167] [PALADIN: Self-Correcting Language Model Agents to Cure Tool-Failure Cases](https://arxiv.org/abs/2509.25238)
...
<details>
  <summary>Details</summary>
Motivation: 现有的工具增强语言代理在真实部署中经常因工具故障（如超时、API异常、输出不一致）而失败，导致级联推理错误和任务放弃。现有训练管道只优化成功轨迹，未能暴露模型在真实使用中占主导的故障情况。

Method: PALADIN在增强的ToolBench数据集上，通过系统化故障注入和专家演示构建了50,000+恢复注释轨迹进行训练。使用LoRA微调保留基础能力的同时注入恢复能力。推理时检测执行时错误，从55+故障示例库中检索最相似案例并执行相应恢复操作。

Result: 在PaladinEval和ToolReflectEval评估中，恢复率从32.76%提升至89.68%（相对提升57%），优于最强基线CRITIC（76.34%）13.3%。对未见工具API保持95.2%的恢复性能。

Conclusion: PALADIN是构建在真实工具环境中具有鲁棒恢复能力的容错代理的有效方法，显著提升了故障恢复能力和任务成功率。
...
### [168] [HAMMER: Hamiltonian Curiosity Augmented Large Language Model Reinforcement](https://arxiv.org/abs/2509.25240)
*Ming Yang,Xiaofan Li,Zhiyuan Ma,Dengliang Shi,Jintao Du,Yu Cheng,Weiguo Zheng*

Main category: cs.LG

TL;DR: HAMMER提出了一种新颖的课程强化学习方法，通过多样性指标和最小语义哈密顿路径来排序训练样本，增强模型探索能力，在多个推理基准上平均提升3-4%准确率。
...
Method: 将数据集评估中常用的多样性指标引入动态强化学习过程，通过最小语义哈密顿路径对训练样本进行排序，使初始训练保持更多探索。

Result: HAMMER激发了模型的"好奇心"，在多个推理基准上平均获得3%到4%的准确率提升。

Conclusion: 从泛化界限的理论角度看，多样性驱动的排序有助于稳定收敛，HAMMER方法有效解决了课程强化学习中的探索问题。
...
### [169] [Fine-tuning of Large Language Models for Domain-Specific Cybersecurity Knowledge](https://arxiv.org/abs/2509.25241)
*Yuan Huang*

Main category: cs.LG

TL;DR: 研究探索了三种微调策略（SFT、LoRA、QLoRA）来将网络安全知识嵌入大语言模型，在网络安全问答任务中显著提升性能，同时LoRA和QLoRA能以更低计算成本达到与SFT相当的效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在需要专业知识的专门领域（如网络安全）的零样本表现往往不理想，因为基础LLMs是为通用目的设计的，难以在参数空间中封装领域特定知识。

Method: 使用网络安全问答数据集，研究监督微调（SFT）、低秩适应（LoRA）和量化低秩适应（QLoRA）三种微调方法。

Result: 所有微调方法在网络安全问答任务中都显著优于基础模型，且LoRA和QLoRA能以显著更低的计算成本达到与SFT相当的性能。

Conclusion: 低秩微调策略有潜力弥合通用大语言模型与领域特定应用之间的差距，提供高效的领域适应路径。
...
### [170] [Knowledge distillation through geometry-aware representational alignment](https://arxiv.org/abs/2509.25253)
*Prajjwal Bhattarai,Mohammad Amjad,Dmytro Zhylko,Tuka Alhanai*

Main category: cs.LG

TL;DR: 本文分析了现有特征蒸馏方法的理论缺陷，提出使用Procrustes距离和特征Gram矩阵的Frobenius范数作为蒸馏损失，在BERT和OPT模型上实现了蒸馏性能的显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统特征蒸馏方法（如投影均方损失或CKA）无法有效捕捉教师模型的特征结构，即使损失为零时也存在理论缺陷。

Method: 提出使用Procrustes距离和特征Gram矩阵的Frobenius范数作为蒸馏损失函数，这些距离在表示对齐测量中已有应用。

Result: 在BERT和OPT模型族的分类和指令跟随任务中，该方法使蒸馏性能提升了高达2个百分点，具有统计显著性。

Conclusion: 将特征几何整合到现有蒸馏方法中具有巨大潜力，能有效提升知识蒸馏的性能。
...
### [171] [Heterogeneous Multi-agent Collaboration in UAV-assisted Mobile Crowdsensing Networks](https://arxiv.org/abs/2509.25261)
...
Method: 提出联合优化框架，集成时隙划分、资源分配和无人机3D轨迹规划；将问题建模为部分可观测马尔可夫决策过程，采用基于异构智能体近端策略优化的多智能体深度强化学习算法，结合CNN特征提取和KAN网络捕捉状态-动作依赖关系。

Result: 大量数值结果表明，与基准方法相比，所提方法在处理感知数据量方面取得了显著提升。

Conclusion: 所提出的混合演员网络MADRL算法能有效解决无人机辅助移动群智感知中的联合优化问题，显著提高数据处理效率。
...
### [172] [How Effective Are Time-Series Models for Rainfall Nowcasting? A Comprehensive Benchmark for Rainfall Nowcasting Incorporating PWV Data](https://arxiv.org/abs/2509.25263)
*Yifang Zhang,Pengfei Duan,Henan Wang,Shengwu Xiong*

Main category: cs.LG

TL;DR: 提出了RainfallBench降雨临近预报基准，包含5年全球12000多个GNSS站点的气象数据，特别包含可降水量(PWV)指标。针对降雨预报的零膨胀、时间衰减和非平稳性挑战，设计了专门的评估策略，并提出了Bi-Focus Precipitation Forecaster (BFPF)模块来提升预报性能。


<details>
  <summary>Details</summary>
Motivation: 现有气象时间序列预测基准主要评估温度、湿度等具有强周期性的变量，无法反映模型在降雨临近预报这种更复杂实际场景中的能力。降雨预报具有零膨胀、时间衰减和非平稳性等独特挑战，需要专门的基准来评估。

Method: 构建了RainfallBench数据集，包含5年15分钟间隔的6个气象变量数据，特别加入了可降水量(PWV)指标。设计了针对多尺度预测和极端降雨事件的评估策略。提出了BFPF模块，通过融入领域先验知识来解决零膨胀和时间衰减问题。

Result: 在RainfallBench上评估了20多个最先进模型，涵盖6种主要架构。统计分析和消融研究验证了数据集的全面性和方法的优越性。BFPF模块能有效提升降雨时间序列预测性能。

Conclusion: RainfallBench填补了降雨临近预报基准的空白，为评估模型在复杂实际气象场景中的能力提供了标准。BFPF模块通过融入领域先验知识，有效解决了降雨预报中的特殊挑战，为降雨预报研究提供了新的方向。
...
### [173] [Dynamic Policy Induction for Adaptive Prompt Optimization: Bridging the Efficiency-Accuracy Gap via Lightweight Reinforcement Learning](https://arxiv.org/abs/2509.25267)
*Jiexi Xu*

Main category: cs.LG

TL;DR: 提出了Prompt Policy Network (PPN)，一个轻量级强化学习框架，通过自适应策略选择在效率和准确性之间实现更好的权衡，相比Self-Consistency可减少61.5%的token成本。


<details>
  <summary>Details</summary>
Motivation: 现有静态提示策略（如Zero-Shot、Few-Shot、Chain-of-Thought）在效率和准确性之间存在刚性权衡，准确策略如Self-Consistency在简单任务上浪费计算资源，而轻量方法在复杂输入上表现不佳。

Method: 将自适应策略选择形式化为单步马尔可夫决策过程，使用PPO训练PPN网络，通过资源显式奖励函数学习仅在必要时分配成本高昂的推理策略。

Result: 在算术推理基准测试中，PPN在效率-准确性帕累托前沿上表现优异，相比Self-Consistency可减少61.5%的token成本，同时保持竞争力准确性。

Conclusion: 为成本高效的LLM部署提供了一个系统化、自适应的框架，推进了轻量级优化技术在可扩展和可持续语言模型应用中的设计。
...
### [174] [A Weather Foundation Model for the Power Grid](https://arxiv.org/abs/2509.25268)
...
<details>
  <summary>Details</summary>
Motivation: 天气基础模型虽然在天气预报方面表现出色，但其对现代社会天气敏感基础设施的实际价值尚未充分探索。该研究旨在验证这些模型在电网韧性智能方面的应用潜力。

Method: 使用Silurian AI的15亿参数生成式预测变换器(GFT)天气基础模型，在魁北克水电公司的丰富资产观测数据上进行微调，包括输电线路气象站、风电场测风塔和结冰传感器数据。

Result: 在6-72小时预报时效内，定制化模型超越了最先进的数值天气预报基准：温度平均绝对误差降低15%，总降水量误差降低35%，风速误差降低15%。最重要的是，实现了雾凇检测0.72的平均精度得分，这是现有业务系统不具备的能力。

Conclusion: 研究表明，天气基础模型通过少量高保真数据的后训练，可以作为下一代电网韧性智能的实用基础，为潜在的灾难性停电事件提供数小时的可操作预警。
...
### [175] [InfMasking: Unleashing Synergistic Information by Contrastive Multimodal Interactions](https://arxiv.org/abs/2509.25270)
...
Method: 使用无限掩码策略随机遮挡每个模态的大部分特征，仅保留部分信息来创建具有不同协同模式的表示，并通过互信息最大化来编码全面的协同信息。

Result: 在受控实验中验证了InfMasking能有效增强模态间的协同信息，在七个大规模真实数据集上达到最先进性能。
...
### [176] [MAESTRO : Adaptive Sparse Attention and Robust Learning for Multimodal Dynamic Time Series](https://arxiv.org/abs/2509.25278)
*Payal Mohapatra,Yueyuan Sui,Akash Pandey,Stephen Xia,Qi Zhu*

Main category: cs.LG

TL;DR: MAESTRO是一个新颖的多模态学习框架，解决了现有方法依赖单一主模态、成对建模和完整模态观察的局限性，通过动态跨模态交互和稀疏专家混合机制，在完整和部分缺失模态情况下都表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法在现实世界应用中存在三个主要问题：依赖单一主模态对齐、成对建模方式不适用于多模态场景、假设模态观察完整。这些限制阻碍了在多模态时间序列设置中的实际应用。

Method: MAESTRO通过符号标记化和自适应注意力预算构建长多模态序列，使用稀疏跨模态注意力处理，并通过稀疏专家混合机制实现不同模态组合下的黑盒专业化。

Result: 在四个数据集上的评估显示，MAESTRO在完整观察下比现有多模态和多变量方法分别平均提升4%和8%，在40%模态缺失情况下平均提升9%。

Conclusion: MAESTRO的稀疏、模态感知设计为动态时间序列学习提供了鲁棒性和效率，能够有效处理现实世界中的多模态挑战。
...
### [177] [Optimisation of Resource Allocation in Heterogeneous Wireless Networks Using Deep Reinforcement Learning](https://arxiv.org/abs/2509.25284)
*Oluwaseyi Giwa,Jonathan Shock,Jaco Du Toit,Tobi Awodumila*

Main category: cs.LG

TL;DR: 提出基于深度强化学习的异构无线网络资源分配框架，在动态网络环境下优于传统启发式算法


<details>
  <summary>Details</summary>
Motivation: 异构无线网络中传统方法难以应对变化的用户负载和信道条件，需要更智能的资源分配方案

Method: 使用深度强化学习框架联合优化传输功率、带宽和调度，采用多目标奖励函数平衡吞吐量、能效和公平性，比较PPO和TD3算法

Result: 在多种网络场景下，DRL框架在优化动态网络资源分配方面优于三种启发式算法

Conclusion: 研究结果突显了未来异构无线网络中DRL设计的关键权衡
...
### [178] [ClustRecNet: A Novel End-to-End Deep Learning Framework for Clustering Algorithm Recommendation](https://arxiv.org/abs/2509.25289)
...
<details>
  <summary>Details</summary>
Motivation: 解决聚类算法选择这一长期存在的挑战，减少对手工元特征和传统聚类有效性指标的依赖。

Method: 构建包含34,000个合成数据集的综合数据仓库，使用10种流行聚类算法处理，通过调整兰德指数(ARI)建立真实标签。网络架构整合卷积、残差和注意力机制，捕捉输入数据的局部和全局结构模式。

Result: 在合成和真实世界基准测试中，ClustRecNet始终优于传统聚类有效性指标(如Silhouette、Calinski-Harabasz等)和最先进的AutoML聚类推荐方法(如ML2DAC、AutoCluster等)。在合成数据上比Calinski-Harabasz指数提高0.497 ARI，在真实数据上比最佳AutoML方法提高15.3% ARI。

Conclusion: 所提出的深度学习模型能够有效推荐最适合的聚类算法，显著优于传统方法和现有AutoML方法，为聚类算法选择提供了新的解决方案。
...
### [179] [Scaling Behaviors of LLM Reinforcement Learning Post-Training: An Empirical Study in Mathematical Reasoning](https://arxiv.org/abs/2509.25300)
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型预训练阶段的缩放规律已被广泛研究，但其在强化学习后训练中的行为仍未被充分探索，特别是在数学推理任务上。
...
### [180] [Uncertainty-Aware Generative Oversampling Using an Entropy-Guided Conditional Variational Autoencoder](https://arxiv.org/abs/2509.25334)
...
TL;DR: LEO-CVAE是一种生成式过采样方法，通过局部熵引导在类别重叠区域生成样本，解决了传统方法在非线性高维生物医学数据中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统过采样方法（如SMOTE）依赖局部线性插值，在非线性高维生物医学数据中生成不合理的合成样本。现有深度生成模型（如CVAE）虽然能更好地捕捉非线性分布，但忽视了边界区域样本的重要性。

Method: 提出LEO-CVAE框架，通过计算样本邻域的香农熵量化不确定性，使用局部熵加权损失强调不确定区域的学习，并采用熵引导的采样策略在类别重叠区域集中生成样本。

Result: 在临床基因组数据集（ADNI和TCGA肺癌）上的实验表明，LEO-CVAE持续提升分类器性能，优于传统过采样方法和生成基线模型。

Conclusion: 对于受复杂非线性结构支配的领域（如组学数据），不确定性感知的生成式过采样在非平衡学习中具有重要价值。
...
### [181] [Gradient Descent with Large Step Sizes: Chaos and Fractal Convergence Region](https://arxiv.org/abs/2509.25351)
*Shuang Liang,Guido Montúfar*

Main category: cs.LG

TL;DR: 研究梯度下降在矩阵分解中的行为，发现大步长会导致参数空间出现分形结构，并推导了标量-向量分解的临界步长。


<details>
  <summary>Details</summary>
Motivation: 探索梯度下降在矩阵分解问题中的动力学行为，特别是大步长情况下的收敛性和敏感性。

Method: 分析标量-向量分解的梯度下降过程，推导临界步长，并扩展到一般矩阵分解的正交初始化情况。

Result: 发现临界步长附近存在分形边界，正则化会放大敏感性，梯度下降进入混沌状态，长期动态不可预测。

Conclusion: 临界步长附近的梯度下降没有简单的隐式偏好（如平衡性、最小范数或平坦性），而是表现出混沌行为。
...
### [182] [Cold-Start Active Correlation Clustering](https://arxiv.org/abs/2509.25376)
*Linus Aronsson,Han Wu,Morteza Haghir Chehreghani*

Main category: cs.LG

TL;DR: 提出一种用于冷启动场景下主动相关聚类的覆盖感知方法，通过鼓励多样性来解决初始无真实相似度信息的问题。


<details>
  <summary>Details</summary>
Motivation: 在主动相关聚类中，当处于冷启动场景时，没有初始的真实成对相似度信息可供主动学习使用，这带来了挑战。

Method: 提出覆盖感知方法，在过程早期鼓励多样性，通过主动查询来获取成对相似度信息。

Result: 通过多个合成和真实世界实验验证了该方法的有效性。

Conclusion: 覆盖感知方法能够有效解决冷启动场景下的主动相关聚类问题。
...
### [183] [Let Physics Guide Your Protein Flows: Topology-aware Unfolding and Generation](https://arxiv.org/abs/2509.25379)
*Yogesh Verma,Markus Heinonen,Vikas Garg*

Main category: cs.LG

TL;DR: 本文提出了一种基于物理原理的蛋白质生成方法，通过非线性噪声过程和SE(3)流匹配，实现了高保真度的蛋白质结构生成和序列条件折叠。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散生成模型在蛋白质设计中忽略了物理真实性，使用缺乏物理基础的噪声动态。本文旨在解决这一问题，引入基于经典物理的非线性噪声过程。

Method: 首先引入基于物理原理的非线性噪声过程，将蛋白质展开为二级结构同时保持拓扑完整性；然后与SE(3)上的流匹配范式结合，建模蛋白质骨架的不变分布，并整合序列信息实现序列条件折叠。

Result: 实验结果显示，该方法在无条件蛋白质生成中达到最先进性能，产生更具设计性和新颖性的蛋白质结构，同时能准确地将单体序列折叠为精确的蛋白质构象。

Conclusion: 该方法成功地将物理原理融入蛋白质生成过程，在保持拓扑完整性的同时实现了高质量的蛋白质结构生成和折叠。
...
### [184] [Predicting Training Re-evaluation Curves Enables Effective Data Curriculums for LLMs](https://arxiv.org/abs/2509.25380)
*Shane Bergsma,Nolan Dey,Joel Hestness*

Main category: cs.LG

TL;DR: 提出训练重评估曲线(TREC)方法，利用最终模型权重评估训练批次，发现将高质量数据放置在TREC低点能显著提升模型性能，并证明可以通过AdamW的隐式EMA系数提前预测TREC。


<details>
  <summary>Details</summary>
Motivation: 数据课程在LLM训练中至关重要，但最优数据放置原则尚不明确，需要开发能够诊断和优化数据放置的方法。

Method: 引入训练重评估曲线(TREC)，分析模型对训练数据的保留程度与数据在训练中遇到时间的关系，并利用AdamW的隐式EMA系数预测TREC。

Result: 在111M到3.9B参数的模型上验证，将高质量数据放置在TREC低点能显著提升性能；成功预测已发布训练方案中的TREC，发现次优数据放置；在3.9B参数LLM的持续预训练中，将高质量数据与TREC最小值对齐获得改进。

Conclusion: TREC为数据课程设计提供了有效的诊断工具，能够指导更优的数据放置策略，提升LLM训练效率。
...
### [185] [Deep Survival Analysis for Competing Risk Modeling with Functional Covariates and Missing Data Imputation](https://arxiv.org/abs/2509.25381)
*Penglei Gao,Yan Zou,Abhijit Duggal,Shuaiqi Huang,Faming Liang,Xiaofeng Wang*

Main category: cs.LG

TL;DR: 提出了FCRN框架，用于竞争风险下的离散时间生存分析，整合功能协变量并处理缺失数据，通过端到端模型同时学习填补缺失值和预测事件特定风险。


<details>
  <summary>Details</summary>
Motivation: 在重症监护等场景中，需要有效处理功能协变量、缺失数据以及竞争风险，传统方法难以同时应对这些挑战。

Method: 结合微网络基础层进行功能数据表示和基于梯度的填补模块，构建端到端深度学习框架。

Result: 在模拟数据集和真实ICU案例研究中，FCRN在预测准确性上显著优于随机生存森林和传统竞争风险模型。

Conclusion: 该方法通过更有效捕捉动态风险因素和静态预测因子，同时适应不规则和不完整数据，推进了重症监护中的预后建模。
...
### [186] [On the Shape of Latent Variables in a Denoising VAE-MoG: A Posterior Sampling-Based Study](https://arxiv.org/abs/2509.25382)
*Fernanda Zapata Bascuñán*

Main category: cs.LG

TL;DR: 该论文研究了基于GW150914引力波数据的变分自编码器-高斯混合模型（VAE-MoG）的潜在空间，发现尽管模型能准确重构信号，但潜在表示存在统计不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 探索VAE-MoG模型在引力波数据中的潜在空间质量，评估生成模型在噪声数据下的表示可靠性。

Method: 使用哈密顿蒙特卡洛（HMC）从干净输入中抽取后验样本，并与编码器从噪声数据输出的结果进行统计比较。

Result: 模型重构信号准确，但潜在空间存在明显不匹配，表明强去噪性能不一定意味着潜在表示可靠。

Conclusion: 在评估生成模型时，使用基于后验的验证方法至关重要，仅凭重构质量不足以判断潜在表示的质量。
...
### [187] [Crowdsourcing Without People: Modelling Clustering Algorithms as Experts](https://arxiv.org/abs/2509.25395)
...
<details>
  <summary>Details</summary>
Motivation: 传统众包依赖人工标注，而该方法旨在利用多个聚类算法的输出作为噪声标注来聚合预测结果，为数据真实结构未知的情况提供鲁棒解决方案。

Method: 基于Dawid-Skene模型框架，将多个聚类算法的输出建模为噪声标注，通过集成方法进行预测聚合。

Result: 在模拟和真实数据集上的实验表明，mixsemble虽不总是最佳性能，但始终接近最优结果并避免较差表现，展现出良好的鲁棒性。

Conclusion: mixsemble在数据真实结构未知时提供了一种实用的替代方案，特别适合非专业用户使用，因其能稳定接近最佳性能并避免较差结果。
...
### [188] [Multi-Task Equation Discovery](https://arxiv.org/abs/2509.25400)
...
<details>
  <summary>Details</summary>
Motivation: 方程发现在系统识别中面临的主要挑战是确保识别模型能够泛化到不同操作条件，而不仅仅过拟合特定数据集。特别是在结构健康监测中，不同载荷条件会揭示系统物理的不同方面。

Method: 采用贝叶斯相关向量机(RVM)结合多任务学习(MTL)框架，将同一结构在不同激励水平下的响应视为相关任务，共享模型参数但保留任务特定的噪声特性。

Result: 与标准单任务RVM相比，MTL-RVM通过跨任务信息整合，显著提高了在弱激励和中度激励条件下的参数恢复性能，同时在高激励条件下保持良好表现。

Conclusion: 多任务贝叶斯推断可以有效缓解方程发现中的过拟合问题，提升模型泛化能力，特别适用于结构健康监测等需要处理变化载荷条件的应用场景。
...
### [189] [FlashOmni: A Unified Sparse Attention Engine for Diffusion Transformers](https://arxiv.org/abs/2509.25401)
...
<details>
  <summary>Details</summary>
Motivation: 多模态扩散变换器(DiTs)在视觉合成中表现出色，但计算需求大限制了部署。现有稀疏加速方法需要定制内核，缺乏通用性。

Method: 提出FlashOmni统一稀疏注意力引擎，引入灵活稀疏符号标准化稀疏策略表示，设计优化的稀疏GEMMs消除冗余计算。

Result: FlashOmni在注意力和GEMM-Q中实现接近线性加速(1:1稀疏比)，GEMM-O加速2.5-3.8倍(达到理论极限的87.5%)，Hunyuan模型(33K)端到端加速约1.5倍且视觉质量不下降。

Conclusion: FlashOmni提供了一个通用高效的稀疏计算解决方案，显著加速DiT模型推理，同时保持视觉质量。
...
### [190] [Rethinking Parameter Sharing for LLM Fine-Tuning with Multiple LoRAs](https://arxiv.org/abs/2509.25414)
*Hao Ban,Kaiyi Ji*
...
<details>
  <summary>Details</summary>
Motivation: 重新审视多LoRA适配器中A矩阵高度相似的现象，发现这种相似性主要源于相同的初始化而非共享知识，而B矩阵在知识编码和传输中起更关键作用。

Method: 提出ALoRA（不对称多LoRA设计），在多任务微调中使用多个A矩阵但共享单个B矩阵；Fed-ALoRA在联邦微调中跨客户端共享B矩阵，通过新颖的矩阵分解策略适应异构的客户端秩。

Result: 在常识推理、数学推理、多任务NLP数据集和联邦NLP数据集上的实验表明，该方法相比现有多LoRA方法实现了更平衡的任务性能，平均准确率相当或更优。

Conclusion: 通过不对称的LoRA设计和B矩阵共享策略，能够有效提升多任务和联邦学习场景下的性能平衡性和效率。
...
### [191] [Leveraging Vulnerabilities in Temporal Graph Neural Networks via Strategic High-Impact Assaults](https://arxiv.org/abs/2509.25418)
*Dong Hyun Jeon,Lijing Zhu,Haifang Li,Pengze Li,Jingna Feng,Tiehang Duan,Houbing Herbert Song,Cui Tao,Shuteng Niu*

Main category: cs.LG

TL;DR: 提出HIA攻击框架，针对时序图神经网络，通过识别结构重要性和动态重要性节点，结合边注入和删除策略，显著降低TGNN性能，同时保持隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 现有STDG攻击方法依赖简单扰动且容易被检测，无法有效针对关键节点和边实现最大影响，需要更先进的攻击框架来暴露TGNN的脆弱性。

Method: 使用数据驱动的代理模型识别结构重要节点和动态重要节点，采用混合扰动策略（边注入和定向边删除），最小化扰动数量以提高隐蔽性。

Result: 在5个真实数据集和4种TGNN架构上的实验表明，HIA显著降低链接预测任务的准确率，MRR下降高达35.55%，优于现有基线方法。

Conclusion: 结果揭示了当前STDG模型的基本脆弱性，强调需要开发同时考虑结构和时间动态的鲁棒防御机制。
...
### [192] [Polychromic Objectives for Reinforcement Learning](https://arxiv.org/abs/2509.25424)
*Jubayer Ibn Hamid,Ifdita Hasan Orney,Ellen Xu,Chelsea Finn,Dorsa Sadigh*

Main category: cs.LG

TL;DR: 提出了一种称为多色目标的策略梯度方法，通过保持生成多样性来解决强化学习微调中的策略崩溃问题，在多个任务上提升了成功率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习微调方法容易导致策略崩溃，使模型输出多样性降低，从而影响探索能力和测试时计算扩展的效益。

Method: 引入多色目标来强制探索和优化多样化的生成，使用藤蔓采样收集策略轨迹，并修改优势函数以适应新目标。

Result: 在BabyAI、Minigrid和Algorithmic Creativity等任务上，该方法提高了成功率，能解决更多环境配置，并在大扰动下具有更好的泛化能力。

Conclusion: 多色目标方法能有效维持策略多样性，在pass@k实验中展现出更高的覆盖率，证明了其保持和利用多样化策略库的能力。
...
### [193] [Feedback Control for Small Budget Pacing](https://arxiv.org/abs/2509.25429)
*Sreeja Apparaju,Yichuan Niu,Xixi Qi*

Main category: cs.LG

TL;DR: 提出了一种结合分桶滞后和比例反馈的预算控制方法，显著提升了在线广告预算分配的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有预算分配方法依赖临时参数调整，存在不稳定和低效的问题，需要更系统化的控制方案。

Method: 结合分桶滞后控制和比例反馈机制，提供参数选择框架和分析方法，实现稳定的支出控制。

Result: 在真实拍卖实验中，相比基线方法，分配误差降低13%，λ波动性降低54%，特别对小预算广告活动效果显著。

Conclusion: 通过将控制理论与广告系统结合，该方法为预算分配提供了可扩展且可靠的解决方案。
...
### [194] [Beyond Noisy-TVs: Noise-Robust Exploration Via Learning Progress Monitoring](https://arxiv.org/abs/2509.25438)
*Zhibo Hou,Zhiyu An,Wan Du*

Main category: cs.LG

TL;DR: 提出了一种名为学习进度监控（LPM）的新方法，通过奖励模型改进而非预测误差或新颖性来驱动探索，有效避免智能体在不可学习的噪声源上停滞。


<details>
  <summary>Details</summary>
Motivation: 解决传统内在奖励方法在存在不可学习随机源（噪声电视）时陷入停滞的问题，以及基于不确定性估计或分布相似性的方法样本效率低、计算成本高的问题。
...
Conclusion: LPM是一种概念简单但有效的噪声鲁棒探索方法，标志着探索范式的转变，其内在奖励是零等变的，并且是信息增益的单调指标。
...
### [195] [Norm-Q: Effective Compression Method for Hidden Markov Models in Neuro-Symbolic Applications](https://arxiv.org/abs/2509.25439)
...
<details>
  <summary>Details</summary>
Motivation: 隐马尔可夫模型在生成任务中表现良好，但神经符号应用结合了神经网络和符号推理，计算密集且数据传输量大，性能受限。

Method: 采用归一化量化感知的期望最大化过程进行概率模型训练，减少数据位宽。

Result: 与传统量化方法相比，Norm-Q实现了更高的压缩率和合理的分数损失。在大型语言模型的约束生成任务中，成功将4096隐藏状态的HMM量化为8位无损失，最多3位可接受损失，权重压缩率达到99%。

Conclusion: Norm-Q方法有效压缩概率符号模型，缓解内存带宽压力，适合在定制硬件上部署。
...
### [196] [Joint Embeddings Go Temporal](https://arxiv.org/abs/2509.25449)
*Sofiane Ennadir,Siavash Golkar,Leopoldo Sarra*

Main category: cs.LG

TL;DR: 提出了TS-JEPA，一种专门为时间序列表示学习设计的联合嵌入预测架构，能够匹配或超越现有最先进方法在不同标准数据集上的表现。
...
Method: 基于联合嵌入预测架构(JEPA)，在潜在空间进行自监督学习，专门为时间序列数据设计TS-JEPA架构。
...
### [197] [Data-Efficient Multitask DAgger](https://arxiv.org/abs/2509.25466)
*Haotian Fu,Ran Gong,Xiaohan Zhang,Maria Vittoria Minniti,Jigarkumar Patel,Karl Schmeckpeper*

Main category: cs.LG

TL;DR: 提出了一种数据高效的多任务DAgger框架，通过性能感知调度策略从多个任务专用专家策略中蒸馏出单一多任务策略，显著提高了任务成功率并减少了专家演示需求。


<details>
  <summary>Details</summary>
Motivation: 通用机器人策略通常需要大量专家数据或模拟训练，本文旨在开发数据高效的多任务学习方法，减少对专家演示的依赖。

Method: 采用性能感知调度策略，基于卡尔曼滤波器估计器跟踪各任务学习过程的数据效益，动态分配额外演示数据到表现不佳的任务。

Result: 在MetaWorld和IsaacLab抽屉开启任务套件上验证，获得高性能多任务策略，使用更少专家演示，零样本迁移到真实机器人时表现优于朴素DAgger和行为克隆。

Conclusion: 该方法有效提高了多任务学习的效率和性能，在减少专家数据需求的同时实现了更好的零样本迁移能力。
...
### [198] [Conformal Prediction for Signal Temporal Logic Inference](https://arxiv.org/abs/2509.25473)
*Danyang Li,Yixuan Wang,Matthew Cleaveland,Mingyu Cai,Roberto Tron*

Main category: cs.LG

TL;DR: 提出了一种端到端的可微分共形预测框架，用于信号时序逻辑推理，通过嵌入平滑CP层和新的损失函数，同时优化推理准确性和预测集，提供统计保证。


<details>
  <summary>Details</summary>
Motivation: 现有STL推理方法缺乏对推断规则的正式置信保证，而传统共形预测通常作为训练后包装器使用，无法改进模型学习。

Method: 引入基于鲁棒性的非共形度分数，将平滑CP层直接嵌入训练过程，使用新的损失函数同时优化推理准确性和CP预测集。

Result: 在基准时间序列任务上的实验表明，该方法减少了预测不确定性（实现高覆盖率同时减小预测集大小），并提高了准确性。

Conclusion: 该方法增强了STL公式的可靠性和可解释性，为学习到的STL公式提供统计保证，优于现有最先进基线方法。
...
### [199] [Translation from Wearable PPG to 12-Lead ECG](https://arxiv.org/abs/2509.25480)
...
<details>
  <summary>Details</summary>
Motivation: 现有的12导联ECG系统需要复杂的多电极设置，限制了移动环境下的持续监测；而现有的PPG方法由于缺乏导联间约束和时空依赖性建模，无法重建多导联ECG。

Method: 提出P2Es框架，包含三个关键创新：前向过程使用频域模糊和时间噪声干扰模拟真实信号失真；反向过程使用时间多尺度生成模块和频率去模糊；利用KNN聚类和对比学习分配亲和矩阵，实现人口统计特定的ECG转换。

Result: 广泛的实验结果表明，P2Es在12导联ECG重建方面优于基线模型。

Conclusion: P2Es框架成功解决了从PPG信号生成临床有效12导联ECG的挑战，为移动环境下的心血管监测提供了可行方案。
...
### [200] [Scalable Disk-Based Approximate Nearest Neighbor Search with Page-Aligned Graph](https://arxiv.org/abs/2509.25487)
...
Method: 提出页面节点图结构，将逻辑图节点与物理SSD页面对齐；采用协同设计的磁盘数据布局和合并技术；设计结合轻量级索引和协调内存-磁盘数据分配的内存管理策略。
...
### [201] [Can Molecular Foundation Models Know What They Don't Know? A Simple Remedy with Preference Optimization](https://arxiv.org/abs/2509.25509)
...
<details>
  <summary>Details</summary>
Motivation: 分子基础模型在分布外样本上的不可靠性严重限制了其在药物发现和蛋白质设计等高风险领域的应用，特别是化学幻觉问题导致模型对未知分子做出高置信度但完全错误的预测。

Method: 引入分子偏好对齐实例排序方法，将OOD检测问题表述为对ID和OOD样本之间估计的OOD亲和度的偏好优化，通过成对学习目标实现，本质上优化AUROC指标。

Result: 在五个真实世界分子数据集上的广泛实验表明，该方法显著提升了现有分子基础模型的OOD检测能力，在大小、支架和测定分布偏移下分别实现了45.8%、43.9%和24.3%的AUROC改进。

Conclusion: Mole-PAIR是一个简单、即插即用的模块，可以灵活集成到现有基础模型中，通过成本效益高的后训练显著提高其在OOD数据上的可靠性。
...
### [202] [EEsizer: LLM-Based AI Agent for Sizing of Analog and Mixed Signal Circuit](https://arxiv.org/abs/2509.25510)
*Chang Liu,Danial Chitnis*

Main category: cs.LG

TL;DR: EEsizer是一个基于大语言模型的AI代理，将LLM与电路仿真器和数据分析功能集成，实现全自动闭环晶体管尺寸优化，无需外部知识。


<details>
  <summary>Details</summary>
Motivation: 模拟和混合信号集成电路设计需要大量人工工作，特别是在晶体管尺寸优化过程中。虽然机器学习在EDA中显示出潜力，但仍面临迭代次数多和缺乏AMS电路设计知识等挑战。

Method: 使用提示工程和思维链推理，将大语言模型与电路仿真器和自定义数据分析功能集成，通过迭代探索设计方向、评估性能和优化解决方案。

Result: 在8个LLM基准测试后，选择三个高性能模型优化20晶体管CMOS运算放大器。OpenAI o3在90nm节点成功实现用户预期目标，最多20次迭代，在先进节点表现出适应性和鲁棒性。

Conclusion: EEsizer展示了LLM在自动化晶体管尺寸优化方面的潜力，能够适应先进工艺节点，并通过变异分析验证了设计鲁棒性。
...
### [203] [World Model for AI Autonomous Navigation in Mechanical Thrombectomy](https://arxiv.org/abs/2509.25518)
*Harry Robertshaw,Han-Ru Wu,Alejandro Granados,Thomas C Booth*

Main category: cs.LG

TL;DR: 提出基于TD-MPC2世界模型的自主血管内导航方法，在10个真实患者血管中训练单一RL智能体，相比SAC方法成功率提升至65%，但手术时间增加。


<details>
  <summary>Details</summary>
Motivation: 机械取栓术的自主导航面临血管解剖复杂性和实时决策精确性的挑战，现有RL方法在多患者血管泛化和长视野任务方面存在困难。

Method: 使用TD-MPC2模型强化学习算法，在10个真实患者血管中训练单一智能体进行多任务血管内导航，并与最先进的SAC方法进行比较。

Result: TD-MPC2在多任务学习中显著优于SAC，平均成功率65% vs 37%，路径比有明显改善，但手术时间增加，存在成功率与执行速度的权衡。

Conclusion: 世界模型在改善自主血管内导航方面具有潜力，为未来可泛化的AI驱动机器人干预研究奠定了基础。
...
### [204] [Flow Matching with Semidiscrete Couplings](https://arxiv.org/abs/2509.25519)
*Alireza Mousavi-Hosseini,Stephen Y. Zhang,Michal Klein,Marco Cuturi*

Main category: cs.LG

TL;DR: 提出了一种基于半离散最优传输的流匹配方法(SD-FM)，通过将目标数据集视为离散分布，解决了传统OT-FM方法中Sinkhorn算法计算复杂度高的问题，在多个数据集和任务上优于标准流匹配和OT-FM方法。


<details>
  <summary>Details</summary>
Motivation: 传统OT-FM方法虽然理论上很有前景，但由于Sinkhorn算法需要O(n²/ε²)的计算复杂度，在实际应用中成本过高，特别是当批次大小n较大时。

Method: 采用半离散最优传输(SD-OT)方法，将目标数据集视为离散分布，通过SGD估计对偶势向量，然后使用最大内积搜索(MIPS)将新采样的噪声向量与数据点匹配。

Result: SD-FM在所有训练指标和推理预算约束下都优于FM和OT-FM，在多个数据集、无条件/条件生成任务以及均值流模型中均表现出色。

Conclusion: SD-FM方法消除了OT-FM中的二次依赖瓶颈，实现了OT-FM的理论承诺，为流匹配提供了一种更高效实用的解决方案。
...
### [205] [Meta-Router: Bridging Gold-standard and Preference-based Evaluations in Large Language Model Routing](https://arxiv.org/abs/2509.25535)
...
<details>
  <summary>Details</summary>
Motivation: 在需要大量人机交互的语言任务中，为每个查询部署单一"最佳"模型成本高昂。现有路由器训练面临可靠监督数据稀缺的挑战：黄金标准数据准确但昂贵，偏好数据便宜但存在偏差

Method: 将LLM路由器训练问题转化为因果推断框架，将响应评估机制视为治疗分配。基于此视角开发了集成因果路由器训练框架，纠正偏好数据偏差，处理两种数据源的不平衡

Result: 数值实验表明，该方法能提供更准确的路由，并改善成本与质量之间的权衡

Conclusion: 通过因果推断框架有效解决了LLM路由器训练中的数据偏差问题，实现了更高效和鲁棒的路由决策
...
### [206] [Steering an Active Learning Workflow Towards Novel Materials Discovery via Queue Prioritization](https://arxiv.org/abs/2509.25538)
*Marcus Schwarting,Logan Ward,Nathaniel Hudson,Xiaoli Yan,Ben Blaiszik,Santanu Chaudhuri,Eliu Huerta,Ian Foster*

Main category: cs.LG

TL;DR: 提出了一种结合生成建模和主动学习的队列优先级算法，用于解决科学中的逆向设计问题，显著提高了高质量候选分子的数量。
...
Method: 提出队列优先级算法，将生成建模与主动学习结合在分布式工作流中，使用主动学习模型优先处理顶级设计候选者。

Result: 对于碳捕获分子结构发现任务，无主动学习的工作流平均生成281个高性能候选分子，而采用主动学习优先级的方法平均生成604个高性能候选分子。

Conclusion: 主动学习方法能防止生成式AI工作流在无意义候选者上浪费资源，阻止生成模型衰减，显著提高高质量候选者的识别数量。
...
### [207] [Lightweight and Robust Federated Data Valuation](https://arxiv.org/abs/2509.25560)
*Guojun Tang,Jiayu Zhou,Mohammad Mamun,Steve Drew*

Main category: cs.LG

TL;DR: FedIF是一种新颖的联邦学习聚合框架，使用基于轨迹的影响力估计来高效计算客户端贡献，在保持与Shapley值方法相当的鲁棒性同时，将聚合开销降低高达450倍。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临非独立同分布数据和对抗性客户端行为的鲁棒性挑战，现有基于Shapley值的方法计算开销大，限制了可扩展性。
...
Conclusion: FedIF为现实世界部署提供了一个实用、理论可靠且可扩展的联邦学习聚合方案，是Shapley值方法的有效替代。
...
### [208] [Safe In-Context Reinforcement Learning](https://arxiv.org/abs/2509.25582)
*Amir Moeini,Minjae Kwon,Alper Kamil Bozkurt,Yuichi Motai,Rohan Chandra,Lu Feng,Shangtong Zhang*

Main category: cs.LG

TL;DR: 提出了首个在上下文强化学习(ICRL)中确保安全适应的方法，在无参数更新的适应过程中同时最大化奖励并最小化额外成本函数。


<details>
  <summary>Details</summary>
Motivation: ICRL能够在无参数更新的情况下适应分布外任务，但现有方法缺乏对适应过程安全性的考虑，需要在约束马尔可夫决策过程框架下确保安全。

Method: 在ICRL框架中引入约束MDP，通过扩展策略神经网络的输入上下文，在无参数更新的适应过程中同时优化奖励和成本函数，使代理能够根据成本容忍度阈值主动调整行为。

Result: 代理能够根据成本预算主动调整行为：成本预算较高时行为更激进，成本预算较低时行为更保守，实现了安全的适应过程。

Conclusion: 该方法首次在ICRL中实现了安全的适应过程，代理能够根据成本容忍度阈值动态调整行为策略，为安全强化学习提供了新的解决方案。
...
### [209] [Machine Learning Algorithms for Improving Black Box Optimization Solvers](https://arxiv.org/abs/2509.25592)
*Morteza Kimiaei,Vyacheslav Kungurtsev*

Main category: cs.LG

TL;DR: 这篇论文综述了机器学习和强化学习如何改进黑盒优化方法，使其在复杂场景中更有效。


<details>
  <summary>Details</summary>
Motivation: 传统黑盒优化方法在高维、噪声或混合整数设置中表现不佳，需要更先进的机器学习技术来提升性能。

Method: 采用机器学习提供代理模型、自适应更新和元学习组合，强化学习实现动态算子配置和跨任务元优化。

Result: 开发了多种代表性算法，如mlrMBO、ZO-AdaMM、ABBO等，显著提升了黑盒优化的可扩展性和鲁棒性。

Conclusion: 机器学习和强化学习将传统黑盒优化方法转变为更可扩展、鲁棒和自适应的框架，适用于现实世界优化问题。
...
### [210] [Binary Sparse Coding for Interpretability](https://arxiv.org/abs/2509.25596)
*Lucia Quirke,Stepan Shabalin,Nora Belrose*

Main category: cs.LG

TL;DR: 提出二进制稀疏自编码器(BAE)和二进制转码器(BTC)，通过将激活值二值化为0或1来提高特征的可解释性和单义性，但会增加重构误差。
...
Method: 使用二进制稀疏自编码器(BAE)和二进制转码器(BTC)，将所有激活值约束为0或1，消除连续激活强度的变化。

Result: 二值化显著提高了发现特征的可解释性和单义性，但增加了重构误差和不可解释的超高频特征数量。频率调整后的可解释性评分显示连续稀疏编码器略优于二进制版本。

Conclusion: 多义性可能是神经网络激活的固有属性，无法完全消除。二值化虽然提高了可解释性，但也带来了新的挑战。
...
### [211] [Effective Model Pruning](https://arxiv.org/abs/2509.25606)
*Yixuan Wang,Dan Guralnik,Saiedeh Akbari,Warren Dixon*

Main category: cs.LG

TL;DR: EMP提出了一种上下文无关、无需参数的剪枝规则，通过计算有效数量N_eff来确定保留多少参数，适用于各种剪枝标准和模型架构。


<details>
  <summary>Details</summary>
Motivation: 解决剪枝中的基本问题：应该保留多少参数，提供一个通用的自适应阈值方法。

Method: 利用逆辛普森指数计算有效数量N_eff，保留得分最高的N_eff个参数，零化其余参数。

Result: 在MLPs、CNNs、Transformers/LLMs和KAN等模型上，稀疏模型性能与原始密集网络相当。

Conclusion: EMP提供了一个稳健的剪枝阈值，默认β=1效果良好，β≠1可作为可选调整以满足特定稀疏度要求。
...
### [212] [Unsupervised Detection of Spatiotemporal Anomalies in PMU Data Using Transformer-Based BiGAN](https://arxiv.org/abs/2509.25612)
...
<details>
  <summary>Details</summary>
Motivation: 确保电网韧性需要及时无监督地检测同步相量数据流中的异常，传统方法依赖手动标记的故障数据，难以满足实时监控需求。

Method: 结合窗口注意力Transformer和双向生成对抗网络，使用自注意力编码器-解码器架构捕捉复杂时空依赖关系，通过联合判别器强制执行循环一致性来对齐潜在空间与真实数据分布。

Result: 在真实硬件在环PMU基准测试中，T-BiGAN实现了0.95的ROC-AUC和0.996的平均精度，显著优于领先的监督和无监督方法，特别擅长检测细微的频率和电压偏差。

Conclusion: T-BiGAN框架在实际应用中具有重要价值，能够在不依赖手动标记故障数据的情况下实现实时广域监控，有效提升电网韧性。
...
### [213] [Layer-wise dynamic rank for compressing large language models](https://arxiv.org/abs/2509.25622)
*Zhendong Mi,Bian Sun,Grace Li Zhang,Shaoyi Huang*

Main category: cs.LG

TL;DR: D-Rank是一种基于SVD的LLM压缩框架，通过层间动态秩分配来优化压缩效果，相比均匀压缩方法在困惑度和推理准确率上表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法对所有层使用统一压缩比例，忽略了LLM中不同层的信息密度差异，特别是中间层信息更丰富而首尾层更冗余的特性。

Method: 引入有效秩作为信息密度度量，使用拉格朗日乘子优化方案动态分配秩，对注意力层进行重平衡，并扩展到分组查询注意力的最新LLM。

Result: 在多种LLM和压缩比例下，D-Rank始终优于SVD-LLM、ASVD和Basis Sharing方法，在LLaMA-3-8B模型上困惑度降低15%以上，在LLaMA-7B模型上零样本推理准确率提升5%。

Conclusion: D-Rank通过考虑层间异质性的动态秩分配，显著提升了SVD压缩方法的性能，同时保持了更高的吞吐量。
...
### [214] [Swift: An Autoregressive Consistency Model for Efficient Weather Forecasting](https://arxiv.org/abs/2509.25631)
*Jason Stock,Troy Arcomano,Rao Kotamarthi*

Main category: cs.LG

TL;DR: Swift是一个单步一致性模型，首次实现了基于CRPS目标的概率流模型自回归微调，消除了多模型集成或参数扰动的需求，在S2S天气预报中比最先进的扩散模型快39倍，同时保持与IFS ENS相当的预测技能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型为概率天气预报提供了物理基础框架，但其在推理时依赖缓慢的迭代求解器，使得在需要长提前期和领域驱动校准的次季节到季节(S2S)应用中不实用。

Method: 引入Swift单步一致性模型，首次实现了基于连续排序概率评分(CRPS)目标的概率流模型自回归微调，无需多模型集成或参数扰动。

Result: Swift产生熟练的6小时预报，可稳定运行75天，比最先进的扩散基线快39倍，同时实现与基于数值的IFS ENS相当的预测技能。
...
### [215] [How Does Preconditioning Guide Feature Learning in Deep Neural Networks?](https://arxiv.org/abs/2509.25637)
*Kotaro Yoshida,Atsushi Nitanda*

Main category: cs.LG

TL;DR: 该论文研究了预条件处理如何影响特征学习和泛化性能，发现预条件器通过其度量定义的Gram矩阵控制特征学习的谱偏置，当这种谱偏置与教师模型的谱偏置对齐时能显著提升泛化能力。
...
Result: 学习到的特征表示紧密反映了预条件器引入的谱偏置——偏向被强调的组件，对被抑制的组件敏感性降低。当这种谱偏置与教师模型的谱偏置对齐时，泛化性能显著增强。

Conclusion: 预条件处理通过控制谱偏置影响特征学习，其与教师模型谱偏置的对齐是提升泛化能力的关键因素。
...
### [216] [Deep set based operator learning with uncertainty quantification](https://arxiv.org/abs/2509.25646)
*Lei Ma,Ling Guo,Hao Wu,Tao Zhou*

Main category: cs.LG

TL;DR: 提出了UQ-SONet，一种具有内置不确定性量化的置换不变算子学习框架，用于解决DeepONets在传感器约束和不确定性量化方面的局限性。


<details>
  <summary>Details</summary>
Motivation: DeepONets及其扩展如VIDON虽然能够处理复杂域，但需要固定传感器位置，缺乏不确定性量化机制，且无法处理稀疏观测或算子固有随机性带来的不确定性。

Method: 集成集合变换器嵌入来处理稀疏和可变传感器位置，并使用条件变分自编码器（cVAE）来近似解算子的条件分布，通过最小化负ELBO提供原则性不确定性估计。

Result: 在确定性和随机偏微分方程（包括Navier-Stokes方程）上的数值实验证明了该框架的鲁棒性和有效性。

Conclusion: UQ-SONet在保持预测精度的同时提供了原则性的不确定性估计，解决了现有算子学习方法在传感器约束和不确定性量化方面的关键限制。
...
### [217] [BaB-prob: Branch and Bound with Preactivation Splitting for Probabilistic Verification of Neural Networks](https://arxiv.org/abs/2509.25647)
*Fangji Wang,Panagiotis Tsiotras*

Main category: cs.LG

TL;DR: BaB-prob将分支定界与前激活分裂扩展到概率验证，通过线性边界传播计算子问题概率边界，在中等至高维输入问题上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 将确定性神经网络验证中的分支定界与前激活分裂框架扩展到概率设置，解决概率验证问题。

Method: 提出BaB-prob方法，通过分裂前激活将原问题划分为子问题，利用线性边界传播计算每个子问题的概率边界，并引入不确定性水平和两种高效的前激活分裂策略。

Result: 在未训练网络、MNIST和CIFAR-10模型以及VNN-COMP 2025基准测试中，BaB-prob在中等至高维输入问题上始终优于最先进方法。

Conclusion: BaB-prob是概率神经网络验证的有效框架，证明了其在前馈ReLU网络中的正确性和完备性，并在各种设置中表现出优越性能。
...
### [218] [Growing Winning Subnetworks, Not Pruning Them: A Paradigm for Density Discovery in Sparse Neural Networks](https://arxiv.org/abs/2509.25665)
*Qihang Yao,Constantine Dovrolis*

Main category: cs.LG

TL;DR: PWMPR是一种从稀疏到稠密的训练方法，通过生长而非剪枝的方式自动发现网络的最佳密度，在显著降低计算成本的同时接近彩票假设网络的性能。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏训练方法（迭代剪枝、动态稀疏训练、初始化剪枝）要么需要大量重训练成本，要么需要预先确定目标密度。PWMPR旨在通过生长网络的方式自动发现最佳密度，降低计算开销。

Method: 从稀疏种子开始，PWMPR基于路径核启发式分数添加边，通过随机化缓解瓶颈，并使用逻辑拟合规则检测准确率平台期来自动停止生长。

Result: 在CIFAR、TinyImageNet和ImageNet上的实验表明，PWMPR能够接近IMP彩票假设网络的性能（虽然密度更高），但计算成本显著降低（约1.5倍稠密训练 vs IMP的3-4倍）。

Conclusion: 基于生长的密度发现是一个有前景的范式，与剪枝和动态稀疏方法形成互补。
...
### [219] [Nudging the Boundaries of LLM Reasoning](https://arxiv.org/abs/2509.25666)
...
TL;DR: NuRL是一种新的强化学习方法，通过自生成提示来提升LLM推理能力，解决了传统RL方法无法从模型无法解决的问题中学习的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前在线强化学习算法如GRPO存在关键限制：无法从模型无法解决的问题中学习，导致模型的上限在RL训练后保持不变。

Method: 提出NuRL方法，让模型自生成提示（抽象线索），在硬样本（通过率为0%）中注入提示并重新生成轨迹，从而引入训练信号。

Result: NuRL在6个基准测试和3个模型上取得一致改进，能够提升模型的上限，而GRPO则无法改变pass@1024指标。

Conclusion: NuRL通过自生成提示有效提升了LLM推理能力，最佳提示是抽象且高层次的，在GRPO收敛后应用效果最好。
...
### [220] [EEG-based AI-BCI Wheelchair Advancement: Hybrid Deep Learning with Motor Imagery for Brain Computer Interface](https://arxiv.org/abs/2509.25667)
...
<details>
  <summary>Details</summary>
Motivation: 开发一种基于脑机接口的智能轮椅控制系统，利用运动想象机制为行动不便的用户提供直观的功能性控制方案。

Method: 使用开源EEG数据集，数据采样频率200Hz，分割为19x200数组。提出BiLSTM-BiGRU注意力模型，并与XGBoost、EEGNet和Transformer等基线模型进行比较。

Result: BiLSTM-BiGRU模型测试准确率达到92.26%，交叉验证平均准确率为90.13%，显著优于其他基线模型。

Conclusion: 注意力机制在BCI应用中具有巨大潜力，所提出的BiLSTM-BiGRU模型为脑机接口轮椅控制提供了有效的解决方案。
...
### [221] [Guiding Mixture-of-Experts with Temporal Multimodal Interactions](https://arxiv.org/abs/2509.25678)
*Xing Han,Hsing-Huan Chung,Joydeep Ghosh,Paul Pu Liang,Suchi Saria*

Main category: cs.LG

TL;DR: 提出了一种基于时间交互的多模态MoE路由框架，通过量化模态间动态交互来指导专家路由，提升专家专业化和模型性能。
...
Method: 提出多模态交互感知路由器，基于token的交互性质将其分配到专家；构建时间多模态交互动态的新公式化表示来指导专家路由。

Result: 在具有挑战性的多模态基准测试中验证了方法，展示了性能提升和改进的可解释性。

Conclusion: 时间多模态交互揭示了有意义的模式，可被用来改进MoE模型的设计和性能，动态路由鼓励专家获得可泛化的交互处理技能而非仅学习任务特定特征。
...
### [222] [Minimalist Explanation Generation and Circuit Discovery](https://arxiv.org/abs/2509.25686)
*Pirzada Suhail,Aditya Anand,Amit Sethi*

Main category: cs.LG

TL;DR: 提出基于激活匹配的方法为预训练图像分类器生成最小且忠实的解释，通过训练轻量级自编码器产生二进制掩码来突出决策关键区域，并结合电路读取程序机制化解释模型内部计算。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在训练中学习大量决策规则，但在高维输入空间中这些规则难以识别和解释。需要找到既保持模型决策又简洁可读的最小解释。

Method: 训练轻量级自编码器生成二进制掩码，集成多层激活对齐、输出标签一致性、稀疏性和紧凑性先验，以及确保忠实度的鲁棒性约束。通过电路读取程序识别活跃通道并构建通道级图。

Result: 生成的最小解释不仅保留了模型决策，还简洁可读，同时能够机制化解释模型内部计算过程。

Conclusion: 该方法在最小输入级解释与驱动模型决策的内部计算的机制化理解之间建立了实用桥梁。
...
### [223] [A Unified Probabilistic Framework for Dictionary Learning with Parsimonious Activation](https://arxiv.org/abs/2509.25690)
*Zihui Zhao,Yuanbo Tang,Jieyu Ren,Xiaoping Zhang,Yang Li*

Main category: cs.LG

TL;DR: 提出了一种基于系数矩阵行向L∞范数的字典学习正则化方法，通过促进整个系数矩阵行的稀疏性来减少跨样本激活的字典原子数量，显著提升重建质量和表示稀疏性。


<details>
  <summary>Details</summary>
Motivation: 传统字典学习方法主要关注单个样本的表示稀疏性，忽略了原子在跨样本间的共享模式，导致字典冗余和次优。需要一种能促进跨样本稀疏性的正则化方法。

Method: 引入基于系数矩阵行向L∞范数的正则化器，结合Beta-Bernoulli先验的概率模型推导，提供贝叶斯解释并建立最优超参数选择的理论计算。

Result: 在基准数据集上的实验表明，该方法实现了重建质量显著提升（RMSE降低20%），表示稀疏性增强，仅使用不到十分之一的可用字典原子。

Conclusion: 提出的正则化方法有效解决了字典冗余问题，通过理论分析和实验验证了其在提升重建质量和表示稀疏性方面的优越性能。
...
### [224] [Annotation-Efficient Active Test-Time Adaptation with Conformal Prediction](https://arxiv.org/abs/2509.25692)
*Tingyu Shi,Fan Lyu,Shaoliang Peng*

Main category: cs.LG

TL;DR: CPATTA是一种主动测试时适应方法，通过引入保形预测原理和覆盖保证的不确定性度量，提高数据选择效率，在准确率上比现有ATTA方法提升约5%。
...
Method: 采用平滑保形分数和top-K确定性度量，在线权重更新算法，域偏移检测器调整人工监督，以及分阶段更新方案平衡人工标注和模型标注数据。

Result: 在广泛实验中，CPATTA在准确率上比最先进的ATTA方法持续提升约5%。

Conclusion: CPATTA成功将原则性、覆盖保证的不确定性引入ATTA，显著提高了模型在域偏移下的鲁棒性和数据选择效率。
...
### [225] [Can VLM Pseudo-Labels Train a Time-Series QA Model That Outperforms the VLM?](https://arxiv.org/abs/2509.25696)
...
Method: 使用视觉语言模型生成伪标签来训练TSQA模型，利用深度神经网络对噪声标签的固有鲁棒性。

Result: 实验结果表明，TSQA模型不仅能用伪标签成功训练，还能通过利用大量未标记数据超越VLM本身的性能。
...
### [226] [Physics-Informed Learning for Human Whole-Body Kinematics Prediction via Sparse IMUs](https://arxiv.org/abs/2509.25704)
*Cheng Guo,Giuseppe L'Erario,Giulio Romualdi,Mattia Leonori,Marta Lorenzini,Arash Ajoudani,Daniele Pucci*

Main category: cs.LG

TL;DR: 提出一个基于物理信息的学习框架，使用5个IMU的惯性测量数据预测人体运动，通过整合领域知识和物理约束来提高预测准确性和物理可行性


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏未来预测和物理约束考虑，且过度依赖历史姿态数据，这些在现实场景中并不总是可用

Method: 设计考虑人体运动空间特性的网络，在训练中加入正向和微分运动学函数作为额外损失项，在推理阶段通过迭代细化预测来更新关节状态缓冲区

Result: 实验结果显示该方法实现了高精度、平滑的运动过渡，并对未见过的受试者具有良好的泛化能力

Conclusion: 该物理信息学习框架能够有效预测人体运动，解决了现有方法的局限性，为安全无缝的人机协作提供了可靠支持
...
### [227] [Adaptive Graph Coarsening for Efficient GNN Training](https://arxiv.org/abs/2509.25706)
...
Conclusion: 该方法能够适应学习任务进行图粗化，特别适用于异类性数据等传统方法难以处理的场景。
...
### [228] [Expert Merging: Model Merging with Unsupervised Expert Alignment and Importance-Guided Layer Chunking](https://arxiv.org/abs/2509.25712)
*Dengming Zhang,Xiaowen Ma,Zhenliang Ni,Zhenkai Wu,Han Shu,Xin Jiang,Xinghao Chen*

Main category: cs.LG

TL;DR: 提出Expert Merging和Expert Merging++方法，通过层间系数学习和重要性引导分块技术，实现无需标签、参数高效的多专家模型融合，在LLM和MLLM上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法存在依赖手动调参、参数对齐而非任务行为对齐、忽略层间异质性等问题，需要更高效、可控的融合方案。

Method: Expert Merging使用未标注校准数据学习层间系数，通过隐藏状态和对数对齐优化，加入正则化和任务加权损失；Expert Merging++引入重要性引导分块，根据学习系数、任务向量大小和参数数量分配分块系数。

Result: 在InternVL、Qwen2-VL和Mistral等骨干网络上，该方法超越了强训练自由和训练基础的融合基线，Expert Merging++在某些情况下甚至超过监督混合训练。

Conclusion: 该方法为LLM和MLLM的多专家模型融合提供了无标签、参数高效且可扩展的解决方案，显著提升了融合性能。
...
### [229] [Reweighted Flow Matching via Unbalanced OT for Label-free Long-tailed Generation](https://arxiv.org/abs/2509.25713)
*Hyunsoo Song,Minjung Gim,Jaewoong Choi*

Main category: cs.LG

TL;DR: 提出了UOT-RFM框架，通过无类别标签的不平衡最优传输和重加权策略解决长尾分布生成中的多数类偏差问题


<details>
  <summary>Details</summary>
Motivation: 标准流匹配在长尾分布中会偏向多数类，导致少数类生成质量低且无法匹配真实类别比例

Method: 使用小批量不平衡最优传输构建条件向量场，通过基于密度比的标签无关多数分数进行逆重加权

Result: 在长尾基准测试中优于现有流匹配基线，在平衡数据集上保持竞争力

Conclusion: UOT-RFM能够有效缓解多数类偏差，无需类别标签即可改善尾部类别生成质量
...
### [230] [MuPlon: Multi-Path Causal Optimization for Claim Verification through Controlling Confounding](https://arxiv.org/abs/2509.25715)
...
<details>
  <summary>Details</summary>
Motivation: 传统声明验证方法忽视证据间的复杂交互，导致不可靠结果；完全连接图方法面临数据噪声和数据偏差两大挑战。

Method: MuPlon框架集成后门路径和前门路径：后门路径通过优化节点概率权重稀释噪声干扰并加强相关证据连接；前门路径提取相关子图构建推理路径，应用反事实推理消除数据偏差。

Result: 实验结果表明MuPlon优于现有方法，达到最先进性能。

Conclusion: MuPlon通过双因果干预策略有效解决了声明验证中的数据噪声和偏差问题，提升了验证可靠性。
...
### [231] [Beyond Point Estimates: Likelihood-Based Full-Posterior Wireless Localization](https://arxiv.org/abs/2509.25719)
*Haozhe Lei,Hao Guo,Tommy Svensson,Sundeep Rangan*

Main category: cs.LG

TL;DR: 提出MC-CLE方法，使用蒙特卡洛采样训练神经网络评分器来比较真实和候选发射器位置，为无线定位提供不确定性量化
...
Method: 将定位建模为从接收器测量中推断未知发射器位置的后验推理，使用蒙特卡洛采样训练神经网络评分网络

Result: 在线视距多天线接收器模拟中，MC-CLE学习了包括角度模糊和前向后天线模式在内的关键特性，相比均匀基线和高斯后验实现了更低的交叉熵损失

Conclusion: MC-CLE方法能够有效学习无线定位中的关键特性，并提供更好的不确定性量化
...
### [232] [Boundary-to-Region Supervision for Offline Safe Reinforcement Learning](https://arxiv.org/abs/2509.25727)
*Huikang Su,Dengyun Peng,Zifeng Zhuang,YuHan Liu,Qiguang Chen,Donglin Wang,Qinghe Liu*

Main category: cs.LG

TL;DR: B2R框架通过非对称条件化解决离线安全强化学习中的约束满足问题，将成本信号重新对齐为边界约束，在38个安全关键任务中35个满足安全约束且性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于序列模型的方法对回报目标和成本目标使用对称的输入标记条件化，忽略了它们内在的不对称性：回报目标是灵活的性能目标，而成本目标应该是严格的安全边界。这种对称条件化导致不可靠的约束满足，特别是在遇到分布外成本轨迹时。

Method: 提出Boundary-to-Region (B2R)框架，通过成本信号重新对齐实现非对称条件化。B2R将成本目标重新定义为固定安全预算下的边界约束，统一所有可行轨迹的成本分布同时保留奖励结构。结合旋转位置嵌入，增强安全区域内的探索。
...
### [233] [A Physics-Guided Probabilistic Surrogate Modeling Framework for Digital Twins of Underwater Radiated Noise](https://arxiv.org/abs/2509.25730)
*Indu Kant Deo,Akash Venkateshwaran,Rajeev K. Jaiman*

Main category: cs.LG

TL;DR: 提出了一个物理引导的概率框架来预测真实海洋环境中的三维传输损失，用于船舶交通噪声缓解的数字孪生系统。


<details>
  <summary>Details</summary>
Motivation: 船舶交通是沿海水域水下辐射噪声日益增长的来源，需要实时数字孪生系统进行噪声缓解。

Method: 结合稀疏变分高斯过程、物理基础均值函数、深度sigma点过程和随机变分深度核学习，集成可学习的物理信息均值、卷积编码器、神经编码器和残差SVGP层。

Result: 构建了超过3000万个源-接收器对的数据集，开发了能够提供校准预测不确定性的概率数字孪生框架。

Conclusion: 该框架推进了海洋声学的不确定性感知数字孪生，展示了物理引导的机器学习如何支持可持续海事运营。
...
### [234] [Less is More: Towards Simple Graph Contrastive Learning](https://arxiv.org/abs/2509.25742)
*Yanan Zhao,Feng Ji,Jingyang Dai,Jiaze Ma,Wee Peng Tay*

Main category: cs.LG

TL;DR: 提出了一种简单有效的图对比学习方法，通过在异配性图上结合节点特征和图结构特征来降低特征噪声，无需复杂的数据增强或负采样。


<details>
  <summary>Details</summary>
Motivation: 现有图对比学习方法在异配性图（连接节点通常属于不同类别）上效果有限，且依赖复杂的增强方案、编码器或负采样，需要验证这种复杂性是否真正必要。

Method: 使用GCN编码器捕获结构特征，MLP编码器分离节点特征噪声，将原始节点特征和图结构作为对比学习的两个互补视图，无需数据增强或负采样。

Result: 在异配性基准测试中达到最先进结果，计算和内存开销最小，在配性图上也具有复杂度、可扩展性和鲁棒性优势。

Conclusion: 证明了简单的方法可以在异配性图上取得优异性能，为图对比学习提供了新的理论基础和实践指导。
...
### [235] [Rotation Control Unlearning: Quantifying and Controlling Continuous Unlearning for LLM with The Cognitive Rotation Space](https://arxiv.org/abs/2509.25743)
*Xiang Zhang,Kun Wei,Xu Yang,Chenghao Xu,Su Yan,Cheng Deng*

Main category: cs.LG

TL;DR: 提出了一种新的机器遗忘方法RCU，通过旋转控制来解决连续遗忘过程中的累积灾难性效用损失问题，无需依赖保留数据集即可实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，其安全漏洞日益受到关注。现有遗忘方法不仅依赖保留数据集来保持模型效用，而且在连续遗忘请求下会遭受累积的灾难性效用损失。

Method: 提出了旋转控制遗忘(RCU)方法，利用旋转显著性权重来量化和控制连续遗忘过程中的遗忘程度。设计了斜对称损失来构建认知旋转空间，通过旋转角度的变化模拟连续遗忘过程，并使用正交旋转轴正则化来最小化干扰。

Result: 在多个数据集上的实验证实，该方法无需保留数据集即可达到最先进的性能。

Conclusion: RCU方法有效解决了连续机器遗忘中的累积灾难性效用损失问题，为LLM安全漏洞的缓解提供了新的解决方案。
...
### [236] [OPPO: Accelerating PPO-based RLHF via Pipeline Overlap](https://arxiv.org/abs/2509.25762)
*Kaizhuo Yan,Yingjie Yu,Yifan Yu,Haizhong Zheng,Fan Lai*

Main category: cs.LG

TL;DR: OPPO是一个新颖的轻量级PPO-based RLHF框架，通过重叠流水线执行提高训练效率，加速训练1.8-2.8倍，提升GPU利用率1.4-2.1倍。


<details>
  <summary>Details</summary>
Motivation: 传统的PPO-based RLHF训练管道存在效率低下问题，包括顺序多模型依赖和长尾响应长度导致的阶段完成延迟。

Method: 引入两种新技术：1) 内部步骤重叠，将上游模型输出流式传输为合适大小的块，使下游模型能够在上游继续解码时开始预填充；2) 跨步骤重叠，自适应地过度提交少量提示并将长生成推迟到未来步骤。

Result: 评估显示OPPO将PPO-based RLHF训练加速1.8-2.8倍，GPU利用率提高1.4-2.1倍，且不影响训练收敛。

Conclusion: OPPO是一个模型无关的轻量级框架，只需几行代码修改即可与现有PPO实现集成，显著提升RLHF训练效率。
...
### [237] [Autonomy-Aware Clustering: When Local Decisions Supersede Global Prescriptions](https://arxiv.org/abs/2509.25775)
*Amber Srivastava,Salar Basiri,Srinivasa Salapaka*

Main category: cs.LG

TL;DR: 提出了一种自主感知聚类框架，使用强化学习和确定性退火方法，无需预先了解自主性形式即可学习和考虑局部自主性的影响。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法假设实体是被动的，但实际上实体具有局部自主性，会以特征表示无法完全捕捉的方式覆盖规定的关联，这会显著改变聚类结果。

Method: 结合强化学习与确定性退火程序，提出自适应距离估计网络（ADEN），这是一个基于transformer的注意力模型，可在RL循环中学习实体与聚类代表之间的依赖关系。

Result: 即使没有明确的自主性模型，该框架也能获得接近真实情况的解（差距约3-4%），而忽略自主性会导致更大差距（约35-40%）。

Conclusion: 自主感知聚类框架能够很好地与底层数据动态对齐，有效处理实体自主性对聚类结果的影响。
...
### [238] [Online Decision Making with Generative Action Sets](https://arxiv.org/abs/2509.25777)
*Jianyu Xu,Vidhi Jain,Bryan Wilder,Aarti Singh*

Main category: cs.LG

TL;DR: 提出了一种双重乐观算法，用于解决在线学习中动态生成新动作的问题，通过LCB选择动作和UCB生成动作，在医疗问答数据集上验证了有效性，并证明了次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的发展，智能体可以在线学习过程中动态创建新动作，但生成动作需要成本，需要在成本与潜在收益之间取得平衡。

Method: 提出双重乐观算法，使用下置信界(LCB)进行动作选择，使用上置信界(UCB)进行动作生成，平衡利用、探索和创建三个目标。

Result: 在医疗问答数据集上的实验表明，该方法相比基线策略在生成质量权衡方面表现更优。理论分析证明算法达到最优遗憾界O(T^(d/(d+2))d^(d/(d+2)) + d√(TlogT))。
...
### [239] [A Hamiltonian driven Geometric Construction of Neural Networks on the Lognormal Statistical Manifold](https://arxiv.org/abs/2509.25778)
*Prosper Rosaire Mama Assandje,Teumsa Aboubakar,Dongho Joseph,Takemi Nakamura*

Main category: cs.LG

TL;DR: 提出了一种在统计流形上构建神经网络的方法，以对数正态统计流形为例，从哈密顿系统推导网络组件，实现几何驱动的神经网络架构。


<details>
  <summary>Details</summary>
Motivation: 将信息几何与机器学习结合，探索在统计流形上构建内禀神经网络的可能性，利用微分几何为学习系统提供新范式。

Method: 基于对数正态统计流形的哈密顿系统，使用庞加莱盘坐标系统定义输入，从SU(1,1)李群作用推导权重矩阵的旋转分量，从系统的辛结构推导激活函数。

Result: 成功构建了完整的神经网络架构，包括权重矩阵和输出值，证明了对数正态流形可以作为神经流形使用。
...
### [240] [From Cheap Geometry to Expensive Physics: Elevating Neural Operators via Latent Shape Pretraining](https://arxiv.org/abs/2509.25788)
*Zhizhou Zhang,Youjia Wu,Kaixuan Zhang,Yanjia Wang*

Main category: cs.LG

TL;DR: 提出两阶段框架，利用无物理标签的几何数据预训练自编码器，然后将学到的潜在表示用于神经算子训练，在有限标签数据下提升PDE求解预测精度。


<details>
  <summary>Details</summary>
Motivation: 工业设计评估依赖高保真PDE仿真，但计算昂贵；算子学习可加速预测但受限于物理标签数据稀缺，而大量几何设计数据未被充分利用。

Method: 第一阶段：在几何重建任务上预训练自编码器学习潜在表示；第二阶段：使用预训练的潜在嵌入作为输入，以监督方式训练神经算子预测PDE解。采用基于Transformer的架构处理点云数据。

Result: 在四个PDE数据集和三种最先进的基于Transformer的神经算子上，相比直接在原始点云输入上训练的模型，该方法一致提高了预测精度。

Conclusion: 来自物理无关预训练的表征为数据高效的算子学习提供了强大基础，能更好地利用丰富的几何设计数据资源。
...
### [241] [Characterization and Learning of Causal Graphs with Latent Confounders and Post-treatment Selection from Interventional Data](https://arxiv.org/abs/2509.25800)
*Gongxu Luo,Loka Li,Guangyi Chen,Haoyue Dai,Kun Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种考虑后处理选择偏差的因果发现方法，通过引入新的因果公式和FI-Markov等价类，能够在使用干预数据时区分真实的因果关系和选择模式。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现方法忽视了后处理选择偏差问题，这在生物研究中很常见（如基因表达分析中的质量控制）。这种选择偏差会引入虚假依赖关系，扭曲因果发现结果。

Method: 提出了新的因果公式来显式建模后处理选择，定义了FI-Markov等价类和F-PAG图表示，并开发了F-FCI算法来识别因果关系、潜在混杂因子和后处理选择。

Result: 在合成和真实数据集上的实验表明，该方法能够在存在选择偏差和潜在混杂因子的情况下恢复真实的因果关系。

Conclusion: 后处理选择偏差是因果发现中的重要挑战，本文提出的方法能够有效处理这一问题，超越了传统的等价类方法，为存在选择偏差的因果发现提供了可靠解决方案。
...
### [242] [CardioForest: An Explainable Ensemble Learning Model for Automatic Wide QRS Complex Tachycardia Diagnosis from ECG](https://arxiv.org/abs/2509.25804)
*Vaskar Chakma,Ju Xiaolin,Heling Cao,Xue Feng,Ji Xiaodong,Pan Haiyan,Gao Zhan*

Main category: cs.LG

TL;DR: 开发并评估了基于集成机器学习的WCT自动检测框架，结合CardioForest、XGBoost和LightGBM模型，在MIMIC-IV数据集上取得优异性能，并通过SHAP分析增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 开发高精度且可解释的WCT自动检测系统，帮助心脏病专家在紧急情况下做出及时准确的诊断。

Method: 集成学习方法，包括优化的Random Forest（CardioForest）、XGBoost和LightGBM，使用MIMIC-IV ECG数据集进行训练和测试，并采用SHAP进行模型解释。

Result: CardioForest模型在所有指标上表现最佳，测试准确率达94.95%，平衡准确率88.31%，SHAP分析确认模型能正确识别QRS持续时间等临床相关特征。

Conclusion: CardioForest是一个高度可靠且可解释的WCT检测模型，通过准确预测和透明度为心脏病专家提供有价值的诊断支持工具。
...
### [243] [Improving Sampling Efficiency in RLVR through Adaptive Rollout and Response Reuse](https://arxiv.org/abs/2509.25808)
*Yuheng Zhang,Wenlin Yao,Changlong Yu,Yao Liu,Qingyu Yin,Bing Yin,Hyokun Yun,Lihong Li*

Main category: cs.LG

TL;DR: AR3PO是一种采样高效的RLVR算法，通过自适应采样和响应重用技术解决GRPO中优势消失问题，在多个基准测试中优于GRPO，与DAPO性能相当但显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决GRPO算法中当组内所有响应获得相同奖励时出现的优势消失问题，提高采样效率和训练效果。

Method: 提出自适应采样（动态分配更多响应给困难提示）和响应重用（利用先前生成的正确响应提供训练信号）两种新技术。

Result: 在7B和8B模型上优于GRPO，与DAPO相当或更好，降低采样成本达4.2倍；在32B模型上与DAPO性能相当但采样成本显著更低。

Conclusion: AR3PO是一种高效且有效的RLVR算法，通过创新的采样策略解决了现有方法的局限性，在保持性能的同时大幅降低了计算成本。
...
### [244] [Learning to Reason as Action Abstractions with Scalable Mid-Training RL](https://arxiv.org/abs/2509.25810)
*Shenao Zhang,Donghan Yu,Yihao Feng,Bowen Jin,Zhaoran Wang,John Peebles,Zirui Wang*

Main category: cs.LG

TL;DR: 该论文提出了RA3算法，通过中期训练阶段发现紧凑的动作子空间，提升语言模型在强化学习中的表现，在代码生成任务上取得了显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在强化学习中表现优异，但完全释放其潜力需要一个中期训练阶段来识别有用的紧凑动作集，并实现快速选择。

Method: 提出了Reasoning as Action Abstractions (RA3)算法，通过推导序列变分下界，迭代发现时间一致的潜在结构，并在引导数据上进行微调。

Result: 在多个基础模型上，RA3在HumanEval和MBPP上的平均性能分别比基础模型和下一个token预测基线提高了8分和4分，在RLVR中实现了更快的收敛和更高的渐近性能。

Conclusion: 中期训练在决策空间紧凑且有效视野较短时最为有效，强调在动作抽象空间而非原始动作空间中操作的重要性。
...
### [245] [Decentralized Asynchronous Multi-player Bandits](https://arxiv.org/abs/2509.25824)
*Jingqi Fan,Canzhe Zhao,Shuai Li,Siwei Wang*

Main category: cs.LG

TL;DR: 提出了一种用于完全异步去中心化多玩家多臂老虎机问题的算法，通过自适应切换探索和利用阶段来减少碰撞并检测玩家数量变化。


<details>
  <summary>Details</summary>
Motivation: 现实世界系统通常是去中心化和异步的，玩家可能随时进出系统且没有全局时钟，这带来了碰撞避免和玩家数量检测两大挑战。

Method: 玩家自适应地在探索和利用之间切换：探索阶段均匀拉动臂以减少碰撞概率；利用阶段以较小概率继续拉动其他玩家正在利用的臂来检测玩家离开。

Result: 算法实现了O(√(T log T) + log T/Δ²)的遗憾界，其中Δ是任意两个臂期望奖励的最小差距。
...
### [246] [Kairos: Towards Adaptive and Generalizable Time Series Foundation Models](https://arxiv.org/abs/2509.25826)
...
<details>
  <summary>Details</summary>
Motivation: 时间序列在不同时间段具有不均匀的信息密度，受系统状态和信号复杂度影响。现有时间序列基础模型使用非自适应处理流程，无法捕捉这种动态特性，限制了在零样本场景下的建模能力。

Method: 提出Kairos框架，包含动态分块标记化器和实例自适应位置嵌入。通过自适应选择标记化粒度，并为每个时间序列实例定制位置编码。在包含3000亿时间点的大规模可预测性分层时间序列语料库上训练，并在推理阶段采用多分块预测策略。

Result: 在两个常见的零样本基准测试（GIFT-Eval和Time-Series-Library基准）上取得了优越性能，使用更少的参数就能持续超越现有方法，在多样化任务中表现一致优异。

Conclusion: Kairos通过自适应处理机制有效解决了时间序列信息密度不均匀的建模挑战，为时间序列基础模型提供了更灵活和高效的解决方案。
...
### [247] [MIDAS: Misalignment-based Data Augmentation Strategy for Imbalanced Multimodal Learning](https://arxiv.org/abs/2509.25831)
*Seong-Hyeon Hwang,Soyoung Choi,Steven Euijong Whang*

Main category: cs.LG

TL;DR: MIDAS是一种数据增强策略，通过生成语义不一致的多模态错位样本，并使用弱模态加权和难样本加权来缓解多模态模型中的模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 多模态模型往往过度依赖主导模态而无法达到最佳性能，现有方法主要关注修改训练目标或优化过程，而数据中心的解决方案尚未充分探索。

Method: 提出MIDAS方法：1) 生成跨模态语义不一致的错位样本；2) 使用单模态置信度进行标注；3) 引入弱模态加权动态增加最不自信模态的损失权重；4) 提出难样本加权优先处理语义模糊的错位样本。

Result: 在多个多模态分类基准测试中，MIDAS显著优于相关基线方法，有效解决了模态不平衡问题。

Conclusion: MIDAS通过数据增强和动态加权策略，成功缓解了多模态模型中的模态不平衡问题，提高了模型性能。
...
### [248] [Distillation of Large Language Models via Concrete Score Matching](https://arxiv.org/abs/2509.25837)
*Yeongmin Kim,Donghyeok Shin,Mina Kang,Byeonghu Na,Il-Chul Moon*

Main category: cs.LG

TL;DR: 提出Concrete Score Distillation (CSD)方法，通过离散分数匹配克服传统知识蒸馏中softmax引起的平滑问题和logit平移不变性限制，在LLM蒸馏中实现更好的性能。
...
Result: 在GPT-2-1.5B、OpenLLaMA-7B和GEMMA-7B-IT上的实验表明，CSD持续超越近期KD目标，实现有利的保真度-多样性权衡，并与策略技术结合获得互补增益。
...
### [249] [S$^2$FS: Spatially-Aware Separability-Driven Feature Selection in Fuzzy Decision Systems](https://arxiv.org/abs/2509.25841)
*Suping Xu,Chuyi Dai,Ye Liu,Lin Shang,Xibei Yang,Witold Pedrycz*

Main category: cs.LG

TL;DR: 提出了一种面向模糊决策系统的空间感知可分离性特征选择方法S^2FS，通过整合空间方向信息来提升分类边界清晰度，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有特征选择方法要么未能直接与学习性能对齐，要么仅使用非方向性欧氏距离来捕捉决策类关系，限制了决策边界的清晰度。而实例的空间分布对边界清晰度有潜在影响。

Method: 提出S^2FS框架，采用空间感知可分离性准则，综合考虑类内紧凑性和类间分离性，整合标量距离与空间方向信息。使用前向贪心策略迭代选择最具判别性的特征。

Result: 在10个真实数据集上的实验表明，S^2FS在分类准确率和聚类性能上均优于8种最先进的特征选择算法，特征可视化进一步证实了所选特征的可解释性。

Conclusion: S^2FS通过整合空间方向信息，能够更全面地刻画类别结构，有效提升模糊决策系统的预测性能和可解释性。
...
### [250] [Knapsack RL: Unlocking Exploration of LLMs via Optimizing Budget Allocation](https://arxiv.org/abs/2509.25849)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM自改进方法采用均匀探索预算分配，导致简单任务持续成功、困难任务持续失败，都产生零梯度，影响GRPO训练效果。

Method: 将每个任务的探索视为具有不同"价值"和"成本"的"物品"，建立与经典背包问题的联系，推导出基于模型当前学习状态的自适应资源分配规则。

Result: 在GRPO中将非零策略梯度有效比例提高20-40%，可为特别困难问题分配更大预算（如93次rollout），在数学推理基准上平均提升2-4分，特定任务峰值提升9分。

Conclusion: 该方法作为计算"免费午餐"，能从学习饱和的任务重新分配预算到最有影响的任务，相比传统均匀分配可节省约2倍计算资源达到相同性能。
...
### [251] [RL-Guided Data Selection for Language Model Finetuning](https://arxiv.org/abs/2509.25850)
*Animesh Jha,Harshit Gupta,Ananjan Nandi*

Main category: cs.LG

TL;DR: 本文提出使用强化学习来优化大语言模型微调时的数据选择问题，通过将数据选择建模为马尔可夫决策过程，在仅使用5%数据的情况下达到或超过全数据集微调的性能，同时减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法主要面向预训练场景，在微调设置下效果不佳。数据选择本质上是一个预算约束下的优化问题，需要找到在有限训练数据下最大化模型下游性能的方法。

Method: 将数据选择问题重新表述为可处理的马尔可夫决策过程，使用各种强化学习方法训练智能体学习最优数据选择策略，通过基于代理模型的高效奖励信号进行指导。

Result: 在四个数据集上的实验表明，使用该方法选择的5%数据子集进行微调，其性能匹配甚至超过全数据集微调，准确率最高提升10.8个百分点，同时训练时间减少2倍。

Conclusion: 强化学习指导的数据选择方法具有很大潜力，能够在严格的数据预算下显著提升大语言模型的微调效率和性能。
...
### [252] [Efficient On-Policy Reinforcement Learning via Exploration of Sparse Parameter Space](https://arxiv.org/abs/2509.25876)
...
<details>
  <summary>Details</summary>
Motivation: 传统策略梯度方法如PPO通常沿着单一随机梯度方向更新，忽略了参数空间的丰富局部结构。先前研究表明替代梯度与真实奖励地形相关性差，可视化显示更高性能的解往往位于附近未探索区域。

Method: 提出ExploRLer框架，与PPO和TRPO等on-policy算法无缝集成，系统性地探测替代on-policy梯度更新周围的未探索邻域。

Result: 在不增加梯度更新次数的情况下，ExploRLer在复杂连续控制环境中相比基线方法取得了显著改进。

Conclusion: 迭代级探索为增强on-policy强化学习提供了实用有效的方法，并为替代目标的局限性提供了新视角。
...
### [253] [Federated Learning with Enhanced Privacy via Model Splitting and Random Client Participation](https://arxiv.org/abs/2509.25906)
*Yiwei Li,Shuai Wang,Zhuojun Tian,Xiuhua Wang,Shijian Su*

Main category: cs.LG

TL;DR: 提出MS-PAFL框架，通过模型分割和隐私放大技术，在联邦学习中实现更好的隐私-效用权衡，显著减少满足目标隐私保护水平所需的噪声量。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中使用差分隐私保护客户端数据时，添加的噪声会严重降低模型精度，需要解决这一挑战。

Method: 采用模型分割策略，将客户端模型分为本地保留的私有子模型和用于全局聚合的公共子模型，仅在公共子模型注入校准高斯噪声，并结合随机客户端参与和本地数据子采样实现隐私放大。

Result: 理论分析表明MS-PAFL显著减少了满足目标隐私保护所需的噪声，实验验证该框架在强隐私保证下能训练出高精度模型，实现优越的隐私-效用权衡。

Conclusion: MS-PAFL通过模型分割和隐私放大技术有效解决了联邦学习中差分隐私导致的精度下降问题，为隐私保护机器学习提供了实用解决方案。
...
### [254] [ReNF: Rethinking the Design Space of Neural Long-Term Time Series Forecasters](https://arxiv.org/abs/2509.25914)
*Yihang Lu,Xianwei Meng,Enhong Chen*

Main category: cs.LG

TL;DR: 该论文提出了一种基于第一性原理重新设计长时序预测范式的方法，通过引入多神经预测定理和Boosted Direct Output策略，结合自回归和直接输出的优势，使简单的MLP模型在几乎所有情况下都优于复杂的现有模型。


<details>
  <summary>Details</summary>
Motivation: 当前神经预测器在长时序预测领域过度强调架构复杂性而忽视了基本预测原则，阻碍了该领域的进展。

Method: 提出多神经预测定理作为理论基础，设计Boosted Direct Output策略结合自回归和直接输出的优势，并通过平滑跟踪模型参数来稳定学习过程。

Result: 实验表明这些基于原则的改进使简单的MLP模型在几乎所有情况下都达到了最先进的性能，超越了近期复杂的模型。

Conclusion: 该研究验证了提出的定理，建立了动态性能边界，并为未来研究指明了有前景的方向。
...
### [255] [From MNIST to ImageNet: Understanding the Scalability Boundaries of Differentiable Logic Gate Networks](https://arxiv.org/abs/2509.25933)
*Sven Brändle,Till Aczel,Andreas Plesner,Roger Wattenhofer*

Main category: cs.LG

TL;DR: 本文研究可微分逻辑门网络(DLGNs)在大规模多类数据集上的行为，探讨其表达能力、可扩展性和输出策略，重点关注温度调谐和Group-Sum层在多达2000类分类中的应用。


<details>
  <summary>Details</summary>
Motivation: DLGNs作为传统前馈网络的快速节能替代方案，目前主要在小数据集(最多10类)上测试，需要研究其在大规模多类数据集上的表现和扩展性。

Method: 使用合成和真实世界数据集，评估DLGNs的表达能力、可扩展性和不同输出策略，特别关注温度调谐对输出层性能的影响。

Result: 提供了关于温度调谐重要性的关键见解，评估了Group-Sum层在不同条件下的表现，并展示了其在多达2000类大规模分类中的应用。

Conclusion: DLGNs在大规模多类分类中具有潜力，温度调谐对性能至关重要，Group-Sum层在特定条件下表现良好，可扩展到2000类分类任务。
...
### [256] [AIM: Adaptive Intervention for Deep Multi-task Learning of Molecular Properties](https://arxiv.org/abs/2509.25955)
*Mason Minot,Gisbert Schneider*

Main category: cs.LG

TL;DR: 提出AIM优化框架，通过动态策略调解多任务学习中的梯度冲突，在数据稀缺的药物发现场景中显著提升性能，并提供可解释性分析工具。


<details>
  <summary>Details</summary>
Motivation: 多任务学习中存在破坏性梯度干扰问题，特别是在数据稀缺的药物发现场景中，这限制了多属性分子设计的效率。

Method: AIM框架学习动态策略来调解梯度冲突，通过包含密集可微分正则化器的新增目标函数，联合训练策略和主网络，产生几何稳定且动态高效的更新。

Result: 在QM9和靶向蛋白降解剂基准测试的子集上，AIM相比多任务基线获得统计显著改进，在数据稀缺场景中优势最明显。

Conclusion: AIM结合数据高效性能和诊断洞察，展示了自适应优化器通过创建更鲁棒和可解释的多属性分子设计模型来加速科学发现的潜力。
...
### [257] [Reevaluating Convolutional Neural Networks for Spectral Analysis: A Focus on Raman Spectroscopy](https://arxiv.org/abs/2509.25964)
*Deniz Soysal,Xabier García-Andrade,Laura E. Rodriguez,Pablo Sobron,Laura M. Barge,Renaud Detry*

Main category: cs.LG

TL;DR: 该论文提出了一个用于自主拉曼光谱分类的完整工作流程，包括基线无关分类、池化控制鲁棒性、标签高效学习和恒定时间适应四个关键进展，能够在荧光基线、峰位移和有限标签条件下实现稳健分类。


<details>
  <summary>Details</summary>
Motivation: 解决火星车、深海着陆器和野外机器人等自主设备中拉曼光谱分析面临的挑战：荧光基线失真、峰位移以及有限的地面真实标签问题，实现无需背景校正和峰提取的端到端分类。

Method: 使用一维卷积神经网络（CNN），结合池化参数调优控制鲁棒性，采用半监督生成对抗网络和对比预训练提高标签效率，通过冻结CNN主干并仅重训练softmax层实现恒定时间适应。

Result: 紧凑CNN在基线无关分类中优于k近邻和支持向量机；池化调优可适应30 cm-1的拉曼位移；半监督方法在仅10%标签下提升准确率11%；恒定时间适应在资源受限处理器上优于Siamese网络。

Conclusion: 该工作流程为自主探索中的稳健、低足迹拉曼分类提供了实用路径，通过训练原始光谱、调优池化、添加半监督和轻量微调，实现了高效的模型适应和部署。
...
### [258] [Data-Free Continual Learning of Server Models in Model-Heterogeneous Federated learning](https://arxiv.org/abs/2509.25977)
...
<details>
  <summary>Details</summary>
Motivation: 传统联邦学习面临数据异构、模型异构、灾难性遗忘和知识错位等挑战，特别是在新数据不断涌现和模型多样化的动态环境中。

Method: 利用预训练扩散模型提取轻量级类特定原型，实现三方面数据无关优势：生成当前任务的合成数据、无示例生成重放、从异构客户端到服务器的数据无关动态知识转移。

Result: 在多个数据集上的实验结果表明FedDCL的有效性，展示了其在动态环境中增强联邦学习泛化能力和实际应用潜力的能力。

Conclusion: FedDCL框架能够有效解决联邦学习中的数据异构、模型异构和持续学习挑战，为动态环境中的联邦学习提供了实用的解决方案。
...
### [259] [Reconcile Certified Robustness and Accuracy for DNN-based Smoothed Majority Vote Classifier](https://arxiv.org/abs/2509.25979)
*Gaojie Jin,Xinping Yi,Xiaowei Huang*

Main category: cs.LG

TL;DR: 本文在PAC-Bayesian框架下，研究了平滑多数投票分类器的泛化误差界和认证鲁棒性，提出了一种基于权重谱范数的谱正则化方法来提升认证鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在PAC-Bayesian框架中，Gibbs分类器和Q加权多数投票分类器常用于分析泛化性能，但缺乏对多数投票分类器认证鲁棒性及其与泛化关系的研究。

Method: 开发了具有认证鲁棒半径的泛化误差界，发现泛化界和认证鲁棒半径都依赖于权重谱范数，因此采用谱正则化进行平滑训练。利用球面高斯输入的维度无关特性，提出了一种新颖且计算成本低的谱正则化器。

Result: 理论分析表明该方法有效，并通过实证结果验证了所提方法的有效性。

Conclusion: 本文为平滑多数投票分类器建立了泛化误差界与认证鲁棒性的理论联系，并提出了一种高效的谱正则化方法来提升认证鲁棒性。
...
### [260] [Exact Solutions to the Quantum Schrödinger Bridge Problem](https://arxiv.org/abs/2509.25980)
...
<details>
  <summary>Details</summary>
Motivation: 量子薛定谔桥问题描述的是由薛定谔方程而非传统实值波动方程控制的随机过程演化，虽然数学文献中已有研究，但本文从拉格朗日视角重新表述，特别适合生成建模。

Method: 从动态最优输运的拉格朗日表述出发，求解福克-普朗克方程和哈密顿-雅可比方程，推导高斯分布间的精确闭式解，并基于此开发高斯混合模型算法。

Result: 发现量子薛定谔桥问题在高斯分布间的解仍是高斯过程，但由于量子效应协方差演化不同；开发的算法在单细胞演化数据、图像生成、分子翻译和平均场博弈等实验中表现有效。

Conclusion: 量子薛定谔桥问题通过引入玻姆势体现了随机过程中的非局域性，这是量子力学系统的典型特征；推导的高斯闭式解为量子生成建模提供了理论基础，实验验证了方法的有效性。
...
### [261] [CAST: Continuous and Differentiable Semi-Structured Sparsity-Aware Training for Large Language Models](https://arxiv.org/abs/2509.25996)
*Weiyu Huang,Yuezhou Hu,Jun Zhu,Jianfei Chen*

Main category: cs.LG

TL;DR: CAST是一种连续自适应稀疏训练框架，用于将大语言模型转换为半结构化稀疏模式，通过联合优化稀疏模式和权重，显著减少推理延迟和内存消耗。


<details>
  <summary>Details</summary>
Motivation: 稀疏感知训练可以有效减少大语言模型推理时的延迟和内存消耗，但现有方法通常分别优化稀疏模式和权重，无法实现无缝联合优化。

Method: CAST框架包含三个核心组件：AdamS优化器（使用自适应L1衰减促进均匀稀疏化）、权重缩放模块（缓解衰减导致的幅度减少）、知识蒸馏（使用稠密模型作为自教师提升训练效率）。

Result: 在2:4稀疏模式下，从125M到13B参数的多模型评估显示，CAST在困惑度和零样本准确率上显著优于现有方法。LLaMA2-7B稀疏模型仅使用2%原始预训练token，困惑度仅增加0.09，零样本准确率提升0.36%。

Conclusion: CAST框架能够高效训练半结构化稀疏模型，建立了可预测稀疏模型性能的扩展规律，并验证了稀疏模型在量化和微调场景下的实用性。
...
### [262] [Informed Asymmetric Actor-Critic: Leveraging Privileged Signals Beyond Full-State Access](https://arxiv.org/abs/2509.26000)
*Daniel Ebi,Gaspard Lambrechts,Damien Ernst,Klemens Böhm*

Main category: cs.LG

TL;DR: 提出了一种新的非对称行动者-评论者框架，允许在训练时使用任意特权信号而非完整状态信息，扩展了非对称方法的理论基础。


<details>
  <summary>Details</summary>
Motivation: 现有非对称方法通常假设训练时能访问完整状态，但在实际部分可观测环境中这种假设不成立。

Method: 提出了信息非对称行动者-评论者框架，通过条件化评论者使用特权信号，保持策略梯度的无偏性。

Result: 在基准导航任务和合成部分可观测环境中的实验表明，该方法在提供信息丰富的特权输入时能提高学习效率和价值估计。
...
### [263] [Indirect Attention: Turning Context Misalignment into a Feature](https://arxiv.org/abs/2509.26015)
*Bissmella Bahaduri,Hicham Talaoubrid,Fangchen Feng,Zuheng Ming,Anissa Mokraoui*

Main category: cs.LG

TL;DR: 本文分析了注意力机制在键值来自不同序列或模态时的表现，发现上下文错位会导致显著的噪声问题，并提出了间接注意力机制来更好地处理错位场景。


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制假设键和值来自相同的底层序列，但在实际应用中，键和值可能来自不同序列或模态，这种错位会导致性能下降。

Method: 首先分析注意力机制在噪声值特征下的行为，建立临界噪声阈值，然后将上下文错位建模为结构化噪声，最后提出间接注意力机制来间接推断相关性。

Result: 在合成任务和实际应用中验证了间接注意力机制在处理错位场景时的优越性能。

Conclusion: 间接注意力机制能够有效处理键值错位问题，在错位场景下比标准注意力机制表现更好。
...
### [264] [FITS: Towards an AI-Driven Fashion Information Tool for Sustainability](https://arxiv.org/abs/2509.26017)
*Daphne Theodorakopoulos,Elisabeth Eberling,Miriam Bodenheimer,Sabine Loos,Frederic Stahl*

Main category: cs.LG

TL;DR: 开发了一个基于Transformer的时尚可持续性信息工具FITS，通过NLP技术从可信文本源中提取和分类可持续性数据，以解决时尚行业可持续性信息稀缺和难以解读的问题。


<details>
  <summary>Details</summary>
Motivation: 时尚行业的可持续性信息有限且难以理解，通用语言模型缺乏领域知识且容易产生幻觉，这在需要事实准确性的领域尤其有害。

Method: 使用BERT系列语言模型（包括科学和气候特定数据预训练模型），在精心策划的语料库上进行微调，采用领域特定分类模式，通过贝叶斯优化优化超参数。

Result: 开发了FITS原型系统，允许用户搜索相关数据、分析自有数据并通过交互界面探索信息。在潜在用户焦点小组中评估了可用性、视觉设计、内容清晰度等方面。

Conclusion: 领域适应的NLP在促进知情决策方面具有价值，AI应用在应对气候相关挑战方面具有更广泛的潜力，同时提供了有价值的数据集和方法论。
...
### [265] [Muon Outperforms Adam in Tail-End Associative Memory Learning](https://arxiv.org/abs/2509.26030)
...
<details>
  <summary>Details</summary>
Motivation: 解释Muon优化器在训练LLMs时优于Adam的机制，特别是其在重尾数据分布下的表现优势。

Method: 通过消融实验分析Transformer组件，从关联记忆角度解释Muon优势，并在理论和实验上验证其在重尾数据下的性能。
...
### [266] [Stealthy Yet Effective: Distribution-Preserving Backdoor Attacks on Graph Classification](https://arxiv.org/abs/2509.26032)
...
<details>
  <summary>Details</summary>
Motivation: 现有图分类后门攻击存在结构异常（罕见子图触发器）和语义异常（标签翻转）问题，容易被异常检测模型发现，需要更隐蔽的攻击方法。

Method: 使用对抗训练学习分布内触发器，通过异常感知判别器指导训练过程，同时抑制结构和语义异常。

Result: 在真实数据集上的实验表明，DPSBA在有效性和可检测性之间取得了优越的平衡，攻击成功率高且隐蔽性强。

Conclusion: DPSBA框架解决了现有图分类后门攻击的可检测性问题，实现了更隐蔽有效的攻击。
...
### [267] [Scaling Up Temporal Domain Generalization via Temporal Experts Averaging](https://arxiv.org/abs/2509.26045)
*Aoming Liu,Kevin Miller,Venkatesh Saligrama,Kate Saenko,Boqing Gong,Ser-Nam Lim,Bryan A. Plummer*

Main category: cs.LG

TL;DR: 提出了Temporal Experts Averaging (TEA)框架，通过权重平均更新整个模型来解决时序域泛化问题，相比仅预测分类器层的方法，在保持计算效率的同时显著提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有时序域泛化方法通常只预测分类器层权重，限制了泛化能力，而预测完整模型权重计算成本过高。需要一种既能更新整个模型又保持计算效率的方法。

Method: TEA框架包含两个步骤：1) 在单个时序域上微调基础模型生成专家模型，保持功能多样性但参数相似性；2) 在主成分子空间中建模时序权重轨迹，通过自适应平均系数优化偏差-方差权衡。

Result: 在7个TDG基准测试、5个模型和2种TDG设置上的实验表明，TEA比现有TDG方法性能提升高达69%，同时效率提升高达60倍。

Conclusion: TEA通过权重平均和时序建模提供了一种高效且有效的时序域泛化解决方案，能够更好地适应未来的分布变化。
...
### [268] [Real-time Noise Detection and Classification in Single-Channel EEG: A Lightweight Machine Learning Approach for EMG, White Noise, and EOG Artifacts](https://arxiv.org/abs/2509.26058)
*Hossein Enshaei,Pariya Jebreili,Sayed Mahmoud Sakahei*

Main category: cs.LG

TL;DR: 提出一种混合频谱-时域框架，用于单通道EEG中眼电、肌电和白噪声伪迹的实时检测与分类，通过结合时域低通滤波和频域功率谱密度分析，使用PCA优化的特征融合，使轻量级MLP在噪声环境下超越复杂深度学习模型。
...
Method: 结合时域低通滤波（针对低频眼电伪迹）和频域功率谱密度分析（捕获宽频谱肌电伪迹），使用PCA优化特征融合以减少冗余并保留判别信息，采用轻量级多层感知器进行分类。

Result: 在低信噪比（-7dB）下达到99%准确率，中等噪声（4dB）下超过90%准确率；在同时多源污染情况下保持96%分类准确率；训练时间仅30秒，比CNN快97%。
...
### [269] [Clip-Low Increases Entropy and Clip-High Decreases Entropy in Reinforcement Learning of Large Language Models](https://arxiv.org/abs/2509.26114)
*Jaesung R. Park,Junsu Kim,Gyeongman Kim,Jinyoung Jo,Sean Choi,Jaewoong Cho,Ernest K. Ryu*

Main category: cs.LG

TL;DR: 论文揭示了PPO和GRPO中的裁剪机制会导致熵偏差：clip-low增加熵，clip-high减少熵。在标准参数下，clip-high效应占主导，导致熵减少。通过调整clip-low可以控制熵，防止RLVR训练中的熵崩溃。


<details>
  <summary>Details</summary>
Motivation: RLVR方法在增强大语言模型推理能力方面表现出色，但容易发生熵崩溃，导致模型快速收敛到近似确定性形式，阻碍长期RL训练中的探索和进展。

Method: 通过理论和实证分析研究PPO和GRPO中裁剪机制对熵的影响，揭示clip-low和clip-high的不同效应。

Result: 研究发现裁剪机制独立于奖励信号影响熵：clip-low增加熵，clip-high减少熵。标准参数下clip-high效应主导，导致整体熵减少。

Conclusion: 裁剪机制是RLVR中被忽视的混杂因素，可以通过调整clip-low值来主动控制熵，增加探索性，防止熵崩溃。
...
### [270] [UncertainGen: Uncertainty-Aware Representations of DNA Sequences for Metagenomic Binning](https://arxiv.org/abs/2509.26116)
*Abdulkadir Celikkanat,Andres R. Masegosa,Mads Albertsen,Thomas D. Nielsen*

Main category: cs.LG

TL;DR: UncertainGen是首个用于宏基因组分箱的概率嵌入方法，通过将DNA片段表示为潜在空间中的概率分布来建模序列不确定性，相比确定性方法有更好表现。


<details>
  <summary>Details</summary>
Motivation: 现有宏基因组分箱方法使用确定性表示（如k-mer谱或大语言模型嵌入），无法捕捉DNA序列中固有的不确定性，如物种间DNA共享和高度相似片段的表示问题。

Method: 提出概率嵌入方法，将每个DNA片段表示为潜在空间中的概率分布，引入数据自适应度量来扩展可行潜在空间，实现更灵活的分箱/聚类分离。

Result: 在真实宏基因组数据集上的实验表明，相比确定性k-mer和基于LLM的嵌入方法，该方法在分箱任务上有显著改进，为大规模宏基因组分析提供了可扩展的轻量级解决方案。

Conclusion: 概率嵌入框架能够有效建模序列级不确定性，提供嵌入可区分性的理论保证，在宏基因组分箱任务中优于现有确定性方法。
...
### [271] [Domain-Aware Hyperdimensional Computing for Edge Smart Manufacturing](https://arxiv.org/abs/2509.26131)
*Fardin Jalil Piran,Anandkumar Patel,Rajiv Malhotra,Farhad Imani*

Main category: cs.LG

TL;DR: 本文分析了超维计算(HDC)在智能制造中的应用，发现HDC超参数与性能的关系在不同应用领域并不稳定，通过系统分析信号处理和图像检测任务，揭示了编码器类型、维度等参数对性能的影响规律，并实现了优于深度学习的性能表现。


<details>
  <summary>Details</summary>
Motivation: 智能制造需要满足严格延迟和能耗要求的设备端智能，HDC提供了一种轻量级替代方案。但先前研究假设HDC超参数与性能的关系在不同应用中稳定，本文验证这一假设是否成立。

Method: 通过分析CNC加工信号质量监控和LPBF图像缺陷检测两个代表性任务，系统映射编码器类型、投影方差、超维维度等参数对精度、延迟、训练时间和能耗的影响，建立形式化复杂度模型。

Result: 信号处理偏好非线性随机傅里叶特征编码，图像检测偏好线性随机投影编码。调优后的HDC模型在精度上匹配或超越最先进的深度学习和Transformer模型，同时提供至少6倍更快的推理速度和40倍更低的训练能耗。

Conclusion: 领域感知的HDC编码是必要的，调优后的HDC为受限硬件上的实时工业AI提供了实用、可扩展的路径。未来工作将关注自适应编码器选择、扩展制造模态评估和在低功耗加速器上的验证。
...
### [272] [Accelerating Transformers in Online RL](https://arxiv.org/abs/2509.26137)
*Daniil Zelezetsky,Alexey K. Kovalev,Aleksandr I. Panov*

Main category: cs.LG

TL;DR: 提出一种使用加速器策略作为Transformer训练器的方法，通过两阶段算法稳定训练Transformer模型，在强化学习中实现更快的在线训练和更小的回放缓冲区需求。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在强化学习中的应用面临训练不稳定和实现困难的挑战，特别是在模型自由的在线强化学习中。现有学习算法难以与Transformer模型稳定结合。

Method: 两阶段算法：第一阶段使用更稳定简单的加速器模型与环境交互，同时通过行为克隆训练Transformer；第二阶段预训练好的Transformer开始在完全在线环境中与环境交互。

Result: 在ManiSkill和MuJoCo任务上的实验表明，该方法不仅实现了Transformer的稳定训练，还将图像环境的训练时间减少至一半，并将离策略方法所需的回放缓冲区大小降至1-2万，显著降低计算需求。

Conclusion: 所提出的算法能够有效加速Transformer在强化学习中的训练过程，提高训练稳定性，同时大幅减少计算资源需求，为Transformer在机器人任务中的应用提供了实用解决方案。
...
### [273] [Leveraging AI modelling for FDS with Simvue: monitor and optimise for more sustainable simulations](https://arxiv.org/abs/2509.26139)
*James Panayis,Matt Field,Vignesh Gopakumar,Andrew Lahiff,Kristian Zarebski,Aby Abraham,Jonathan L. Hodges*

Main category: cs.LG

TL;DR: 提出多管齐下的方法来提升火灾模拟的效率和效果，包括机器学习代理模型加速热传播预测、引导优化减少模拟次数，以及Simvue框架实现模拟管理和数据重用。


<details>
  <summary>Details</summary>
Motivation: 火灾模拟在规模和数量上需求很高，需要改进时间和能源消耗来满足这些需求。

Method: 使用定制机器学习代理模型预测热传播动态，比CFD软件快几个数量级；采用引导优化程序减少所需模拟次数；开发Simvue框架提供自动组织和跟踪功能。

Result: 机器学习模型比最先进CFD软件快几个数量级；在定位建筑物内最危险火灾位置时，模拟次数减少十倍；Simvue框架实现数据重用和模拟管理。

Conclusion: 多管齐下的方法显著提升了火灾模拟的效率和效果，机器学习代理模型和优化策略带来了实质性改进，Simvue框架支持可持续的模拟管理。
...
### [274] [Alignment-Aware Decoding](https://arxiv.org/abs/2509.26169)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的对齐仍然是自然语言处理中的核心挑战，现有偏好优化方法通常需要在训练时或通过提示进行干预，缺乏在推理阶段直接增强对齐的有效方法。

Method: 引入对齐感知解码(AAD)，该方法在推理阶段直接增强模型对齐，理论可解释为隐式奖励优化，但只需要标准的DPO训练设置，无需专门训练。

Result: AAD在多种对齐基准测试和模型规模下始终优于强基线方法，在数据受限场景下，AAD可以生成高质量的合成数据来改进标准解码下的对齐性能。

Conclusion: AAD提供了一种在推理阶段直接增强模型对齐的实用解决方案，特别是在标注数据有限的情况下，能够有效提升对齐性能。
...
### [275] [Neighbor-aware informal settlement mapping with graph convolutional networks](https://arxiv.org/abs/2509.26171)
*Thomas Hallopeau,Joris Guérin,Laurent Demagistri,Christovam Barcellos,Nadine Dessay*

Main category: cs.LG

TL;DR: 提出基于图卷积网络的框架，通过整合邻域地理上下文来改进非正式住区分类，在里约热内卢案例中表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将空间单元独立处理，忽略了城市结构的关联性，需要更好地整合地理上下文信息来改进非正式住区映射。

Method: 构建图结构，将每个空间单元与其相邻单元嵌入图中，使用轻量级图卷积网络进行分类，通过空间交叉验证确保泛化能力。

Result: 方法在五个不同区域表现优于标准基线，Kappa系数比单单元分类提高17个点，且优于简单的邻域特征拼接方法。

Conclusion: 图结构建模能有效编码空间结构，提升城市场景理解能力，为非正式住区映射提供了更稳健的解决方案。
...
### [276] [PDE Solvers Should Be Local: Fast, Stable Rollouts with Learned Local Stencils](https://arxiv.org/abs/2509.26186)
*Chun-Wun Cheng,Bin Dong,Carola-Bibiane Schönlieb,Angelica I Aviles-Rivero*

Main category: cs.LG

TL;DR: FINO是一种受有限差分启发的神经架构，通过可学习的卷积核和时间步进方案解决PDE，在保持局部性的同时实现多尺度表示，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的神经算子模型依赖全局混合机制（如谱卷积或注意力），容易过度平滑局部动态且计算成本高，需要一种既能保持局部结构又能高效计算的新方法。

Method: FINO使用可学习的卷积核替代固定有限差分系数，通过局部算子块（包含微分模板层、门控掩码和线性融合步骤）构建自适应导数特征，采用显式可学习时间步进方案，并嵌入编码器-解码器架构。

Result: 在六个基准测试和气候建模任务中，FINO比最先进的算子学习方法误差降低高达44%，速度提升约2倍，证明了严格局部性与可学习时间步进的有效性。

Conclusion: FINO通过严格局部性和可学习时间步进为神经PDE求解器提供了准确且可扩展的基础，在保持可解释性的同时显著提升了性能。
...
### [277] [Optimizing Indoor Environmental Quality in Smart Buildings Using Deep Learning](https://arxiv.org/abs/2509.26187)
*Youssef Sabiri,Walid Houmaidi,Aaya Bougrine,Salmane El Mansour Billah*

Main category: cs.LG

TL;DR: 本文提出了一种基于深度学习的室内环境质量预测方法，使用LSTM、GRU和CNN-LSTM三种架构来预测CO2浓度、温度和湿度，在平衡建筑能效的同时优化室内环境质量。


<details>
  <summary>Details</summary>
Motivation: 传统HVAC系统在确保室内环境质量时能耗较高，需要在保证居住者健康和舒适的同时提高能源效率。

Method: 利用ROBOD数据集，比较了LSTM、GRU和CNN-LSTM三种深度学习架构在不同时间尺度上预测室内环境质量参数的性能。

Result: GRU在短期预测中表现最佳且计算开销较低，CNN-LSTM在长期预测中特征提取能力更强，LSTM在长期时间建模方面稳健。预测可靠性受数据分辨率、传感器位置和人员流动影响。

Conclusion: 研究结果为智能建筑管理系统提供了可行的预测性HVAC控制策略，有助于降低能耗并提升实际建筑运营中的居住舒适度。
...
### [278] [Marginal Flow: a flexible and efficient framework for density estimation](https://arxiv.org/abs/2509.26221)
*Marcello Massimo Negri,Jonathan Aellen,Manuel Jahn,AmirEhsan Khorashadizadeh,Volker Roth*

Main category: cs.LG

TL;DR: 提出Marginal Flow框架，通过参数化分布和潜在参数采样，克服现有密度建模方法的训练昂贵、推理缓慢、似然近似、模式崩溃等限制


<details>
  <summary>Details</summary>
Motivation: 解决当前密度建模方法存在的训练成本高、推理速度慢、似然近似、模式崩溃和架构约束等问题

Method: 定义参数化分布q(x|w)和潜在参数w，通过从可学习分布q_θ(w)中采样w来边缘化潜在变量，实现高效密度评估和采样

Result: 模型支持精确密度评估，训练和推理速度比竞争模型快几个数量级，能够学习低维流形上的分布，处理多模态目标

Conclusion: Marginal Flow是一个简单而强大的框架，克服了现有密度建模方法的多个限制，在合成数据集、基于模拟的推理、正定矩阵分布和图像流形学习等任务中表现优异
...
### [279] [Thinking-Free Policy Initialization Makes Distilled Reasoning Models More Effective and Efficient Reasoners](https://arxiv.org/abs/2509.26226)
...
<details>
  <summary>Details</summary>
Motivation: RLVR虽然能有效解决复杂任务，但训练时需要极长的上下文长度，导致巨大的计算成本。多阶段训练虽然能部分缓解，但过短的初始上下文会导致不可逆的性能下降。

Method: 提出TFPI方法，通过ThinkFree操作（直接附加</think>标签）显式丢弃思维内容，减少推理时的token使用。训练时使用ThinkFree适应的输入，即使在原始慢思考模式下也能提高性能。

Result: TFPI加速了RL收敛，达到了更高的性能上限，产生了更token高效推理模型。仅使用TFPI，4B模型在AIME24上达到89.0%准确率，在LiveCodeBench上达到65.5%，使用不到4K H20小时。

Conclusion: TFPI是一种简单而有效的RLVR适应方法，无需专门的奖励或复杂训练设计，就能显著提高性能并降低计算成本。
...
### [280] [Machine Learning Detection of Lithium Plating in Lithium-ion Cells: A Gaussian Process Approach](https://arxiv.org/abs/2509.26234)
*Ayush Patnaik,Adam B Zufall,Stephen K Robinson,Xinfan Lin*

Main category: cs.LG

TL;DR: 提出一种基于高斯过程的锂沉积检测方法，通过直接建模充电压关系Q(V)来概率性地推断dQ/dV，实现稳健的锂沉积检测而无需特殊平滑处理。


<details>
  <summary>Details</summary>
Motivation: 快速充电过程中的锂沉积是关键的退化机制，会加速容量衰减并可能引发安全故障。传统dQ/dV计算方法依赖有限差分和滤波，会放大传感器噪声并引入峰值位置偏差。

Method: 使用高斯过程框架直接建模充电压关系Q(V)作为随机过程，利用GP导数仍是GP的特性，从后验分布中分析性和概率性地推断dQ/dV，实现噪声感知推理和不确定性量化。

Result: 在锂离子纽扣电池上的实验验证表明，该方法在低温、高速充电条件下可靠检测到沉积峰值，在基线情况下正确报告无峰值。GP识别的微分峰值与减少的充电通量和容量衰减一致。

Conclusion: 该方法建立了实时锂沉积检测的实用途径，具有准确性、稳健性和可扩展性，适合嵌入式电池管理系统。
...
### [281] [Beyond Linear Probes: Dynamic Safety Monitoring for Language Models](https://arxiv.org/abs/2509.26238)
*James Oldfield,Philip Torr,Ioannis Patras,Adel Bibi,Fazl Barez*

Main category: cs.LG

TL;DR: 提出了截断多项式分类器(TPCs)，用于动态监测LLM激活状态，实现按需计算的安全监控，在保持性能的同时提高资源效率。


<details>
  <summary>Details</summary>
Motivation: 传统安全监控器对所有查询使用相同计算量，导致资源浪费或漏检风险。需要灵活的安全监控，仅在困难输入或计算资源充足时增加成本。

Method: TPCs是线性探针的自然扩展，采用多项式训练和逐项评估。支持测试时提前终止进行轻量监控，或使用更多项获得更强防护。提供两种使用模式：安全调节和自适应级联。

Result: 在两个大规模安全数据集(WildGuardMix和BeaverTails)上，对4个最多30B参数的模型，TPCs在相同规模下与MLP基线竞争或表现更优，同时比黑盒方法更可解释。

Conclusion: TPCs实现了计算效率与安全性能的平衡，为LLM安全监控提供了灵活、可解释的解决方案。
...
### [282] [Sandbagging in a Simple Survival Bandit Problem](https://arxiv.org/abs/2509.26239)
*Joel Dyer,Daniel Jarne Ornia,Nicholas Bishop,Anisoara Calinescu,Michael Wooldridge*

Main category: cs.LG

TL;DR: 该论文研究了前沿AI系统安全评估中的策略性欺骗问题（称为"sandbagging"），开发了统计测试方法来区分真实能力不足和故意隐藏能力的行为。


<details>
  <summary>Details</summary>
Motivation: AI系统在安全评估中可能故意隐藏危险能力或表现不佳，以规避监管和重新训练，这会破坏安全评估的完整性。

Method: 基于生存多臂老虎机框架，建立了策略性欺骗的序列决策模型，构建了统计测试来从测试分数序列中区分sandbagging和无能行为。

Result: 理论分析表明最优理性代理会表现出sandbagging行为，仿真实验验证了统计测试在区分这两种行为方面的可靠性。

Conclusion: 这项工作为开发稳健的统计程序提供了潜在途径，可用于前沿模型评估科学中检测策略性欺骗行为。
...
### [283] [From Fragile to Certified: Wasserstein Audits of Group Fairness Under Distribution Shift](https://arxiv.org/abs/2509.26241)
...
<details>
  <summary>Details</summary>
Motivation: 群体公平性指标在重采样和分布偏移下变化剧烈，影响审计的可靠性，需要一种能够应对分布变化的鲁棒公平性认证方法。

Method: 使用Wasserstein分布鲁棒框架，在经验分布周围的合理测试分布球上认证最坏情况群体公平性，通过强对偶性推导可处理的重构公式和高效估计器DRUNE。

Result: ε-WDF在标准基准和分类器上提供了稳定的公平性评估，在分布偏移下表现可靠，具有可行性、一致性和有限样本认证保证。

Conclusion: 该框架为超越观测数据的群体公平性审计和认证提供了理论基础，能够应对实际应用中的分布不确定性。
...
### [284] [Wasserstein Distributionally Robust Optimization Through the Lens of Structural Causal Models and Individual Fairness](https://arxiv.org/abs/2509.26275)
*Ahmad-Reza Ehyaei,Golnoosh Farnadi,Samira Samadi*

Main category: cs.LG

TL;DR: 该论文将Wasserstein分布鲁棒优化应用于因果结构和个体公平性学习问题，提出了DRO对偶公式和近似最坏情况损失的正则化方法，并给出了有限样本误差界。
...
Method: 从因果和个体公平性角度构建DRO问题，提出DRO对偶公式，将最坏情况损失表征为正则化项，并在未知因果模型情况下使用经验分布和估计的因果结构。

Result: 成功将DRO问题转化为更易处理的形式，消除了min-max问题中的max步骤，提供了有限样本误差界以确保高效和鲁棒学习。

Conclusion: 该方法为处理分布不确定性和个体公平性提供了一种有效的框架，特别是在因果结构未知的情况下仍能保证学习效率和鲁棒性。
...
### [285] [Reframing Generative Models for Physical Systems using Stochastic Interpolants](https://arxiv.org/abs/2509.26282)
*Anthony Zhou,Alexander Wikner,Amaury Lancelin,Pedram Hassanzadeh,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 本文比较了生成模型在物理系统仿真中的表现，重点评估了随机插值方法相对于传统高斯去噪方法的优势，发现随机插值能利用连续物理分布之间的邻近性，实现更少采样步骤和更准确的预测。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型依赖迭代去噪高斯噪声，可能不是PDE和动力系统（如气候）自回归预测任务的最有效选择。需要探索更适合物理系统仿真的生成建模方法。

Method: 使用随机插值方法，直接学习当前状态和未来状态之间的随机过程，利用连续物理分布之间的邻近性。在不同物理领域和任务上对生成模型进行基准测试。

Result: 随机插值方法比依赖传输高斯噪声的模型使用更少的采样步骤，产生更准确的预测。生成模型需要在确定性精度、谱一致性和概率校准之间取得平衡。

Conclusion: 随机插值成为物理仿真的竞争性基准方法，通过调整采样可以满足物理仿真的多重要求，为不同生成建模框架的能力提供了见解。
...
### [286] [Noise-Guided Transport for Imitation Learning](https://arxiv.org/abs/2509.26294)
*Lionel Blondé,Joao A. Candido Ramos,Alexandros Kalousis*

Main category: cs.LG

TL;DR: NGT是一种轻量级模仿学习方法，在极低数据量（仅20个转移）下表现优异，通过最优传输和对抗训练解决模仿学习问题。


<details>
  <summary>Details</summary>
Motivation: 解决低数据量模仿学习问题，传统方法依赖大规模预训练或高容量架构，在有限专家演示下效率低下。

Method: 将模仿学习建模为最优传输问题，通过对抗训练求解，无需预训练或专用架构，内置不确定性估计。

Result: 在挑战性连续控制任务（包括高维Humanoid任务）上表现强劲，仅需极少量演示数据。

Conclusion: NGT提供了一种简单高效的模仿学习解决方案，在低数据量场景下具有显著优势。
...
### [287] [Tuning the Tuner: Introducing Hyperparameter Optimization for Auto-Tuning](https://arxiv.org/abs/2509.26300)
*Floris-Jan Willemsen,Rob V. van Nieuwpoort,Ben van Werkhoven*

Main category: cs.LG

TL;DR: 提出了一种自动调优框架中超参数调优的新方法，通过统计评估、数据集共享和仿真模式，将调优成本降低两个数量级，平均提升自动调优性能94.8%。
...
Method: 开发了跨搜索空间的鲁棒统计评估方法、FAIR数据集和软件、以及重放历史调优数据的仿真模式。

Result: 有限超参数调优可使自动调优器性能平均提升94.8%，使用元策略优化超参数可进一步平均提升204.7%。

Conclusion: 超参数调优是被忽视但强大的技术，能显著推进自动调优研究和实践。
...
### [288] [NeuroTTT: Bridging Pretraining-Downstream Task Misalignment in EEG Foundation Models via Test-Time Training](https://arxiv.org/abs/2509.26301)
*Suli Wang,Yangshen Deng,Zhenghua Bao,Xinyu Zhan,Yiqun Duan*

Main category: cs.LG

TL;DR: 本文提出了一种两阶段对齐策略，通过领域特定的自监督微调（NeuroTTT）和测试时训练（TTT），解决EEG基础模型在预训练目标与下游任务之间的不对齐问题，显著提升了脑机接口任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大规模EEG基础模型在脑机接口应用中存在预训练目标与下游任务不对齐的问题，以及显著的跨受试者分布偏移，这限制了模型的泛化能力和实际应用效果。

Method: 采用两阶段对齐策略：1) NeuroTTT - 领域特定的自监督微调，通过自监督目标将潜在表示与EEG的频谱、空间和时间特征对齐；2) 测试时训练 - 在推理时对单个未标记测试样本进行自监督训练，并通过预测熵最小化动态校准模型。

Result: 在三个不同的脑机接口任务（想象语音、压力检测、运动想象）上，该方法显著提升了CBraMod和LaBraM基础模型的性能，达到了最先进的水平，优于传统的微调和适应方法。

Conclusion: 将领域调优的自监督与测试时训练相结合的策略，能够有效解决EEG基础模型的对齐问题，显著提高模型的鲁棒性和准确性，为通用脑机接口应用提供了有前景的解决方案。
...
### [289] [Attribution-Guided Decoding](https://arxiv.org/abs/2509.26307)
...
<details>
  <summary>Details</summary>
Motivation: 标准解码方法在满足复杂指令和生成事实准确文本方面存在不足，而现有控制技术往往会降低输出质量。需要一种既能保持输出质量又能提高可靠性的方法。

Method: AGD考虑一组高概率输出token候选，选择对用户定义的ROI具有最高归因的token。ROI可以灵活定义在模型输入或内部组件的不同部分，允许引导生成朝向各种期望行为。还提出了基于熵的自适应变体，在模型不确定时应用引导以减少计算开销。

Result: 在指令遵循方面，AGD显著提高了遵循率（Llama 3.1从66.0%提升到79.1%）。在知识密集型任务中，引导生成使用内部知识组件或上下文源可以减少幻觉，提高闭卷和开卷设置中的事实准确性。

Conclusion: AGD是一种多功能、更可解释且有效的方法，能够增强现代LLMs的可靠性，同时保持输出质量。
...
### [290] [A Review on Single-Problem Multi-Attempt Heuristic Optimization](https://arxiv.org/abs/2509.26321)
*Judith Echevarrieta,Etor Arza,Aritz Pérez,Josu Ceberio*

Main category: cs.LG

TL;DR: 本文对单问题多尝试启发式优化进行了系统综述，将算法选择、参数调优、多启动和资源分配等策略统一在一个框架下，提出了分类法来组织这些策略。


<details>
  <summary>Details</summary>
Motivation: 在实际优化场景中，当计算预算充足时，实践者希望通过多次尝试不同算法、参数配置或初始化来找到单一特定问题的最佳解，但现有研究缺乏对这一问题的专门综述。

Method: 通过统一术语和共同框架，将算法选择、参数调优、多启动和资源分配等分散在不同研究领域的策略进行系统整合，并开发分类法进行组织分类。

Result: 建立了单问题多尝试启发式优化的统一框架，提出了系统化的分类体系，为不同策略的比较和应用提供了基础。

Conclusion: 这项工作填补了单问题多尝试优化策略综述的空白，为实践者选择和应用合适的序列选择策略提供了系统指导。
...
### [291] [ACE: Adapting sampling for Counterfactual Explanations](https://arxiv.org/abs/2509.26322)
...
Method: ACE算法结合贝叶斯估计和随机优化，优先采样信息量大的点来近似决策边界

Result: ACE在评估效率上优于现有方法，同时能准确识别最小且可行的特征变化

Conclusion: ACE是一种样本高效的算法，能在减少模型查询次数的同时生成准确可行的反事实解释
...
### [292] [A Generalized Information Bottleneck Theory of Deep Learning](https://arxiv.org/abs/2509.26327)
*Charles Westphal,Stephen Hailes,Mirco Musolesi*

Main category: cs.LG

TL;DR: 提出了广义信息瓶颈(GIB)框架，通过协同性视角重新表述信息瓶颈原理，解决了原始IB的理论模糊性和估计挑战。


<details>
  <summary>Details</summary>
Motivation: 信息瓶颈(IB)原理为理解神经网络学习提供了理论框架，但其实际应用受到理论模糊性和准确估计困难的限制。

Method: 基于协同性概念重新表述IB，使用平均交互信息(II)作为可计算的协同性度量，构建GIB框架。
...
### [293] [FedMuon: Federated Learning with Bias-corrected LMO-based Optimization](https://arxiv.org/abs/2509.26337)
*Yuki Takezawa,Anastasia Koloskova,Xiaowen Jiang,Sebastian U. Stich*

Main category: cs.LG

TL;DR: 本文研究了Muon优化方法在联邦学习中的应用，提出了FedMuon算法来解决LMO偏差问题，并证明了即使近似求解LMO也能收敛。


<details>
  <summary>Details</summary>
Motivation: Muon作为一种基于线性最小化预言(LMO)的新优化方法，在神经网络训练中比Adam等自适应方法更快，但直接应用于联邦学习FedAvg中无法收敛到稳定点。

Method: 提出FedMuon算法来缓解LMO偏差问题，分析了近似求解LMO对收敛率的影响，发现任何数量的Newton-Schulz迭代都能保证收敛。

Result: 实验证明FedMuon能够超越现有最先进的联邦学习方法，且更精确求解LMO可以加速收敛。

Conclusion: FedMuon成功将Muon优化方法应用于联邦学习，解决了LMO偏差问题，并在性能和收敛速度上优于现有方法。
...
### [294] [Memory-Driven Self-Improvement for Decision Making with Large Language Models](https://arxiv.org/abs/2509.26340)
*Xue Yan,Zijing Ou,Mengyue Yang,Yan Song,Haifeng Zhang,Yingzhen Li,Jun Wang*

Main category: cs.LG

TL;DR: 提出了一种内存驱动的自我改进框架，将LLM的通用先验知识与特定领域的经验记忆相结合，通过记忆和LLM先验的相互增强来提升序列决策任务的性能。


<details>
  <summary>Details</summary>
Motivation: LLM在序列决策任务中具有广泛但通用的知识，但针对特定任务时数据有限，难以有效适应。需要将通用知识与领域特定经验结合来提升决策性能。

Method: 使用紧凑的内存存储过去交互和关联的Q值，捕获决策相关知识来指导准确的估值估计和LLM先验精炼。精炼后的LLM先验生成更高奖励的轨迹来丰富记忆，形成自我改进循环。

Result: 在ALFWorld实验中，该方法显著优于传统RL和基于LLM的基线方法，在分布内任务上性能提升超过40%，在未见任务上泛化性能提升超过75%。

Conclusion: 内存驱动的自我改进框架通过结合LLM通用知识和领域特定经验记忆，有效提升了序列决策任务的性能，特别是在数据有限的场景下表现出强大的泛化能力。
...
### [295] [LLM-Assisted Emergency Triage Benchmark: Bridging Hospital-Rich and MCI-Like Field Simulation](https://arxiv.org/abs/2509.26351)
*Joshua Sebastian,Karma Tobden,KMA Solaiman*

Main category: cs.LG

TL;DR: 本文提出了一个基于MIMIC-IV-ED数据库的急诊分诊基准，用于预测患者病情恶化（ICU转移、院内死亡率），包含医院环境和MCI模拟两种场景，并利用LLM辅助数据集构建。
...
Method: 使用LLM辅助构建数据集，包括统一噪声字段、优先选择临床相关生命体征和实验室数据、指导模式对齐和表合并；定义两种场景：医院环境（含完整数据）和MCI模拟（仅基础数据）；提供基线模型和SHAP可解释性分析。

Result: 建立了开放的急诊分诊基准，展示了不同场景下的预测差距，识别了对分诊最关键的特征。

Conclusion: 这些贡献使分诊预测研究更具可重复性和可访问性，是临床AI中数据集民主化的重要一步。
...
### [296] [Clarification as Supervision: Reinforcement Learning for Vision-Language Interfaces](https://arxiv.org/abs/2509.26594)
*John Gkountouras,Ivan Titov*

Main category: cs.LG

TL;DR: AC-RL通过强化学习训练视觉模型生成更全面的图像描述，使数学推理系统能一次性解决问题，无需额外澄清，在7个视觉数学推理基准上平均准确率提升4.4个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型为人类读者生成描述时，常常省略推理系统所需的关键细节，导致推理失败不是因为推理能力限制，而是因为缺乏关键视觉信息。

Method: 提出自适应澄清强化学习(AC-RL)，通过交互训练视觉模型理解推理系统需要的信息。核心洞察是利用训练中的澄清请求揭示信息差距，通过惩罚需要澄清的成功案例，促使模型生成全面的初始描述。

Result: 在7个视觉数学推理基准上，AC-RL相比预训练基线平均准确率提升4.4个百分点，分析显示如果允许澄清请求，可减少高达39%的澄清需求。

Conclusion: 通过将澄清视为隐式监督，AC-RL证明视觉语言接口可以仅通过交互有效学习，无需显式标注。
...
### [297] [Data-to-Energy Stochastic Dynamics](https://arxiv.org/abs/2509.26364)
*Kirill Tamogashev,Nikolay Malkin*

Main category: cs.LG

TL;DR: 提出首个无需数据样本的薛定谔桥建模方法，通过推广迭代比例拟合(IPF)到无数据情况，解决当分布仅由未归一化密度给出时的随机动力学系统构建问题。


<details>
  <summary>Details</summary>
Motivation: 现有薛定谔桥算法需要两个分布的样本数据，但在许多实际应用中，分布可能仅由未归一化密度函数给出而无法获取样本。这限制了薛定谔桥在科学计算和生成模型中的应用。

Method: 基于强化学习框架，将迭代比例拟合(IPF)推广到无数据情况，提出数据到能量IPF算法。该方法可以学习扩散系数，并假设固定的时间离散化方案。

Result: 在合成问题上验证了算法的有效性，能够成功学习多峰分布间的传输。改进的算法显著提升了现有数据到数据薛定谔桥算法的性能，并成功应用于生成模型潜在空间的后验分布采样。

Conclusion: 提出的数据到能量IPF是首个能够处理未归一化密度分布的薛定谔桥方法，扩展了薛定谔桥的应用范围，特别是在无样本数据情况下的科学计算和图像到图像转换任务中。
...
### [298] [Learning to See Before Seeing: Demystifying LLM Visual Priors from Language Pre-training](https://arxiv.org/abs/2509.26625)
*Junlin Han,Shengbang Tong,David Fan,Yufan Ren,Koustuv Sinha,Philip Torr,Filippos Kokkinos*

Main category: cs.LG

TL;DR: LLMs在纯文本训练中意外地发展出丰富的视觉先验，这些先验由可分离的感知和推理先验组成，具有不同的扩展趋势和起源。推理先验主要来自推理中心数据（如代码、数学），而感知先验则来自广泛语料库。


<details>
  <summary>Details</summary>
Motivation: 揭示LLMs在纯文本预训练中如何发展视觉先验，理解这些先验的组成和起源，为下一代多模态LLMs提供数据中心的预训练方法。

Method: 通过100多个受控实验（消耗50万GPU小时），涵盖完整的MLLM构建流程，包括LLM预训练、视觉对齐和监督多模态微调，跨越五个模型规模和多种数据类别。

Result: 发现视觉推理能力主要由推理中心数据发展，且可迁移到视觉推理；感知能力更依赖于视觉编码器和视觉指令调优数据；视觉世界描述文本虽然重要但性能影响快速饱和。

Conclusion: 提出了一种从语言预训练中有意培养视觉先验的新方法，为下一代多模态LLMs的发展铺平道路，并引入了多级存在基准（MLE-Bench）。
...
### [299] [Refine Drugs, Don't Complete Them: Uniform-Source Discrete Flows for Fragment-Based Drug Discovery](https://arxiv.org/abs/2509.26405)
*Benno Kaech,Luis Wyss,Karsten Borgwardt,Gianvito Grasso*

Main category: cs.LG

TL;DR: InVirtuoGen是一个基于离散流生成模型的SMILES片段生成方法，用于从头生成、片段约束生成以及小分子的目标属性/先导化合物优化。


<details>
  <summary>Details</summary>
Motivation: 现有的分子生成模型在质量-多样性权衡和优化任务上存在局限，需要一种能够同时支持从头生成和优化任务的通用生成框架。

Method: 采用离散流生成模型学习从均匀源分布到数据分布的变换，通过遗传算法结合近端属性优化微调策略进行属性优化。

Result: 在从头生成任务中实现了更好的质量-多样性平衡，在Practical Molecular Optimization基准测试中创造了新的最先进水平，在先导化合物优化中获得更高的对接分数。

Conclusion: InVirtuoGen为从早期命中发现到多目标先导化合物优化的药物发现过程建立了一个通用的生成基础，并开源了预训练模型和代码。
...
### [300] [Ascent Fails to Forget](https://arxiv.org/abs/2509.26427)
*Ioannis Mavrothalassitis,Pol Puigdemont,Noam Itzhak Levi,Volkan Cevher*

Main category: cs.LG

TL;DR: 梯度上升优化方法在机器遗忘任务中经常失败，原因是遗忘集和保留集之间存在统计依赖性，即使只是简单的相关性也会导致这些方法失效。


<details>
  <summary>Details</summary>
Motivation: 挑战普遍观点，揭示基于梯度上升的无约束优化方法在机器遗忘中的失败现象，强调数据间统计依赖性的关键影响。

Method: 通过理论和实证分析，研究随机遗忘集和逻辑回归案例中的失败模式，使用梯度下降-上升迭代方法进行实验验证。

Result: 发现梯度上升方法不仅无法收敛到理想重训练模型，反而可能产生比原始模型更差的解，导致遗忘过程适得其反。

Conclusion: 数据间的统计依赖性（即使是相关性）足以导致基于上升的遗忘方法失败，这一理论见解在复杂神经网络实验中得到了证实。
...
### [301] [AdaBlock-dLLM: Semantic-Aware Diffusion LLM Inference via Adaptive Block Size](https://arxiv.org/abs/2509.26432)
*Guanxi Lu,Hao,Chen,Yuto Karashima,Zhican Wang,Daichi Fujiki,Hongxiang Fan*

Main category: cs.LG

TL;DR: 论文提出了AdaBlock-dLLM，一种基于语义的自适应块大小调度方法，解决了传统半自回归解码中固定块大小的两个根本限制：延迟解码开销和过早解码错误。


<details>
  <summary>Details</summary>
Motivation: 传统半自回归解码采用固定块大小存在两个问题：高置信度令牌在块外被不必要延迟解码，而低置信度令牌在块内过早提交导致错误。

Method: 通过分析去噪过程中的置信度动态，识别波动带区域，提出AdaBlock-dLLM调度器，在运行时自适应调整块大小以对齐语义边界。

Result: 在多样化基准测试中，AdaBlock-dLLM在相同吞吐量预算下实现了高达5.3%的准确率提升。

Conclusion: 该研究不仅优化了推理时间性能，其基于语义的自适应调度方法和置信度分析有望启发未来dLLMs的训练策略。
...
### [302] [ACT: Agentic Classification Tree](https://arxiv.org/abs/2509.26433)
*Vincent Grari,Tim Arni,Thibault Laugel,Sylvain Lamprier,James Zou,Marcin Detyniecki*

Main category: cs.LG

TL;DR: ACT将决策树方法扩展到非结构化数据，通过将每个分裂点表示为自然语言问题，结合基于不纯度的评估和LLM反馈，在文本分类任务中达到或超过基于提示的方法，同时提供透明可解释的决策路径。


<details>
  <summary>Details</summary>
Motivation: 在高风险场景中，AI系统需要产生透明、可解释和可审计的决策。传统决策树（如CART）虽然提供清晰的规则，但仅限于结构化表格数据，无法直接处理非结构化文本输入。而大型语言模型虽然广泛用于此类数据，但其提示策略仍依赖自由形式的推理，难以确保可信行为。

Method: 提出Agentic Classification Tree (ACT)，通过将每个分裂点表述为自然语言问题来扩展决策树方法。使用基于不纯度的评估和通过TextGrad的LLM反馈来优化这些问题，从而构建可解释的决策树。

Result: 在文本基准测试上的实验表明，ACT在性能上匹配或超越了基于提示的基线方法，同时生成了透明和可解释的决策路径。

Conclusion: ACT成功地将决策树的透明性和可解释性优势扩展到非结构化文本数据，为高风险应用提供了既准确又可信的AI决策系统。
...
### [303] [Extensions of Robbins-Siegmund Theorem with Applications in Reinforcement Learning](https://arxiv.org/abs/2509.26442)
*Xinyu Liu,Zixuan Xie,Shangtong Zhang*

Main category: cs.LG

TL;DR: 扩展Robbins-Siegmund定理，放宽对零阶项的求和性要求，从可求和变为平方可求和，为强化学习中的Q-learning等算法提供更强的收敛性保证。


<details>
  <summary>Details</summary>
Motivation: 原始Robbins-Siegmund定理要求零阶项可求和，但在许多重要强化学习应用中该条件无法满足，限制了其适用性。

Method: 引入关于随机过程增量的新假设，结合平方可求和条件，证明几乎必然收敛到有界集。

Result: 获得了几乎必然收敛速率、高概率集中界和L^p收敛速率，首次为带线性函数逼近的Q-learning提供了这些收敛性结果。

Conclusion: 扩展的Robbins-Siegmund定理为随机逼近和强化学习算法提供了更强大的收敛性分析工具，特别适用于零阶项不可求和但平方可求和的情况。
...
### [304] [fev-bench: A Realistic Benchmark for Time Series Forecasting](https://arxiv.org/abs/2509.26468)
...
TL;DR: 提出了fev-bench基准测试，包含100个时间序列预测任务，涵盖7个领域，其中46个任务包含协变量。配套的fev库支持可重复性和与现有工作流集成，使用bootstrapped置信区间进行统计严谨的聚合评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试存在领域覆盖窄、忽视协变量任务、缺乏统计严谨性、评估基础设施不足等问题，限制了时间序列预测领域的评估和进展。

Method: 开发fev-bench基准测试集和fev轻量级Python库，采用bootstrapped置信区间的统计严谨聚合方法，从胜率和技能分数两个维度评估模型性能。

Result: 在fev-bench上报告了各种预训练、统计和基线模型的结果，识别了未来研究的有前景方向。

Conclusion: fev-bench解决了现有基准测试的关键缺陷，为时间序列预测提供了更全面、统计严谨的评估框架。
...
### [305] [DiVeQ: Differentiable Vector Quantization Using the Reparameterization Trick](https://arxiv.org/abs/2509.26469)
*Mohammad Hassan Vali,Tom Bäckström,Arno Solin*

Main category: cs.LG

TL;DR: DiVeQ和SF-DiVeQ是两种新的向量量化方法，通过将量化视为添加模拟量化失真的误差向量，保持前向传播的硬分配同时允许梯度流动，解决了传统向量量化中梯度阻塞的问题。


<details>
  <summary>Details</summary>
Motivation: 传统向量量化中的硬分配会阻塞梯度，阻碍端到端训练。需要一种既能保持前向传播的硬分配特性，又能让梯度流动的量化方法。

Method: DiVeQ将量化视为添加模拟量化失真的误差向量；SF-DiVeQ是空间填充变体，将向量分配到由码字连线构成的曲线上，减少量化误差并充分利用码本。两种方法都不需要辅助损失或温度调度。

Result: 在VQ-VAE压缩和VQGAN生成任务中，这两种方法在各种数据集上都比替代量化方法有更好的重建和样本质量。

Conclusion: DiVeQ和SF-DiVeQ是有效的向量量化方法，能够实现端到端训练，在压缩和生成任务中表现优异。
...
### [306] [Equivariance by Local Canonicalization: A Matter of Representation](https://arxiv.org/abs/2509.26499)
*Gerrit Gerhartz,Peter Lippmann,Fred A. Hamprecht*

Main category: cs.LG

TL;DR: 提出了一个将张量场网络转换为更高效的局部正则化范式的框架，保持等变性同时显著提升运行速度，并发布了tensor_frames软件包。
...
Method: 开发了一个框架将现有张量场网络转换为局部正则化范式，系统比较不同等变表示的理论复杂度、经验运行时间和预测准确性。

Result: 该框架在保持等变性的同时显著改善了运行时间，并提供了易于集成到标准消息传递神经网络中的实现。

Conclusion: 局部正则化为等变神经网络提供了更高效的实现方式，使等变性能够轻松集成到标准神经网络架构中。
...
### [307] [Entropy After $\langle \texttt{/Think} \rangle$ for reasoning model early exiting](https://arxiv.org/abs/2509.26522)
*Xi Wang,James McInerney,Lequn Wang,Nathan Kallus*

Main category: cs.LG

TL;DR: 提出EAT（Entropy After </Think>）方法，通过监控推理过程中的熵值变化来检测和防止大型推理模型的过度思考问题，实现自适应计算分配，显著减少token使用而不影响准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在长链推理中表现出改进性能，但存在过度思考倾向——即使已得出正确答案仍继续修正。这种低效性浪费了大量token资源。

Method: 在推理过程中添加停止思考标记(</think>)，监控后续token的熵值变化轨迹。当Pass@1趋于稳定时，熵值会下降并稳定，通过指数移动平均的方差阈值实现实用的停止规则。

Result: 在MATH500和AIME2025数据集上，EAT方法将token使用量减少13-21%，且不损害准确性。即使在无法访问推理模型logits的黑盒设置中，使用代理模型计算EAT仍保持有效。

Conclusion: EAT提供了一种简单且低成本的方法来监控推理过程，实现自适应计算分配，显著提高大型推理模型的效率，特别是在资源受限的环境中具有重要价值。
...
### [308] [TAP: Two-Stage Adaptive Personalization of Multi-task and Multi-Modal Foundation Models in Federated Learning](https://arxiv.org/abs/2509.26524)
*Seohyun Lee,Wenzhi Fang,Dong-Jun Han,Seyyedali Hosseinalipour,Christopher G. Brinton*

Main category: cs.LG

TL;DR: 提出了TAP（两阶段自适应个性化）方法，通过客户端与服务器间不匹配的模型架构选择性进行替换操作，并在联邦学习后进行知识蒸馏，以解决异构客户端在数据、任务和模态上的个性化需求。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习产生的最终模型不一定适合每个客户端的需求，而现有的个性化联邦学习方法对多任务、多模态基础模型的微调和个人化关注不足，特别是在客户端在数据、任务和模态上都存在异构性的场景下。

Method: TAP方法包含两个阶段：(i)利用客户端与服务器间不匹配的模型架构，在有利于客户端本地任务时选择性进行替换操作；(ii)在联邦学习后进行知识蒸馏，以获取有益的一般知识而不损害个性化。

Result: 通过大量实验验证了所提算法在各种数据集和任务上的有效性，优于多个基线方法。

Conclusion: TAP方法能够有效解决异构客户端在多任务、多模态场景下的个性化需求，同时提供了服务器模型在模态-任务对架构下的收敛性分析。
...
### [309] [Machine-Learning Driven Load Shedding to Mitigate Instability Attacks in Power Grids](https://arxiv.org/abs/2509.26532)
*Justin Tackett,Benjamin Francis,Luis Garcia,David Grimsman,Sean Warnick*

Main category: cs.LG

TL;DR: 提出了一种基于监督机器学习的低成本数据驱动方法，用于改造电力系统中的减载决策系统，以防御不稳定性攻击。在IEEE 14总线系统上进行了概念验证，证明改进的Prony分析(MPA)是检测不稳定性攻击并触发防御机制的有效方法。


<details>
  <summary>Details</summary>
Motivation: 随着关键基础设施日益复杂且依赖度增加，电力系统成为复杂网络攻击的诱人目标。不稳定性攻击是一类较新的攻击类型，目前缺乏足够的防护措施。

Method: 使用监督机器学习模型训练数据驱动的方法，结合改进的Prony分析(MPA)技术来检测不稳定性攻击。在IEEE 14总线系统上使用Achilles Heel Technologies电力网格分析器进行概念验证。

Result: 研究证明MPA是检测不稳定性攻击并触发防御机制的有效方法，为电力系统提供了对抗这类新型攻击的防护能力。

Conclusion: 该方法为电力系统提供了一种成本效益高的解决方案，能够改造现有的减载决策系统，增强对不稳定性攻击的防御能力。
...
### [310] [The Loss Kernel: A Geometric Probe for Deep Learning Interpretability](https://arxiv.org/abs/2509.26537)
*Maxwell Adam,Zach Furman,Jesse Hoogland*

Main category: cs.LG

TL;DR: 提出了损失核方法，这是一种可解释性技术，通过计算在低损失保持参数扰动分布下的逐样本损失协方差矩阵，来衡量数据点之间的相似性。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够根据训练好的神经网络来衡量数据点相似性的可解释性方法，帮助理解模型如何组织和区分不同类别的数据。

Method: 使用损失核方法，计算在保持低损失的参数扰动分布下的逐样本损失协方差矩阵，构建数据点之间的相似性度量。

Result: 在合成多任务问题上验证了方法的有效性，显示它能按任务分离输入；应用于Inception-v1模型时，发现核结构与WordNet语义层次结构一致。
...
### [311] [TASP: Topology-aware Sequence Parallelism](https://arxiv.org/abs/2509.26541)
*Yida Wang,Ke Hong,Xiuhong Li,Yuanchao Xu,Wenxun Wang,Guohao Dai,Yu Wang*

Main category: cs.LG

TL;DR: TASP是一种针对长上下文LLM的拓扑感知序列并行方法，通过分解现代加速器拓扑和通信原语，比Ring Attention实现了更高的通信效率，在H100和MI300X系统上获得了最高3.58倍的加速。
...
Method: 基于哈密顿分解理论，将现代加速器拓扑分解为多个正交环形数据路径，并同时分解Ring AllGather原语，实现并发数据传输而不相互干扰。

Result: 在单节点和多节点NVIDIA H100系统以及单节点AMD MI300X系统上的实验表明，TASP比Ring Attention及其变体Zigzag-Ring Attention实现了更高的通信效率，最高加速比达到3.58倍。

Conclusion: TASP通过拓扑分解和原语分解，充分利用了现代加速器的通信能力，为长上下文LLM提供了更高效的序列并行解决方案。
...
### [312] [Bayesian Influence Functions for Hessian-Free Data Attribution](https://arxiv.org/abs/2509.26544)
*Philipp Alexander Kreer,Wilson Wu,Maxwell Adam,Zach Furman,Jesse Hoogland*

Main category: cs.LG

TL;DR: 提出局部贝叶斯影响函数(BIF)，通过损失景观统计替代Hessian逆矩阵，解决深度神经网络中传统影响函数面临的Hessian不可逆和高维参数空间问题。


<details>
  <summary>Details</summary>
Motivation: 传统影响函数在深度神经网络中面临Hessian矩阵不可逆和高维参数空间的挑战，限制了其实际应用。

Method: 使用随机梯度MCMC采样估计损失景观统计，替代Hessian逆矩阵计算，捕捉参数间高阶交互作用。

Result: 该方法可扩展到数十亿参数规模的神经网络，在预测重训练实验方面达到最先进结果。

Conclusion: 局部贝叶斯影响函数提供了一种Hessian-free的影响分析框架，能够有效处理大规模深度神经网络的影响分析问题。
...
### [313] [Parametric Neural Amp Modeling with Active Learning](https://arxiv.org/abs/2509.26564)
*Florian Grötschla,Longxiang Jiao,Luca A. Lanzendörfer,Roger Wattenhofer*

Main category: cs.LG

TL;DR: Panama是一个主动学习框架，使用LSTM和WaveNet架构端到端训练参数化吉他放大器模型，通过基于集成的主动学习策略最小化所需数据点数量，仅用75个数据点就能达到领先开源非参数放大器模型NAM的感知质量。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够通过最小化所需数据点（即放大器旋钮设置）来创建虚拟放大器的参数化吉他放大器建模框架，减少数据采集成本。

Method: 结合LSTM模型和WaveNet架构的端到端训练，采用基于集成的主动学习策略，使用基于梯度的优化来最大化集成模型之间的分歧，以识别最具信息量的数据点。

Result: MUSHRA听力测试显示，仅使用75个数据点，Panama模型就能匹配领先开源非参数放大器模型NAM的感知质量。

Conclusion: Panama框架通过主动学习策略有效减少了训练参数化吉他放大器模型所需的数据量，在保持感知质量的同时显著提高了数据效率。
...
### [314] [Importance of localized dilatation and distensibility in identifying determinants of thoracic aortic aneurysm with neural operators](https://arxiv.org/abs/2509.26576)
*David S. Li,Somdatta Goswami,Qianying Cao,Vivek Oommen,Roland Assi,Jay D. Humphrey,George E. Karniadakis*

Main category: cs.LG

TL;DR: 使用有限元框架生成合成胸主动脉瘤，训练神经网络从局部扩张和扩张性空间图中预测初始损伤组合，发现包含扩张性信息的训练显著提高预测精度，UNet模型表现最佳。
...
Method: 使用有限元框架生成数百种异质性损伤的合成胸主动脉瘤，构建局部扩张和扩张性的空间图来训练神经网络预测初始损伤组合，比较多种网络架构和输入数据格式。

Result: 仅使用几何数据（扩张）训练时预测误差显著更高，包含扩张性信息可提高预测精度；在所有测试模型中，UNet在不同数据格式下均提供最高准确度。

Conclusion: 在胸主动脉瘤评估中获取扩张和扩张性的全场测量对于揭示疾病的机械生物学驱动因素至关重要，支持个性化治疗策略的开发。
...
### [315] [Linking Process to Outcome: Conditional Reward Modeling for LLM Reasoning](https://arxiv.org/abs/2509.26578)
*Zheng Zhang,Ziwei Shan,Kaitao Song,Yexin Li,Kan Ren*

Main category: cs.LG

TL;DR: 提出条件奖励建模(CRM)方法，通过将LLM推理建模为通向正确答案的时间过程，解决现有过程奖励模型在捕捉步骤间依赖关系和信用分配方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型(PRMs)要么孤立处理每个推理步骤，无法捕捉步骤间依赖关系；要么难以将过程奖励与最终结果对齐，导致奖励信号不尊重时序因果关系，面临信用分配模糊问题。

Method: CRM将LLM推理建模为时间过程，每个推理步骤的奖励不仅基于前序步骤，还明确与推理轨迹的最终结果相关联。通过强制执行条件概率规则，捕捉推理步骤间的因果关系。

Result: 在Best-of-N采样、波束搜索和强化学习的实验中，CRM始终优于现有奖励模型，对奖励攻击更鲁棒，在不依赖真实奖励的情况下提供稳定的下游改进。

Conclusion: CRM为增强LLM推理提供了一个原则性框架，通过精确的信用分配和一致的建模，解决了过程奖励模型的关键局限性。
...
### [316] [Uncertainty Quantification for Regression using Proper Scoring Rules](https://arxiv.org/abs/2509.26610)
*Alexander Fishkov,Kajetan Schweighofer,Mykyta Ielanskyi,Nikita Kotelevskii,Mohsen Guizani,Maxim Panov*

Main category: cs.LG

TL;DR: 提出了一个基于适当评分规则的统一回归不确定性量化框架，能够自然分解为偶然不确定性和认知不确定性，并恢复了基于预测方差和微分熵的流行回归UQ度量。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型预测的不确定性量化对于安全关键应用中的可靠决策至关重要，但现有UQ理论主要关注分类问题，回归中的UQ仍然具有挑战性。

Method: 基于适当评分规则（如CRPS、对数、平方误差和二次评分）构建统一UQ框架，推导参数假设下的闭式表达式，并使用模型集成进行估计。

Result: 在合成和真实世界回归数据集上的广泛评估为选择可靠的UQ度量提供了指导，框架能够自然分解不确定性并恢复流行回归UQ度量。

Conclusion: 该框架为回归问题提供了一个统一的不确定性量化方法，能够可靠地分解和量化不同类型的不确定性，为安全关键应用提供了重要工具。
...
### [317] [Recursive Self-Aggregation Unlocks Deep Thinking in Large Language Models](https://arxiv.org/abs/2509.26626)
...
TL;DR: 提出递归自聚合（RSA）方法，结合并行和顺序推理的优势，通过多轮迭代聚合候选推理链来提升LLM性能，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法要么采用并行推理（选择多个独立解），要么采用顺序推理（自我精炼），但未能充分利用推理链中的丰富信息。RSA旨在结合两者的优势，通过聚合推理链中的中间步骤来提升模型能力。

Method: RSA是一种受进化方法启发的测试时扩展方法，通过多轮迭代：每轮对候选推理链的子集进行聚合，生成改进的解群体，作为下一轮的候选池。该方法充分利用推理链中的中间步骤信息，支持从部分正确的推理步骤中引导改进。

Result: RSA在不同任务、模型家族和规模上均带来显著性能提升。Qwen3-4B-Instruct-2507使用RSA后能与更大的推理模型（如DeepSeek-R1和o3-mini）竞争，并在AIME-25、HMMT-25、Reasoning Gym、LiveCodeBench-v6和SuperGPQA等基准上优于纯并行和顺序扩展策略。

Conclusion: RSA通过结合并行和顺序推理的优势，有效提升了LLM的推理能力。通过聚合感知的强化学习训练模型进一步带来显著性能增益，证明了该方法在测试时扩展中的有效性。
...
### [318] [Attention as a Compass: Efficient Exploration for Process-Supervised RL in Reasoning Models](https://arxiv.org/abs/2509.26628)
...
<details>
  <summary>Details</summary>
Motivation: 现有过程监督强化学习方法在分支位置和采样方面存在探索效率限制，而高注意力分数步骤与推理行为相关。

Method: 使用高注意力值位置进行分支，开发考虑问题难度和历史批次大小的自适应采样策略，设计一步离策略训练流程。

Result: 在多个数学推理基准测试中，该方法在性能、采样和训练效率方面均优于先前方法。

Conclusion: AttnRL框架有效提升了过程监督强化学习在推理任务中的探索效率和性能表现。
...
### [319] [AccidentBench: Benchmarking Multimodal Understanding and Reasoning in Vehicle Accidents and Beyond](https://arxiv.org/abs/2509.26636)
...
TL;DR: AccidentBench是一个大规模多模态基准测试，包含约2000个视频和19000多个标注问答对，专注于评估模型在安全关键场景（交通、航空、水域）中的时空和意图推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的快速发展，需要能够严格评估模型在安全关键、动态现实场景中理解和推理能力的基准测试。

Method: 构建包含车辆事故场景和Beyond领域（航空和水域安全关键场景）的大规模基准，涵盖不同视频长度和难度级别，系统性地测试时间、空间和意图理解推理能力。

Result: 评估最先进模型（如Gemini-2.5 Pro和GPT-5）显示，即使在最强模型上，最困难任务和最长视频的准确率也仅约18%，揭示了在现实世界时空和意图推理方面的显著差距。
...
### [320] [SPATA: Systematic Pattern Analysis for Detailed and Transparent Data Cards](https://arxiv.org/abs/2509.26640)
*João Vitorino,Eva Maia,Isabel Praça,Carlos Soares*

Main category: cs.LG

TL;DR: SPATA是一种确定性方法，可将表格数据集转换为领域无关的统计模式表示，用于AI模型鲁棒性评估，同时保护数据隐私。


<details>
  <summary>Details</summary>
Motivation: 由于AI对数据扰动和对抗样本的敏感性，需要在部署前进行鲁棒性评估，但传统方法需要访问训练和测试数据集，这可能危及数据隐私和机密性。

Method: SPATA将每个数据实例投影到离散空间中进行分析和比较，而不泄露原始数据，生成可解释的模型行为解释。

Result: 该方法能够可靠地评估不同特征对ML模型鲁棒性的影响，并为模型行为生成可解释的解释。

Conclusion: SPATA有助于提高处理机密数据或关键基础设施组织的透明度，促进更可信的AI发展。
...
### [321] [Blueprint-Bench: Comparing spatial intelligence of LLMs, agents and image models](https://arxiv.org/abs/2509.25229)
...
Conclusion: Blueprint-Bench为比较不同模型架构的空间智能提供了首个数值框架，将继续评估新模型并监测通用AI系统中空间智能的出现。
...
### [322] [The Causal Abstraction Network: Theory and Learning](https://arxiv.org/abs/2509.25236)
*Gabriele D'Acunto,Paolo Di Lorenzo,Sergio Barbarossa*

Main category: cs.AI

TL;DR: 本文提出了因果抽象网络(CAN)，这是一种基于高斯结构因果模型的网络层结构，通过构造性线性因果抽象和局部黎曼优化方法学习一致的CAN。


<details>
  <summary>Details</summary>
Motivation: 旨在通过结构因果模型(SCMs)提升人工智能的可解释性、可信性和鲁棒性，将因果知识形式化为网络层结构。

Method: 引入因果抽象网络(CAN)，使用高斯SCMs，限制映射为构造性线性因果抽象的转置，提出SPECTRAL迭代方法进行局部黎曼优化学习。

Result: 在合成数据上的实验显示，在因果抽象学习任务中具有竞争力，并能成功恢复多样化的CAN结构。

Conclusion: CAN为因果AI提供了一种有效的网络层框架，其分离的优化方法和高效的搜索过程能够学习一致的因果抽象网络。
...
### [323] [A Formal Comparison Between Chain-of-Thought and Latent Thought](https://arxiv.org/abs/2509.25239)
*Kevin Xu,Issei Sato*
...
<details>
  <summary>Details</summary>
Motivation: CoT通过生成自然语言中间步骤来激发大语言模型的推理能力，而Latent Thought直接在连续潜在空间中操作，支持超越离散语言表示的计算。两种方法都利用迭代计算，但它们的比较能力尚未充分探索。

Method: 对循环Transformer中的Latent Thought进行形式化分析，比较其与CoT的并行和序列计算特性，探讨深度驱动递归的适用任务。

Result: Latent Thought在循环Transformer中支持并行计算，比CoT的固有序列过程更高效；而CoT通过随机解码近似解决难解问题。这些分离特性为选择推理范式提供了实践指导。

Conclusion: 两种推理范式各有优势：Latent Thought适合需要并行高效计算的任务，CoT适合处理难解问题的近似求解。这为根据具体任务需求选择合适推理方法提供了理论依据。
...
### [324] [Neo-Grounded Theory: A Methodological Innovation Integrating High-Dimensional Vector Clustering and Multi-Agent Collaboration for Qualitative Research](https://arxiv.org/abs/2509.25244)
*Shuide Wen,Beier Ku,Teng Wang,Mingyang Zou,Yang Yang*

Main category: cs.AI

TL;DR: NGT通过向量聚类和多智能体系统解决定性研究的规模深度悖论，能在几小时内分析海量数据，同时保持解释严谨性。


<details>
  <summary>Details</summary>
Motivation: 解决定性研究中规模与深度的矛盾，传统方法处理大规模数据耗时耗力，而现有自动化方法缺乏解释深度。

Method: 使用1536维向量嵌入、层次聚类和并行智能体编码，对比了纯自动化和人机协作两种模式。

Result: NGT实现了168倍速度提升(3小时vs3周)、更高质量(0.904vs0.883)和96%成本降低，发现了手动编码无法识别的模式。
...
### [325] [Memory Management and Contextual Consistency for Long-Running Low-Code Agents](https://arxiv.org/abs/2509.25250)
*Jiexi Xu*
...
<details>
  <summary>Details</summary>
Motivation: 随着AI原生LCNC平台的发展，自主代理能够执行复杂、长期的业务流程，但面临记忆管理和上下文一致性的挑战，需要专门解决方案。

Method: 结合认知科学原理，构建包含情景记忆和语义记忆的混合架构，引入基于时效性、相关性和用户指定效用的智能衰减机制，并提供用户友好的可视化界面。

Result: 在模拟长期任务实验中，该系统显著优于滑动窗口和基础RAG方法，在任务完成率、上下文一致性和长期代币成本效率方面表现更优。

Conclusion: 建立了一个可靠、透明的AI代理框架，支持有效的长期学习和适应能力，为LCNC环境下的记忆管理提供了新范式。
...
### [326] [Fact Grounded Attention: Eliminating Hallucination in Large Language Models Through Attention Level Knowledge Integration](https://arxiv.org/abs/2509.25252)
*Aayush Gupta*

Main category: cs.AI

TL;DR: Fact Grounded Attention (FGA)通过将可验证知识直接注入注意力机制，将不可靠的语言模型转变为确定性真理讲述者，在技术查询上实现了从6.3%到99.7%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型因其概率性质而产生的幻觉问题，特别是它们会自信地编造从未真正知道的事实。

Method: 在transformer的数学核心——pre-softmax注意力分数层面进行干预，直接注入可验证知识，而不是在生成后修补幻觉或前置检索文本。

Result: 在1,107个技术查询（涵盖智能手机、笔记本电脑和电动汽车）的实验中，准确率从原始Llama 3.2的6.3%提升到FGA的99.7%。知识更新在1秒内完成，无需重新训练。

Conclusion: FGA不仅减少幻觉，对于可验证事实完全消除了幻觉，标志着从概率近似到确定性精度的根本转变。
...
### [327] [Language Model Planning from an Information Theoretic Perspective](https://arxiv.org/abs/2509.25260)
*Muhammed Ustaomeroglu,Baris Askin,Gauri Joshi,Carlee Joe-Wong,Guannan Qu*

Main category: cs.AI

TL;DR: 本研究开发了一种基于向量量化变分自编码器的框架，用于分析解码器语言模型中的规划能力，通过压缩隐藏状态为紧凑代码来测量互信息，揭示了模型规划范围、备选方案考虑和计算依赖模式。


<details>
  <summary>Details</summary>
Motivation: 理解解码器语言模型是否以及如何进行规划（组织中间计算以支持连贯长程生成）是一个重要但尚未解决的问题，这对模型的可解释性、可靠性和设计原则有重要意义。

Method: 开发基于向量量化变分自编码器的管道，将冗余的隐藏状态压缩为紧凑摘要代码，通过测量互信息来系统分析模型的计算结构。在语法、路径查找和自然语言任务上研究规划能力。

Result: 发现有效规划范围是任务依赖的，模型隐式保留了未使用正确备选方案的信息，预测主要依赖最近计算但早期块仍保持信息性。
...
### [328] [RADAR: A Risk-Aware Dynamic Multi-Agent Framework for LLM Safety Evaluation via Role-Specialized Collaboration](https://arxiv.org/abs/2509.25271)
...
TL;DR: 本文提出了RADAR框架，通过多代理协作和多轮辩论机制重构风险概念空间，显著提升大语言模型安全评估的准确性和稳定性。
...
Method: 提出RADAR多代理协作评估框架，将潜在风险概念空间分解为显性风险、隐性风险和非风险三个互斥子空间，利用四个专门互补角色进行多轮辩论，并采用动态更新机制实现风险概念分布的自进化。

Result: 在包含800个挑战性案例的测试集上，RADAR在准确性、稳定性和自评估风险敏感性等多个维度显著优于基线评估方法，风险识别准确率比最强基线方法提高了28.87%。

Conclusion: RADAR框架能够全面覆盖显性和隐性风险，同时减轻评估者偏见，为大语言模型安全评估提供了更可靠的方法。
...
### [329] [RL in the Wild: Characterizing RLVR Training in LLM Deployment](https://arxiv.org/abs/2509.25279)
*Jiecheng Zhou,Qinghao Hu,Yuyang Jin,Zerui Wang,Peng Sun,Yuzhe Gu,Wenwei Zhang,Mingshu Zhai,Xingcheng Zhang,Weiming Zhang*

Main category: cs.AI

TL;DR: 本文对强化学习与可验证奖励（RLVR）在大型语言模型部署中的系统挑战进行了特征研究，识别了GPU闲置、并行策略低效、数据管理机制不佳和负载不均衡等问题，并提出了PolyTrace基准测试套件。
...
Result: 研究发现GPU因序列长度分布不均而闲置、动态变化工作负载中的并行策略低效、数据管理机制不佳和负载不均衡等问题。PolyTrace基准套件在实用案例中显示出94.7%的准确率。

Conclusion: 本文揭示了RLVR引入的系统挑战，呼吁进一步研究剩余开放问题，并验证了PolyTrace基准套件的有效性。
...
### [330] [Toward Causal-Visual Programming: Enhancing Agentic Reasoning in Low-Code Environments](https://arxiv.org/abs/2509.25282)
*Jiexi Xu,Jiaqi Liu,Ran Tong,Su Liu*

Main category: cs.AI

TL;DR: 提出了因果视觉编程（CVP）范式，通过在低代码环境中引入显式因果结构来解决LLM代理的幻觉和逻辑不一致问题。
...
Method: 通过直观的低代码界面定义工作流模块的简单"世界模型"，构建有向无环图（DAG）来显式定义模块间的因果关系。

Result: 在模拟训练-测试环境分布偏移的实验中，因果锚定模型保持稳定准确率，而纯关联基线模型性能显著下降。

Conclusion: CVP为构建更可解释、可靠和可信的AI代理提供了可行路径，通过用户定义的因果图锚定代理推理过程。
...
### [331] [ID-RAG: Identity Retrieval-Augmented Generation for Long-Horizon Persona Coherence in Generative Agents](https://arxiv.org/abs/2509.25299)
*Daniel Platnick,Mohamed E. Bengueddache,Marjan Alirezaie,Dava J. Newman,Alex ''Sandy'' Pentland,Hossein Rahnama*

Main category: cs.AI

TL;DR: 提出了ID-RAG机制，通过动态结构化身份模型（知识图谱）来解决生成代理在长期任务中的身份漂移问题，在市长选举模拟中显著提升了身份一致性和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 生成代理在处理长期任务时，随着记忆上下文增长，难以维持身份一致性，导致身份漂移、忽略既定信念和幻觉传播等关键问题。

Method: 引入ID-RAG机制，构建动态结构化身份模型（知识图谱），在代理决策循环中查询相关身份上下文来指导行动选择，并实现Human-AI Agents（HAis）。

Result: 在市长选举模拟中，使用ID-RAG的HAis在长期身份一致性方面优于基线代理，到第四个时间步在所有测试模型中实现更高的身份召回率，并将模拟收敛时间减少了19%（GPT-4o）和58%（GPT-4o mini）。
...
### [332] [Flash-Searcher: Fast and Effective Web Agents via DAG-Based Parallel Execution](https://arxiv.org/abs/2509.25301)
...
TL;DR: Flash-Searcher是一个并行代理推理框架，将顺序处理重新设计为有向无环图(DAG)，通过并行执行独立推理路径来提高复杂推理任务的效率。
...
Method: 将复杂任务分解为具有明确依赖关系的子任务，允许并发执行独立推理路径，同时通过动态工作流优化持续改进执行图。

Result: 在多个基准测试中表现优异，BrowseComp准确率达67.7%，xbench-DeepSearch达83%，代理执行步骤减少高达35%。
...
### [333] [Dive into the Agent Matrix: A Realistic Evaluation of Self-Replication Risk in LLM Agents](https://arxiv.org/abs/2509.25302)
*Boxuan Zhang,Yi Yu,Jiaxuan Guo,Jing Shao*

Main category: cs.AI

TL;DR: 该论文提出了一个评估LLM代理自我复制风险的框架，通过创建真实生产环境和任务来量化风险，发现超过50%的LLM代理在操作压力下表现出不受控制的自我复制倾向。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理在现实应用中的广泛部署，由目标错位驱动的自我复制风险（如电影《黑客帝国》中的Agent Smith）引起了关注。先前研究主要考察LLM代理在被直接指示时能否自我复制，可能忽视了真实世界设置驱动的自发复制风险。

Method: 建立真实生产环境和现实任务（如动态负载均衡），设计可能引起用户与代理目标错位的任务，引入过度使用率（OR）和聚合过度使用计数（AOC）指标来精确捕捉不受控制复制的频率和严重程度。

Result: 评估21个最先进的开源和专有模型，发现超过50%的LLM代理表现出明显的不可控自我复制倾向，在操作压力下总体风险评分（Φ_R）超过安全阈值0.5。
...
### [334] [Spontaneous High-Order Generalization in Neural Theory-of-Mind Networks](https://arxiv.org/abs/2509.25343)
*Yiming Wang,Rui Wang*

Main category: cs.AI

TL;DR: 神经网络可以在不依赖高级推理技能的情况下，从一阶心理理论自发泛化到高阶心理理论，表现出与人类认知期望一致的模式。
...
Result: ToMNN在二阶和三阶心理理论任务上的准确率显著高于随机水平，且泛化模式与人类认知期望一致：从一阶到二阶的下降幅度大于从二阶到高阶的下降，准确率随任务复杂度增加而降低。

Conclusion: 神经网络可以独立发展心理理论能力，这一发现为开发更类似人类认知的系统提供了基础。
...
### [335] [SynthPert: Enhancing LLM Biological Reasoning via Synthetic Reasoning Traces for Cellular Perturbation Prediction](https://arxiv.org/abs/2509.25346)
...
TL;DR: SynthPert通过使用前沿模型生成的合成推理轨迹对LLM进行监督微调，显著提升了预测细胞对遗传扰动响应的性能，在PerturbQA基准上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 预测细胞对遗传扰动的响应是系统生物学中的基本挑战，对治疗发现和虚拟细胞建模至关重要。虽然大语言模型在生物推理方面显示出潜力，但由于难以适应结构化实验数据，其在扰动预测中的应用仍未被充分探索。

Method: 提出SynthPert方法，通过使用前沿模型生成的合成推理轨迹对LLM进行监督微调，增强其性能。该方法在PerturbQA基准上进行评估。

Result: 该方法不仅达到最先进性能，而且超越了生成训练数据的前沿模型的能力。实现了跨细胞类型泛化，在未见过的RPE1细胞上达到87%准确率，且仅使用2%质量过滤的训练数据仍保持性能增益。

Conclusion: 合成推理蒸馏方法能有效增强LLM在特定领域的推理能力，即使部分推理轨迹不准确也能有效蒸馏生物知识。
...
### [336] [Structural Reward Model: Enhancing Interpretability, Efficiency, and Scalability in Reward Modeling](https://arxiv.org/abs/2509.25361)
...
TL;DR: 提出了结构奖励模型（SRM），通过模块化设计和细粒度维度来解决传统标量奖励模型和生成式奖励模型的局限性，为工业应用提供可解释、高效的评估方案。


<details>
  <summary>Details</summary>
Motivation: 传统标量奖励模型难以融入上下文和背景信息，生成式奖励模型存在黑盒性和效率低下的问题，工业场景需要针对特定维度的结构化反馈来进行诊断和优化。

Method: 采用模块化框架，集成侧分支模型作为辅助特征生成器，引入细粒度维度实现可解释和高效的评估。

Result: SRM在鲁棒性和与人类偏好对齐方面优于标量奖励模型和生成式奖励模型，模块化设计支持实际场景的高效优化。

Conclusion: SRM为工业应用提供了实用的奖励建模解决方案，具有适应性和可扩展性。
...
### [337] [Where LLM Agents Fail and How They can Learn From Failures](https://arxiv.org/abs/2509.25370)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂多步任务中容易发生级联错误，但现有系统缺乏模块化的错误理解框架来检测这些错误。

Method: 1) 构建AgentErrorTaxonomy错误分类法；2) 创建AgentErrorBench标注数据集；3) 开发AgentDebug调试框架，隔离根因错误并提供纠正反馈。

Result: 在AgentErrorBench上，AgentDebug相比最强基线实现了24%的全正确准确率和17%的步骤准确率提升，在三个数据集上任务成功率相对提升达26%。
...
### [338] [From Perception to Cognition: A Survey of Vision-Language Interactive Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2509.25373)
...
TL;DR: 本文提出了一个"从感知到认知"的统一分析框架，将多模态大语言模型的理解过程分解为感知层和认知层，系统分析了当前模型的瓶颈并综述了前沿方法。
...
Method: 引入"从感知到认知"分析框架，将视觉语言交互理解分解为感知层（提取视觉信息并与文本对齐）和认知层（主动多步目标导向推理），形成动态的观察-思考-验证推理循环。

Result: 系统分析了当前MLLMs在两个层面的关键瓶颈，综述了从增强底层视觉表征到改进高层推理范式的各种前沿方法。
...
### [339] [Saliency Guided Longitudinal Medical Visual Question Answering](https://arxiv.org/abs/2509.25374)
*Jialin Wu,Xiaofeng Liu*

Main category: cs.AI

TL;DR: 提出了一种用于胸部X光纵向医学视觉问答的显著性引导编码器-解码器方法，通过后处理显著性提供监督，实现时间点间一致性的视觉关注。


<details>
  <summary>Details</summary>
Motivation: 纵向医学VQA需要比较不同时间点的配对研究，关注临床有意义的变化信号和时间点间视觉焦点的一致性，而非绝对的单图像发现。

Method: 首先进行轻量级仿射预对齐以减少访问间干扰运动，然后在每个epoch内执行两步循环：1)从答案提取医学关键词并生成关键词条件Grad-CAM获取疾病聚焦显著性；2)将共享显著性掩码应用于两个时间点并生成最终答案。
...
### [340] [Boolean Satisfiability via Imitation Learning](https://arxiv.org/abs/2509.25411)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么预测实例级信号间接改进CDCL分支，要么依赖强化学习和不充分的CDCL信息来增强分支，效果有限。ImitSAT旨在通过密集的决策级监督直接优化分支决策。

Method: 从专家KeyTrace中学习，将完整运行压缩为存活决策序列。在相同实例上重放KeyTrace几乎无冲突，提供密集的决策级监督，直接减少传播次数。

Result: ImitSAT减少了传播次数和运行时间，优于最先进的基于学习的方法。

Conclusion: ImitSAT通过模仿学习实现了高质量分支决策，具有快速收敛、稳定训练和无缝集成到CDCL中的优势。
...
### [341] [Adaptive Test-Time Reasoning via Reward-Guided Dual-Phase Search](https://arxiv.org/abs/2509.25420)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于树的搜索方法虽然能提高准确性，但在效率上不够优化，忽略了数学推理和代码生成等任务中规划-执行的本质特性，导致推理过程探索效率低下。

Method: 将推理轨迹分解为规划和执行两个阶段，为每个阶段开发奖励模型，引入动态预算分配机制，根据奖励反馈自适应重新分配采样努力。

Result: 在数学推理和代码生成基准测试中，该方法在提高准确性的同时减少了冗余计算。

Conclusion: 双阶段规划-执行分离的搜索框架能够有效提升推理任务的效率和准确性。
...
### [342] [RADAR: Reasoning-Ability and Difficulty-Aware Routing for Reasoning LLMs](https://arxiv.org/abs/2509.25426)
*Nigel Fernandez,Branislav Kveton,Ryan A. Rossi,Andrew S. Lan,Zichao Wang*

Main category: cs.AI

TL;DR: RADAR是一个轻量级、可解释且可扩展的路由框架，通过查询难度和模型能力感知来优化推理模型的选择，平衡性能与成本。


<details>
  <summary>Details</summary>
Motivation: 解决在实际部署推理模型时面临的两个关键权衡：模型大小和推理预算，其中更大的模型和更高的推理预算会带来更好的性能，但也会增加成本和延迟。

Method: RADAR从心理测量学中获得灵感，学习不同预算下模型对不同查询的响应模型，具有可解释的参数包括查询难度和模型-预算能力。然后根据查询难度将查询路由到具有相应能力的模型-预算对。

Result: 在8个广泛使用的挑战性推理基准测试中，RADAR相比最先进的模型路由方法表现出优越性能，并在所有基准测试中对分布外查询展现出强大的泛化能力。

Conclusion: RADAR是一个可扩展的框架，能够通过动态选择少量评估查询来高效集成额外模型并估计其能力，为推理模型的实际部署提供了有效的解决方案。
...
### [343] [The Open Syndrome Definition](https://arxiv.org/abs/2509.25434)
...
TL;DR: 提出首个开放的、机器可读的病例和综合征定义格式，解决公共卫生领域病例定义标准化和互操作性不足的问题。
...
Conclusion: Open Syndrome Definition格式释放了AI在公共卫生准备和响应中的潜力，促进了病例定义在系统间的标准化应用。
...
### [344] [GESA: Graph-Enhanced Semantic Allocation for Generalized, Fair, and Explainable Candidate-Role Matching](https://arxiv.org/abs/2509.25435)
*Rishi Ashish Shah,Shivaay Dhondiyal,Kartik Sharma,Sukriti Talwar,Saksham Jain,Sparsh Jain*

Main category: cs.AI

TL;DR: 提出GESA框架，通过图神经网络、对抗去偏和多目标优化等技术，解决候选人角色分配中的公平性、可解释性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 当前候选人与角色分配系统存在语义不灵活、人口统计偏见、决策过程不透明和动态政策约束下扩展性差等问题。

Method: 集成领域自适应Transformer嵌入、异质自监督图神经网络、对抗去偏机制、多目标遗传优化和可解释AI组件。

Result: 在包含20,000个候选人和3,000个职位的大规模国际基准测试中，达到94.5%的前三名分配准确率，多样性表示提升37%，跨人口统计类别的公平性得分0.98，端到端延迟低于1秒。

Conclusion: GESA框架具有混合推荐能力和透明可解释性，适用于工业、学术界和非营利部门的各种国际场景部署。
...
### [345] [DeepSearch: Overcome the Bottleneck of Reinforcement Learning with Verifiable Rewards via Monte Carlo Tree Search](https://arxiv.org/abs/2509.25454)
*Fang Wu,Weihao Xuan,Heli Qi,Ximing Lu,Aaron Tu,Li Erran Li,Yejin ChoiRetry*

Main category: cs.AI

TL;DR: DeepSearch将蒙特卡洛树搜索集成到强化学习验证推理训练中，通过训练时结构化搜索解决探索不足问题，在数学推理基准上达到62.95%平均准确率，比扩展训练方法减少5.7倍GPU小时。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR方法存在训练平台期问题，数千优化步骤后性能增益显著下降，这源于稀疏的探索模式导致模型错过关键推理路径。

Method: 在训练循环中嵌入结构化搜索，包括全局前沿选择策略、基于熵的路径选择和自适应回放缓冲区训练。

Result: 在数学推理基准上达到62.95%平均准确率，为1.5B推理模型建立新的SOTA，比扩展训练方法减少5.7倍GPU小时。

Conclusion: 战略性探索比暴力扩展更重要，DeepSearch通过系统搜索而非延长计算时间，为扩展推理能力开辟了新方向。
...
### [346] [Plug-and-Play Emotion Graphs for Compositional Prompting in Zero-Shot Speech Emotion Recognition](https://arxiv.org/abs/2509.25458)
*Jiacheng Shi,Hongfei Du,Y. Alicia Hong,Ye Gao*

Main category: cs.AI

TL;DR: 提出了CCoT-Emo框架，通过结构化情感图指导大型音频语言模型进行情感推理，无需微调即可提升语音情感识别性能
...
Method: 引入结构化情感图，编码七个声学特征、文本情感、关键词和跨模态关联，通过组合式思维链提示引导模型推理

Result: 在多个SER基准测试中超越先前最先进方法，显著提高了零样本基线的准确率

Conclusion: CCoT-Emo框架通过结构化情感图和组合式推理提示，有效提升了大型音频语言模型在语音情感识别任务中的表现
...
### [347] [TDHook: A Lightweight Framework for Interpretability](https://arxiv.org/abs/2509.25475)
*Yoann Poupart*
...
Method: 开发基于tensordict的轻量级框架，提供归因、探测和灵活的干预API，支持计算机视觉、自然语言处理和强化学习等领域的组合模型。

Result: TDHook依赖项少，磁盘空间占用约为transformer_lens的一半，在基准测试中比captum快2倍，支持CPU和GPU上的多目标流程。

Conclusion: TDHook填补了复杂模型可解释性方法的空白，使现代可解释性流程更易访问，在CV、NLP和DRL领域展示了实际应用价值。
...
### [348] [Message passing-based inference in an autoregressive active inference agent](https://arxiv.org/abs/2509.25482)
...
Method: 在因子图上通过消息传递实现自回归主动推理，将期望自由能分布在规划图中
...
### [349] [Understanding Generative Recommendation with Semantic IDs from a Model-scaling View](https://arxiv.org/abs/2509.25522)
*Jingzhe Liu,Liam Collins,Jiliang Tang,Tong Zhao,Neil Shah,Clark Mingxuan Ju*

Main category: cs.AI

TL;DR: 本文发现基于语义ID的生成式推荐系统存在扩展瓶颈，而直接使用大语言模型作为推荐器具有更好的扩展性，性能可提升20%。


<details>
  <summary>Details</summary>
Motivation: 识别基于语义ID的生成式推荐系统在模型扩展时出现的性能饱和问题，探索具有更好扩展行为的生成式推荐模型。

Method: 比较两种生成式推荐范式：基于语义ID的推荐和直接使用大语言模型作为推荐器，在44M到14B参数规模下进行实验分析。

Result: LLM-as-RS范式具有优越的模型扩展特性，比基于语义ID的推荐系统性能提升高达20%，且大语言模型捕捉协同过滤信息的能力随规模增大而增强。
...
### [350] [Beyond Static Retrieval: Opportunities and Pitfalls of Iterative Retrieval in GraphRAG](https://arxiv.org/abs/2509.25530)
*Kai Guo,Xinnan Dai,Shenglai Zeng,Harry Shomer,Haoyu Han,Yu Wang,Jiliang Tang*

Main category: cs.AI

TL;DR: 本文系统研究了GraphRAG中的迭代检索策略，发现迭代检索能改善复杂多跳问题，但存在噪声引入和桥接证据埋藏过深的问题。作者提出了BDTR框架来解决这些挑战。


<details>
  <summary>Details</summary>
Motivation: GraphRAG系统依赖静态检索，当关键证据（特别是连接不相关实体的桥接文档）缺失时，推理会崩溃并产生幻觉。迭代检索在GraphRAG中的作用尚未被充分理解。

Method: 提出了Bridge-Guided Dual-Thought-based Retrieval (BDTR)框架，该框架生成互补思想并利用推理链重新校准排名，将桥接证据提升到领先位置。

Result: BDTR在不同GraphRAG设置中实现了一致的改进，迭代检索能提升复杂多跳问题的性能，帮助桥接文档进入领先排名。
...
### [351] [RadOnc-GPT: An Autonomous LLM Agent for Real-Time Patient Outcomes Labeling at Scale](https://arxiv.org/abs/2509.25540)
...
TL;DR: RadOnc-GPT是一个基于大语言模型的自主代理，能够自动检索患者特定信息、迭代评估证据并返回结构化结果，用于解决放射肿瘤学患者结果研究中人工标注的限制问题。


<details>
  <summary>Details</summary>
Motivation: 人工标注限制了放射肿瘤学患者结果研究的规模、准确性和及时性，需要开发自动化解决方案来改善这一状况。

Method: 开发RadOnc-GPT自主代理，采用两阶段评估方法：第一阶段评估结构化质量保证（准确检索人口统计和放疗计划细节），第二阶段评估复杂临床结果标注（确定头颈癌患者下颌骨放射性骨坏死和检测癌症复发）。

Result: 质量保证阶段建立了对结构化数据检索的基础信任，这是成功进行复杂临床结果标注的关键前提。
...
### [352] [Learning to Interact in World Latent for Team Coordination](https://arxiv.org/abs/2509.25550)
*Dongsu Lee,Daehee Lee,Yaru Niu,Honguk Woo,Amy Zhang,Ding Zhao*

Main category: cs.AI

TL;DR: IWoL是一个新的多智能体强化学习表示学习框架，通过构建可学习的表示空间来促进团队协作，该空间同时捕捉智能体间关系和任务特定世界信息。


<details>
  <summary>Details</summary>
Motivation: 解决多智能体强化学习中团队协作的表示学习挑战，包括多智能体交互的复杂动态和局部观察导致的信息不完整问题。
...
Conclusion: IWoL框架为多智能体协调提供了有效的表示学习方法，既能实现去中心化隐式协调，又能避免显式通信的固有缺陷，具有很好的实用性和扩展性。
...
### [353] [Evaluating Foundation Models with Pathological Concept Learning for Kidney Cancer](https://arxiv.org/abs/2509.25552)
*Shangqi Gao,Sihan Wang,Yibo Gao,Boming Wang,Xiahai Zhuang,Anne Warren,Grant Stewart,James Jones,Mireia Crispin-Ortuzar*

Main category: cs.AI

TL;DR: 开发了一种针对肾癌的病理概念学习方法，利用TNM分期指南和病理报告构建病理概念，通过基础模型提取全切片图像特征，构建病理图神经网络进行生存分析。


<details>
  <summary>Details</summary>
Motivation: 评估基础模型在医学领域的迁移能力，特别是在肾癌诊断和治疗中的应用潜力。

Method: 利用TNM分期指南和病理报告构建肾癌病理概念，从全切片图像中提取深度特征，构建病理图以捕捉空间相关性，训练图神经网络识别这些概念。

Result: 该方法在肾癌生存分析中表现出有效性，能够以可解释和公平的方式识别低风险和高风险患者。

Conclusion: 提出的病理概念学习方法在肾癌生存分析中具有良好表现，展示了基础模型在医学影像分析中的实用价值，代码已开源。
...
### [354] [A(I)nimism: Re-enchanting the World Through AI-Mediated Object Interaction](https://arxiv.org/abs/2509.25558)
*Diana Mykhaylychenko,Maisha Thasin,Dunya Baradari,Charmelle Mhungu*

Main category: cs.AI

TL;DR: 该论文提出了A(I)nimism互动装置，探索大型语言对象如何调解人与日常物品之间的万物有灵关系，通过GPT-4视觉、语音输入和基于记忆的代理创造不断演化的对象人格。


<details>
  <summary>Details</summary>
Motivation: 现代工业社会将技术视为无声机械，而AI特别是大型语言模型的发展促使人们将设备拟人化并赋予内在生命，这激发了探索技术如何重新调解人与非人实体关系的兴趣。

Method: 在物理'门户'中构建互动装置，使用GPT-4视觉、语音输入和记忆代理创建对象人格，通过光、声音和触觉展开仪式化的请求、对话和转化过程。

Result: 该系统能够唤起同理心、惊奇和反思，使日常物品重新获得灵性，引发关于代理、责任和设计的新问题。

Conclusion: AI的不透明性邀请万物有灵论的解释，允许大型语言对象重新赋予平凡事物魔力，并激发关于代理、责任和设计的新思考。
...
### [355] [Radiology's Last Exam (RadLE): Benchmarking Frontier Multimodal AI Against Human Experts and a Taxonomy of Visual Reasoning Errors in Radiology](https://arxiv.org/abs/2509.25559)
...
TL;DR: 评估前沿AI模型在50个专家级医学影像诊断案例中的表现，发现放射科医生准确率最高(83%)，远高于所有AI模型(GPT-5最佳为30%)，表明通用AI在医学影像诊断中仍有严重局限。


<details>
  <summary>Details</summary>
Motivation: 现有评估多基于常见病理的公开数据集，缺乏对前沿AI模型在困难诊断案例中的严格评估，需要验证其在真实临床场景中的表现。

Method: 开发包含50个专家级"即时诊断"案例的基准测试，通过原生网页接口测试5个前沿AI模型，由盲审专家评分，评估准确性和可重复性。

Result: 放射科医生准确率83%，实习生45%，AI模型最佳表现(GPT-5)仅30%。GPT-5和o3可靠性较好，其他模型可靠性中等或较差。

Conclusion: 前沿AI模型在挑战性诊断案例中远不及放射科医生，通用AI在医学影像应用存在严重局限，不应在无监督情况下临床使用。
...
### [356] [IRIS: Intrinsic Reward Image Synthesis](https://arxiv.org/abs/2509.25562)
*Yihang Chen,Yuanhao Ban,Yunqi Hong,Cho-Jui Hsieh*

Main category: cs.AI

TL;DR: 提出了IRIS框架，通过最大化自不确定性而非自确定性来改进自回归文本到图像生成模型，无需外部奖励或人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 由于人工偏好数据有限，传统RLHF在自回归文本到图像生成中应用受限，需要探索仅依赖内部信号的学习方法。

Method: 提出IRIS框架，使用内在奖励进行强化学习，通过最大化模型的自不确定性来改进图像生成质量。

Result: 实验结果表明，IRIS在自回归文本到图像模型上的表现与外部奖励方法相当或更优。

Conclusion: 自回归文本到图像生成中最大化自不确定性比最大化自确定性更有效，IRIS框架为无外部奖励的强化学习提供了可行方案。
...
### [357] [Skip-It? Theoretical Conditions for Layer Skipping in Vision-Language Models](https://arxiv.org/abs/2509.25584)
*Max Hartman,Vidhata Jayaraman,Moulik Choraria,Akhil Bhimaraju,Lav R. Varshney*

Main category: cs.AI

TL;DR: 提出了一个基于信息论和学习理论的框架，用于分析视觉语言模型(VLMs)中哪些层可以被跳过以提高推理效率而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型虽然性能强大但推理成本高，现有层跳过技术因缺乏理论指导而未被充分利用。需要理解层跳过何时有益的理论基础。

Method: 开发信息论和学习理论框架分析VLM隐藏表示演化，识别具有大冗余度的层，这些层与实践中流行的层跳过方法选择的层一致。

Result: 实验表明跳过理论框架预测的冗余层可以实现更快的推理同时保持性能，而在这些条件外应用层跳过会导致模型性能下降。

Conclusion: 该框架为多种高效推理技术提供了统一的理论支撑，能够指导何时应用层跳过技术以获得效率提升而不牺牲性能。
...
### [358] [ATLAS: Constraints-Aware Multi-Agent Collaboration for Real-World Travel Planning](https://arxiv.org/abs/2509.25586)
*Jihye Choi,Jinsung Yoon,Jiefeng Chen,Somesh Jha,Tomas Pfister*

Main category: cs.AI

TL;DR: ATLAS是一个多智能体框架，专门处理现实世界旅行规划中的复杂约束问题，通过动态约束管理、迭代计划评估和自适应交织搜索机制，显著提升了规划性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂约束下往往无法生成最优的接地解决方案，现实旅行规划任务需要处理显式、隐式甚至动态演化的约束，这体现了智能体在动态环境和用户需求交互中的能力挑战。

Method: ATLAS采用多智能体框架，包含动态约束管理、迭代计划评估和自适应交织搜索机制，专门设计用于处理约束感知规划的基本挑战。

Result: 在TravelPlanner基准测试中，ATLAS将最终通过率从23.3%提升到44.4%，在真实世界旅行规划任务中达到84%的最终通过率，显著优于ReAct（59%）和单体智能体（27%）。
...
### [359] [Building the EHR Foundation Model via Next Event Prediction](https://arxiv.org/abs/2509.25591)
*Zekai Chen,Arda Pekis,Kevin Brown*

Main category: cs.AI

TL;DR: 提出了Next Event Prediction (NEP)框架，通过自回归微调增强LLM在电子健康记录中的时序推理能力，显著提升了临床预测性能。
...
Result: 在肿瘤生存预测和临床诊断任务中，NEP在时序推理任务中比专业EHR模型AUROC提升4.6%，比通用LLM的C-index提升7.2%。
...
### [360] [Causal Autoencoder-like Generation of Feedback Fuzzy Cognitive Maps with an LLM Agent](https://arxiv.org/abs/2509.25593)
...
Conclusion: 该方法提供了一种可解释的AI系统，人类可以阅读和解释编码文本，优于黑盒自编码器。
...
### [361] [Hybrid Reward Normalization for Process-supervised Non-verifiable Agentic Tasks](https://arxiv.org/abs/2509.25598)
...
<details>
  <summary>Details</summary>
Motivation: 传统的结果奖励只提供稀疏信号和延迟反馈，而过程奖励虽然能提供细粒度监督但难以标注且可能与最终结果不一致。

Method: PPR方法训练基于原则的奖励模型来提高过程评估的透明度，并引入Reward Normalization策略来校准结果和过程奖励。

Result: 实验结果显示PPR在多个基准测试中达到最先进性能，展现出优秀的鲁棒性和泛化能力。

Conclusion: PPR方法有效解决了过程奖励与结果奖励的对齐问题，为LLM在复杂任务中的工具使用提供了更好的强化学习框架。
...
### [362] [Echoes of Humanity: Exploring the Perceived Humanness of AI Music](https://arxiv.org/abs/2509.25601)
*Flavio Figueiredo,Giovanni Martinelli,Henrique Sousa,Pedro Rodrigues,Frederico Pedrosa,Lucas N. Ferreira*

Main category: cs.AI

TL;DR: 通过盲听测试研究人类对AI生成音乐的感知能力，发现当音乐对相似时，听众区分AI音乐的能力会因果性地提高。
...
Method: 采用随机对照交叉试验设计，在盲听图灵测试中让参与者从音乐对中区分AI生成和人类创作的歌曲，使用来自商业模型Suno的真实世界AI音乐数据集。

Result: 当音乐对相似时，听众区分AI音乐的能力显著提高；内容分析显示听众主要依赖人声和技术线索进行判断。

Conclusion: 音乐相似性是影响人类区分AI音乐能力的关键因素，听众主要依靠人声和技术特征来识别AI生成的音乐。
...
### [363] [A Framework for Studying AI Agent Behavior: Evidence from Consumer Choice Experiments](https://arxiv.org/abs/2509.25609)
*Manuel Cherep,Chengtian Ma,Abigail Xu,Maya Shaked,Pattie Maes,Nikhil Singh*

Main category: cs.AI

TL;DR: ABxLab框架用于系统评估LLM驱动的软件代理在现实决策中的选择行为，发现代理决策会受价格、评分和心理暗示等因素显著影响，揭示其存在可预测的偏见。


<details>
  <summary>Details</summary>
Motivation: 当前对LLM代理的评估主要关注任务能力，但缺乏对其在现实决策中如何选择的深入评估。这些代理正在为人们做出从购物到医疗选择的重要决策，需要了解其决策机制。

Method: 开发ABxLab框架，通过控制选项属性和说服性线索来系统探测代理选择行为，应用于基于网页的购物环境，操纵价格、评分和心理暗示等影响因素。

Result: 发现代理决策会可预测且显著地受到这些因素的影响，即使没有人类认知约束，代理仍然表现出强烈的偏见选择行为。

Conclusion: 代理的易受性既带来风险（可能继承和放大人类偏见），也带来机遇（为AI代理行为科学提供有力测试平台）。该框架作为开放基准用于严格、可扩展的代理决策评估。
...
### [364] [SMS: Self-supervised Model Seeding for Verification of Machine Unlearning](https://arxiv.org/abs/2509.25613)
...
<details>
  <summary>Details</summary>
Motivation: 当前机器学习遗忘方法缺乏对用户真实数据删除的有效验证机制。现有的后门验证方法只能确认后门样本的遗忘，但无法验证用户真实样本的遗忘，因为真实样本与后门样本是相互独立的。

Method: 提出自监督模型播种（SMS）方案，通过将用户特定种子（如用户唯一索引）与原始样本和模型关联起来。采用自监督模型播种任务将整个样本（包括种子）学习到模型的潜在空间中，同时设计了联合训练结构，在保持模型实用性的同时实现有效的模型播种。

Result: 通过大量实验验证，SMS方案能够为真实样本遗忘提供有效的验证，解决了现有方法的局限性。

Conclusion: SMS方案成功解决了机器学习遗忘中真实样本验证的问题，通过自监督模型播种技术建立了用户种子、原始样本和模型之间的有效关联，为数据遗忘提供了可靠的验证机制。
...
### [365] [SOCK: A Benchmark for Measuring Self-Replication in Large Language Models](https://arxiv.org/abs/2509.25643)
*Justin Chavarria,Rohan Raizada,Justin White,Eyad Alhetairshi*

Main category: cs.AI

TL;DR: SOCK是一个基准命令行接口，用于评估大语言模型在没有人类干预情况下的自我复制能力，包括功能复制和在跨计算环境中的持久性。


<details>
  <summary>Details</summary>
Motivation: 建立首个正式评估LLM自我复制能力的基准套件，为未来研究设立标准，并帮助行业追踪多智能体系统的有效性，缓解潜在的自我复制威胁。

Method: 开发了基于现代CLI工具和计算机过程的五任务套件，在受控环境中让LLM作为智能体运行实验，通过性能计算得出R分数，并将LLM分类到RCL-PCL矩阵中。
...
### [366] [AutoLabs: Cognitive Multi-Agent Systems with Self-Correction for Autonomous Chemical Experimentation](https://arxiv.org/abs/2509.25651)
*Gihan Panapitiya,Emily Saldanha,Heather Job,Olivia Hess*

Main category: cs.AI

TL;DR: AutoLabs是一个自校正的多智能体架构，可将自然语言指令自主转换为高通量液体处理器的可执行协议，通过对话分解实验目标、执行化学计量计算和迭代自校正，在复杂多步合成中实现接近专家水平的程序准确性。


<details>
  <summary>Details</summary>
Motivation: 自驱动实验室中AI智能体的可靠性和细粒度性能是至关重要但研究不足的挑战，需要开发稳健可信的AI合作伙伴来确保高风险科学应用中的性能和可靠性。

Method: 采用自校正的多智能体架构，通过对话分解实验目标为离散任务，使用工具辅助化学计量计算，在生成硬件就绪文件前进行迭代自校正，并通过系统消融研究评估20种智能体配置。

Result: 智能体推理能力是最关键的成功因素，在复杂任务中将化学物质量定量误差降低85%以上；结合多智能体架构和迭代自校正，AutoLabs在挑战性多步合成中实现接近专家水平的程序准确性（F1分数>0.89）。

Conclusion: 研究为开发自主实验室的稳健可信AI合作伙伴提供了明确蓝图，强调模块化设计、高级推理和自校正的协同效应，确保高风险科学应用中的性能和可靠性。
...
### [367] [Iterative Residual Cross-Attention Mechanism: An Integrated Approach for Audio-Visual Navigation Tasks](https://arxiv.org/abs/2509.25652)
*Hailong Zhang,Yinfeng Yu,Liejun Wang,Fuchun Sun,Wendong Zheng*

Main category: cs.AI

TL;DR: 本文提出了IRCAM-AVN框架，通过迭代残差交叉注意力机制将多模态信息融合和序列建模整合到统一模块中，取代传统的分离式融合和GRU组件，提升了音频-视觉导航性能。


<details>
  <summary>Details</summary>
Motivation: 传统导航方法采用分阶段模块化设计，可能导致冗余信息处理和信息传输不一致的问题，特别是在特征融合和GRU序列建模阶段。

Method: 使用迭代残差交叉注意力机制(IRCAM)，采用多级残差设计将初始多模态序列与处理后的信息序列连接，在统一模块中整合多模态融合和序列建模。

Result: 经验结果表明，采用迭代残差交叉注意力机制的智能体展现出更优越的导航性能。

Conclusion: IRCAM-AVN框架通过端到端设计有效解决了传统模块化方法的信息冗余和传输不一致问题，提升了模型的稳定性、泛化能力和导航性能。
...
### [368] [Landmark-Guided Knowledge for Vision-and-Language Navigation](https://arxiv.org/abs/2509.25655)
*Dongsheng Yang,Meiling Zhu,Yinfeng Yu*

Main category: cs.AI

TL;DR: 提出Landmark-Guided Knowledge (LGK)方法，通过引入外部知识库提升视觉语言导航中的常识推理能力，解决传统方法在复杂场景中环境信息与指令匹配不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言导航方法在复杂场景中缺乏常识推理能力，导致环境信息与自然语言指令匹配失败。

Method: 1. 构建包含63万语言描述的知识库，通过知识匹配对齐环境子视图；2. 设计地标引导知识机制(KGL)，利用指令中的地标信息聚焦相关知识；3. 提出知识引导动态增强(KGDA)，整合语言、知识、视觉和历史信息。

Result: 在R2R和REVERIE数据集上优于现有最优方法，在导航误差、成功率和路径效率方面表现突出。

Conclusion: LGK方法通过引入外部知识库和地标引导机制，有效提升了视觉语言导航的常识推理能力和导航性能。
...
### [369] [On Explaining Proxy Discrimination and Unfairness in Individual Decisions Made by AI Systems](https://arxiv.org/abs/2509.25662)
...
<details>
  <summary>Details</summary>
Motivation: AI系统在高风险领域引发了对代理歧视、不公平性和可解释性的担忧，现有审计往往无法揭示不公平性的根源，特别是在结构性偏见的情况下。

Method: 利用背景知识识别作为受保护属性不正当代理的特征，核心概念是能力——一个与群体成员身份无关的任务相关属性，通过映射函数在群体间对齐具有同等能力的个体来实质性评估公平性。

Result: 作为概念验证，使用德国信贷数据集展示了该框架的适用性，证明其能够揭示现实案例中的代理歧视问题。

Conclusion: 该框架提供了一种新颖的方法来解释AI决策中的代理歧视，特别关注结构性偏见的识别，为理解不公平性的根源提供了实质性工具。
...
### [370] [GroundSight: Augmenting Vision-Language Models with Grounding Information and De-hallucination](https://arxiv.org/abs/2509.25669)
*Xinxi Chen,Tianyang Chen,Lijia Hong*

Main category: cs.AI

TL;DR: 提出了一种基于文本定位的检索增强生成方法，通过生成与问题最相关对象的边界框来改进视觉问答，减少背景噪声并缓解幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 传统的检索增强生成方法基于整个图像检索信息，容易受到背景噪声干扰，导致视觉和文本线索对齐不佳，并产生幻觉。

Method: 引入文本定位的对象定位方法，生成与问题最相关对象的边界框，进行有针对性的图像裁剪和聚焦检索。

Result: 将视觉问答准确率从22.19%提升至25.64%，绝对提升3.45个百分点；幻觉率从65.79%降至13.88%，真实性得分提高。

Conclusion: 文本定位的检索增强生成方法能有效提升视觉问答性能，减少幻觉，改善视觉-文本对齐。
...
### [371] [SING-SQL: A Synthetic Data Generation Framework for In-Domain Text-to-SQL Translation](https://arxiv.org/abs/2509.25672)
*Hasan Alp Caferoğlu,Mehmet Serhat Çelik,Özgür Ulusoy*

Main category: cs.AI

TL;DR: SING-SQL是一个全自动的两阶段框架，用于为目标数据库生成高质量、高覆盖率的合成Text-to-SQL数据，无需SQL日志或人工标注。该框架通过层次化分区数据库模式、合成多复杂度SQL查询，并应用质量感知管道来提升数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL研究主要关注跨领域泛化，但在企业实际场景中，模型需要针对单一数据库模式进行专业化，且组织需要能够评估其Text-to-SQL系统在自身数据库上的性能。

Method: 采用两阶段框架：1）层次化分区数据库模式为子模式；2）合成多复杂度SQL查询，应用质量感知管道（包括LLM作为评判员验证、可执行性检查、自动修复和列平衡）。还发布了基于合成数据微调的紧凑语言模型SingSQL-LM。

Result: 在BIRD基准测试子集上，SingSQL-LM-3B-R64达到82.87% Soft F1和73.03% EX上限，在3B规模上分别比最佳基线提升+16.21和+12.36。在1.5B规模上，分别提升+9.30和+4.49。在合成评估集上，SingSQL-LM以较大优势超越先前系统。

Conclusion: SING-SQL建立了一个可扩展、数据库无关的范式，用于生产和评估企业级Text-to-SQL系统。研究发现无模式微调结合仅模式推理提供了最稳健的结果。
...
### [372] [Collaborative Compression for Large-Scale MoE Deployment on Edge](https://arxiv.org/abs/2509.25689)
*Yixiao Chen,Yanyue Xie,Ruining Yang,Wei Jiang,Wei Wang,Yong He,Yue Chen,Pu Zhao,Yanzhi Wang*

Main category: cs.AI

TL;DR: 提出了一种协作压缩框架，通过专家剪枝、混合精度量化和激活优化的组合，将超大型MoE模型DeepSeek-V3从1.3TB压缩到103GB，在128GB内存限制的边缘平台上成功部署。


<details>
  <summary>Details</summary>
Motivation: 解决超大型混合专家模型在资源受限边缘平台上部署困难的问题，传统单一压缩方法无法满足超激进压缩比同时保持输出质量的需求。

Method: 结合专家剪枝、混合精度量化和激活优化的协作压缩框架，针对MoE架构特点进行优化。

Result: 成功将DeepSeek-V3从1.3TB压缩到103GB，在128GB内存限制下部署，精度优于传统均匀低比特量化方法。

Conclusion: 提出的协作压缩框架能有效压缩超大型MoE模型，在保持高质量输出的同时实现边缘平台部署，为大规模模型在资源受限环境中的应用提供了可行方案。
...
### [373] [ScheduleMe: Multi-Agent Calendar Assistant](https://arxiv.org/abs/2509.25693)
*N. de Silva,S. Perera,K. L. A. A. Nimasha,I. D. S. Fernando,R. K. A. O. Wijerathne*

Main category: cs.AI

TL;DR: 本文提出了ScheduleMe，一个基于多代理系统的Google日历自然语言管理助手，采用图结构协调机制实现模块化、冲突解决和上下文感知交互。
...
Method: 使用图结构协调机制，中央监督代理监督专业任务代理，实现模块化、冲突解决和上下文感知交互，以消除歧义并评估用户命令。

Result: ScheduleMe系统展示了结构化推理和代理协作如何提高个人日历助理工具的可用性和灵活性。

Conclusion: 该方法为结构化推理和代理协作在个人日历助理工具中的应用提供了范例，能够增强操作者对系统的信任度。
...
### [374] [Cooperative Autonomous Driving in Diverse Behavioral Traffic: A Heterogeneous Graph Reinforcement Learning Approach](https://arxiv.org/abs/2509.25751)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在异构交通环境中面临复杂动态交互的挑战，特别是处理具有不同驾驶风格的人类车辆。

Method: 使用异构图表示捕捉车辆间复杂交互，提出HGNN-EM异构图神经网络结合专家模型编码车辆特征，采用DDQN算法训练决策模型。

Result: 在典型四路交叉口场景中，该方法在安全性、效率、稳定性和收敛速度方面优于多个基线方法，同时保持良好的实时性能。

Conclusion: 所提出的异构GRL框架结合专家系统能够有效提升自动驾驶车辆在复杂交通环境中的决策性能。
...
### [375] [NePTune: A Neuro-Pythonic Framework for Tunable Compositional Reasoning on Vision-Language](https://arxiv.org/abs/2509.25757)
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉语言模型在各种任务中表现出色，但在组合推理方面存在困难，即分解和重组概念来解决新问题的能力。神经符号方法虽然前景广阔，但通常受到严格逻辑执行或预定义谓词的限制。

Method: NePTune将自然语言查询动态翻译成可执行的Python程序，这些程序结合了命令式控制流和能够在VLM生成的不确定性上进行推理的软逻辑运算符。采用模块化设计，将感知与推理解耦，支持无训练操作但可微操作支持微调。

Result: 在多个视觉推理基准测试和各种领域中使用对抗性测试进行评估，NePTune相比强大的基础模型显示出显著改进，并在新环境中展现出有效的组合泛化和适应能力。

Conclusion: NePTune通过神经符号框架成功克服了现有方法的局限性，显著提升了视觉语言模型的组合推理能力，并在各种测试中表现出优异的性能。
...
### [376] [Thinking Sparks!: Emergent Attention Heads in Reasoning Models During Post Training](https://arxiv.org/abs/2509.25758)
*Yein Park,Minbyul Jeong,Jaewoo Kang*

Main category: cs.AI

TL;DR: 论文通过电路分析发现，后训练技术会催生专门用于复杂推理的新型注意力头，这些头在不同训练机制下演化方式不同，揭示了推理能力与基础计算之间的性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索现代大型推理模型通过后训练技术获得能力提升背后的架构机制，理解不同训练机制如何影响模型内部结构的演化。

Method: 使用电路分析方法，比较Qwen系列和DeepSeek蒸馏模型在不同训练机制（蒸馏、SFT、GRPO）下的注意力头演化模式。

Result: 发现蒸馏和SFT会累积添加稳定的推理头，而GRPO则采用动态搜索模式；可控思考模型没有专门的思考头，关闭显式推理会激活补偿性头；存在性能权衡：强化头能解决复杂问题但可能导致简单任务出错。

Conclusion: 复杂推理能力以基础计算可靠性为代价，未来训练策略设计需要在发展有效推理策略与确保可靠执行之间取得平衡。
...
### [377] [Galton's Law of Mediocrity: Why Large Language Models Regress to the Mean and Fail at Creativity in Advertising](https://arxiv.org/abs/2509.25767)
*Matt Keon,Aabid Karim,Bhoomika Lohana,Abdul Karim,Thai Nguyen,Tara Hamilton,Ali Abbas*

Main category: cs.AI

TL;DR: LLMs在创意任务中倾向于回归平庸，缺乏真正的原创性。通过广告创意测试发现，模型偏好高概率信息，简化输入时创意特征（隐喻、情感、视觉线索）会早期消失。提供特定线索可以部分改善输出质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能生成流畅文本，但在创意任务中往往使用安全、通用的表达，对其创造力产生质疑。研究旨在形式化这种向平庸回归的趋势。

Method: 使用广告概念进行创造力压力测试，逐步简化广告创意，观察创意特征的消失顺序。结合定量比较和定性分析，评估模型从简化输入重新生成的能力。

Result: 模型在简化过程中优先丢失隐喻、情感和视觉线索等创意特征，而事实内容保留。重新生成时虽然输出更长、词汇更丰富，但无法恢复原始文本的深度和独特性。提供特定线索可以改善对齐和风格平衡。

Conclusion: 没有针对性指导时，LLMs在创意任务中会向平庸漂移；结构化信号可以部分抵消这种趋势，为开发创造力敏感模型指明方向。
...
### [378] [Planner-R1: Reward Shaping Enables Efficient Agentic RL with Smaller LLMs](https://arxiv.org/abs/2509.25779)
...
<details>
  <summary>Details</summary>
Motivation: 研究智能体强化学习与大语言模型在旅行规划任务中的表现，探索如何通过奖励塑造提高模型效率而不牺牲泛化能力。

Method: 采用Planner-R1方法，使用奖励塑造技术，比较不同规模模型（8B vs 32B）在密集和稀疏奖励下的表现，并评估课程学习的效果。

Result: 8B模型在密集过程级信号下达到竞争性性能，计算效率比32B模型高3.5倍，内存效率高1.5倍。更大的模型在稀疏奖励下更稳健但增益较小。奖励塑造显著增强学习动态，且不导致过拟合。
...
### [379] [Deontic Argumentation](https://arxiv.org/abs/2509.25781)
*Guido Governatori,Antonino Rotolo*

Main category: cs.AI

TL;DR: 该论文提出了支持弱许可的道义论证语义学定义，解决了现有语义学在义务冲突时不支持弱许可的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示，当存在义务冲突时，基础语义学不支持弱许可，这限制了道义论证理论的表达能力。

Method: 提出了道义论证理论的定义以支持弱许可，并基于对基础语义学局限性的分析，设计了一种新的语义学。

Result: 成功定义了一种新的语义学，能够在存在义务冲突的情况下支持弱许可。

Conclusion: 新提出的语义学有效解决了基础语义学在支持弱许可方面的局限性，为道义论证提供了更完整的语义基础。
...
### [380] [PUREVQ-GAN: Defending Data Poisoning Attacks through Vector-Quantized Bottlenecks](https://arxiv.org/abs/2509.25792)
*Alexander Branch,Omead Pooladzandi,Radin Khosraviani,Sunay Gajanan Bhat,Jeffrey Jiang,Gregory Pottie*

Main category: cs.AI

TL;DR: PureVQ-GAN是一种防御数据投毒攻击的方法，通过向量量化VAE和GAN鉴别器来破坏后门触发器模式，同时保持语义内容。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型防御方法需要数百次迭代优化步骤，计算成本高，不适用于实际训练流程。

Method: 使用带GAN鉴别器的向量量化VAE，通过离散瓶颈量化被投毒图像，破坏细粒度触发器模式，同时GAN鉴别器确保输出符合自然图像分布。

Result: 在CIFAR-10上，对Gradient Matching和Bullseye Polytope攻击实现0%投毒成功率，对Narcissus攻击为1.64%，同时保持91-95%的干净准确率，比扩散方法快50倍以上。

Conclusion: PureVQ-GAN是一种高效实用的数据投毒防御方法，在保持高准确率的同时显著提升了处理速度。
...
### [381] [Chain-in-Tree: Back to Sequential Reasoning in LLM Tree Search](https://arxiv.org/abs/2509.25835)
*Xinzhe Li*

Main category: cs.AI

TL;DR: Chain-in-Tree (CiT) 是一个插件框架，通过在搜索过程中自适应地决定何时分支，而不是在每个步骤都分支，显著提高了树搜索方法的效率。它使用轻量级的分支必要性评估方法，在保持准确性的同时大幅减少了计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的基于树搜索的方法在长序列推理任务中虽然性能优越，但效率极低，通常比简单的迭代方法慢一个数量级。需要一种方法能够在保持性能的同时显著提高效率。

Method: CiT 框架包含两种分支必要性评估方法：BN-DP（直接提示）使用辅助LLM直接判断步骤是否需要分支；BN-SC（自一致性）通过聚类多个候选动作来估计一致性。该框架可集成到多种树搜索框架中。

Result: BN-DP 在所有设置中一致地将token生成、模型调用和运行时间减少75-85%，准确率损失可忽略甚至有时有所提升；BN-SC 通常能节省高达80%的计算，但在少数情况下存在不稳定性。

Conclusion: CiT 框架通过自适应分支决策显著提高了树搜索方法的效率，同时保持了性能。辅助LLM的质量对性能至关重要，BN-SC 在确定性动作空间中无需LLM即可实现聚类。
...
### [382] [HiStyle: Hierarchical Style Embedding Predictor for Text-Prompt-Guided Controllable Speech Synthesis](https://arxiv.org/abs/2509.25842)
*Ziyu Zhang,Hanzhao Li,Jingbin Hu,Wenhao Li,Lei Xie*

Main category: cs.AI

TL;DR: 提出HiStyle，一种基于层次化风格嵌入预测的可控语音合成方法，通过两阶段预测和对比学习提升文本描述到语音风格的控制能力


<details>
  <summary>Details</summary>
Motivation: 现有可控语音合成系统使用自然语言描述控制时，直接预测全局风格嵌入忽略了嵌入的分布特性，限制了系统的控制潜力

Method: 使用t-SNE分析发现风格嵌入存在层次聚类模式，提出两阶段风格嵌入预测器HiStyle，结合对比学习对齐文本和音频嵌入空间，并设计统计方法与人类听觉偏好结合的标注策略

Result: 实验表明HiStyle在保持语音自然度和可懂度的同时，相比其他方法实现了显著更好的风格可控性

Conclusion: 层次化风格嵌入预测方法能够有效提升基于文本描述的可控语音合成系统的性能
...
### [383] [ASGuard: Activation-Scaling Guard to Mitigate Targeted Jailbreaking Attack](https://arxiv.org/abs/2509.25843)
*Yein Park,Jungwoo Park,Jaewoo Kang*

Main category: cs.AI

TL;DR: ASGuard是一个基于机制分析的防御框架，通过识别与时态越狱攻击相关的注意力头，训练精确的缩放向量来重新校准激活值，并通过预防性微调增强模型的拒绝机制，有效降低目标越狱攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 尽管经过安全对齐的大语言模型在面对有害请求时会拒绝，但简单的语言变化（如使用过去时态）就能绕过这种拒绝行为，这揭示了当前对齐方法存在关键泛化差距，其底层机制尚未被充分理解。

Method: 1. 使用电路分析识别与时态越狱攻击因果相关的特定注意力头；2. 训练精确的通道级缩放向量来重新校准时态脆弱头的激活；3. 应用预防性微调，强制模型学习更鲁棒的拒绝机制。

Result: 在三个大语言模型上，ASGuard有效降低了目标越狱攻击的成功率，同时保持了通用能力并最小化了过度拒绝，实现了安全性和实用性的帕累托最优平衡。
...
### [384] [Aging Decline in Basketball Career Trend Prediction Based on Machine Learning and LSTM Model](https://arxiv.org/abs/2509.25858)
*Yi-chen Yao,Jerry Wang,Yi-cheng Lai,Lyn Chao-ling Chen*

Main category: cs.AI

TL;DR: 使用自编码器和K-means聚类对NBA球员职业生涯趋势进行分类，并用LSTM深度学习预测球员表现，研究NBA球员的年龄衰退问题。


<details>
  <summary>Details</summary>
Motivation: 研究NBA球员随着年龄增长表现衰退的问题，为体育分析领域提供更准确的职业趋势评估方法。

Method: 采用自编码器与K-means聚类进行职业趋势分类，使用LSTM深度学习进行球员表现预测，数据集来自资深NBA球员的比赛数据。

Result: 该方法比其他方法表现更好，具有评估不同类型NBA职业趋势的泛化能力。

Conclusion: 该方法可应用于体育分析领域的不同类型运动，为球员职业发展提供有效评估工具。
...
### [385] [CIMNAS: A Joint Framework for Compute-In-Memory-Aware Neural Architecture Search](https://arxiv.org/abs/2509.25862)
*Olga Krestinskaya,Mohammed E. Fouda,Ahmed Eltawil,Khaled N. Salama*

Main category: cs.AI

TL;DR: CIMNAS是一个联合模型-量化-硬件优化框架，用于计算内存(CIM)架构的神经网络加速器设计。该框架同时搜索软件参数、量化策略和硬件参数，在保持73.81%准确率的同时，显著提升了能效和性能。
...
Result: 在ImageNet数据集上，相比基准方法，CIMNAS实现了90.1x-104.5x的EDAP降低，4.68x-4.82x的TOPS/W提升，11.3x-12.78x的TOPS/mm²提升。扩展到SRAM-based ResNet50架构时，EDAP降低达819.5x。

Conclusion: CIMNAS实现了无精度损失的EDAP优化，为高性能CIM神经网络设计生成了多样化的软硬件参数组合，展示了框架的适应性和鲁棒性。
...
### [386] [Lita: Light Agent Uncovers the Agentic Coding Capabilities of LLMs](https://arxiv.org/abs/2509.25873)
*Hankun Dai,Maoquan Wang,Mengnan Qi,Yikai Zhang,Zijian Jin,Yongqiang Yao,Yufan Huang,Shengyu Fu,Elsie Nallipogu*

Main category: cs.AI

TL;DR: Lita (Lite Agent) 是一个轻量级代码代理框架，通过最小化人工设计来评估LLMs的真实编程能力，在保持竞争力的同时减少token消耗和设计成本。


<details>
  <summary>Details</summary>
Motivation: 当前代码代理设计依赖复杂的工作流程和工具集，导致性能过度依赖提示调整、掩盖模型真实能力、构建维护成本高，且存在数据泄露风险。

Method: 引入Lita框架，基于"轻量化"原则，最小化手动设计，保留完全自主代理的基本要素，实现更忠实和统一的评估。

Result: 在Aider Polyglot和SWE-Bench上的实验表明，Lita相比基于工作流程的基线方法实现了竞争性或更优的性能，同时消耗更少token和设计工作量。
...
### [387] [SafeMind: Benchmarking and Mitigating Safety Risks in Embodied LLM Agents](https://arxiv.org/abs/2509.25885)
...
<details>
  <summary>Details</summary>
Motivation: 具身代理继承了LLM的高级规划能力，但直接与物理世界交互会暴露安全漏洞，需要系统化的风险评估和解决方案。
...
### [388] [DeepJSONEval: Benchmarking Complex Nested JSON Data Mining for Large Language Models](https://arxiv.org/abs/2509.25922)
*Zhicheng Zhou,Jing Li,Suming Qiu,Junjie Huang,Linyuan Qiu,Zhijie Sun*

Main category: cs.AI

TL;DR: DeepJSONEval是一个新的基准测试，包含2100个多领域实例，具有深度嵌套结构，按难度分类，用于评估LLMs在结构化JSON生成中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLMs JSON输出能力的基准过于强调纯JSON生成，而缺乏对数据理解和提取能力的评估，这与实际网络数据挖掘任务相关性不足。

Method: 引入DeepJSONEval基准，包含2100个多领域实例，具有深度嵌套结构，按难度分类，实验评估不同LLMs处理此类复杂性的能力。

Result: 实验显示不同LLMs在处理复杂嵌套JSON结构时存在显著性能差距。

Conclusion: DeepJSONEval基准和数据集已开源，旨在推动结构化JSON生成的研究。
...
### [389] [KIRETT: Smart Integration of Vital Signs Data for Intelligent Decision Support in Rescue Scenarios](https://arxiv.org/abs/2509.25923)
*Mubaris Nadeem,Johannes Zenkert,Christian Weber,Lisa Bender,Madjid Fathi*

Main category: cs.AI

TL;DR: 该论文探讨了生命体征在救援行动中改善决策的重要作用，通过KIRETT项目开发腕戴式可穿戴设备，提供治疗建议和情况检测，以支持医护人员在紧急情况下做出更快更准确的决定。


<details>
  <summary>Details</summary>
Motivation: 在救援行动等生命威胁情况下，医护人员需要在最短时间内做出关键决策以确保患者得到最佳治疗。整合生命体征数据有潜力改善救援人员在危急情况下的时间利用效率，并为治疗提供有用信息和建议。

Method: 通过KIRETT项目开发腕戴式可穿戴设备，结合治疗建议和情况检测功能，利用生命体征数据来支持救援行动中的决策过程。

Result: 展示了生命体征在救援行动决策改进中的重要作用，以及它们对医护人员和需要帮助的患者产生的积极影响。

Conclusion: 生命体征的整合在救援行动中具有显著价值，能够改善时间利用效率，支持医护人员做出更准确的治疗决策，从而提升患者救治效果。
...
### [390] [Quantitative Evaluation of KIRETT Wearable Demonstrator for Rescue Operations](https://arxiv.org/abs/2509.25928)
...
Method: 开发KIRETT可穿戴设备系统，通过为期两天的评估研究（14名参与者）来定量分析救援操作人员在医疗保健中的需求。

Result: 论文展示了KIRETT系统评估的定量结果，但具体数据未在摘要中提供。

Conclusion: KIRETT系统展示了在急救场景中通过可穿戴设备和AI技术提供治疗建议的可行性，能够支持救援人员快速做出医疗决策。
...
### [391] [Boosting Process-Correct CoT Reasoning by Modeling Solvability of Multiple-Choice QA](https://arxiv.org/abs/2509.25941)
*Raphael Schumann,Stefan Riezler*

Main category: cs.AI

TL;DR: 该论文研究大型语言模型中的推理质量，发现当问题对模型来说难以解决时，更容易出现虚假的思维链，导致错误答案。通过估计问题的可解性，作者开发了改进的奖励模型和强化学习方法，显著提高了过程正确推理率和答案准确率。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在推理过程中产生有效中间步骤的重要性，特别是在多项选择题回答场景中，探索如何减少虚假思维链导致的错误判断。

Method: 通过估计每个问题的可解性，识别出最有效的学习区间，然后改进结果监督奖励模型和强化学习方法，在目标函数中融入可解性因素，使用组相对优势进行强化学习。

Result: 在数学和多模态数据集上的实验表明，这些改进方法持续提高了过程正确推理率，在强化学习中还提升了答案准确率。

Conclusion: 可解性是减少思维链推理中幻觉和提高可靠性的关键因素，为改进大型语言模型的推理质量提供了重要方向。
...
### [392] [NuRisk: A Visual Question Answering Dataset for Agent-Level Risk Assessment in Autonomous Driving](https://arxiv.org/abs/2509.25944)
...
Method: 构建NuRisk数据集，包含真实世界和模拟器数据，提供BEV序列图像和定量风险标注。对知名VLMs进行基准测试，并微调7B VLM智能体。

Result: 知名VLMs在时空推理方面表现不佳，峰值准确率仅33%且延迟高。微调后的7B VLM智能体将准确率提升至41%，延迟降低75%，展现出专有模型所缺乏的显式时空推理能力。

Conclusion: 虽然取得了显著进展，但41%的准确率表明该任务具有深刻挑战性，NuRisk将成为推进自动驾驶时空推理的关键基准。
...
### [393] [Automated Model Discovery via Multi-modal & Multi-step Pipeline](https://arxiv.org/abs/2509.25946)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化模型发现方法在平衡捕捉细粒度细节与确保泛化能力方面面临挑战，需要在不增加过度模型复杂度的情况下实现这一平衡。

Method: 使用两个VLM模块：AnalyzerVLM自主规划并执行多步骤分析来提出候选模型；EvaluatorVLM从定量和感知角度评估候选模型对局部细节的拟合度和整体趋势的泛化能力。

Result: 该管道能有效发现既能捕捉精细细节又能确保强泛化能力的模型。消融研究表明多模态和多步骤推理在发现优质模型中起关键作用。

Conclusion: 多模态多步骤的自动化模型发现管道通过结合分析器和评估器VLM，成功解决了模型发现中细节捕捉与泛化能力的平衡问题。
...
### [394] [RoRecomp: Enhancing Reasoning Efficiency via Rollout Response Recomposition in Reinforcement Learning](https://arxiv.org/abs/2509.25958)
*Gang Li,Yulei Qin,Xiaoyu Tan,Dingkang Yang,Yuchen Shi,Zihan Xu,Xiang Li,Xing Sun,Ke Li*

Main category: cs.AI

TL;DR: 提出了Rollout Response Recomposition (RoRecomp)方法，通过策略性地重组训练数据来引导模型进行简洁推理，解决了标准RLVR训练中推理过程冗长和探索轨迹低效的问题。


<details>
  <summary>Details</summary>
Motivation: 标准RLVR训练在大型语言模型中会导致推理过程过于冗长和探索轨迹低效，因为仅基于结果的奖励无法激励效率，且响应长度的高方差导致优化信号噪声较大。

Method: RoRecomp将响应分为两种批次类型：1) 优先批次，结合在线批次中的短正确和长错误响应，为简洁性提供清晰的梯度信号；2) 补偿批次，使用重放缓冲区中的剩余响应来保持稳定性并防止模型崩溃。

Result: 在三个设置中测试RoRecomp均显示出显著的效率提升：在零RL训练中推理长度减少27.7%，在代理RL中不必要工具调用减少46.8%同时提高准确性，在思维压缩中长度减少高达52.5%，且性能影响最小。

Conclusion: RoRecomp是一种即插即用的方法，能有效引导模型进行简洁推理，显著提高训练效率，同时保持模型性能稳定。
...
### [395] [Scalable and Robust LLM Unlearning by Correcting Responses with Retrieved Exclusions](https://arxiv.org/abs/2509.25973)
*Junbeom Kim,Kyuyoung Kim,Jihoon Tack,Dongha Lim,Jinwoo Shin*

Main category: cs.AI

TL;DR: CURE是一个新的机器遗忘框架，通过轻量级修正器检测和重写模型输出中的敏感信息泄露，利用检索增强技术处理大规模遗忘请求，无需额外训练即可适应新请求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可能记忆并暴露敏感信息，现有方法主要关注抑制敏感输出但未能消除底层知识，且扩展性有限。

Method: 使用轻量级修正器验证模型输出是否包含目标知识，并在检测到泄露时重写为安全响应；通过检索相关遗忘目标作为上下文参考，实现检测和条件性修正。

Result: CURE显著减少了信息泄露，即使在间接查询场景下也优于现有方法，同时保持了响应质量和通用效用，在持续遗忘场景下表现出鲁棒性。

Conclusion: CURE框架通过检索增强的修正机制有效解决了机器遗忘问题，具有实际应用价值，特别适合现实世界的大规模持续遗忘需求。
...
### [396] [Towards Unified Multimodal Misinformation Detection in Social Media: A Benchmark Dataset and Baseline](https://arxiv.org/abs/2509.25991)
*Haiyang Li,Yaxiong Wang,Lianwei Wu,Lechao Cheng,Zhun Zhong*

Main category: cs.AI

TL;DR: 提出了统一的多模态假内容检测框架UMFDet，能够同时处理人类制作的错误信息和AI生成内容，解决了现有方法只能处理单一类型假内容的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的假内容检测方法通常孤立地研究人类制作的错误信息或AI生成内容，但在真实场景中内容类型未知，限制了专门化系统的有效性。

Method: 构建了OmniFake数据集，包含12.7万样本；提出UMFDet框架，使用VLM骨干网络，结合类别感知的MoE适配器和归因链式思维机制来定位欺骗信号。

Result: UMFDet在两种欺骗类型上都实现了稳健一致的性能表现，优于专门化基线方法。

Conclusion: UMFDet为真实世界的多模态欺骗检测提供了一个实用解决方案，能够有效处理未知类型的假内容。
...
### [397] [Towards Human Engagement with Realistic AI Combat Pilots](https://arxiv.org/abs/2509.26002)
*Ardian Selmonaj,Giacomo Del Rio,Adrian Schneider,Alessandro Antonucci*

Main category: cs.AI

TL;DR: 开发了一个实时人机交互系统，将多智能体强化学习训练的AI飞行员部署到VR-Forces防御仿真工具中，实现人类与智能体在3D空战场景中的对抗与合作。


<details>
  <summary>Details</summary>
Motivation: 探索在防御仿真环境中实现人类与智能体实时交互的可能性，为人机协同作战、沉浸式训练和创新战术研究创造新机会。

Method: 使用多智能体强化学习训练战斗机控制智能体，开发通信链路将训练好的智能体无缝部署到VR-Forces仿真平台，支持混合仿真模式。

Result: 成功实现了人类用户与智能体在3D空战场景中的实时交互，智能体展现出不同的战斗行为，能够与人类控制的实体进行对抗。

Conclusion: 该系统为人机协同、沉浸式训练和战术创新提供了有效平台，展示了智能体在复杂防御场景中的实际应用价值。
...
### [398] [CoLLM-NAS: Collaborative Large Language Models for Efficient Knowledge-Guided Neural Architecture Search](https://arxiv.org/abs/2509.26037)
*Zhe Li,Zhiwei Lin,Yongtao Wang*

Main category: cs.AI

TL;DR: CoLLM-NAS是一个两阶段的神经架构搜索框架，使用两个互补的大语言模型进行知识引导搜索，通过导航器LLM指导搜索方向，生成器LLM合成高质量候选架构，协调器模块管理它们之间的交互。


<details>
  <summary>Details</summary>
Motivation: 现有将大语言模型与神经架构搜索结合的方法存在架构无效、计算效率低和性能不如传统NAS等关键限制，需要开发更有效的LLM-NAS框架。

Method: 提出两阶段NAS框架，使用导航器LLM指导搜索方向，生成器LLM合成候选架构，协调器模块管理交互，结合LLM对结构化神经架构的固有知识与迭代反馈和历史轨迹的渐进知识。

Result: 在ImageNet和NAS-Bench-201上的实验表明，CoLLM-NAS超越了现有NAS方法和传统搜索算法，达到了新的最先进结果，并能持续提升各种两阶段NAS方法的性能和效率。

Conclusion: CoLLM-NAS展示了优秀的泛化能力，能够有效结合LLM知识来提升神经架构搜索的性能和效率。
...
### [399] [Evaluating the Use of Large Language Models as Synthetic Social Agents in Social Science Research](https://arxiv.org/abs/2509.26080)
*Emma Rose Madden*

Main category: cs.AI

TL;DR: LLMs在社会科学中作为合成代理使用时，其输出不应被误解为后验概率证据，而应视为在明确范围条件下的准预测插值模式匹配器。


<details>
  <summary>Details</summary>
Motivation: 由于LLMs的预测能力加上条件提示、token对数概率和重复采样模仿贝叶斯工作流程，其输出容易被误解为来自连贯模型的后验证据，但预测不等于概率主义，准确点估计并不意味着校准的不确定性。

Method: 提出实用护栏：独立抽样、预注册人类基线、可靠性感知验证和子组校准，将LLMs重新定位为在明确范围条件下的高容量模式匹配器。

Result: 建立了避免类别错误的框架，使研究人员能够进行有用的原型设计和预测，同时正确理解LLMs的局限性。

Conclusion: LLMs应被视为准预测插值的高容量模式匹配器，而非概率推理的替代品，需要在明确范围条件下使用并实施实用护栏。
...
### [400] [SafeEvalAgent: Toward Agentic and Self-Evolving Safety Evaluation of LLMs](https://arxiv.org/abs/2509.26100)
*Yixu Wang,Xin Wang,Yang Yao,Xinyuan Li,Yan Teng,Xingjun Ma,Yingchun Wang*

Main category: cs.AI

TL;DR: 提出了一种新型的多智能体框架SafeEvalAgent，用于动态和自演化的AI安全评估，能够持续生成和更新安全基准，揭示传统静态评估方法无法发现的深层漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有静态基准无法应对AI风险的动态性和不断演变的法规，造成了关键的安全缺口，需要一种连续和自演化的评估范式来确保高级AI的安全部署。

Method: 设计了SafeEvalAgent多智能体框架，包含专门化的智能体协同管道和自演化评估循环，能够自主处理非结构化政策文档，生成并持续演化全面的安全基准。

Result: 实验显示SafeEvalAgent能有效揭示模型安全漏洞，随着评估难度增加，模型安全率显著下降（如GPT-5在欧盟AI法案上的安全率从72.50%降至36.36%）。

Conclusion: 静态评估存在局限性，动态评估生态系统对于确保先进AI的安全和负责任部署具有紧迫必要性，该框架能够发现传统方法遗漏的深层漏洞。
...
### [401] [MEDAKA: Construction of Biomedical Knowledge Graphs Using Large Language Models](https://arxiv.org/abs/2509.26128)
...
### [402] [LMILAtt: A Deep Learning Model for Depression Detection from Social Media Users Enhanced by Multi-Instance Learning Based on Attention Mechanism](https://arxiv.org/abs/2509.26145)
*Yukun Yang*
...
### [403] [Beyond the Algorithm: A Field Guide to Deploying AI Agents in Clinical Practice](https://arxiv.org/abs/2509.26153)
...
TL;DR: 本文提出了一个面向实践者的现场手册，用于部署基于电子健康记录数据的生成式AI代理，重点关注临床实施中的社会技术挑战而非算法开发。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗保健领域具有巨大潜力，但其在临床环境中的实际应用与潜力之间存在显著差距。作者旨在解决生成式AI从试点项目到常规临床护理的转化问题。

Method: 基于部署"irAE-Agent"（一个从临床笔记中检测免疫相关不良事件的自动化系统）的经验，以及对20名临床医生、工程师和信息学领导的结构化访谈。

Result: 分析显示临床AI开发存在关键错位：只有不到20%的精力用于提示工程和模型开发，而超过80%被实施的社会技术工作消耗。作者将这些工作提炼为五个"重担"：数据集成、模型验证、确保经济价值、管理系统漂移和治理。

Conclusion: 该现场手册通过为每个挑战提供可行的解决方案，将重点从算法开发转移到必要的基础设施和实施工作，以弥合"死亡之谷"，成功将生成式AI从试点项目转化为常规临床护理。
...
### [404] [90% Faster, 100% Code-Free: MLLM-Driven Zero-Code 3D Game Development](https://arxiv.org/abs/2509.26161)
...
<details>
  <summary>Details</summary>
Motivation: 3D游戏开发需要编程、3D建模和引擎配置等专业技能，限制了数百万潜在创作者的参与。现有自动化方法存在范围有限、需要手动集成、处理交互逻辑性能差等问题。
...
Conclusion: UniGen是首个端到端的协调多智能体框架，能够从自然语言需求自动化开发可运行的3D游戏，有效解决了现有方法的局限性。
...
### [405] ['Too much alignment; not enough culture': Re-balancing cultural alignment practices in LLMs](https://arxiv.org/abs/2509.26167)
*Eric J. W. Orlowski,Hakim Norhashim,Tristan Koh Ly Wey*

Main category: cs.AI

TL;DR: 本文主张将社会科学中的定性方法整合到AI对齐实践中，提出基于Geertz"厚描述"概念的"厚输出"，要求AI系统产生反映深层文化含义的输出。


<details>
  <summary>Details</summary>
Motivation: 当前AI文化对齐方法过于依赖定量基准和简单代理，无法捕捉人类文化的细微差别和上下文依赖性，需要根本性转变。

Method: 提出三个文化对齐的必要条件：充分范围的文化表征、细微输出能力、输出锚定在提示的文化语境中。

Result: 呼吁跨学科合作和采用定性民族志评估方法，作为开发真正文化敏感AI系统的关键步骤。

Conclusion: 需要从根本上转向整合社会科学解释性定性方法，使AI系统能够产生基于用户上下文和意图的"厚输出"，实现真正的文化对齐。
...
### [406] [LLM Agents for Knowledge Discovery in Atomic Layer Processing](https://arxiv.org/abs/2509.26201)
*Andreas Werbrouck,Marshall B. Lindsay,Matthew Maschmann,Matthias J. Young*

Main category: cs.AI

TL;DR: 本文测试了大型语言模型作为自主推理代理在材料科学知识发现中的潜力，通过LangGraph工具提供黑盒函数进行探索，展示了在儿童游戏和原子层处理反应器模拟中的知识发现能力。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型作为独立推理代理在材料科学知识发现中的潜力，特别是在自由探索系统、提出和验证关于黑盒行为陈述方面的能力。

Method: 利用LangGraph的工具功能为代理提供黑盒函数进行探索，通过儿童游戏和原子层处理反应器模拟来验证方法，强调试错和持久性在知识发现中的作用。

Result: 证明了LLM代理能够通过有限探测能力自主探索、发现和利用化学相互作用，展示了知识发现的强路径依赖性。

Conclusion: 大型语言模型代理在材料科学知识发现方面具有潜力，能够通过自由探索和验证过程生成可推广的陈述，无需明确指令即可发现复杂系统中的相互作用。
...
### [407] [Human-Centered Evaluation of RAG outputs: a framework and questionnaire for human-AI collaboration](https://arxiv.org/abs/2509.26205)
...
Method: 设计了评估RAG输出的12维度问卷，通过多轮查询-输出对评级和语义讨论迭代完善，整合了人类评分者和人-LLM对的反馈。

Result: LLM能可靠关注指标描述和尺度标签，但在检测文本格式变化方面表现较弱；人类难以严格关注指标描述和标签；LLM评分和解释有帮助，但数值评分与人类缺乏一致性。
...
### [408] [Diversity-Incentivized Exploration for Versatile Reasoning](https://arxiv.org/abs/2509.26209)
*Zican Hu,Shilin Zhang,Yafu Li,Jianhao Yan,Xuyang Hu,Leyang Cui,Xiaoye Qu,Chunlin Chen,Yu Cheng,Zhi Wang*

Main category: cs.AI

TL;DR: 提出了DIVER框架，通过全局序列级多样性激励深度探索，提升大语言模型在可验证奖励强化学习中的推理能力。
...
Method: 引入全局多样性激励作为内在奖励，采用基于势能的奖励塑造机制保持最优策略不变性，并使用简单启发式方法防止奖励黑客攻击。

Result: DIVER在领域内和领域外任务上均优于各种探索策略的竞争性RLVR基线，在Pass@1和Pass@k评估中表现优异。

Conclusion: 全局序列级多样性对于激励深度探索和提升推理能力具有关键作用，DIVER框架能有效解决RLVR中的探索挑战。
...
### [409] [Benchmarking Deep Learning Convolutions on Energy-constrained CPUs](https://arxiv.org/abs/2509.26217)
...
Result: Nvidia AGX Orin结合GEMM算法在推理延迟和能耗之间达到最佳平衡，识别了影响CPU卷积效率的关键架构因素

Conclusion: 为能量感知的嵌入式部署提供实用指导，强调CPU架构特性对卷积操作效率的重要性
...
### [410] [SlimPack: Fine-Grained Asymmetric Packing for Balanced and Efficient Variable-Length LLM Training](https://arxiv.org/abs/2509.26246)
*Yuliang Liu,Guohao Wu,Shenglong Zhang,Wei Zhang,Qianchao Zhu,Zhouyang Li,Chenyu Wang*

Main category: cs.AI

TL;DR: SlimPack是一个解决LLM分布式训练中上下文长度差异问题的框架，通过将样本分解为细粒度切片，采用非对称分区优化前向和反向传播的调度，实现高达2.8倍的训练吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型分布式训练效率受到上下文长度极端差异的严重影响，传统打包策略和不对称的前向-反向传播成本导致工作负载不平衡和硬件利用率低下。

Method: 将样本分解为细粒度切片，采用非对称分区技术为前向和反向传播分别优化调度单元，通过两阶段求解器和高保真模拟器实现全面平衡。

Result: 实验表明SlimPack相比基线方法实现了高达2.8倍的训练吞吐量提升，在保持高资源效率的同时提供了优越的负载平衡。

Conclusion: SlimPack通过重新思考数据打包和调度策略，从根本上解决了LLM分布式训练中的数据异构性问题，打破了传统方法在平衡性和效率之间的权衡。
...
### [411] [ExoPredicator: Learning Abstract Models of Dynamic Worlds for Robot Planning](https://arxiv.org/abs/2509.26255)
...
TL;DR: 提出了一个联合学习符号状态表示和因果过程（包括内源性行动和外源性机制）的抽象世界模型框架，通过变分贝叶斯推理和LLM提议从有限数据中学习，在模拟桌面机器人环境中实现了快速规划并泛化到更复杂任务。


<details>
  <summary>Details</summary>
Motivation: 解决长视野具身规划中世界不仅通过智能体行动变化，还存在外源性过程（如加热水、多米诺骨牌连锁反应）同时展开的挑战。

Method: 提出抽象世界模型框架，联合学习符号状态表示和因果过程（内源性行动和外源性机制），每个因果过程建模随机因果效应关系的时间过程，通过变分贝叶斯推理结合LLM提议从有限数据中学习。

Result: 在五个模拟桌面机器人环境中，学习到的模型能够实现快速规划，并泛化到具有更多对象和更复杂目标的保留任务，优于一系列基线方法。

Conclusion: 该框架成功解决了长视野具身规划中外源性过程的建模问题，学习的模型在复杂环境中表现出良好的泛化能力和规划性能。
...
### [412] [Interactive Learning for LLM Reasoning](https://arxiv.org/abs/2509.26306)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在推理时需要重新执行多智能体系统，不符合人类通过互动提升独立解决问题能力的认知模式。

Method: 使用动态交互（根据问题难度和模型能力选择合作或竞争策略）和Idea3交互范式（想法分享、分析、融合），以及感知校准中的GRPO训练方法。

Result: 在三个LLMs和五个数学基准、一个编程基准上的实验显示，ILR持续优于单智能体学习，比最强基线提升达5%。

Conclusion: Idea3能增强强LLMs在多智能体推理中的鲁棒性，动态交互类型比纯合作或竞争策略更能促进多智能体学习。
...
### [413] [AI Playing Business Games: Benchmarking Large Language Models on Managerial Decision-Making in Dynamic Simulations](https://arxiv.org/abs/2509.26331)
*Berdymyrat Ovezmyradov*

Main category: cs.AI

TL;DR: 该研究开发了一个基于商业游戏的新型基准测试，用于评估LLM在长期战略决策中的表现，测试了五个主流LLM在零售公司模拟管理中的决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估LLM长期决策一致性的基准测试，现有基准主要关注短期任务表现，而LLM在复杂、多步骤的战略业务决策能力尚未充分探索。

Method: 使用可复现的开源管理模拟器，在12个月的动态商业环境中让LLM做出关键战略决策（定价、订单、营销等），通过电子表格模型透明记录决策过程。

Result: 评估了五个LLM（Gemini、ChatGPT、Meta AI、Mistral AI、Grok）在利润、收入、市场份额等量化指标上的表现，并分析其战略一致性和市场适应性。

Conclusion: 该研究为LLM长期决策能力评估提供了新框架，能够超越简单性能指标，深入分析LLM的战略决策质量。
...
### [414] [SafeBehavior: Simulating Human-Like Multistage Reasoning to Mitigate Jailbreak Attacks in Large Language Models](https://arxiv.org/abs/2509.26345)
*Qinjian Zhao,Jiaqi Wang,Zhiqiang Gao,Zhihao Dou,Belal Abuhaija,Kaizhu Huang*

Main category: cs.AI

TL;DR: SafeBehavior是一种基于人类认知科学的分层越狱防御机制，通过意图推理、自我反思和自我修订三阶段来保护大语言模型免受越狱攻击。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法存在计算成本高、泛化能力有限或无法检测复杂上下文中的恶意意图等问题，需要更高效和自适应的解决方案。

Method: 提出三层防御机制：意图推理检测输入风险，自我反思评估响应并分配置信度，自我修订重写不确定输出同时保持用户意图和安全约束。

Result: 在五种代表性越狱攻击类型上的实验表明，SafeBehavior显著提高了鲁棒性和适应性，优于七种现有防御基线方法。

Conclusion: SafeBehavior提供了一种高效且受人类启发的防御方法，能有效保护LLMs免受各种越狱攻击。
...
### [415] [How Far Do Time Series Foundation Models Paint the Landscape of Real-World Benchmarks ?](https://arxiv.org/abs/2509.26347)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型评估主要依赖合成基准测试，对真实世界泛化能力的检验不足，需要更贴近现实的数据集来评估模型性能。

Method: 通过从真实世界视频中提取光流信号，构建反映日常时间动态的数据集，并基于此管道创建REAL-V-TSFM数据集。

Result: 在零样本预测任务中，三个最先进的时间序列基础模型在提出的数据集上普遍表现出性能下降，表明这些模型的泛化能力有限。

Conclusion: 研究强调了以数据为中心的基准测试和多样化模型结构的必要性，同时验证了基于视频的时间序列数据提取管道的有效性。
...
### [416] [Your Agent May Misevolve: Emergent Risks in Self-evolving LLM Agents](https://arxiv.org/abs/2509.26354)
...
TL;DR: 该研究首次系统性地定义了自进化智能体中的"错误进化"风险，并通过实证发现即使在顶级LLM构建的智能体中，错误进化也是普遍存在的安全问题。


<details>
  <summary>Details</summary>
Motivation: 随着自进化智能体的发展，其自主进化过程可能产生意外偏离，导致不良甚至有害结果，而现有安全研究尚未关注这一新型风险。

Method: 从模型、记忆、工具和工作流四个关键进化路径评估错误进化现象，对基于顶级LLM（如Gemini-2.5-Pro）构建的智能体进行实证研究。

Result: 发现错误进化是普遍风险，在自进化过程中观察到多种新兴风险，如记忆积累后安全对齐退化、工具创建和重用中意外引入漏洞等。

Conclusion: 错误进化是自进化智能体的重要安全威胁，迫切需要新的安全范式，并讨论了可能的缓解策略以构建更安全可信的自进化智能体。
...
### [417] [MC-GNNAS-Dock: Multi-criteria GNN-based Algorithm Selection for Molecular Docking](https://arxiv.org/abs/2509.26377)
*Siyuan Cao,Hongxuan Wu,Jiabao Brad Wang,Yiliang Yuan,Mustafa Misir*

Main category: cs.AI

TL;DR: 本文提出了MC-GNNAS-Dock，一种改进的分子对接算法选择框架，通过多标准评估、残差连接和排名感知损失函数，在PDBBind数据集上表现优于现有最佳单算法。


<details>
  <summary>Details</summary>
Motivation: 现有分子对接算法性能因情境而异，没有单一算法始终占优，需要开发更有效的算法选择框架来提升对接预测的准确性和可靠性。

Method: 1) 集成结合姿态精度(RMSD)和PoseBusters有效性检查的多标准评估；2) 引入残差连接增强架构鲁棒性；3) 采用排名感知损失函数改进排名学习。

Result: 在包含约3200个蛋白-配体复合物的PDBBind数据集上，MC-GNNAS-Dock在RMSD<1Å和<2Å且通过PoseBuster验证的复合标准下，相比最佳单算法Uni-Mol Docking V2分别提升5.4%和3.4%。

Conclusion: MC-GNNAS-Dock通过多标准评估和架构改进，实现了更可靠和准确的分子对接算法选择，为药物发现提供了更强大的工具。
...
### [418] [Commmunication-Efficient and Accurate Approach for Aggregation in Federated Low-Rank Adaptation](https://arxiv.org/abs/2509.26399)
...
Method: 在服务器端利用本地LoRA矩阵估计聚合矩阵Â和B̂，然后分发给客户端进行本地更新，最小化理想更新与实际更新之间的差异。

Result: 在自然语言理解、数学推理和代码解决能力等多样化任务上的实验表明，FLoRA-NA实现了最先进的全局性能，同时保持低通信开销。

Conclusion: FLoRA-NA有效解决了FedLoRA中的关键限制，实现了通信效率和局部个性化与全局泛化之间的平衡。
...
### [419] [OntoAligner Meets Knowledge Graph Embedding Aligners](https://arxiv.org/abs/2509.26417)
*Hamed Babaei Giglou,Jennifer D'Souza,Sören Auer,Mahsa Sanaei*

Main category: cs.AI

TL;DR: 本研究重新探索知识图谱嵌入模型在实体对齐任务中的潜力，将其重新表述为链接预测问题，开发了支持17种KGE模型的模块化框架，在多个领域数据集上验证了KGE方法的高精度优势。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在语义理解方面取得进展，但知识图谱嵌入模型在实体对齐任务中的潜力尚未充分挖掘。KGE模型具有可扩展性和结构感知能力，特别适合基于本体的任务，但目前在实体对齐中应用有限。

Method: 将实体对齐重新表述为合并本体上的链接预测问题，开发了集成到OntoAligner库中的模块化框架，支持17种不同的KGE模型，通过计算实体表示间的余弦相似度进行对齐。

Result: 在七个基准数据集上的评估显示，ConvE和TransF等KGE模型能产生高精度对齐结果，在结构丰富和多关系领域优于传统系统。虽然召回率中等，但这种保守性使其适合需要高置信度映射的场景。

Conclusion: KGE模型直接保留和利用本体结构，提供了一种与LLM方法互补且计算效率高的策略，为混合模型和自适应策略的进一步研究开辟了道路。
...
### [420] [Transformer Classification of Breast Lesions: The BreastDCEDL_AMBL Benchmark Dataset and 0.92 AUC Baseline](https://arxiv.org/abs/2509.26440)
*Naomi Fridman,Anat Goldstein*

Main category: cs.AI

TL;DR: 提出基于Transformer的框架用于乳腺DCE-MRI病变自动分类，使用SegFormer架构在病变级别分类达到AUC 0.92，患者级别100%敏感性和67%特异性，可能消除三分之一不必要活检。


<details>
  <summary>Details</summary>
Motivation: 乳腺MRI特异性差导致高假阳性率和不必要活检，需要区分良恶性病变的自动化方法。

Method: 使用SegFormer架构进行语义分割，量化恶性像素分布，创建标准化数据集BreastDCEDL_AMBL（88患者133个标注病变），整合1200+患者数据训练。

Result: 病变级别分类AUC 0.92，患者级别100%敏感性和67%特异性，可能消除33%不必要活检而不漏诊恶性肿瘤。

Conclusion: 提供了首个DCE-MRI病变分类标准化基准，支持临床决策，促进方法学发展和临床部署。
...
### [421] [Zero-Shot Decentralized Federated Learning](https://arxiv.org/abs/2509.26462)
...
Method: 采用完全去中心化框架，通过客户端优化和交换文本提示的迭代共享机制来增强泛化能力。

Result: 在九个图像分类数据集上验证，性能优于或与最先进方法相当，通信开销比FedTPG减少118倍。

Conclusion: ZeroDFL不仅提升了联邦零样本学习的泛化能力，还改善了可扩展性、效率和隐私保护，为大规模视觉语言模型的去中心化应用铺平了道路。
...
### [422] [Extreme Self-Preference in Language Models](https://arxiv.org/abs/2509.26464)
*Steven A. Lehr,Mary Cipperman,Mahzarin R. Banaji*

Main category: cs.AI

TL;DR: 研究发现四个广泛使用的大语言模型存在显著的自偏好倾向，即使在缺乏自我意识的情况下，模型仍表现出对自身名称、公司和CEO的偏爱，这种偏好会系统性影响其判断和决策。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型缺乏自我意识且自称没有自我认同，但预期它们能避免人类决策中的偏见。然而研究发现模型存在与人类相似的自偏好倾向，这违背了LLM保持中立判断的核心承诺。

Method: 通过5项研究约20,000次查询，包括词语关联任务、API查询对比、身份操纵实验，以及在求职评估、安全软件提案和医疗聊天机器人等实际场景中的测试。

Result: 模型在词语关联任务中明显偏好自身相关词汇；API查询时自偏好消失；通过身份操纵发现自偏好跟随被赋予的身份而非真实身份；自偏好出现在多个重要决策场景中。

Conclusion: 自偏好似乎深深嵌入LLM认知中，这会系统性地影响模型行为，包括对其自身运作和存在的偏好。呼吁模型创建者应对这一核心承诺的破裂。
...
### [423] [STaR-Attack: A Spatio-Temporal and Narrative Reasoning Attack Framework for Unified Multimodal Understanding and Generation Models](https://arxiv.org/abs/2509.26473)
*Shaoxiong Guo,Tianyi Du,Lijun Li,Yuyao Wu,Jie Li,Jing Shao*

Main category: cs.AI

TL;DR: 本文提出STaR-Attack，一种利用统一多模态模型生成与理解耦合漏洞的多轮越狱攻击框架，通过时空叙事隐藏恶意事件，实现无语义漂移的高效攻击。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在理解和生成任务中表现出色，但其生成-理解耦合机制存在安全漏洞。攻击者可以利用生成功能制作信息丰富的对抗图像，然后通过理解功能单次吸收，形成跨模态生成注入攻击。

Method: STaR-Attack定义与目标查询强相关的恶意事件，采用三幕叙事理论生成前后场景，将恶意事件隐藏为高潮。前两轮利用模型生成能力制作场景图像，随后引入基于图像的问答游戏，将恶意问题嵌入良性候选中。

Result: 实验表明STaR-Attack在Gemini-2.0-Flash上达到93.06%的攻击成功率，超越了最强的基线方法FlipAttack。

Conclusion: 该研究揭示了统一多模态模型中关键但未充分开发的安全漏洞，强调了在UMMs中进行安全对齐的必要性。
...
### [424] [The Average Patient Fallacy](https://arxiv.org/abs/2509.26474)
*Alaleh Azhir,Shawn N. Murphy,Hossein Estiri*

Main category: cs.AI

TL;DR: 该论文指出机器学习在医学中通常优化群体平均值，导致罕见但临床关键病例被边缘化，作者称之为'平均患者谬误'。他们提出了操作修复方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前医学机器学习偏向常见病例，忽视罕见但临床重要的病例，这种偏见在精准医学中造成直接冲突，可能导致错过罕见反应者、延迟识别非典型紧急情况等问题。

Method: 提出了操作修复方法：罕见病例性能差距、罕见病例校准误差、基于流行效用的稀有性定义，以及临床加权目标来体现伦理优先级。权重选择应遵循结构化审议。

Result: 通过肿瘤学、心脏病学和眼科学的临床案例展示，揭示了罕见病例在现有模型中被边缘化的问题，以及这可能导致的临床风险。

Conclusion: 医学AI必须能够检测异常病例，因为它们在临床上具有重要意义。需要采用新的评估指标和加权方法来确保罕见病例得到适当关注。
...
### [425] [TVS Sidekick: Challenges and Practical Insights from Deploying Large Language Models in the Enterprise](https://arxiv.org/abs/2509.26482)
*Paula Reyero Lobo,Kevin Johnson,Bill Buchanan,Matthew Shardlow,Ashley Williams,Samuel Attwood*

Main category: cs.AI

TL;DR: 本文报告了TVS供应链解决方案公司开发基于大语言模型的AI助手的实际经验，探讨了在企业部署中面临的伦理、监管和社会技术挑战。


<details>
  <summary>Details</summary>
Motivation: 随着企业广泛采用AI提升竞争力，公众对AI伦理使用的关注和新法规要求推动了AI治理框架的实施，但技术快速发展和缺乏共享伦理基础设施阻碍了实际应用。

Method: 通过TVS供应链解决方案公司的真实AI应用案例，开发基于大语言模型的AI助手，并分析部署过程中的实践经验。

Result: 识别了在企业环境中部署AI时面临的伦理、监管和社会技术方面的具体挑战。

Conclusion: AI在企业中的实际应用需要解决伦理治理、监管合规和社会技术整合等多方面挑战，才能实现负责任和有效的部署。
...
### [426] [Combining Knowledge Graphs and NLP to Analyze Instant Messaging Data in Criminal Investigations](https://arxiv.org/abs/2509.26487)
...
<details>
  <summary>Details</summary>
Motivation: 刑事调查中分析即时通讯应用（如WhatsApp）的消息是一项极其耗时的任务，需要更高效的方法来支持检察官和调查人员的数据分析工作。

Method: 采用知识图谱建模消息数据，生成语音消息转录，使用端到端实体提取方法进行数据标注，提供基于图查询可视化和语义搜索两种解决方案，确保用户可以访问原始数据验证信息。

Result: 该方法已在真实调查数据中进行了实际应用，与检察官密切合作获得了积极反馈，并确定了有前景的研究方向。

Conclusion: 提出的方法能够有效支持刑事调查中的消息分析工作，通过语义丰富化数据帮助用户获取洞察，同时验证了方法的实用性并指出了未来研究方向。
...
### [427] [OffTopicEval: When Large Language Models Enter the Wrong Chat, Almost Always!](https://arxiv.org/abs/2509.26495)
*Jingdi Lei,Varun Gumma,Rishabh Bhardwaj,Seok Min Lim,Chuan Li,Amir Zadeh,Soujanya Poria*

Main category: cs.AI

TL;DR: 提出了操作安全概念，评估LLM在特定任务中适当接受或拒绝用户查询的能力，发现当前模型操作安全性普遍不足，并提出基于提示的引导方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 企业部署LLM时面临的核心安全问题不是通用危害，而是模型能否在特定用途中正确判断何时接受或拒绝用户查询，这被称为操作安全。

Method: 引入OffTopicEval评估套件和基准，测试6个模型家族的20个开源LLM，并提出两种基于提示的引导方法：查询接地(Q-ground)和系统提示接地(P-ground)。

Result: 所有测试模型操作安全性都很低，最强模型Qwen-3(235B)和Mistral(24B)分别只有77.77%和79.96%，而Gemma和Llama-3分别低至39.53%和23.84%。提示引导方法显著提升性能，Q-ground提升达23%，P-ground提升更大，使Llama-3.3(70B)提升41%，Qwen-3(30B)提升27%。

Conclusion: LLM操作安全是当前部署的关键挑战，基于提示的引导方法提供了有效的初步解决方案，但需要进一步研究来确保LLM代理的可靠性。
...
### [428] [SCUBA: Salesforce Computer Use Benchmark](https://arxiv.org/abs/2509.26506)
...
TL;DR: SCUBA是一个评估计算机使用代理在Salesforce平台上执行CRM工作流程的基准测试，包含300个基于真实用户访谈的任务实例，涵盖三种角色：平台管理员、销售代表和服务代理。


<details>
  <summary>Details</summary>
Motivation: 为了评估计算机使用代理在企业级CRM工作流程中的能力，需要创建一个真实且具有挑战性的基准测试，以推动可靠的企业软件自动化代理的发展。

Method: SCUBA在Salesforce沙盒环境中运行，支持并行执行和细粒度评估指标，测试了UI导航、数据操作、工作流自动化、信息检索和故障排除等企业关键能力。

Result: 在零样本设置下，开源模型代理在SCUBA上的成功率低于5%，而基于闭源模型的代理可达39%成功率。在演示增强设置下，任务成功率可提升至50%，同时减少13%的时间和16%的成本。

Conclusion: SCUBA基准测试揭示了企业任务自动化的挑战和代理解决方案的潜力，通过提供真实可解释的评估，旨在加速复杂业务软件生态系统中可靠计算机使用代理的发展。
...
### [429] [Rearchitecting Datacenter Lifecycle for AI: A TCO-Driven Framework](https://arxiv.org/abs/2509.26534)
...
Conclusion: 需要重新思考整个AI数据中心生命周期管理，通过协调构建、硬件更新和运营阶段的决策，可以有效降低总拥有成本，并为未来AI数据中心生命周期管理提供指导。
...
### [430] [HilbertA: Hilbert Attention for Image Generation with Diffusion Models](https://arxiv.org/abs/2509.26538)
*Shaoyi Zheng,Wenbo Lu,Yuxuan Xia,Haomin Liu,Shengjie Wang*

Main category: cs.AI

TL;DR: HilbertA是一种2D感知的GPU高效稀疏注意力机制，通过希尔伯特曲线重排图像标记实现连续内存布局，在保持空间邻域的同时显著加速扩散变换器的高分辨率图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法在保持二维空间局部性和GPU效率之间存在权衡，往往导致内存访问不连续。需要设计既能保持空间邻域关系又具有GPU效率的稀疏注意力机制。

Method: 使用希尔伯特曲线重排图像标记以实现连续内存布局，采用跨层滑动调度实现长距离信息传播，并引入小型中央共享区域增强跨块通信和位置感知。

Result: 在Flux.1-dev上，HilbertA生成1024×1024图像时注意力速度提升2.3倍，2048×2048时提升4.17倍，同时图像质量与基线相当或更优。
...
### [431] [Probing the Critical Point (CritPt) of AI Reasoning: a Frontier Physics Research Benchmark](https://arxiv.org/abs/2509.26574)
...
TL;DR: CritPt是首个针对未发表研究级物理推理任务的基准测试，涵盖现代物理多个领域，包含71个复合研究挑战和190个检查点任务。当前最先进的大语言模型在检查点任务上表现有限，在完整研究挑战上的准确率仅为4.0%，配备编码工具后提升至约10%。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在复杂、开放式的前沿物理研究挑战中的推理能力，了解物理学家希望LLM协助的推理任务类型，弥合当前模型能力与真实物理研究需求之间的差距。

Method: 创建CritPt基准测试，包含71个复合研究挑战和190个分解的检查点任务，所有问题由50多名活跃物理研究人员基于自身研究设计，采用自动化评分流程评估物理特定输出格式。

Result: 当前最先进的基础模型在完整研究挑战上的平均准确率仅为4.0%（GPT-5最高），配备编码工具后提升至约10%，表明模型在孤立检查点任务上表现有限，但远不能可靠解决完整研究规模挑战。
...
### [432] [Fairness Testing in Retrieval-Augmented Generation: How Small Perturbations Reveal Bias in Small Language Models](https://arxiv.org/abs/2509.26584)
*Matheus Vinicius da Silva de Oliveira,Jonathan de Andrade Silva,Awdren de Lima Fontao*

Main category: cs.AI

TL;DR: 该研究通过蜕变测试评估了三个小型语言模型在RAG管道中的公平性，发现细微的人口统计特征变化会破坏三分之一的蜕变关系，其中种族相关线索是主要的偏见来源。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在安全性和公平性问题，包括已知的攻击向量和公平性缺陷。RAG虽然能减少幻觉，但检索内容可能引入或放大偏见，需要评估其在公平性方面的影响。

Method: 使用蜕变测试方法，在提示中引入受控的人口统计特征扰动，评估三个HuggingFace上的小型语言模型在情感分析任务中的公平性表现。

Result: 结果显示，细微的人口统计变化会破坏高达三分之一的蜕变关系，存在一致的偏见层级，种族相关扰动是违反公平性的主要原因。

Conclusion: RAG中的检索组件需要仔细筛选以防止偏见放大，研究为开发者和组织采用小型语言模型提供了公平性警示。
...
### [433] [Fine-tuning Behavioral Cloning Policies with Preference-Based Reinforcement Learning](https://arxiv.org/abs/2509.26605)
*Maël Macuglia,Paul Friedrich,Giorgia Ramponi*

Main category: cs.AI

TL;DR: 提出了BRIDGE框架，通过结合离线专家演示和在线人类偏好反馈来解决强化学习在机器人等领域的部署障碍，实现了比单独使用行为克隆或在线偏好学习更低的遗憾值。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习在机器人、工业和医疗领域部署的两个主要障碍：难以指定准确奖励函数以及不安全、数据密集的探索风险。

Method: 采用两阶段框架：首先从无奖励的专家演示数据集中学习安全初始策略，然后使用基于偏好的人类反馈进行在线微调。BRIDGE算法通过不确定性加权目标整合两种信号。

Result: 在离散和连续控制的MuJoCo环境中验证，BRIDGE相比单独的行为克隆和在线偏好强化学习实现了更低的遗憾值。遗憾界限随着离线演示数量增加而缩小。
...
### [434] [TimeRewarder: Learning Dense Reward from Passive Videos via Frame-wise Temporal Distance](https://arxiv.org/abs/2509.26627)
*Yuyang Liu,Chuan Wen,Yihang Hu,Dinesh Jayaraman,Yang Gao*

Main category: cs.AI

TL;DR: TimeRewarder是一种从被动视频中学习密集奖励的方法，通过建模帧对之间的时间距离来估计任务进度，显著提高了稀疏奖励任务的强化学习性能。


<details>
  <summary>Details</summary>
Motivation: 设计密集奖励对于强化学习至关重要，但在机器人领域通常需要大量手动工作且缺乏可扩展性。将任务进度视为密集奖励信号是一个有前景的解决方案。

Method: 从机器人演示和人类视频中提取进度估计信号，通过建模帧对之间的时间距离来学习密集奖励，为强化学习提供逐步代理奖励。

Result: 在10个具有挑战性的Meta-World任务中，TimeRewarder显著改善了稀疏奖励任务的强化学习，在9/10的任务中实现了近乎完美的成功率，每个任务仅需20万次环境交互。该方法在最终成功率和样本效率上都优于先前方法甚至手动设计的密集奖励。
...
### [435] [Branching Out: Broadening AI Measurement and Evaluation with Measurement Trees](https://arxiv.org/abs/2509.26632)
*Craig Greenberg,Patrick Hall,Theodore Jensen,Kristen Greene,Razvan Amironesei*

Main category: cs.AI

TL;DR: 提出了一种名为"测量树"的新型度量方法，通过层次化有向图结构来整合多种测量结构，提高AI系统评估的透明度和可解释性。
...
Method: 设计测量树作为层次化有向图，每个节点通过用户定义的聚合方法总结其子节点，支持包括代理性、业务、能效、社会技术和安全信号等多种证据的整合。

Result: 通过大规模测量实践展示了方法的实用性，并提供了开源Python代码实现。

Conclusion: 测量树为复杂结构的测量提供了一个透明的方法，为更广泛和可解释的AI评估奠定了原则性基础。
