...
Motivation: 视觉语言模型的幻觉问题（视觉内容与生成文本之间的不匹配）会削弱模型的可靠性。现有的检测方法通常使用相同或不同的VLM来评估生成输出，这计算量大且会增加模型延迟。

Method: 利用VLM的下一个标记概率（NTPs）作为模型不确定性的直接量化指标，训练传统ML模型进行幻觉检测。还引入了语言NTPs和VLM幻觉预测分数来增强检测性能。

Result: 实验结果表明，基于NTP的特征是有效的幻觉预测指标，轻量级ML模型能达到与强VLM相当的性能。结合语言NTPs和VLM预测分数能进一步提升检测性能。

Conclusion: 这项研究为开发简单、轻量级的解决方案铺平了道路，有助于提高VLMs的可靠性，NTPs为基础的方法在保持高效的同时实现了良好的幻觉检测效果。
...
<details>
  <summary>Details</summary>
Motivation: 离线手写签名验证在跨作者独立设置下具有挑战性，传统方法依赖真实数据集训练分类器，限制了模型的泛化能力

Method: 利用SPD矩阵的黎曼几何特性，通过黎曼高斯混合模型识别合成作者中心，生成正负样本对，构建度量学习框架

Result: 在包含西方和亚洲书写风格的两个流行签名数据集上实验，在数据集内和跨数据集评估协议下均获得低错误率

Conclusion: 黎曼空间中生成合成数据的方法在作者独立签名验证系统中具有潜力，能够有效提升模型泛化性能
...
<details>
  <summary>Details</summary>
Motivation: 传统文本到图像系统功能单一，缺乏交互性和多维创作能力。Seedream 4.0旨在将传统T2I系统扩展为更互动和多维的创作工具，推动生成式AI在创意和专业应用中的边界。

Method: 开发高效扩散变换器与强大VAE，大幅减少图像token数量；通过数十亿文本-图像对进行预训练；采用多模态后训练联合训练T2I和图像编辑任务；集成对抗蒸馏、分布匹配、量化和推测解码等技术加速推理。

Result: 推理时间最快达1.8秒生成2K图像；在T2I和多模态图像编辑任务上达到最先进水平；在复杂任务中展示出色的多模态能力，包括精确图像编辑和上下文推理；支持多图像参考和生成多个输出图像。

Conclusion: Seedream 4.0成功将传统T2I系统扩展为互动性更强、功能更丰富的创作工具，为创意和专业应用开辟了新可能性，代表了生成式AI的重要进展。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球第二大癌症死因，早期检测对治疗结果至关重要。深度学习虽然有效但受限于标注数据不足，需要开发能在小标注数据集上表现优异的方法

Method: 使用Resnet-50在对比学习框架下进行半监督训练，利用大量未标注乳腺X光片数据，通过相似性指数和各种数据增强技术提升性能，最后在小规模标注数据上进行微调

Result: 在INbreast和MIAS基准数据集上实现了96.7%的乳腺癌检测准确率，超越了现有最佳方法

Conclusion: 对比学习框架能够有效解决深度学习在医学影像分析中标注数据不足的问题，为乳腺癌早期检测提供了高效可靠的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在文本和图像处理任务上表现出色，能够零样本泛化到不同领域，这使其有望用于系列制造中的自动化质量检测，以替代繁琐的标注任务。

Method: 在自定义的真实工业图像数据和公共图像数据上测试了多个最新的基础模型。

Result: 所有测试的基础模型在真实工业数据上都失败了，而相同的模型在公共基准数据集上表现良好。

Conclusion: 基础模型在工业应用中的实际效果有限，需要进一步研究如何提高其在真实工业场景中的性能。
...
Conclusion: 该架构减少了冗余，提高了跨域偏移的泛化能力，为高效的多任务视觉管道奠定了基础，相比大型transformer骨干网络更轻量且硬件适应性更广。
...
Conclusion: 高置信度流式策略与多样性方法的结合是有效的图像选择策略，能够在保持低传输成本的同时提升模型质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于掩码的虚拟试穿方法存在局限性：生成理想结果的掩码困难、需要背景知识、可能模型依赖，且在某些情况下无法实现特定样式（如卷起袖子）。

Method: 利用视觉语言模型（VLMs）和图像分割模型自动生成二进制掩码，基于用户提供的图像和自由文本样式指令，无需精确绘制掩码，并自动执行多轮图像生成。

Result: InstructVTON与现有虚拟试穿模型互操作，实现了最先进的结果和样式控制。
...
<details>
  <summary>Details</summary>
Motivation: 解决指纹识别系统在面对故意修改的指纹时容易被规避检测的问题，在边境控制、法医和财政准入等应用中需要鲁棒的篡改指纹识别能力。

Method: 使用VGG16骨干网络提取高维特征，通过余弦相似度比较特征嵌入，在SOCOFing Real-Altered子集上评估，包含三个难度级别（简单、中等、困难）。

Result: 在严格阈值下，DeepAFRNet在三个难度级别上的准确率分别为96.7%、98.76%和99.54%。阈值敏感性研究表明，阈值从0.92放宽到0.72会导致准确率急剧下降至7.86%、27.05%和29.51%。

Conclusion: DeepAFRNet通过使用真实篡改样本和报告逐级指标，解决了基于合成篡改或有限验证协议的先前工作的局限性，表明其已准备好用于需要安全性和识别弹性的现实世界部署。
...
Method: 从自监督ViT模型DINOv2提取注意力图，将其解释为RGB图像的像素级显著度分数，然后将这些图提升到3D体素网格中，形成体素级语义线索，并整合到最先进的基于体素的行为克隆策略中。

Result: 在RLBench双手操作基准测试中，所有任务的平均绝对性能提升达到8.2%，相对增益为21.9%。

Conclusion: 注意力引导的特征化方法能显著提升双手机器人操作的性能，证明了预训练视觉模型的注意力机制在机器人操作任务中的有效性。
...
Method: 使用新构建的661张蓝莓冠层图像数据集评估36个YOLO和RT-DETR模型变体，并采用无偏均值教师半监督学习方法在1,035张未标记图像上进行微调。

Result: YOLOv12m获得93.3% mAP@50，RT-DETRv2-X获得93.6% mAP@50；半监督学习后RT-DETR-v2-X达到94.8%的最佳性能，中型模型在精度和速度间取得良好平衡。

Conclusion: 研究提供了蓝莓检测的全面基准分析，证明了实时检测模型的可行性，并指出需要进一步研究半监督学习以更好地利用跨域未标记数据。
...
Result: 在Mini-DDSM数据集上，ROI增强（最佳参数：p_roi=0.10, alpha=0.10）带来了适度的平均ROC-AUC提升，但性能在不同折叠间有差异；PR-AUC基本持平或略有下降。
...
<details>
  <summary>Details</summary>
Motivation: 镜面反射在日常环境中常见，能提供单次拍摄中的立体信息，通过利用反射作为虚拟视图来简化3D重建过程

Method: 设计物理有效的虚拟相机变换，直接生成像素域虚拟视图；提出对称感知损失优化姿态估计；支持动态场景的逐帧几何恢复

Result: 在真实世界和合成数据上的广泛实验证明了方法的有效性，提供了包含16个场景的合成数据集用于定量评估

Conclusion: 该方法能够从单张包含镜面反射的图像实现鲁棒的3D重建，简化成像流程并保持与现有重建模型的兼容性
...
<details>
  <summary>Details</summary>
Motivation: 灾后建筑占用评估对分诊、检查、公用事业恢复和资源分配至关重要。高空图像覆盖迅速但缺乏立面细节，街景图像细节丰富但稀疏且难以与地块对齐。

Method: 提出FacadeTrack框架，包括全景视频与地块链接、立面视图校正、可解释属性提取，并采用两种决策策略：透明的一阶段规则和将感知与保守推理分离的两阶段设计。

Result: 在两次飓风Helene灾后调查中，两阶段方法达到精度0.927、召回率0.781、F-1分数0.848；一阶段基线为精度0.943、召回率0.728、F-1分数0.822。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是了解人类在识别社交互动时使用的语义表征如何补充视觉特征，特别是通过简单的移动形状动画来研究社会感知。

Method: 研究1：让参与者直接标记移动形状动画的印象；研究2：通过人类相似性判断测量27种社交互动的表征几何，并与基于视觉特征、标签和动画描述语义嵌入的模型预测进行比较。

Result: 人类反应呈分布状态；语义模型为解释人类判断提供了对视觉特征的补充信息；基于动词的嵌入最能解释人类相似性判断。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法面临两个关键挑战：(1) 无人机不同方向和视野导致的严重外观变化影响跨域泛化能力；(2) 需要建立同时捕获全局场景语义和细粒度局部细节的可靠对应关系

Method: EGS框架包含：1) E(2)-Steerable CNN编码器提取旋转和视角变化下的稳定特征；2) 构建带有虚拟超级节点的图结构，连接所有局部节点，实现全局语义聚合和局部区域重分配，强制全局-局部一致性

Result: 在University-1652和SUES-200基准测试上的广泛实验表明，EGS持续获得显著性能提升，在跨域CVGL中建立了新的最先进水平

Conclusion: EGS框架通过结合E(2)-Steerable特征提取和全局-局部一致性图结构，有效提升了跨视角地理定位的跨域泛化能力
...
Result: 该方法能够精确定位不同尺寸的目标边缘，有效抑制噪声，为红外小目标检测和定位提供了有前景的解决方案。
...
Method: 提出SHOT数据集包含1,979个篮球视频片段，具有多个体信息、多视角适应性和多层次意图特征；开发GIFT框架提取细粒度个体特征并建模动态群体互动。

Result: 实验结果验证了SHOT数据集和GIFT框架的有效性，为群体意图预测研究奠定了基础。

Conclusion: 该研究为群体意图预测领域建立了坚实基础，提出的数据集和框架为未来研究提供了重要资源。
...
Method: Neptune-X框架包含两个核心组件：1）X-to-Maritime多模态条件生成模型，通过双向目标-水域注意力模块捕捉目标与水域环境的边界交互；2）属性相关主动采样，基于任务相关性动态选择合成样本。

Result: 实验表明该方法在海事场景合成方面设立了新基准，显著提高了检测准确性，特别是在具有挑战性和先前代表性不足的环境中。

Conclusion: Neptune-X通过数据中心的生成-选择框架有效解决了海事目标检测中的数据稀缺和泛化问题，为海事视觉任务提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有陨石坑导航技术主要针对短期的动力下降和着陆任务，而长期月球测绘任务面临稀疏、倾斜图像采集、变化光照条件等更大挑战，需要开发专门适用于长期测绘的导航系统。

Method: STELLA系统包含：1）基于Mask R-CNN的陨石坑检测器；2）无描述符陨石坑识别模块；3）鲁棒的透视-n-陨石坑姿态求解器；4）批量轨道确定后端。使用CRESENT-365数据集进行测试，该数据集模拟了长达一年的月球测绘任务。

Result: 在CRESENT+和CRESENT-365数据集上的实验表明，STELLA在各种视角、光照条件和月球纬度范围内平均保持米级位置精度和亚度级姿态精度。

Conclusion: 这是首次在真实月球测绘环境下对陨石坑导航技术进行全面评估，为未来任务的运行条件提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 虽然已有研究表明深度视觉和语言模型在不相交模态上训练后仍能部分对齐表征空间，但对齐的具体位置、支持对齐的线索、是否反映人类偏好以及样例聚合的影响仍不清楚

Method: 通过分析模型不同层的表征对齐、测试对视觉外观变化和语义变化的鲁棒性、设计多对多图像-文本匹配的强制选择任务来评估人类偏好与模型对齐的一致性

Result: 对齐在两种模型的中后期层达到峰值；对齐对外观变化鲁棒但对语义变化敏感；人类偏好与模型嵌入空间匹配；样例平均增强而非模糊对齐

Conclusion: 单模态网络收敛于共享语义编码，该编码与人类判断一致且通过样例聚合得到加强
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在个性化对象插入任务中存在三个主要问题：缺乏对插入对象的几何控制、风格一致性不足以及需要大量训练。这些限制影响了编辑结果的真实性和精确性。

Method: 利用现有3D生成模型，将2D对象转换为3D，在3D层面进行交互式编辑后重新渲染到指定视角的2D图像。结合扩散适配器实现几何、风格和内容的综合控制。

Result: 该方法能够生成具有精确几何控制（如形状和视角）且风格一致性的编辑图像，无需额外训练即可实现高质量的对象插入。

Conclusion: FreeInsert通过引入3D几何信息有效解决了图像编辑中的几何控制和风格一致性问题，为个性化对象插入提供了训练自由的解决方案。
...
Method: 提出CustomEnhancer框架，利用人脸交换技术和预训练扩散模型获取额外表示；采用三重流融合生成方法，结合两个兼容的反向潜在空间来操纵个性化模型的关键空间；引入ResInversion方法通过预扩散机制进行噪声校正。

Result: 实验表明CustomEnhancer在场景多样性、身份保真度和训练自由控制方面达到SOTA结果，ResInversion比NTI效率显著提升。

Conclusion: CustomEnhancer通过创新的增强管道和高效反演方法，为个性化模型提供了精确控制和高效生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着基于神经网络的计算机视觉应用对图像视频数据处理需求的增长，需要专门针对视觉任务优化的压缩技术，但缺乏统一的评估平台来比较不同压缩方法在保持任务准确性方面的表现。

Method: 开发了CompressAI-Vision开源评估平台，集成了标准编解码器，通过比特率与任务准确性的权衡来评估压缩效果，支持远程和分割推理两种场景。

Result: 该平台已在多个数据集上展示了压缩增益评估，并被MPEG采纳用于Feature Coding for Machines标准的开发。

Conclusion: CompressAI-Vision为面向机器视觉的压缩技术提供了标准化的评估框架，促进了该领域的研究和标准化进程。
...
<details>
  <summary>Details</summary>
Motivation: 传统半监督域泛化方法假设每个源域都有标注和未标注数据，但实际应用中常存在标注数据有限且域偏移同时存在的情况。本文探索更实用且具有挑战性的CD-SSDG场景，其中训练集内标注和未标注数据之间存在域偏移。

Method: 提出DAC框架，基于两个子模型的协同训练范式，集成了额外的特征级监督和非对称辅助任务。特征级监督解决域偏移导致的伪标签不准确问题，两个不同的自监督辅助任务增强域不变特征学习并防止模型坍塌。

Result: 在真实医学图像分割数据集（眼底、息肉、SCGM）上的广泛实验表明，所提DAC框架具有强大的泛化能力。

Conclusion: DAC框架有效解决了CD-SSDG场景中的挑战，在医学图像分割任务中展现出优越的泛化性能。
...
Method: 1) 对于X/L/M/S型号：采用DINOv3预训练或蒸馏骨干网络，引入STA将单尺度输出转换为多尺度特征；2) 对于超轻量型号：使用HGNetv2并进行深度和宽度剪枝，配合简化解码器和升级的Dense O2O。

Result: DEIMv2-X仅用5030万参数达到57.8 AP，超越需要6000万+参数才能达到56.5 AP的先前模型；DEIMv2-S成为首个参数量低于1000万但AP超过50的模型(50.9 AP)；DEIMv2-Pico仅用150万参数实现38.5 AP，性能与参数量多50%的YOLOv10-Nano相当。

Conclusion: DEIMv2通过统一设计在不同模型尺寸上实现了卓越的性能-成本平衡，为实时目标检测任务建立了新的技术标杆，特别在轻量化模型方面表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）在关键应用中易受对抗攻击影响，而现有的参数高效微调方法（如LoRA）在提升模型鲁棒性方面存在不足。CLIP作为众多下游VLM的骨干网络，其脆弱性可能在整个多模态AI生态系统中传播。

Method: 提出DAC-LoRA框架，集成对抗训练到PEFT中，采用动态对抗课程学习策略，基于一阶平稳条件（FOSC）和TRADES启发式损失函数，逐步增加攻击难度。

Result: DAC-LoRA在显著提升对抗鲁棒性的同时，不会明显损害清洁准确率，是一种轻量级且广泛适用的方法。

Conclusion: DAC-LoRA框架可以轻松集成到标准PEFT流程中，显著增强模型鲁棒性，为安全关键应用提供有效保护。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于提示学习的联邦领域泛化方法学习的提示多样性有限，且容易忽略未知领域的信息，需要一种更有效的方法来处理领域偏移问题。

Method: FedDSPG方法在训练阶段为每个领域引入领域特定软提示，将内容和领域知识整合到客户端间的生成模型中；在推理阶段利用生成器获取未知目标领域的软提示来指导下游任务。

Result: 在多个公共数据集上的综合评估表明，该方法在联邦领域泛化任务中优于现有的强基线方法。

Conclusion: 从生成视角提出的FedDSPG方法有效解决了联邦学习中领域偏移问题，实现了state-of-the-art的性能。
...
Method: 基于对比语言-图像预训练，整合视觉编码器（ResNet-50、Vision Transformer、Swin Transformer）和BERT文本编码器，通过可学习的投影头将特征投影到共享嵌入空间，使用软CLIP损失进行稳定对比训练。

Result: 在测试集上达到95.00%的准确率和94.75%的F1分数，尽管存在类别不平衡问题。消融研究表明线性投影头比非线性变体产生更有效的跨模态对齐。
...
Method: 使用基于影子VideoLLM和GPT-4o-mini构建的转述有害描述集，通过闭环优化策略迭代优化通用扰动来抑制有害帧的相关性分数。

Result: 在三种提示引导采样策略和三个先进VideoLLMs上的全面评估显示，PoisonVID的攻击成功率达到82%-99%。
...
<details>
  <summary>Details</summary>
Motivation: 现有的QAT-KD方法在低比特量化下难以平衡任务特定损失和蒸馏损失，由于梯度幅度异质性导致性能受限。

Method: 提出Game of Regularizer (GoR)方法，使用两个可训练参数进行动态损失加权；同时引入QAT-EKD-GoR集成蒸馏框架，利用多个异构教师模型。

Result: 在图像分类、目标检测和LLM压缩任务上，GoR一致优于最先进的QAT-KD方法，在低功耗边缘设备上实现更快的推理速度同时保持全精度准确率。

Conclusion: GoR为实际部署提供了稳健解决方案，在最优条件下，EKD-GoR甚至可以超越全精度模型性能。
...
Method: 使用两个训练数据集：大规模网络收集的噪声数据集和小型专家验证数据集，测试数据来自Pl@ntNet移动应用，通过挑战赛形式比较不同训练策略的效果。

Result: 挑战赛结果显示，尽管网络数据存在噪声，但在大规模植物识别任务中仍能取得有竞争力的表现，为利用网络资源提供了实证依据。

Conclusion: 大规模网络数据虽然存在噪声问题，但在植物识别领域具有重要价值，结合专家验证数据可以构建更强大的自动化识别系统。
...
<details>
  <summary>Details</summary>
Motivation: 传统植物计数方法需要为每种植物构建特定模型，但植物种类繁多且新品种不断出现，这种方法不可持续。受计算机视觉中类无关计数（CAC）的启发，需要重新思考植物计数的问题定义。

Method: TasselNetV4继承TasselNet的局部计数思想，结合CAC的提取-匹配范式，基于普通视觉变换器构建，并引入新颖的多分支框感知局部计数器来增强跨尺度鲁棒性。

Result: 在两个具有挑战性的数据集PAC-105和PAC-Somalia上的广泛实验表明，TasselNetV4不仅实现了优异的计数性能，而且具有高效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有半监督学习方法在视网膜OCT图像分割中常产生解剖学上不合理的分割结果，无法有效建模层-病变相互作用，且缺乏拓扑正确性保证。
...
<details>
  <summary>Details</summary>
Motivation: 该挑战赛旨在在接近真实世界生物多样性监测场景的条件下，评估植物识别方法和系统，解决大规模开放集识别问题。

Method: 使用包含11万张图像、1000种植物的数据集，将识别任务作为开放集识别问题评估，要求系统在已知类别分类的同时自动拒绝未知类别。

Result: 挑战赛成功评估了各种参与研究组的方法和系统，分析了大规模植物识别在开放集场景下的主要成果。

Conclusion: LifeCLEF 2016为大规模植物识别提供了重要基准，特别强调了开放集识别在实际应用中的重要性，为未来研究提供了方向。
...
Conclusion: SCRA-VQA通过描述总结和重排有效提升了LLMs在知识型VQA任务中的表现，无需昂贵的端到端训练。
...
<details>
  <summary>Details</summary>
Motivation: 探索感知优化目标（保真度目标和对抗目标）作为优化目标的有效性与它们作为图像质量评估指标能力之间的相关性，这一领域尚未被充分研究。

Method: 通过系统分析，研究不同保真度指标在感知优化中的表现，以及判别器设计对优化效果的影响，包括补丁级和卷积架构与Transformer架构的对比。

Result: 发现保真度指标在IQA和优化任务中存在明显不对齐；判别器设计对优化效果起决定性作用，补丁级和卷积架构比传统或基于Transformer的替代方案能提供更忠实的细节重建。
...
Method: 提出IOG-VQA模型，结合对象交互自注意力机制（捕捉图像中对象间的复杂交互）和GAN去偏框架（生成无偏见数据分布）。

Result: 在VQA-CP v1和VQA-CP v2数据集上的实验表明，该模型在处理有偏见和不平衡数据分布方面优于现有方法。

Conclusion: 同时解决对象交互和数据集偏见对于推进VQA任务至关重要，IOG-VQA为这一方向提供了有效解决方案。
...
Method: 集成低秩时间建模与扩散后验采样的混合框架，通过核范数约束和扩散模型先验来提升视频恢复质量。
...
<details>
  <summary>Details</summary>
Motivation: 随着VAE、GAN、LDM等先进模型生成的合成图像越来越逼真，合成图像检测面临重大挑战。

Method: 基于马尔可夫随机场的局部像素依赖特性，重建合成图像以暴露纹理连续性和边缘一致性的破坏，并设计仅110万参数的轻量级神经网络FerretNet。

Result: 仅在4类ProGAN数据集上训练的FerretNet，在包含22个生成模型的开放世界基准测试中平均准确率达到97.1%，比现有最优方法高出10.6%。

Conclusion: FerretNet通过有效利用生成过程中的伪影特征，实现了高效且鲁棒的合成图像检测，显著优于现有方法。
...
Method: 采用transformer架构设计，处理任意长度的视频序列，通过全局概念重要性、局部概念相关性和时间依赖性三个互补视角来分析视频概念。

Result: 实验结果表明概念建模范式可以有效地迁移到视频数据，在保持竞争性性能的同时，更好地理解时间上下文中的概念贡献。

Conclusion: MoTIF成功地将概念瓶颈框架扩展到视频领域，为视频分类提供了更可解释的分析方法，代码已在GitHub开源。
...
Result: 在两个公开数据集上的实验结果表明，该方法在低数据量的挑战性场景下表现出有效的目标检测性能，优于多个基于最先进模型建立的基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 在远距离物体定位或计算资源受限的任务中，传统的密集深度估计和3D场景重建方法不可行，需要一种更高效的解决方案

Method: 使用粒子滤波器处理单目标和多目标场景，结合相机姿态估计和图像分割结果进行3D定位

Result: 通过3D仿真和无人机图像分割序列验证，粒子滤波器能够在其他方法失效的情况下成功解决实际定位任务

Conclusion: 粒子滤波器独立于检测方法，具有灵活性，可与现有图像分割模型结合用于无人机野火监测等实际应用
...
TL;DR: 提出了一种基于打包的多实例学习框架，通过将可变长度的特征序列打包成固定长度序列来解决计算病理学中全切片图像序列长度极端变化的问题，同时引入残差分支和注意力驱动下采样器来提高训练效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 计算病理学中的全切片图像具有极长的序列长度（高达20万）、显著的长度变化和有限的监督，导致数据高度异质性和冗余。传统方法在有限监督下为了保持这种异质性而牺牲训练效率和优化。

Method: 1）打包MIL框架：将多个采样的可变长度特征序列打包成固定长度序列，实现批处理训练；2）残差分支：将多个切片中丢弃的特征组合成超切片进行训练；3）注意力驱动下采样器：压缩特征以减少冗余。

Result: 在PANDA(UNI)数据集上实现了高达8%的准确率提升，同时仅使用12%的训练时间。

Conclusion: 专注于解决计算病理学中的数据挑战在基础模型时代具有重要潜力，该方法有效解决了序列长度极端变化、数据异质性和监督有限等问题。
...
TL;DR: SimDiff是一个模拟器约束的扩散模型，通过将环境参数直接整合到去噪过程中，无需在推理时重复调用模拟器即可高效生成物理合理的人体运动。
...
Method: 将模拟器运动投影解释为扩散过程中的一种引导形式，提出SimDiff模型，通过将环境参数（如重力、风力）直接整合到去噪过程中，实现物理约束。

Result: SimDiff能够高效生成物理合理的运动，无需推理时重复调用模拟器，并提供对不同物理系数的细粒度控制。
...
TL;DR: 本文研究了视觉架构对高斯噪声鲁棒性的设计原则，通过评估1174个预训练模型发现四个关键设计模式，并提供了理论分析解释这些发现。


<details>
  <summary>Details</summary>
Motivation: 研究视觉模型对高斯噪声鲁棒性的内在机制，将经验观察转化为可操作的设计规则，为构建更鲁棒的视觉模型提供指导。

Method: 对1174个预训练视觉模型进行广泛评估，识别出四个提高鲁棒性的设计模式，并通过理论分析（包括频域分析、池化操作比较和Lipschitz边界分析）解释这些发现。

Result: 发现四个关键设计模式：更大的stem核、更小的输入分辨率、平均池化、监督ViT而非CLIP ViT，这些改进可带来高达506位的排名提升和21.6%的准确率增益。理论分析证实了这些模式的因果机制。

Conclusion: 研究将鲁棒性分解为可解释的模块，提供了理论解释，并建立了实用的即插即用指南，用于设计对高斯噪声更鲁棒的视觉模型。
...
TL;DR: 本文通过PRISMA-ScR指导的范围综述，系统梳理了场景图在手术领域的研究进展，发现存在数据鸿沟问题，并展示了场景图在手术分析和生成任务中的重要应用价值。


<details>
  <summary>Details</summary>
Motivation: 场景图提供结构化关系表示，对解码复杂动态手术环境至关重要。本文旨在系统梳理手术场景图研究的应用、方法进展和未来方向。

Method: 采用PRISMA-ScR指导的范围综述方法，系统分析手术场景图研究的演变格局。

Result: 分析显示该领域快速增长，但发现关键的数据鸿沟：内部视图研究主要使用真实2D视频，而外部视图4D建模依赖模拟数据。方法上从基础图神经网络发展到专门的基础模型，在手术场景中显著优于通用大视觉语言模型。

Conclusion: 手术场景图正在成熟为重要的语义桥梁，能够支持新一代智能系统提升手术安全性、效率和培训质量，尽管数据标注和实时实现挑战仍然存在，但正在通过新兴技术解决。
...
TL;DR: 提出了一种基于视觉的激光功率计传感器涂层缺陷自动检测与分类系统，采用无监督异常检测框架，仅使用正常样本训练，能够检测已知和新型缺陷
...
Result: 在366个真实传感器图像上的实验评估显示，缺陷样本准确率93.8%，正常样本准确率89.3%，图像级AUROC为0.957，像素级AUROC为0.961
...
TL;DR: FASTER是一个用于金融咨询视频多模态摘要的模块化框架，通过提取模态特征、生成优化摘要和视觉关键帧对齐，解决长视频内容理解挑战。


<details>
  <summary>Details</summary>
Motivation: 社交媒体传播扩大了金融咨询内容的覆盖范围，但30-40分钟的多模态视频内容难以有效提取关键信息。

Method: 使用BLIP进行视觉语义描述，OCR提取文本模式，Whisper转录和说话人识别作为BOS特征，采用改进的DPO损失函数和基于排序的检索机制。

Result: 跨领域实验证实FASTER在性能、鲁棒性和泛化性方面优于大型语言模型和视觉语言模型。

Conclusion: FASTER为多模态摘要设立了新标准，使金融咨询内容更易获取和实用，并开源了Fin-APT数据集促进相关研究。
...
TL;DR: ASD是一个适配器，能够在无需任何预训练、聚类学习或已训练聚类模型的前提下，实现自监督学习（SSL）的冷启动，用于深度图像聚类任务。


<details>
  <summary>Details</summary>
Motivation: 现有的将SSL技术整合到深度聚类框架中的方法都需要预训练、聚类学习或已训练模型作为前提条件，这限制了SSL学习器在图像聚类任务中的灵活性和即插即用应用。

Method: 1. 从所有未标记数据中随机采样伪标记数据，并设置实例级分类器学习语义对齐的实例级标签；2. 利用实例级分类能力跟踪未标记数据预测的类别转换，提取实例级类别的高层相似性，为伪标记数据分配聚类级标签；3. 使用带有聚类级标签的伪标记数据触发在未标记数据上训练的通用SSL学习器进行图像聚类。

Result: ASD在各种基准测试中表现出优于最新深度图像聚类方法的性能，与使用真实标签的SSL方法相比仅有很小的准确率差距（例如在CIFAR-10上仅差1.33%）。此外，ASD还能进一步提升现有SSL嵌入的深度图像聚类方法的性能。

Conclusion: ASD提供了一种无需任何先决条件的SSL冷启动解决方案，显著提高了SSL学习器在图像聚类任务中的灵活性和实用性，同时保持了优异的聚类性能。
...
TL;DR: 提出SiNGER框架解决Vision Transformers在知识蒸馏中产生的高范数伪影问题，通过零空间引导的能量重分配来抑制伪影同时保留信息信号


<details>
  <summary>Details</summary>
Motivation: Vision Transformers作为视觉基础模型会产生高范数伪影，这些伪影在知识蒸馏过程中会主导目标函数，导致学生模型过度拟合伪影而忽略信息信号，限制了大型模型的增益

Method: SiNGER框架采用零空间引导的扰动方法对教师特征进行精细化处理，在抑制伪影的同时保留信息信号。通过基于LoRA的适配器实现高效扰动，只需最小结构修改

Result: 实验表明SiNGER能持续提升学生模型性能，在多个下游任务中达到最先进水平，并产生更清晰、更可解释的表征

Conclusion: SiNGER有效解决了教师特征中伪影与信息信号之间的权衡问题，为知识蒸馏提供了更优的解决方案
...
TL;DR: Fast-SEnSeI是一个轻量级、传感器无关的编码器模块，用于实现跨多光谱传感器（具有不同波段配置）的灵活、星上云分割。


<details>
  <summary>Details</summary>
Motivation: 云分割是许多地球观测任务的关键预处理步骤，但大多数模型与特定传感器配置紧密耦合，并依赖地面处理。

Method: 基于SEnSeI-v2，Fast-SEnSeI集成了改进的光谱描述符、轻量级架构和鲁棒的填充波段处理。它接受任意组合的光谱波段及其波长，生成固定大小的特征图，输入到基于改进U-Net的紧凑量化分割模型中。模块使用Apache TVM在嵌入式CPU上高效运行，分割模型部署在FPGA上，形成适用于空间合格硬件的CPU-FPGA混合流水线。

Result: 在Sentinel-2和Landsat 8数据集上的评估表明，该方法在不同输入配置下都能实现准确的分割。

Conclusion: Fast-SEnSeI为星上云分割提供了一种灵活、高效的解决方案，能够适应不同传感器的波段配置。
...
TL;DR: 提出KD-SS算法用于植物器官点云分割，无需降采样即可处理全分辨率点云，适用于不同植物种类和传感器模态


<details>
  <summary>Details</summary>
Motivation: 现有植物器官点云分割方法通常针对特定植物种类或传感器，需要大量预处理和降采样，限制了应用范围

Method: 开发KD-SS子采样算法，保持点云分辨率，与现有最先进分割模型结合使用

Result: 在摄影测量、激光三角测量和LiDAR等多种模态的不同植物物种上获得满意结果

Conclusion: KD-SS是传统密集预处理和降采样方法的轻量级替代方案，适用于任意植物种类和传感器模态
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在少样本分布外检测中存在鲁棒性低的问题，主要原因是过度依赖局部类别相似性和固定的背景补丁提取策略。
...
Result: 使用新提出的统计异质性指标评估，WDES相比随机采样能持续产生更具代表性的划分。在街景、医学影像和卫星图像等多种分割任务中应用WDES，能降低性能方差并改进模型评估。

Conclusion: WDES在处理小型、不平衡和低多样性数据集时特别有价值，这些情况下传统划分策略最容易产生偏差。该方法能有效提高分割模型评估的可靠性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有细粒度分类模型依赖部件标注或自动注意力图提取，但存在局部特征表示不完整的问题。人类识别物体时还会形成语义关联，因此本文利用语义层次结构作为监督信号。
...
TL;DR: 该论文研究了对抗性补丁在Vision Transformers（ViTs）上的可转移性，发现CNN中使用的对抗攻击技术可以成功转移到ViT模型，不同ViT模型对对抗性补丁的脆弱性存在显著差异，预训练数据集规模和方法强烈影响模型对对抗攻击的抵抗力。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习系统的广泛应用，其安全性成为关键问题。Vision Transformers（ViTs）虽然在性能上优于卷积神经网络（CNNs）且对对抗性扰动具有更强的鲁棒性，但仍然容易受到对抗性补丁攻击。本研究旨在探索CNN中使用的对抗攻击技术在ViT分类模型上的可转移性。

Method: 研究设计了现实的对抗性补丁，使用褶皱变换（CT）技术添加类似于穿着衣物时自然发生的细微几何畸变，在人物与非人物分类任务中引起误分类。实验评估了四个微调的ViT模型在二元人物分类任务上的表现。

Result: 实验结果显示不同ViT模型存在显著的脆弱性差异：攻击成功率从40.04%（google/vit-base-patch16-224-in21k）到99.97%（facebook/dino-vitb16）不等，google/vit-base-patch16-224达到66.40%，facebook/dinov3-vitb16达到65.17%。

Conclusion: 研究结果证实了对抗性补丁从CNNs到ViTs的跨架构可转移性，预训练数据集规模和方法强烈影响模型对对抗攻击的恢复力。
...
TL;DR: UniTransfer是一种新颖的视频概念转移架构，通过空间和时间步分解实现精确可控的视频概念转移。


<details>
  <summary>Details</summary>
Motivation: 现有视频概念转移方法缺乏对视频不同组件的精细控制能力，需要一种能够解耦视频内容并实现渐进式控制的架构。

Method: 采用空间分解（前景主体、背景、运动流）和扩散时间步分解（Chain-of-Prompt机制），结合双流到单流的DiT架构和自监督预训练策略。

Result: 在OpenAnimal数据集上的实验表明，该方法在视觉保真度和可编辑性方面均优于现有基线方法。

Conclusion: UniTransfer通过空间和时间分解实现了高质量可控的视频概念转移，为视频编辑领域提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖LLM推理分析解析后的视觉信息，受限于静态感知阶段，需要动态迭代感知机制来提升多模态大模型的推理能力。

Method: VTTS采用迭代感知机制，结合强化学习和时空监督来优化推理过程，通过逐步细化对高置信度时空区域的关注，并引入VTTS-80K数据集支持该范式。

Result: 实验表明VTTS在多个基准测试上显著提升性能，Videochat-R1.5模型相比Qwen2.5VL-3B和-7B等基线模型平均提升超过5%，涵盖视频对话、视频推理和时空感知等15+个基准。

Conclusion: VTTS通过增加感知计算有效增强MLLMs性能，验证了该方法在不同任务和基准上的有效性和泛化能力。
...
TL;DR: 提出了Mammo-CLIP Dissect，首个基于概念的可解释性框架，用于系统分析乳腺X光片DL视觉模型，通过视觉语言模型为神经元分配可解释的文本概念标签。
...
Method: 利用乳腺X光片专用视觉语言模型(Mammo-CLIP)作为"解剖器"，为指定层神经元分配人类可理解的文本概念标签，并量化其与领域知识的对齐程度。

Result: 乳腺X光片数据训练的模型捕获更多临床相关概念且更符合放射科医生工作流程；任务特定微调增强某些概念类别捕获但减少其他类别覆盖。

Conclusion: Mammo-CLIP Dissect能深入了解CNN如何捕获乳腺X光片特定知识，揭示了领域特定训练和任务特定适应如何塑造概念学习。
...
TL;DR: 本文提出了MOSS-ChatV，一个基于强化学习的框架，通过动态时间规整（DTW）的过程奖励来解决多模态大语言模型在视频推理中的过程不一致性问题。
...
Method: 采用强化学习框架，结合基于动态时间规整（DTW）的过程奖励，使推理轨迹与时间基础参考对齐，无需辅助奖励模型即可实现高效的过程监督。

Result: MOSS-ChatV在MOSS-Video测试集上达到87.2%的准确率，并在MVBench和MMVU等通用视频基准测试中表现提升。该框架在不同架构（如Qwen2.5-VL和Phi-2）上均能带来性能增益。

Conclusion: MOSS-ChatV框架能有效提升视频推理的过程一致性和稳定性，具有广泛的适用性，为多模态大语言模型的视频推理能力提供了重要改进。
...
TL;DR: 提出一种新方法，通过将相机和物体运动转换为像素运动来生成符合相机轨迹的视频，解决了现有方法分离学习两种运动导致的相对运动混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常分别学习相机和物体运动，这可能导致对相机与物体之间相对运动的混淆，难以在同时存在相机和物体运动时实现一致性和泛化性。

Method: 利用稳定扩散网络学习参考运动映射图，结合提取的语义物体先验，通过图像到视频网络生成符合指定相机轨迹的视频。

Result: 大量实验验证表明，该方法在性能上大幅优于现有最先进方法。

Conclusion: 所提出的集成相机和物体运动的方法能够有效生成符合相机轨迹的视频，同时保持物体运动的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 随着图像生成AI的快速发展，合成图像与真实图像的界限日益模糊，需要研究判别器在什么条件下处于劣势。

Method: 使用Kolmogorov复杂度作为数据集内在结构的度量，分析数据维度和复杂度对判别器性能的影响。

Result: 简单数据集几乎可以被生成器完美学习，而极端多样的数据集会掩盖缺陷；中等复杂度数据集最有利于检测，因为生成器无法完全捕捉分布且错误仍然可见。

Conclusion: 数据复杂度对合成图像检测有重要影响，中等复杂度数据集为判别器提供了最有利的条件。
...
TL;DR: WAVECLIP是一个基于小波变换的统一模型，通过多级小波分解替代标准补丁嵌入，支持自适应分辨率推理和计算效率优化。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP模型在处理不同分辨率图像时的计算效率问题，实现单一模型支持多分辨率推理和动态计算精度权衡。

Method: 使用多级小波分解进行token化，采用键值缓存和因果跨层注意力机制，结合基于置信度的门控机制实现自适应提前退出。

Result: 在零样本分类任务中表现出竞争力，仅需轻量级蒸馏训练即可实现显著计算节省。

Conclusion: WAVECLIP为视觉语言模型提供了高效的自适应分辨率推理方案，平衡了计算成本与模型精度。
...
TL;DR: 该论文对CLIP等视觉语言模型的量化效果进行了大规模评估，发现量化不仅能提高效率，还能改善模型可靠性指标，挑战了传统的效率-性能权衡观点。


<details>
  <summary>Details</summary>
Motivation: 虽然CLIP等模型在零样本泛化方面表现出色，但量化对CLIP性能的影响（特别是除了准确率之外的可靠性指标）尚未得到充分研究。

Method: 进行了大规模量化评估，不仅评估分布内准确率，还包括全面的可靠性指标套件，并分析了预训练来源的影响。

Result: 量化能持续改善通常欠自信预训练模型的校准度，但对过度自信模型会降低校准度；有趣的是，即使校准度下降，OOD检测仍可改善；某些QAT方法能同时提升零样本准确率、校准度和OOD鲁棒性。

Conclusion: 量化在高效、可靠、鲁棒的VLM部署中具有超越传统角色的价值，为多目标优化问题提供了重要见解。
...
TL;DR: TABLET是一个大规模视觉表格理解数据集，包含400万个示例，基于200万个独特表格，其中88%保留原始可视化效果，旨在解决现有基准缺乏真实世界表格复杂性和视觉多样性的问题。
...
Method: 引入TABLET数据集，包含配对的图像-HTML表示、全面元数据和溯源信息，将Qwen2.5-VL-7B等视觉语言模型在TABLET上进行微调。
...
TL;DR: 提出一种基于共形预测的新方法，让用户能够直接控制生成解释的保真度，识别出能够保持模型预测的关键特征子集，无需真实解释进行校准。
...
Method: 基于共形预测的方法，提出四种一致性函数来量化解释与模型预测的符合程度，通过识别关键特征子集来保持模型预测。

Result: 在6个图像数据集上使用5种解释器进行实证评估，FastSHAP在保真度和信息效率方面优于竞争方法，基于超像素的一致性度量比像素级更有效。

Conclusion: 提出的共形预测方法能够有效控制解释保真度，FastSHAP表现出最佳性能，超像素方法优于像素级方法。
...
TL;DR: Sigma是一个基于骨架的手语理解统一框架，通过多模态融合和分层对齐学习解决了当前手语理解中的语义基础弱、局部细节与全局上下文不平衡、跨模态学习效率低等问题，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 当前基于骨架的手语理解方法面临三个主要限制：1）语义基础弱，模型难以将骨架运动模式与语言意义关联；2）局部细节与全局上下文不平衡；3）跨模态学习效率低。

Method: 提出Sigma框架，包含：1）手语感知的早期融合机制，促进视觉和文本模态的深度交互；2）分层对齐学习策略，在不同层级上最大化跨模态特征一致性；3）统一的预训练框架，结合对比学习、文本匹配和语言建模。

Result: 在孤立手语识别、连续手语识别和无注释手语翻译等多个基准测试中取得了最先进的结果，证明了语义信息预训练的有效性和骨架数据作为独立解决方案的潜力。

Conclusion: Sigma框架通过语义信息丰富的预训练和有效的骨架数据处理，为手语理解任务提供了一个强大的统一解决方案，展示了骨架数据在手语理解中的独立价值。
...
TL;DR: 本文对文本-图像生成中的自动评估指标进行了广泛研究，发现没有单一指标在所有任务中表现一致，指标性能随组合问题类型而变化，VQA指标并非普遍优越，嵌入指标在特定情况下更强，图像质量指标对组合评估贡献有限。


<details>
  <summary>Details</summary>
Motivation: 文本-图像生成发展迅速，但评估生成结果是否真正捕捉提示中的对象、属性和关系仍是核心挑战。当前评估严重依赖自动指标，但这些指标往往基于惯例或流行度采用，而非基于人类判断验证。

Method: 对广泛使用的组合文本-图像评估指标进行广泛研究，分析其在不同组合挑战中的行为，比较不同指标家族与人类判断的一致性，超越简单的相关性分析。

Result: 结果显示没有单一指标在所有任务中表现一致，指标性能随组合问题类型而变化。VQA指标虽然流行但并非普遍优越，某些嵌入指标在特定情况下更强。图像质量指标对组合评估贡献有限。

Conclusion: 这些发现强调了谨慎和透明选择指标的重要性，既是为了可信的评估，也是为了将其用作生成中的奖励模型。
...
TL;DR: 提出SlideMamba框架，结合Mamba架构和图神经网络，通过熵置信加权机制自适应融合局部空间关系和长距离上下文依赖，在WSI分析中优于现有方法。
...
Conclusion: 集成架构结合自适应融合策略在计算病理学的空间分辨预测建模任务中展现出强大潜力和应用前景。
...
TL;DR: Hunyuan3D-Omni是一个统一的3D资产生成框架，支持多种模态输入（图像、点云、体素、边界框、骨骼姿态），通过跨模态架构实现细粒度控制，解决了现有方法缺乏精细跨模态控制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型主要依赖图像或文本条件，缺乏细粒度的跨模态控制，限制了可控性和实际应用。需要解决多种控制信号的统一处理问题。

Method: 基于Hunyuan3D 2.1构建统一框架，使用单一跨模态架构处理所有信号，采用渐进式难度感知采样策略，偏重困难信号（如骨骼姿态）而降低简单信号（如点云）的权重。

Result: 实验表明额外控制提高了生成准确性，支持几何感知变换，增强了生产工作流的鲁棒性。

Conclusion: Hunyuan3D-Omni通过统一的多模态控制框架，显著提升了3D资产生成的可控性和实用性，为游戏、电影和设计领域的生产流程提供了更强大的工具。
...
TL;DR: 提出了一种利用视觉语言模型自动生成语义注意力图的方法，通过自然语言提示引导CNN注意力，无需人工标注即可提升模型的可靠性和认知合理性。


<details>
  <summary>Details</summary>
Motivation: CNN经常利用表面相关性进行"作弊"预测，现有方法依赖专家标注，可扩展性有限。受认知科学启发，需要更可扩展的方法来引导模型注意力。

Method: 利用视觉语言模型自动生成语义注意力图，通过辅助损失函数将CNN注意力与语言引导的注意力图对齐，无需人工标注。

Result: 在ColoredMNIST和DecoyMNIST数据集上的实验表明，该方法在ColoredMNIST上达到最先进性能，在DecoyMNIST上与标注密集型基线竞争力相当，显示出改进的泛化能力和减少的捷径依赖。

Conclusion: 该方法提供了一种可扩展的解决方案，能够促进更可靠和认知合理的决策，模型注意力更好地反映人类直觉。
...
<details>
  <summary>Details</summary>
Motivation: MRI的复杂性和异质性给自动化分析带来挑战，现有基础模型在MRI应用上受限于数据稀缺和解剖区域狭窄，需要开发专门针对MRI的基础模型。

Method: 结合自监督视觉学习和报告引导的文本监督，采用模块化设计，支持在冻结预训练编码器上调整轻量级任务特定解码器。

Result: 在多个基准测试中表现出优于现有基础模型和任务特定方法的性能，证明了其鲁棒性和泛化能力。
...
TL;DR: SQ-InstructBLIP是一个改进视觉语言理解的方法，通过生成图像感知的子问题和子答案来提升多步推理性能，解决了现有LLM方法无法读取视觉信息和黑盒模型难以复现的问题。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言理解领域在处理需要多步推理的问题时仍存在困难，现有基于LLM的方法存在两个主要问题：1）无法获取图像的细粒度视觉内容；2）使用黑盒LLM导致内部机制不可访问且难以复现。

Method: 提出SQ-InstructBLIP方法，包含共享相同架构的Questioner、Answerer和Reasoner三个模块。Questioner和Answerer迭代生成帮助推理主问题的子问题和子答案，Reasoner则考虑生成的子问题信息对主问题进行推理。

Result: 实验表明，SQ-InstructBLIP在VQA任务中使用生成的子问题作为额外信息时，比之前的工作能够进行更准确的推理。

Conclusion: 该方法通过图像感知的子问题生成机制，有效提升了视觉问答任务的多步推理性能，解决了现有方法的局限性。
...
TL;DR: 本文提出了文本到图像生成模型中幻觉的定义和分类，将其定义为基于模型偏见的偏差，并建立了属性、关系和对象幻觉的三类分类法。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注对齐性，检查提示中指定的元素是否出现，但忽视了模型超出提示生成的内容。需要明确定义文本到图像生成模型中的幻觉现象。

Method: 将文本到图像生成模型中的幻觉定义为基于偏见的偏差，并提出包含属性幻觉、关系幻觉和对象幻觉的三类分类法。

Result: 该框架为评估设定了上限并揭示了隐藏的偏见，为更丰富的文本到图像模型评估提供了基础。

Conclusion: 明确定义文本到图像生成模型中的幻觉现象对于全面评估模型性能至关重要，提出的分类法有助于揭示模型偏见并改进评估方法。
...
TL;DR: 提出了Person Independence Universal Micro-action Recognition Framework，通过分布鲁棒优化学习人员无关表征，解决微动作识别中人员间变异导致的泛化问题


<details>
  <summary>Details</summary>
Motivation: 现有微动作识别方法在真实场景中表现不佳，因为人员间变异导致相同动作表现不同，阻碍了模型的鲁棒泛化能力

Method: 框架包含特征层和损失层两个即插即用组件：特征层的时频对齐模块通过双分支设计（时间分支应用Wasserstein正则化对齐，频率分支引入方差引导扰动）归一化人员特定运动特征；损失层的组不变正则化损失通过伪分组模拟未见人员分布，增强边界样本权重和正则化子组方差

Result: 在大型MA-52数据集上的实验表明，该框架在准确性和鲁棒性方面均优于现有方法，在细粒度条件下实现稳定泛化

Conclusion: 所提出的框架通过分布鲁棒优化和双重归一化机制，有效解决了微动作识别中的人员间变异问题，实现了更好的泛化性能
...
TL;DR: 本文提出了一种基于3D几何基础模型VGGT的语义匹配方法，解决了现有方法在几何歧义和最近邻规则方面的局限性，通过保留VGGT核心优势并针对语义匹配场景进行适配，实现了更好的几何感知和匹配可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有语义匹配方法存在两个主要问题：(1)几何歧义：依赖2D基础模型特征难以区分对称结构，需要额外微调且缺乏泛化能力；(2)最近邻规则：像素级匹配忽略了跨图像不可见性和流形保持。需要几何感知的像素描述符和整体密集对应机制。

Method: 提出保留VGGT早期特征阶段、微调后期阶段并添加语义头的方法，通过循环一致性训练策略、合成数据增强和渐进式训练方案，在数据稀缺条件下将VGGT适配到语义匹配场景。
...
TL;DR: 提出了MedVSR框架，专门针对医学视频超分辨率任务，通过交叉状态空间传播和内部状态空间重建模块，解决医学视频中相机抖动、噪声和帧间突变等独特挑战。


<details>
  <summary>Details</summary>
Motivation: 高分辨率医学视频对准确诊断至关重要，但受硬件限制和生理约束难以获取。现有视频超分辨率模型在处理医学视频时面临相机抖动、噪声、帧间突变等问题，导致光流误差和对齐困难，且容易产生伪影和扭曲特征误导医生。

Method: MedVSR框架包含两个核心模块：1）交叉状态空间传播（CSSP）通过状态空间模型将远距离帧投影为控制矩阵，选择性传播一致且信息丰富的特征以实现有效对齐；2）内部状态空间重建（ISSR）通过联合长程空间特征学习和大核短程信息聚合，增强组织结构并减少伪影。

Result: 在包括内窥镜和白内障手术在内的四个医学场景数据集上的实验表明，MedVSR在重建性能和效率方面显著优于现有视频超分辨率模型。

Conclusion: MedVSR为医学视频超分辨率提供了一种有效的解决方案，能够处理医学视频特有的挑战，在保持组织结构完整性的同时提高视频质量，具有重要的临床应用价值。
...
TL;DR: 该论文提出了Variance-Aware Sampling (VAS)方法来稳定强化学习训练，并发布了大规模多模态推理数据集和模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态推理模型面临的两个主要限制：缺乏高质量长链思维数据，以及强化学习训练的不稳定性问题。

Method: 提出Variance-Aware Sampling (VAS)数据选择策略，结合结果方差和轨迹多样性来促进奖励方差；发布包含160万条长链思维数据和1.5万条RL问答对的大规模数据集。

Result: 在数学推理基准测试中证明了所提出方法和数据的有效性，建立了多尺度多模态推理模型作为社区基准。

Conclusion: VAS通过理论保证和实践机制有效提升了策略优化的稳定性，为多模态推理领域提供了重要的资源和基准。
...
TL;DR: 该论文提出了一种基于Prithvi-EO Vision Transformer架构的海洋科学基础模型，通过在Sentinel-3 OLCI数据上进行预训练，并在两个下游海洋地球观测任务上微调，展示了其在海洋监测中的实用性。


<details>
  <summary>Details</summary>
Motivation: 海洋科学中标记数据稀疏且收集成本高，而AI基础模型在大量未标记数据上预训练后，有望显著改变海洋科学中的AI应用。

Method: 使用Prithvi-EO Vision Transformer架构，在Sentinel-3 OLCI数据上进行预训练以重建数据，然后在两个下游任务（叶绿素浓度量化和海洋初级生产力估计）上进行微调评估。

Result: 模型在利用少量高质量标记数据和捕捉海洋颜色详细空间模式方面表现出色，同时与点观测数据匹配良好，证明了自训练基础模型在海洋监测中的实用性。
...
TL;DR: SHINE是一个无需训练的框架，用于实现无缝、高保真度的图像合成，通过引导潜在空间和自适应背景融合来解决复杂光照和高分辨率输入下的合成问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像合成模型在处理复杂光照（如准确阴影、水面反射）和多样化高分辨率输入时表现不佳，而现有的文本到图像扩散模型虽然编码了物理和分辨率先验，但缺乏有效的框架来释放这些能力。

Method: SHINE引入流形引导锚点损失，利用预训练的自定义适配器（如IP-Adapter）来引导潜在空间，实现忠实的主体表示同时保持背景完整性。还提出了退化抑制引导和自适应背景融合来消除低质量输出和可见接缝。
...
Method: QuantVGGT框架包含两个核心技术：1）双平滑细粒度量化，通过预全局Hadamard旋转和后局部通道平滑来缓解重尾分布和通道间方差；2）噪声过滤多样化采样，通过深层统计过滤异常值，并构建帧感知多样化校准簇来确保稳定的量化范围。

Result: 实验表明QuantVGGT在不同基准测试和位宽下均达到最先进结果，大幅超越之前的通用量化方法。4位QuantVGGT可实现3.7倍内存减少和2.5倍硬件推理加速，同时保持重建精度在完整精度模型的98%以上。
...
TL;DR: NewtonGen是一个结合数据驱动合成与可学习物理原理的框架，通过神经牛顿动力学(NND)建模牛顿运动，为视频生成注入物理约束，实现物理一致性和参数可控的视频合成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成模型存在物理一致性和可控性瓶颈，经常产生不真实的运动（如物体向上坠落）和缺乏精确参数控制，主要原因是模型仅从外观学习运动分布而缺乏对底层动力学的理解。

Method: 提出NewtonGen框架，核心是可训练的神经牛顿动力学(NND)，能够建模和预测各种牛顿运动，将潜在动力学约束注入视频生成过程，联合利用数据先验和动力学指导。

Result: 该方法实现了物理一致的视频合成，并具备精确的参数控制能力。

Conclusion: 通过集成数据驱动合成与可学习物理原理，NewtonGen解决了当前文本到视频生成中的物理一致性和可控性问题。
...
<details>
  <summary>Details</summary>
Motivation: 让高质量图像生成技术能够在计算资源有限的消费级设备（如手机和台式机）上运行，实现生成式AI的普及化部署。

Method: 采用重新构建的分布匹配目标进行少步生成蒸馏，引入"时间步共享"减少梯度噪声和"分时步微调"提升提示对齐，结合文本编码器重构和专用量化等管道优化技术。

Result: 通过大规模用户研究等广泛评估，SD3.5-Flash在少步方法中表现优于现有技术，实现了快速生成和内存高效部署。

Conclusion: 该框架使先进的生成式AI真正能够在实际部署中普及应用，覆盖从手机到台式机的完整设备谱系。
...
TL;DR: 本文分析了社区贡献平台（CCP）上公开医学影像数据集的现状，发现当前CCP治理模式在数据共享、文档记录和评估方面存在质量问题，可能导致医疗AI算法的有害下游影响。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据集对医疗AI至关重要，但当前社区贡献平台的开放数据治理模式未能维护数据质量标准和推荐实践，需要系统分析其局限性。

Method: 对CCP上公开可用的机器学习数据集进行分析，比较数据集在数据共享、文档记录和维护等多个维度的差异。

Result: 发现数据集存在许可证模糊、缺乏持久标识符和存储、重复数据、元数据缺失等问题，不同平台之间存在差异。

Conclusion: 研究强调了负责任数据管理的重要性，为医疗AI算法的负责任数据策展工作做出贡献。
...
TL;DR: 本文提出了多智能体生成流网络的理论框架，包括四种算法，实现了集中式训练和分布式执行，并通过实验验证了其优于强化学习和MCMC方法。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏多智能体生成流网络的理论框架，需要解决多个智能体通过联合动作协作生成对象的问题。

Method: 提出了四种算法：集中式流网络、独立流网络、联合流网络及其条件版本，基于局部-全局原则进行训练。
...
TL;DR: FastEagle是一种非自回归级联草稿器，通过单次前向传播生成整个草稿，替代了传统草稿器的N次顺序传递，实现了无损LLM推理加速


<details>
  <summary>Details</summary>
Motivation: 现有的草稿器（如EAGLE）需要N次顺序传递来提出N个标记，存在效率瓶颈。FastEagle旨在通过消除草稿生成中的顺序依赖关系来加速推理过程

Method: 采用非自回归级联草稿器设计，用轻量级层级联替代时间步骤，通过层级监督训练减轻错误累积，结合约束草稿树保持无损验证成本

Result: 在多个LLM（Vicuna-13B、LLaMA-Instruct 3.x、DeepSeek-R1-Distill-LLaMA）和任务上，FastEagle在贪婪和随机解码下始终优于EAGLE-3的速度提升，同时保持可比较的平均接受长度
...
<details>
  <summary>Details</summary>
Motivation: 大多数CMIP气候模型由于大气化学方案的高计算成本而缺乏臭氧的交互表示，这限制了气候敏感性模拟的准确性。

Method: 开发了基于机器学习的参数化方法mloz，仅使用大气温度剖面信息作为输入，在两款不同气候模型（UKESM和ICON）中进行在线测试。

Result: mloz比UKESM中的化学方案快约31倍，仅占气候模型总运行时间的不到4%，且在十年时间尺度上表现出高保真度，并能跨模型迁移使用。

Conclusion: 该方法有潜力在缺乏交互化学的CMIP级气候模型中广泛应用，特别是在关注气候敏感性模拟时，臭氧趋势和变率对大气反馈过程有显著调节作用。
...
TL;DR: 提出基于Transformer的自编码器方法，用于保护EEG数据的隐私，防止主体重识别，同时保持其在机器学习任务中的实用性。


<details>
  <summary>Details</summary>
Motivation: 随着EEG消费设备的普及，用户隐私保护需求日益迫切。研究表明EEG数据存在重识别和个人信息泄露风险，需要找到既能保护隐私又不影响数据实用性的方法。

Method: 使用基于Transformer的自编码器对EEG数据进行匿名化处理，在自动睡眠分期任务中评估匿名化前后数据的重识别风险和实用性。

Result: 结果显示，该方法能显著降低EEG信号的可重识别性，同时保持其在机器学习任务中的实用性。

Conclusion: 提出的Transformer自编码器方法能有效平衡EEG数据的隐私保护和实用性，为EEG数据的安全使用提供了可行方案。
...
TL;DR: 本文研究了基于记忆化的影响力估计器是否可以被对抗性操纵的问题，提出了针对记忆化分数的攻击方法，并验证了即使是最先进的影响力估计代理也容易受到针对性分数操纵。


<details>
  <summary>Details</summary>
Motivation: 随着影响力估计工具在数据估值和负责任机器学习中的广泛应用，需要研究这些分数本身是否可以被对抗性操纵，以揭示基于影响力归因的关键漏洞。

Method: 提出了一种实用的攻击方法（计算输入的伪逆），仅需要黑盒访问模型输出并产生适度的计算开销。通过图像分类任务的实证验证，展示了攻击的有效性。

Result: 实证验证表明，即使是最先进的影响力估计代理也容易受到针对性分数操纵。理论分析揭示了在对抗性扰动下记忆化分数的稳定性条件。

Conclusion: 研究结果强调了基于影响力归因的关键脆弱性，并表明需要开发鲁棒的防御机制。
...
<details>
  <summary>Details</summary>
Motivation: 目标条件强化学习中的两种有效表示框架（对比表示和时序距离）各有优势，但都存在局限性。本文旨在统一这两种框架，结合各自的优点。

Method: 使用拟度量距离参数化方法，在拟度量表示空间结构的基础上施加适当约束，学习后续表示以实现最优目标到达。

Result: 在现有离线GCRL基准测试中，该方法在对比学习方法难以处理的拼接任务和拟度量网络方法难以处理的高维噪声环境中都表现出更好的性能。

Conclusion: 该方法成功结合了蒙特卡洛对比强化学习的稳定性和长时程能力，以及拟度量网络参数化的自由拼接能力，实现了两种框架的优势互补。
...
TL;DR: 提出了一种新颖的端到端深度学习框架，用于从原始EEG信号中提取多尺度特征，通过注意力机制和门控网络处理噪声和通道变异性，并在多种神经系统疾病分类任务中验证了有效性。
...
Method: 设计了能够显式捕获多尺度频率振荡的编码器；引入基于注意力的编码器学习EEG通道间和通道内局部patch的复杂依赖关系；集成了门控网络动态过滤噪声和非信息通道；使用结合监督和对比学习的新型损失函数指导编码过程。

Result: 在多种应用中验证了该方法，包括中枢神经系统疾病治疗效果的分类以及帕金森病和阿尔茨海默病的诊断。结果表明该学习范式能够从不同物种的原始EEG信号中提取生物学有意义的模式，自主选择高质量通道，并通过创新的架构和损失设计实现鲁棒泛化。

Conclusion: 提出的深度学习框架能够有效解决EEG信号处理中的关键挑战，在多种神经系统疾病相关任务中表现出色，具有重要的临床应用价值。
...
TL;DR: DARTVAE是一个基于规则引导的多模态聚类框架，通过将领域特定约束嵌入到VAE表示学习中，实现更符合领域知识的聚类结果。


<details>
  <summary>Details</summary>
Motivation: 传统聚类方法仅依赖输入数据的相似性，难以捕捉领域中的结构或语义约束，限制了在复杂知识密集型场景中的应用效果。

Method: 扩展VAE架构，嵌入LLM生成的规则、语义表示和数据驱动特征到统一潜在空间，通过包含重构损失、KL散度、一致性惩罚和违规惩罚的损失函数强制约束合规性。

Result: 在飞机和汽车数据集上的实验表明，规则引导的聚类产生更具操作意义和可解释性的聚类结果（如分离无人机、统一隐形飞机、区分SUV和轿车），同时改进了传统聚类指标。

Conclusion: DARTVAE通过结合规则编码和学习表示，比纯数据驱动模型获得更有意义和一致的聚类结果，但面临LLM规则幻觉、规则冲突、过拟合和计算复杂度等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 注意力层的参数依赖于输入元素的成对交互，在没有结构假设的情况下，内存和计算复杂度随序列长度呈二次方增长。现有方法（引入稀疏性或循环依赖）各有缺点，需要结合两者优势

Method: 基于树结构矩阵的高效求逆技术，开发新算法来结合稀疏注意力和循环依赖的优点
...
TL;DR: 本文提出了一种深度学习方法来预测混凝土中时间相关的全场损伤，使用自回归U-Net模型预测微结构几何和收缩剖面演化下的标量损伤场演化，并通过CNN预测关键力学性能。
...
TL;DR: 提出用复杂度奖励替代熵奖励的新方法CDPO，通过平衡随机性和结构化来改进策略梯度方法的探索效率


<details>
  <summary>Details</summary>
Motivation: 传统的熵最大化方法将策略推向均匀随机分布，这种无结构的探索策略有时效率低下。需要一种更鲁棒的复杂度奖励来平衡随机性和结构化

Method: 引入复杂度度量（香农熵与不平衡度的乘积），开发了CDPO算法替代PPO中的熵奖励，抑制极端无序和完全有序，鼓励结构化但可适应的策略

Result: 在离散动作空间任务中，CDPO相比PPO对复杂度系数的选择更鲁棒，在需要更多探索的环境中表现更好

Conclusion: 复杂度奖励比熵奖励更有效，能够引导智能体发现更有用、非平凡的行为策略
...
Method: 开发理论框架分析确定性扩散算法，将噪声卷积分数解释为时变投影，将先前算法视为带变化投影的广义投影梯度下降方法。

Result: 当感知矩阵在模型集上满足受限等距性质时，可以推导出明确依赖噪声调度的定量收敛率。在低秩高斯混合模型等非凸设置下也能建立全局收敛保证。

Conclusion: 该理论框架为基于扩散模型的逆问题求解提供了严格的理论基础，特别是在非凸模型集上也能保证全局收敛性。
...
TL;DR: 提出了MDBench，一个用于评估动态系统模型发现方法的开源基准测试框架，评估了12种算法在14个偏微分方程和63个常微分方程上的表现，发现线性方法和遗传编程方法分别在PDE和ODE上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对动态模型发现的全面基准测试，现有研究主要关注单一方程的识别，需要系统性的评估框架来跟踪进展和理解方法间的权衡。

Method: 开发了MDBench框架，在14个PDE和63个ODE上测试12种算法，考虑不同噪声水平，评估指标包括导数预测精度、模型复杂度和方程保真度。

Result: 线性方法在PDE上获得最低预测误差，遗传编程方法在ODE上表现最佳；线性模型对噪声更具鲁棒性；发现了当前方法在流体动力学和热力学PDE系统上的关键局限性。
...
TL;DR: 本文分析了神经概率电路（NPC）的对抗鲁棒性，并提出了一种新的鲁棒神经概率电路（RNPC），通过类级集成推理方法显著提升了对抗攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: NPC模型虽然具有可解释性和高性能，但其基于神经网络的属性识别模块容易受到对抗攻击的影响，这会危及整个系统的预测可靠性。

Method: 提出了RNPC方法，采用新颖的类级集成推理机制，确保两个模块输出的鲁棒组合。理论分析证明RNPC比NPC具有更好的对抗鲁棒性。

Result: 在图像分类任务上的实验结果表明，RNPC在保持良性输入高准确率的同时，相比现有概念瓶颈模型获得了更优越的对抗鲁棒性。

Conclusion: RNPC是首个针对识别模块对抗攻击的鲁棒神经概率电路，通过理论分析和实证验证了其有效性，为可解释AI系统的安全性提供了重要保障。
...
Method: 构建两种混合分类器：XGBoost+随机森林和SVM+逻辑回归。使用泄漏安全的标准化流程（编码、插补、缩放；仅在训练折叠上使用SMOTE；SVM概率校准），在主数据集上训练并冻结流程，然后在PIMA外部队列上进行验证。
...
TL;DR: 提出PIRF方法，将物理约束生成建模为稀疏奖励优化问题，通过轨迹级奖励直接反向传播梯度，避免传统DPS方法的价值函数近似误差


<details>
  <summary>Details</summary>
Motivation: 扩散模型在科学领域生成能力强，但常违反物理定律。现有方法依赖DPS式价值函数近似，存在误差且导致训练不稳定和推理效率低
...
TL;DR: 该论文通过全局敏感性分析研究贝叶斯神经网络超参数对预测准确性和不确定性量化的影响，发现超参数之间存在复杂交互作用，建议使用全局敏感性分析或贝叶斯优化来改进BNNs在实际应用中的超参数选择。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯神经网络虽然能提供不确定性量化，但在实践中由于近似训练方法和超参数选择困难，难以获得准确的不确定性量化。超参数数量多且影响不透明，需要系统研究其影响。

Method: 采用全局敏感性分析方法，在不同超参数设置下系统评估贝叶斯神经网络的性能表现，分析超参数对预测准确性和不确定性量化的影响。

Result: 研究发现许多超参数之间存在交互作用，共同影响预测准确性和不确定性量化效果。超参数的选择对BNNs性能有显著影响。
...
<details>
  <summary>Details</summary>
Motivation: 尽管FMM在物理和工程中广泛应用，但其与现代机器学习架构的结合仍未被充分探索。作者旨在将FMM的分层计算流程整合到神经网络中，以提高学习格林算子的效率。

Method: 利用FMM方法的分层计算流程，将局部和远场相互作用分开，并有效学习它们各自的表示。通过分层机器学习框架实现这一目标。

Result: 提出了Neural FMM架构，该架构能够有效处理N体问题中的长程力计算，特别是在引力和静电场中。

Conclusion: Neural FMM为将传统数值方法（如FMM）与现代机器学习相结合提供了有前景的途径，有望在物理和工程领域的相关应用中提高计算效率。
...
TL;DR: 提出了一种基于可解释机器学习技术的视频流QoE建模新方法，使用Kolmogorov-Arnold Networks在频域特征上进行透明建模


<details>
  <summary>Details</summary>
Motivation: 传统黑盒方法无法捕捉视频流服务中不同特征与用户体验之间的复杂关系，需要开发既能保持高精度又具有透明可解释性的QoE模型

Method: 在原始时间序列数据上应用可解释ML技术，结合Kolmogorov-Arnold Networks作为可解释读出层，构建在紧凑频域特征之上，以捕捉时序信息

Result: 在流行数据集上评估表明，该方法在QoE预测方面具有更高的准确性，同时保持透明性和可解释性
...
TL;DR: 本文提出了一种新型稳定的近似可逆Runge-Kutta方法（EES方案），用于解决神经SDE求解器在反向传播中的内存效率和梯度精度问题。


<details>
  <summary>Details</summary>
Motivation: 传统神经SDE求解器的反向传播方法存在两难选择：离散化后优化方法内存成本高，而优化后离散化方法梯度近似误差大。现有的代数可逆求解器虽然承诺内存效率和梯度精度，但在复杂模型和大步长下往往不稳定。

Method: 开发了显式且有效对称（EES）的Runge-Kutta方案，这些方案保持了可逆求解器的优点，同时克服了其不稳定性，能够在不受步长或模型复杂度严重限制的情况下实现内存高效训练。

Result: 数值实验表明，EES方案具有优越的稳定性和可靠性，为神经SDE的可扩展和精确训练提供了实用基础。

Conclusion: EES方案成功解决了神经SDE训练中的内存效率与梯度精度权衡问题，为复杂模型和大步长情况下的稳定训练提供了有效解决方案。
...
TL;DR: 函数编码器是一种学习神经网络基函数的技术，通过定义学习特征映射的内积来建立与特征学习和核方法的理论联系，并开发了两种训练算法来学习紧凑基函数，在保持精度的同时显著减少基函数数量。


<details>
  <summary>Details</summary>
Motivation: 建立函数编码器与核方法的理论联系，解释其能够独立于数据集大小进行扩展并适应数据内在结构的能力，同时开发高效的训练算法来学习紧凑的基函数表示。

Method: 提出了两种训练算法：渐进式训练方法（逐步构建基函数）和训练后剪枝方法（训练后计算高效的替代方案），两种方法都利用PCA原理揭示学习空间的内在维度。

Result: 在多项式基准测试和非线性动态系统（Van der Pol振荡器和二体轨道模型）上的验证表明，使用显著更少的基函数可以达到相同的精度。

Conclusion: 这项工作为具有核级保证的神经网络预测器开辟了道路，实现了既高效又具有理论保证的可适应模型。
...
TL;DR: 本文提出了一种基于去噪扩散模型的互信息估计新方法，通过信息论框架将互信息与条件和无条件扩散的最小均方误差差距联系起来
...
Method: 利用去噪扩散模型的信息论公式，将互信息表示为条件和无条件扩散的最小均方误差差距的一半，在所有信噪比上进行积分，并采用自适应重要性采样实现可扩展估计
...
TL;DR: SIL-C是一个新框架，通过双边惰性学习映射技术确保技能与策略的兼容性，使得增量学习的技能改进能够直接提升下游策略性能而无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 技能增量学习过程中，技能库的演化会破坏与现有技能策略的兼容性，限制了技能的重用性和泛化能力。

Method: 采用双边惰性学习映射技术，动态对齐策略引用的子任务空间与技能解码的行为空间，基于轨迹分布相似性为每个子任务选择合适技能。

Result: 在多种SIL场景下评估表明，SIL-C能够在学习过程中保持演化技能与下游策略的兼容性并确保效率。

Conclusion: SIL-C框架成功解决了技能增量学习中的兼容性问题，为高效获取可重用技能的分层策略提供了有效解决方案。
...
TL;DR: Latent Twins提出了一种统一的数学框架，通过在潜在空间中创建底层方程的隐藏代理模型，将表示学习和算法求解方法统一起来。


<details>
  <summary>Details</summary>
Motivation: 科学机器学习在过去十年中取得了显著进展，但表示学习和算法求解方法往往平行发展，缺乏统一的框架。作者希望建立一个能够连接数据驱动的表示学习和传统科学建模的桥梁。

Method: Latent Twins框架在潜在空间中创建数学系统的隐藏代理模型，由算子控制。该框架将经典建模、反演、模型约简和算子逼近都视为单一原理的特殊情况。

Result: 研究建立了Latent Twins对ODE和PDE的基本逼近性质，并在三个代表性场景中进行了验证：典型ODE系统、浅水方程PDE基准测试以及真实地理位势再分析数据集。

Conclusion: Latent Twins提供了一个紧凑、可解释的代理模型，能够在单次评估中跨越任意时间间隔，同时与同化、控制和不确定性量化等科学流程保持兼容，为跨学科的科学建模提供了可扩展的理论基础。
...
TL;DR: 本文提出了一种将多轮任务规划转化为单轮任务推理的新方法，使用GRPO进行高效策略优化，在复杂任务规划基准上取得了优于更大基线模型的性能。
...
Result: 1.5B参数模型在超过30步的长视野规划任务中达到70%成功率，优于14B参数的基线模型。
...
TL;DR: PFedDL是一个新的联邦学习框架，用于解决多站点fMRI研究中的数据隐私和站点异质性挑战，通过分解站点特定字典为共享全局组件和个性化本地组件来提高模型泛化能力。
...
Method: PFedDL在每个站点进行独立的字典学习，将每个站点特定字典分解为共享全局组件和个性化本地组件，通过联邦聚合更新全局原子以促进跨站点一致性，同时独立优化本地原子以捕获站点特异性变异。

Result: 在ABIDE数据集上的实验表明，PFedDL在非独立同分布数据集上的准确性和鲁棒性优于现有方法。

Conclusion: PFedDL框架有效解决了多站点神经影像分析中的数据隐私和异质性挑战，提高了下游分析的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有好奇心机制在稀疏奖励的多智能体强化学习中存在问题：将环境随机性误认为有意义的新颖性，且忽视了同伴行为中包含的任务动态信息，导致次优探索。

Method: 受人类儿童通过观察同伴校准探索行为的启发，CERMIC框架让智能体能够过滤噪声惊喜信号，通过推断多智能体上下文来动态校准内在好奇心，并生成理论上有依据的内在奖励来鼓励高信息增益的状态转换探索。
...
TL;DR: 本文开发了基于机器学习的策略来预测大规模并行化学计算所需的资源成本，帮助用户在超级计算机上运行昂贵实验前做出决策。


<details>
  <summary>Details</summary>
Motivation: 为了指导用户在超级计算机上运行昂贵的化学计算实验前，能够预测所需的资源成本，避免资源浪费。

Method: 使用梯度提升（GB）机器学习模型和主动学习策略，基于CCSD应用在DOE Frontier和Aurora超级计算机上的运行时参数值集合进行预测。

Result: 在预测CCSD迭代总执行时间时，GB模型在Aurora和Frontier上的平均绝对百分比误差分别为0.023和0.073；主动学习策略仅需约450个实验就能达到约0.2的MAPE。

Conclusion: 机器学习方法能够有效预测化学计算资源需求，主动学习策略在数据收集成本高的情况下尤其有用。
...
TL;DR: 本文建立了上下文学习稳定性的非渐近下界，提出了一个两阶段可观测估计器，通过单次校准生成实际可用的提示长度估计，无需分布先验。
...
Method: 建立非渐近下界连接最小演示数量与ICL稳定性，提出两阶段可观测估计器进行单次校准。

Result: 实验显示预测阈值与经验拐点高度一致，理论作为保守但可靠的上界，校准版本进一步缩小差距。

Conclusion: 结果将谱覆盖与稳定ICL联系起来，弥合理论与部署之间的鸿沟，提高了有限样本机制下大规模提示的可解释性和可靠性。
...
TL;DR: 本文提出了Bispectral Optimal Transport（双谱最优传输），一种对称感知的离散最优传输扩展方法，通过使用双谱表示来比较元素，在保持信号结构的同时去除群作用引起的变异。


<details>
  <summary>Details</summary>
Motivation: 在对称性丰富的场景中，仅基于原始特征间几何距离的最优传输对齐可能忽略数据的内在相干结构。传统OT方法无法有效处理由视觉对称性引起的变异。

Method: 引入双谱最优传输，使用双谱（一种群傅里叶不变量）来表示元素，该表示在去除群作用变异的同时保留所有信号结构。

Result: 实验表明，在带有视觉对称性的基准数据集上，Bispectral OT计算的传输计划比原始特征OT实现了更高的类别保持准确性，改善了捕获数据集底层语义标签结构的有意义对应关系的质量。

Conclusion: Bispectral OT能够有效去除不影响类别或内容的冗余变异，同时保持数据的语义结构，为对称性丰富的场景提供了更优的对齐方法。
...
<details>
  <summary>Details</summary>
Motivation: 组织间共享数据微调LLMs存在隐私顾虑，联邦学习被认为能保护隐私，但本文要验证其实际隐私保护效果。

Method: 通过大量实验验证联邦学习框架下LLMs的隐私泄露风险，并开发针对FL的增强攻击策略，同时评估差分隐私等防护措施。

Result: 发现即使使用简单生成方法也能从全局模型提取训练数据，模型规模越大泄露越严重，增强攻击策略进一步加剧隐私泄露。

Conclusion: 联邦学习在LLMs微调中仍存在显著隐私风险，需要采用差分隐私等防护措施来降低风险。
...
TL;DR: FIRM-DTI是一个轻量级药物-靶点结合亲和力预测框架，通过FiLM层实现分子嵌入在蛋白质嵌入上的条件调节，并使用三元组损失强制度量结构，在DTI-DG基准上达到最先进性能。
...
Method: 使用特征级线性调制(FiLM)层将分子嵌入条件化到蛋白质嵌入上，通过三元组损失强制度量结构，采用RBF回归头在嵌入距离上产生平滑可解释的亲和力预测。

Result: 尽管模型规模适中，FIRM-DTI在Therapeutics Data Commons DTI-DG基准上实现了最先进的性能，并通过消融研究和域外评估得到验证。

Conclusion: 结果表明条件调节和度量学习对于稳健的药物-靶点亲和力预测具有重要价值。
...
TL;DR: 该论文提出了一种名为CE-GPPO的新算法，通过保留被裁剪标记的梯度信号来更好地控制策略熵，从而在强化学习训练中实现探索与利用的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法如PPO由于裁剪机制会丢弃低概率标记的梯度信号，而这些标记在调节熵动态中起着关键作用，需要一种能够保留这些梯度的方法。

Method: 提出CE-GPPO算法，以温和且有界的方式重新引入被裁剪标记的梯度，通过控制裁剪区间外标记的梯度大小来实现探索-利用权衡。

Result: 在数学推理基准测试中，CE-GPPO在不同模型规模下均优于强基线方法，有效缓解了熵不稳定性。

Conclusion: CE-GPPO通过梯度保留机制成功解决了PPO中梯度信号丢失的问题，为LLM的强化学习优化提供了更有效的训练方法。
...
TL;DR: SynGA是一种直接在合成路径上操作的遗传算法，通过定制交叉和变异操作符约束分子设计的可合成性，并可与机器学习过滤器结合提升性能。
...
Method: 使用定制交叉和变异操作符的遗传算法，直接操作合成路径；结合机器学习过滤器聚焦构建块集；开发SynGBO变体，在贝叶斯优化内循环中使用SynGA和块过滤。

Result: 在多种设计任务中表现出有效性，包括可合成类似物搜索和样本效率属性优化；与块过滤结合后达到最先进性能。

Conclusion: SynGA作为轻量级且确保可合成性的算法，不仅可作为强基准，还可作为未来合成感知工作流中的多功能模块。
...
TL;DR: 本文揭示了深度学习中的缩放定律本质上是冗余定律，证明了缩放指数取决于数据冗余度而非普适常数


<details>
  <summary>Details</summary>
Motivation: 理解缩放定律的数学起源，特别是缩放指数的形成机制，这一直是深度学习理论中的未解之谜

Method: 使用核回归方法，分析数据协方差谱的多项式尾部，推导出超额风险的幂律表达式，并在多种场景下验证其普适性

Result: 得到了缩放指数的精确表达式α=2s/(2s+1/β)，其中β控制谱尾部，1/β衡量冗余度，表明学习曲线的斜率依赖于数据冗余

Conclusion: 首次为缩放定律提供了严格的数学解释，将其统一为有限样本冗余定律，连接了经验观察与理论基础
...
Method: 构建了Watermark-Spoofing数据集，应用多种手工和神经网络水印方法到现有反欺骗数据集上，并提出知识保持水印学习（KPWL）框架。
...
TL;DR: 本文对Transformer-based表格数据合成工具进行超参数敏感性评估，比较GReaT和REaLTabFormer在不同数据集上的运行时间、机器学习效用和数据相似性表现。


<details>
  <summary>Details</summary>
Motivation: Transformer-based模型在表格数据合成质量上优于GAN和Diffusion模型，但计算成本高。本研究旨在评估超参数选择对合成数据质量和计算性能的影响，为资源有限的用户提供实用指导。

Method: 在两个工具(GReaT和REaLTabFormer)上评估10种不同架构类型和深度的模型配置，在四个真实数据集上从运行时间、机器学习效用和数据分布相似性三个维度进行敏感性分析。

Result: 运行时间与超参数数量成正比；GReaT运行时间更短；小数据集上两个工具都能生成高质量数据，但大数据集上只有REaLTabFormer保持强效用和相似性；REaLTabFormer与轻量级LLM结合提供最佳平衡。

Conclusion: REaLTabFormer与轻量级LLM的组合在保持数据质量的同时降低了计算需求，但运行时间仍高于GReaT和其他TDS工具，表明效率提升存在上限。
...
<details>
  <summary>Details</summary>
Motivation: 传统学习索引在静态数据集上效率很高，但在动态更新时性能下降，因为维护CDF不变性需要全局模型重训练，这会阻塞查询并限制QPS。现有方法未能有效解决这些重训练成本问题。

Method: 1) sigmoid增强近似技术：通过局部sigmoid函数近似数据分布变化，延迟完全重训练；2) 基于高斯混合模型的主动更新训练：识别高更新概率区域进行战略占位符分配；3) 神经联合优化框架：通过梯度学习持续优化sigmoid集成和GMM参数。
...
TL;DR: 提出了一种结合MLP和CNN的时间序列预测模型，MLP用于捕捉长期趋势，CNN（特别是新颖的IConv架构）用于建模局部变化模式，解决了MLP在处理多样化分布通道时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列数据具有非平稳性，现有MLP模型虽然能有效捕捉长期依赖关系，但其线性特性在处理具有多样化分布的通道时存在局限，无法充分建模局部变化如季节性模式和残差分量。

Method: 结合MLP和CNN：MLP建模整体趋势以考虑长期依赖；CNN使用多样化核函数建模细粒度局部模式。提出IConv架构，独立处理时间依赖通道，通过不同层考虑通道间关系，支持大核尺寸并降低计算成本。

Result: 在时间序列数据集上的广泛实验表明，该方法在多元时间序列预测方面具有优越性。

Conclusion: MLP与CNN的结合能够有效解决时间序列预测中长期趋势和局部变化的建模问题，IConv架构通过独立通道处理和通道间关系考虑，实现了高效且准确的预测性能。
...
TL;DR: LiLAW是一种轻量级自适应加权方法，通过三个可学习参数动态调整每个训练样本的损失权重，根据样本难度（简单、中等、困难）进行优先级排序，无需过多超参数调优或干净验证集。
...
Method: 使用三个可学习参数动态调整样本损失权重，在每个训练小批量后通过验证集上的单步梯度下降更新权重，无需干净验证集。

Result: 在多个通用和医学影像数据集、不同噪声水平和类型、损失函数及架构上的实验表明，LiLAW能持续提升性能，即使在强噪声环境下也有效。

Conclusion: LiLAW提供了一种计算高效的方法，可在任何神经网络训练设置中增强模型的泛化能力和鲁棒性，且不依赖数据增强或复杂正则化。
...
<details>
  <summary>Details</summary>
Motivation: 大规模模型的成功依赖于缩放定律，但高质量数据的有限性带来了挑战。现有基础序列模型如状态空间模型使用固定的归纳偏置，当任务结构与模型偏置不匹配时会导致样本效率低下。

Method: 首先形式化线性时不变SSMs的归纳偏置，提出任务依赖初始化(TDI)方法，通过功率谱匹配在训练前将模型偏置与任务光谱特征对齐。

Result: 在多样化真实世界基准测试中，TDI显著提高了泛化能力和样本效率，特别是在低数据情况下效果更明显。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据自由鲁棒性蒸馏方法忽视了鲁棒公平性问题，导致不同类别间的鲁棒性差异严重。研究发现两个关键问题：学生模型在不同类别间表现差异显著，且鲁棒性在不同攻击目标下不稳定。

Method: FERD采用鲁棒性引导的类别重加权策略调整对抗样本比例，为鲁棒性较弱的类别合成更多样本；通过公平感知样本和均匀目标对抗样本调整分布，抑制类别特定非鲁棒特征的支配，实现更平衡的表示。
...
TL;DR: T2I-Diff是一个基于时间频率表示和分类器自由去噪扩散的fMRI生成框架，通过将BOLD信号转换为窗口谱图来捕捉时间动态和频谱演化，从而解决传统生成模型在合成fMRI数据时忽略复杂非平稳性和非线性BOLD动态的问题。


<details>
  <summary>Details</summary>
Motivation: fMRI数据采集资源密集，限制了数据驱动脑分析模型所需的高保真样本可用性。现有生成模型在合成fMRI数据时表现不佳，因为它们忽视了BOLD信号的复杂非平稳性和非线性动态。

Method: 1. 通过时间相关傅里叶变换将BOLD信号转换为窗口谱图；2. 训练分类器自由扩散模型生成类条件频率谱图；3. 通过逆傅里叶变换将谱图还原为BOLD信号。

Result: 该方法在下游基于fMRI的脑网络分类任务中表现出更高的准确性和更好的泛化能力。

Conclusion: T2I-Diff框架通过结合时间频率表示和扩散模型，有效提升了fMRI数据的生成质量，为数据驱动的脑分析提供了更可靠的合成数据源。
...
TL;DR: CaTS-Bench是第一个大规模、真实世界的时间序列字幕基准，包含465k训练和105k测试时间戳，整合了数值序列、元数据、线图图像和字幕，并提供了460个多选问题用于深度时间序列推理。
...
Method: 从11个多样化数据集构建基准，采用可扩展的流水线生成参考字幕：大部分由oracle LLM生成并通过事实检查、人类不可区分性研究和多样性分析验证，同时提供579个人工修订的测试字幕。

Result: 提出了新的定制评估指标，并对领先的VLMs进行基准测试，揭示了它们的优势和持续局限性。
...
TL;DR: 本文通过神经坍缩理论为深度神经网络训练中的后期现象（如grokking和信息瓶颈）提供了统一解释，揭示了类内方差收缩是这些现象的关键机制。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络训练动态中存在令人困惑的后期现象，如grokking（测试性能在训练损失稳定后突然提升）和信息瓶颈（模型逐步丢弃与预测任务无关的输入信息），但这些现象的机制及其相互关系尚不明确。

Method: 通过神经坍缩的视角分析学习表示的几何特性，研究类内方差收缩的动态过程，并将其与训练集上的神经坍缩度量相关联，分析拟合训练集与神经坍缩进展的不同时间尺度。

Result: 发现类内方差收缩是grokking和信息瓶颈现象的关键因素，不同时间尺度的动态解释了后期现象的行为特征，并在多个数据集和架构上验证了理论发现。

Conclusion: 神经坍缩理论为深度神经网络训练中的后期现象提供了统一的解释框架，类内方差收缩的动态过程是理解这些现象的核心机制。
...
TL;DR: 本文提出了一种两阶段训练框架来解决多模态融合中的模态竞争问题，通过单模态训练来塑造初始状态，并引入有效竞争强度(ECS)概念和可计算诊断指标来指导训练过程。


<details>
  <summary>Details</summary>
Motivation: 多模态融合在联合训练时经常出现模态竞争问题，其中一个模态主导学习过程，导致其他模态优化不足。现有方法大多在联合学习阶段解决此问题，而忽视了模型初始状态的关键影响。

Method: 提出两阶段训练框架：1）单模态训练阶段塑造初始状态；2）引入ECS概念量化模态竞争强度；3）开发可计算诊断指标FastPID进行部分信息分解；4）异步训练控制器动态平衡模态并定位理想初始状态。

Result: 在多个基准测试上的实验表明，该方法实现了最先进的性能，证明了塑造预融合模型初始状态是缓解模态竞争的有效策略。

Conclusion: 通过单模态训练塑造初始状态是一种强大的策略，能够在竞争开始前缓解模态竞争，可靠地实现协同多模态融合。
...
TL;DR: MOIRA是一种多组学数据集成方法，能够处理缺失模态的数据，通过表示对齐和自适应聚合实现稳健学习。


<details>
  <summary>Details</summary>
Motivation: 多组学数据能揭示复杂的生物分子相互作用，但数据模态缺失会阻碍异质组学数据的整合分析。

Method: MOIRA采用早期集成方法，将每个组学数据集投影到共享嵌入空间，使用可学习的加权机制进行融合，充分利用包括缺失模态在内的所有样本。

Result: 在ROSMAP阿尔茨海默病数据集上的评估显示，MOIRA优于现有方法，特征重要性分析揭示了与文献一致的AD相关生物标志物。

Conclusion: MOIRA方法具有生物学相关性，能够有效处理多组学数据中的模态缺失问题。
...
Method: 开发了基于扩散模型的CaTSG框架，采用后门调整引导机制，通过后门调整和溯因-行动-预测程序推导因果得分函数。

Result: 在合成和真实数据集上的实验表明，CaTSG在保持观测保真度的同时，能够实现现有基线无法处理的干预和反事实生成。

Conclusion: 提出了因果时间序列生成任务家族，并通过CaTSG提供了初步概念验证，为更可靠的干预模拟和反事实生成开辟了有前景的方向。
...
TL;DR: 提出基于掩码变换器自编码器的方法来重建缺失的胎儿心率信号，解决可穿戴设备监测中信号丢失问题


<details>
  <summary>Details</summary>
Motivation: 约10%的新生儿需要呼吸辅助，5%需要通气支持。可穿戴胎儿心率监测设备在孕妇活动时容易出现信号丢失，传统插值方法无法保持信号频谱特性

Method: 使用掩码变换器自编码器方法，同时捕捉数据的空间和频率成分，实现对缺失FHR信号的重建

Result: 该方法对不同持续时间的缺失数据表现出鲁棒性，可用于信号修复和预测

Conclusion: 该方法可应用于研究数据集支持AI风险算法开发，未来可集成到可穿戴FHR监测设备中实现更早更稳健的风险检测
...
TL;DR: Federated Markov Imputation (FMI) 是一种隐私保护的联邦学习方法，用于解决电子健康记录中时间序列数据缺失的问题，特别是在不同ICU时间粒度不一致的情况下。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的缺失数据是联邦学习面临的主要挑战，尤其是在不同医疗机构收集时间序列数据的时态粒度不一致时。

Method: 提出Federated Markov Imputation (FMI)方法，允许ICU协作构建全局转移模型进行时间插补，同时保护隐私。

Result: 在MIMIC-IV数据集上的脓毒症发作预测任务中，FMI表现优于本地插补基线方法，特别是在ICU采样间隔不规则的场景下。

Conclusion: FMI方法能有效解决联邦学习中时间序列数据缺失问题，在不同时间粒度的ICU数据上表现出优越性能。
...
TL;DR: StyleBench是一个系统性评估不同推理风格在多样化任务和模型上表现的基准测试，分析了5种推理风格在5个推理任务和15个开源模型上的表现，发现没有单一最优推理风格，策略效果高度依赖于模型规模和任务类型。


<details>
  <summary>Details</summary>
Motivation: 当前对大型语言模型中推理策略、模型架构和任务类型之间相互作用的理解不足，需要系统性地评估不同推理风格在不同模型和任务上的表现。

Method: 引入StyleBench基准，评估5种代表性推理风格（CoT、ToT、AoT、SoT、CoD）在5个推理任务上的表现，使用15个开源模型（270M-120B参数），涵盖主要模型家族。

Result: 搜索类方法（AoT、ToT）在开放性问题中表现优异但需要大规模模型，简洁风格（SoT、CoD）在定义明确任务上实现显著效率提升；小模型常无法遵循输出指令而默认猜测，推理鲁棒性随规模增加而提升。

Conclusion: 推理策略的选择应基于具体约束条件，没有通用最优策略；开源了基准测试以促进进一步研究。
...
TL;DR: 该论文提出了一种处理随机传感器延迟的模型强化学习方法，通过基于模型的滤波过程来更新信念状态，使智能体能够有效处理观察延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现实环境中经常存在延迟，但标准强化学习算法通常假设环境感知是即时的。论文研究了POMDP中的随机传感器延迟问题，这种延迟导致观察可能乱序到达，这在RL中尚未得到充分研究。

Method: 提出了一种基于模型的滤波过程，顺序更新基于传入观察流的信念状态。然后将这一思想整合到模型强化学习中，形成了一个简单的延迟感知框架。将该框架应用于Dreamer算法。

Result: 与为MDP开发的延迟感知基线方法相比，该方法始终表现更优，并在部署期间对延迟分布变化表现出鲁棒性。在模拟机器人任务上的实验也验证了方法的有效性。

Conclusion: 明确建模观察延迟对于处理现实环境中的延迟问题至关重要，提出的延迟感知框架能够有效提升智能体在存在随机传感器延迟环境中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端数据不平衡会降低共享模型性能，现有研究主要通过客户端选择方法来缓解这一问题。

Method: 扩展现有FL策略，选择能够使当前标签分布与目标分布（平衡分布或联邦组合标签分布）最佳对齐的活跃客户端。

Result: 实验结果表明，面对局部不平衡时，与平衡分布对齐效果最好；面对全局不平衡时，与联邦组合标签分布对齐更优。

Conclusion: 提出的分布控制客户端选择方法能有效改善联邦学习中的数据不平衡问题，不同目标分布适用于不同类型的失衡场景。
...
TL;DR: 提出了一种基于联邦学习的注意力增强LSTM模型，用于ICU脓毒症发病预测，支持可变预测窗口，在保护患者隐私的同时提高了早期检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决ICU脓毒症早期准确预测的挑战，传统机器学习模型受限于单个医院数据量不足，联邦学习可以在不共享数据的情况下实现跨机构协作训练。

Method: 使用联邦学习的注意力增强LSTM模型，支持可变预测窗口（短期和长期预测），在多中心ICU数据上进行训练。

Result: 联邦学习不仅提高了整体预测性能（接近集中式模型），而且特别有利于早期脓毒症发病预测，可变预测窗口在性能损失不大的情况下显著降低了计算、通信和组织开销。

Conclusion: 该方法在保护隐私的同时有效提高了脓毒症早期检测能力，可变预测窗口设计具有实用价值。
...
TL;DR: 提出一种基于马尔可夫链的离散状态扩散模型确定性去噪算法，通过引入具有弱混沌动力学的放牧算法变体来实现确定性离散状态转换，无需重新训练或连续状态嵌入。


<details>
  <summary>Details</summary>
Motivation: 现有的离散扩散模型使用随机去噪过程，本文旨在开发确定性反向过程来替代随机过程，提升离散扩散在生成建模中的重要性。

Method: 采用马尔可夫链框架，引入弱混沌动力学的放牧算法变体，实现确定性离散状态转换，直接替代随机去噪过程。

Result: 在文本和图像生成任务中，效率和样本质量均得到一致提升，证明确定性反向过程在离散状态空间同样有效。

Conclusion: 这种简单的确定性化方法有望增强离散扩散在生成建模中的重要性，并为离散状态空间中的确定性反向过程提供了有效验证。
...
Method: 使用ConvLSTM网络，整合四个美国城市的犯罪数据、社会人口数据和移动数据，网格化为0.077平方英里单元，预测未来12小时犯罪

Result: 移动特征提升预测性能，特别是短序列输入时。移动+社会人口特征组合效果最佳，深度学习模型在所有城市均表现最优，长序列对暴力犯罪预测更好，短序列对财产犯罪更有效

Conclusion: 整合多样化数据源对时空犯罪预测至关重要，深度学习在处理细粒度时空尺度时具有优势但存在局限
...
<details>
  <summary>Details</summary>
Motivation: 提升重型土方设备（特别是挖掘设备臂架）中线性执行器的能量效率，减少能量损失和环境影响。

Method: 使用MATLAB/Simulink进行液压回路仿真，比较传统PDCV方案与创新的PFCV方案（在作动器两端设置人工泄漏）。采用模糊控制器调谐的PID控制器实现位置控制。

Result: PFCV方案通过绕过泵的多余流量减少热量形式的能量损失，能效比传统PDCV方案提高8.5%。仿真结果与实验数据一致。

Conclusion: 提出的PFCV方法可显著提高重型土方设备线性执行器的能量效率，降低环境影响和运营成本。
...
TL;DR: GenFacts是一个基于类别判别变分自编码器的生成框架，用于为多元时间序列生成反事实解释，通过对比学习和分类一致性目标等方法提升反事实的合理性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法为多元时间序列生成的反事实解释往往无效、不合理或不直观，需要一种能够产生合理且用户可理解的反事实的方法。

Method: 基于类别判别变分自编码器的生成框架，整合对比学习和分类一致性目标、基于原型的初始化以及现实性约束优化。

Result: 在雷达手势数据和手写字母轨迹数据集上，GenFacts在合理性方面优于现有基准方法18.7%，在人类研究中获得最高可解释性评分。

Conclusion: 对于时间序列数据，反事实解释的关键在于合理性和用户中心的可解释性，而不仅仅是稀疏性。
...
TL;DR: 本文通过实验研究发现，在时间序列预测中，Transformer架构经常退化为简单的MLP，注意力机制未能按预期工作，主要原因是当前的嵌入方法无法让Transformer在结构良好的潜在空间中运作。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在NLP和CV领域表现出色，但在时间序列预测中表现不佳甚至不如简单线性基线。现有研究未能深入探索Transformer失败的原因，因此需要系统分析时间序列Transformer(TST)的失效机制。

Method: 设计了一系列渐进式实验，将Transformer逐步修改为MLP来研究注意力机制的影响；创建了可解释的数据集来研究注意力机制失效的原因；进行了理论分析探讨嵌入方法失败的根本原因。

Result: 发现Transformer块在现有时间序列Transformer中经常退化为简单MLP；注意力机制没有按预期方式工作；当前嵌入方法无法为Transformer提供结构良好的潜在空间。

Conclusion: 时间序列Transformer表现不佳的根本原因在于嵌入方法的缺陷，导致注意力机制失效，Transformer无法有效利用其核心架构优势。
...
<details>
  <summary>Details</summary>
Motivation: 标准Transformer注意力在PFNs中对高维回归任务效果有限，而高斯过程具有函数空间完全由输入核表征、预测均值是训练目标加权和的特性，这启发了DVA的设计。

Method: 引入解耦值注意力（DVA），仅从输入计算相似度，通过值传播标签，镜像高斯过程更新但保持无核特性。验证了注意力规则而非架构本身是扩展PFNs的关键因素。

Result: 局部化注意力在不同维度设置下持续降低PFNs的样本外验证损失，5维和10维情况下验证损失减少超过50%。CNN-based PFNs性能与Transformer-based相当。64维功率流方程近似达到1E-3量级的平均绝对误差，比精确GP推理快80倍以上。

Conclusion: DVA注意力机制是提升PFNs性能的关键，该框架为快速物理系统代理模型提供了有效解决方案，在保持精度的同时显著提升计算效率。
...
TL;DR: 本文揭示了流匹配框架在低噪声区域存在不稳定性问题，并提出了一种混合训练协议LCF来解决该问题


<details>
  <summary>Details</summary>
Motivation: 流匹配框架在低噪声区域存在条件数发散的问题，导致优化速度变慢和语义表示质量下降

Method: 提出局部对比流(LCF)，在低噪声水平下用对比特征对齐替代直接速度回归，在中高噪声水平下保留标准流匹配
...
TL;DR: MixGate是一个多视图布尔电路学习框架，通过功能对齐先验训练，将掩码建模从无效技术转变为性能驱动因素


<details>
  <summary>Details</summary>
Motivation: 多视图布尔电路学习具有巨大潜力，但不同视图之间的结构异质性（如AIG与XMG）阻碍了有效融合，特别是对于掩码建模等自监督技术

Method: 提出MixGate框架，基于原则性训练课程：首先通过等价对齐损失教授模型共享的功能感知表示空间，然后引入多视图掩码建模目标

Result: 广泛的实验（包括关键消融研究）表明，对齐优先策略成功地将掩码建模转化为强大的性能驱动因素

Conclusion: 功能对齐是多视图自监督学习发挥效力的必要前提条件，MixGate框架有效解决了视图间结构异质性带来的融合挑战
...
<details>
  <summary>Details</summary>
Motivation: 个性化医学需要基于患者个人基因和环境因素优化治疗方案，但无法随意测试候选治疗方案。现有替代模型难以泛化到未见过的患者-治疗组合，需要利用领域特定先验知识来评估治疗方案的适用性。
...
Conclusion: LEON提供了一种数学上严谨的方法，能够有效利用领域知识来优化个性化治疗方案。
...
TL;DR: 本文提出了CLUE框架，通过电路发现技术识别LLM中负责遗忘不良知识和保留重要技能的不同神经元，并使用CNF形式化方法进行精确神经定位，从而实现更有效的LLM遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的基于定位的LLM遗忘方法将负责遗忘和保留的神经元视为单一纠缠组，采用统一干预策略，容易导致灾难性过度遗忘或目标知识擦除不完整的问题。

Method: 提出CLUE框架：1）使用电路发现技术识别遗忘和保留电路；2）将电路转换为合取范式（CNF）；3）根据CNF可满足性解确定每个神经元应被遗忘还是保留；4）针对不同类别神经元提供有针对性的微调策略。

Result: 大量实验表明，与现有定位方法相比，CLUE通过精确的神经定位实现了更优的遗忘效果和保留效用。

Conclusion: CLUE框架能够有效解决LLM遗忘中的神经元纠缠问题，通过区分遗忘和保留电路实现更精确的干预，为LLM安全遗忘提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 图级异常检测在药物发现等领域至关重要，但高标注成本和数据不平衡限制了图神经网络的性能。
...
Conclusion: FracAug作为一个模型无关的模块，具有显著的通用性和有效性，能够显著提升各种图神经网络在异常检测任务上的性能。
...
TL;DR: LCR是一个实用的基于学习的GPU缓存框架，通过LARU算法结合机器学习预测来优化缓存效率，在准确预测时接近最优性能，在预测不准确时也能优雅地回退到接近LRU的性能。


<details>
  <summary>Details</summary>
Motivation: 现代GPU推理中缓存效率是主要瓶颈，推荐模型中的嵌入命中率和LLM中的KV缓存缺失严重影响性能。现有的启发式策略如LRU在处理结构化访问模式时表现不佳，而基于学习的方法要么在预测不准确时性能急剧下降，要么因保守设计而收益有限。

Method: LCR框架的核心算法LARU在LRU基础上融入机器学习预测，通过在线误差估计动态适应预测准确性。当预测准确时实现接近最优性能，预测不准确时优雅回退到接近LRU性能。

Result: 实验显示LCR在DLRM和LLM场景下，吞吐量提升最高达24.2%，P99 TTFT降低最高达28.3%，即使在预测不佳时性能也保持稳定。

Conclusion: LCR架起了基于学习的缓存实证进展与理论进展之间的桥梁，在保证鲁棒性和效率的同时提供性能增益。
...
TL;DR: 本文提出了一个在截断伊辛模型中基于单样本估计逆温度参数β的高效算法，该算法在近线性时间内实现一致性估计。


<details>
  <summary>Details</summary>
Motivation: 研究截断伊辛模型的参数估计问题，特别是在仅有一个样本的情况下，如何高效准确地估计逆温度参数β。

Method: 基于伪似然最大化方法，利用图结构的有限度约束和k-SAT公式表示的截断集，设计了一个在O(n)时间内运行的估计器。

Result: 提出的估计器在k ≥ log(d²k)Δ³条件下，能够实现O(Δ³/√n)的一致性估计精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在解缠LLM隐藏状态的特征时，通常依赖于单实例训练的隐式正则化，缺乏对全局稀疏性的显式保证，导致大量密集特征，损害了特征的稀疏性和原子化。

Method: 提出二元自编码器（BAE），通过步函数将隐藏激活离散化为1位，应用梯度估计实现反向传播，强制小批量隐藏激活的最小熵，促进特征独立性和跨实例稀疏性。

Result: BAE在特征集熵计算和特征解缠两个主要应用中表现优异：能可靠估计二元隐藏激活的熵以分析LLM推理动态，且在特征提取中避免密集特征，产生最多可解释特征。

Conclusion: BAE作为一种特征提取器，通过熵最小化有效提升了LLM特征解缠的稀疏性和可解释性，证实了其在特征提取方面的有效性。
...
TL;DR: 本文提出了一种基于局部唯一标识符（Local-UID）的图神经网络方法，通过d-hop唯一性着色方案解决整数线性规划（ILP）问题中标准匿名GNN表达能力有限和全局UID导致泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 标准匿名图神经网络在整数线性规划问题上表达能力有限，而常用的全局唯一标识符方法会引入虚假相关性，严重损害模型的泛化能力。需要一种既能保持表达能力又能提升泛化性能的解决方案。

Method: 提出了基于d-hop唯一性着色的局部UID方案，确保标识符仅在每个节点的d跳邻域内唯一。基于此方案开发了ColorGNN（通过颜色条件嵌入整合颜色信息）和ColorUID（轻量级特征级变体）。

Result: 实验表明，该方法在三个ILP基准测试中取得显著提升，在线性规划数据集上表现出强大的分布外泛化能力，并且与最先进方法结合后能进一步提升图级任务的性能。

Conclusion: 局部UID方案在d层网络中实现了与全局UID相同的表达能力，同时提供了更强的泛化性能，为ILP优化问题提供了一种有效的学习优化解决方案。
...
TL;DR: 本文提出将无损压缩作为时间序列模型评估的新范式，通过香农信源编码定理建立压缩长度与负对数似然的等价关系，为建模能力提供严格的信息论标准。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列模型评估主要关注预测、插补、异常检测和分类等任务特定性能，但无法严格衡量模型是否真正捕捉数据的完整生成分布。

Method: 提出TSCom-Bench评估框架，将时间序列模型作为无损压缩的骨干网络，通过标准化评估协议和指标来衡量模型的压缩性能。

Result: 在TimeXer、iTransformer、PatchTST等先进模型上的实验表明，压缩评估能够揭示传统基准测试忽略的分布建模弱点。

Conclusion: 无损压缩作为一种原则性任务，能够补充和扩展现有时间序列建模评估方法，提供更全面的模型能力评估。
...
TL;DR: 提出MAIFormer模型，一种多智能体倒置Transformer架构，用于预测多架飞机的飞行轨迹，通过两个关键注意力模块捕捉个体飞行器的时空模式和多智能体间的社会交互模式。


<details>
  <summary>Details</summary>
Motivation: 多智能体飞行轨迹预测对理解当前空中交通流至关重要，但面临两大挑战：建模个体飞行器行为随时间的变化和复杂交互关系，以及生成可解释的预测结果。

Method: MAIFormer框架包含两个关键注意力模块：(i) 掩码多变量注意力，捕捉个体飞行器的时空模式；(ii) 智能体注意力，建模复杂空中交通场景中多智能体间的社会模式。

Result: 在韩国仁川国际机场终端空域的真实ADS-B飞行轨迹数据集上评估，MAIFormer在多个指标上均取得最佳性能，优于其他方法。
...
TL;DR: ExMoIRL是一个新颖的生成框架，通过整合表型和靶点特异性线索来生成高质量候选分子，解决了当前AI驱动药物设计中表型策略和靶点策略各自的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于表型的策略实验成本高，而基于靶点的策略忽视了系统级细胞响应。需要一种能够结合两者优势的方法来改进de novo分子生成。

Method: 使用多目标强化学习框架，预训练表型引导生成器，奖励函数融合对接亲和力、药物相似性评分、排序损失、先验似然正则化和熵最大化。

Result: 实验表明ExMoIRL在多个特征良好的靶点上优于最先进的表型基和靶点基模型，生成的分子具有良好的药物特性、高靶点亲和力和对癌细胞的抑制效力。

Conclusion: 该统一框架展示了表型引导和靶点感知策略的协同潜力，为de novo药物发现提供了更有效的解决方案。
...
TL;DR: 本文通过信息移除的新视角研究上下文学习(ICL)机制，发现语言模型在零样本场景下会将查询编码为包含所有可能任务信息的非选择性表示，导致输出任意且准确率接近零。通过低秩滤波器选择性移除隐藏状态中的特定信息可以有效引导模型专注于目标任务。


<details>
  <summary>Details</summary>
Motivation: ICL作为一种新兴的少样本学习范式，其内部机制尚不明确。作者希望通过信息移除的角度来揭示ICL的工作原理。

Method: 使用低秩滤波器选择性移除隐藏状态中的冗余信息，并设计度量标准测量隐藏状态变化。识别出诱导信息移除操作的关键注意力头（去噪头），通过阻断这些头的操作进行消融实验。

Result: 发现少样本ICL有效模拟了任务导向的信息移除过程，通过选择性移除纠缠的非选择性表示中的冗余信息来改进输出。阻断去噪头的操作会显著降低ICL准确率，特别是在正确标签缺失的情况下。

Conclusion: 信息移除机制是ICL的关键机制，去噪头在实现这一机制中发挥关键作用，这为理解ICL的工作原理提供了新的理论视角。
...
TL;DR: rBridge是一种利用小模型（≤1B）预测大模型推理能力的方法，通过将预训练目标与目标任务对齐，使用前沿模型的推理轨迹作为黄金标签，显著降低了数据集排名的成本。


<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型的成本过高，需要利用小代理模型来优化数据集。但推理能力在大模型（通常超过7B参数）中才可靠地出现，这使得小模型难以预测大模型的推理表现。

Method: rBridge通过加权负对数似然与任务对齐，使用前沿模型的推理轨迹作为黄金标签，使小代理模型更紧密地对齐预训练目标和目标任务。
...
TL;DR: DELTA-Code是一个用于评估LLMs学习新推理策略能力的基准测试，通过合成编程问题家族来探究学习能力和迁移能力。研究发现RL训练模型会出现grokking相变现象，在经历长时间零奖励后突然达到近乎完美的准确率。
...
Result: RL训练模型显示出明显的grokking相变现象，在问题家族内部和重组技能方面有显著提升，但在转换性案例中仍存在持续弱点。
...
TL;DR: E-CIT是一个基于分治策略的集成条件独立性测试框架，通过将数据分区并聚合p值来显著降低计算复杂度


<details>
  <summary>Details</summary>
Motivation: 解决传统条件独立性测试在因果发现中计算成本过高的问题，特别是样本量较大时的时间复杂度瓶颈

Method: 采用分治策略：将数据划分为子集，在每个子集上应用基础CIT，然后使用基于稳定分布特性的新方法聚合p值

Result: E-CIT将基础CIT的计算复杂度降低到样本大小的线性级别，在复杂测试场景特别是真实数据集上表现优异

Conclusion: E-CIT是一个通用即插即用框架，不仅能显著降低CIT和因果发现的计算负担，还能保持竞争性性能
...
TL;DR: ACA是一种轻量级强化学习框架，消除显式行动者网络，直接从噪声级评论家的梯度场生成动作，简化训练并保持性能。


<details>
  <summary>Details</summary>
Motivation: 传统行动者-评论家方法依赖分离的网络结构，训练复杂且难以扩展；扩散模型虽然表达能力强但计算负担重。需要简化架构同时保持多模态行为表达能力。

Method: 通过消除显式行动者网络，直接从评论家的梯度场生成动作，将策略改进与评论家价值估计紧密结合，避免行动者训练的计算开销。

Result: 在标准在线RL基准测试中，ACA相比传统行动者-评论家和最先进的扩散方法，获得了更好的学习曲线和竞争性性能。

Conclusion: ACA提供了一个简单而强大的在线RL解决方案，结合了简洁性和表达能力，无需扩散模型即可捕获多样化行为。
...
TL;DR: 本文分析了多模态大语言模型（MLLMs）中视觉越狱攻击的局限性，发现现有攻击方法存在高锐度区域问题，导致跨模型迁移性差。作者提出了FORCE方法，通过纠正对层特征和频谱特征的过度依赖，提高攻击的跨模型转移能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉越狱攻击虽然能有效攻击开源MLLMs，但在跨模型迁移到闭源MLLMs时效果不佳。作者旨在解决这一迁移性问题，以更可靠地评估闭源MLLMs的安全性。

Method: 提出FORCE方法：1）分析损失景观发现攻击位于高锐度区域；2）通过中间层和频谱域特征分析揭示对窄层表示和语义贫乏频率分量的过度依赖；3）引导攻击探索更广泛的可行区域，并根据语义内容重新调整频率特征的影响。

Result: 实验表明，FORCE方法有效提高了视觉越狱攻击的跨模型迁移性，能够更可靠地对闭源MLLMs进行红队评估。

Conclusion: 通过消除对层特征和频谱特征的非泛化依赖，FORCE方法发现了更平坦的可行区域，显著提升了视觉越狱攻击的跨模型转移能力，为闭源MLLMs的安全评估提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 尽管RL微调已被证明能提升LLM能力，但其对不同LLM内部机制的具体影响尚不明确，需要系统性的分析来理解RL微调如何改变模型内部信息流。

Method: 使用边缘归因修补技术对比分析多个模型家族在RL微调前后的内部差异，重点关注PPO、GRPO和DPO等不同RL方法的对比。

Result: 在线RL训练导致两个稳健效应：激活强度整体增加和激活模式多样性提高；而DPO微调的模型表现出较弱或不一致的内部变化。

Conclusion: RL微调通过重塑信息流使其更具冗余性和灵活性来提升泛化能力，在线RL方法与基于偏好的方法在内部机制上存在显著差异。
...
TL;DR: 该论文提出了一种基于物理学最小作用量原理的学习拉格朗日方法，从第一性原理推导出经典学习算法、强化学习中的贝尔曼最优方程以及生成模型中的Adam优化器。
...
Method: 基于物理学的最小作用量原理，提出学习拉格朗日方法，将学习过程视为在拉格朗日中寻找平稳路径，并通过寻求平稳轨迹来推导学习算法。

Result: 成功从第一性原理推导出经典学习算法、贝尔曼最优方程和Adam优化器。

Conclusion: 学习过程可以统一地理解为在拉格朗日空间中寻找平稳路径，各种学习算法都可以通过这一框架从基本原理推导得出。
...
Method: 构建GeoRef基准数据集，使用结构化几何形式语言生成大规模合成训练数据，比较SFT和GRPO两种微调方法，并提出验证-再生机制提升准确性。

Result: GRPO方法显著优于SFT，能更好地将模型行为与任务特定奖励对齐。训练后的模型在下游几何推理任务上表现出可测量的改进。

Conclusion: 几何指称理解是几何问题求解的重要基础能力，当前最先进的多模态大语言模型在此任务上仍有困难，需要专门评估和加强几何基础能力。
...
TL;DR: SPREAD是一个基于去噪扩散概率模型(DDPMs)的生成式框架，用于高效解决大规模昂贵多目标优化问题，通过条件扩散过程和自适应梯度下降更新来快速收敛并保持多样性。
...
Method: SPREAD首先学习决策空间采样点的条件扩散过程，然后在每个反向扩散步骤中，通过自适应多梯度下降更新和基于高斯RBF的排斥项来细化候选解，实现快速收敛和多样性保持。

Result: 在多目标优化基准测试（包括离线和贝叶斯代理设置）上的实证结果表明，SPREAD在效率、可扩展性和帕累托前沿覆盖方面匹配或超越了领先的基线方法。

Conclusion: SPREAD框架通过结合生成式扩散模型和优化技术，为大规模多目标优化问题提供了一种有效的解决方案。
...
### [171] [ScaleDiff: Scaling Difficult Problems for Advanced Mathematical Reasoning](https://arxiv.org/abs/2509.21070)
*Qizhi Pei,Zhuoshi Pan,Honglin Lin,Xin Gao,Yu Li,Zinan Tang,Conghui He,Rui Yan,Lijun Wu*

Main category: cs.LG

TL;DR: ScaleDiff是一个简单有效的管道，用于大规模创建困难数学问题。通过自适应思维模型识别现有数据集中的难题，训练专门的难题生成器，无需复杂提示即可大规模生成新难题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在扩展数学问题生成方面面临挑战，包括高计算/API成本、复杂提示和生成问题难度有限。需要克服这些限制来扩展困难问题的创建。

Method: 1. 使用自适应思维模型单次前向传播识别现有数据集中的困难问题；2. 在筛选的困难数据上训练专门的难题生成器(DiffGen-8B)；3. 使用成本效益高的Qwen3-8B模型作为教师模型。

Result: 在ScaleDiff-Math数据集上微调Qwen2.5-Math-7B-Instruct，相比原始数据集性能提升11.3%，在AIME'24等五个困难基准测试中平均准确率达到65.9%，优于OpenThinker3等强模型。

Conclusion: ScaleDiff管道能有效转移高级推理能力，无需依赖更大更昂贵的教师模型。随着困难问题数量的增加，模型在困难基准测试上表现出明显的扩展现象。
...
TL;DR: SATMC是一个新颖的图域自适应框架，通过图结构和属性变换来顺序对齐网络间的分布，解决了传统方法在处理图结构异质性方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统图域自适应方法主要关注在原始图结构上变换节点属性并跨网络对齐变换后的节点特征分布，但这些方法往往难以处理不同图域之间的结构异质性，导致分布对齐效果不佳。

Method: SATMC框架通过图结构和属性变换顺序对齐网络分布，引入了私有域信息减少机制和经验Wasserstein距离来减轻域私有信息的负面影响并增强模型泛化能力。

Result: 在9对公开跨域数据集上的大量实验表明，SATMC在跨网络节点分类任务中优于现有最先进的方法。理论证明显示SATMC能够实现比现有图域自适应方法更紧的跨网络节点分类误差界。
...
TL;DR: TyphoonMLA是一种混合注意力计算方法，结合了naive和absorb两种核实现，在MLA架构中实现了高达3倍的注意力计算吞吐量提升。
...
TL;DR: GraphUniverse是一个生成图家族的系统框架，用于首次大规模评估归纳泛化能力，解决了现有基准局限于单图转导设置的局限性。


<details>
  <summary>Details</summary>
Motivation: 图学习中的核心挑战是理解模型如何泛化到新的未见图。现有基准仅限于单图转导设置，无法系统评估归纳泛化能力。

Method: 通过生成具有持久语义社区的图家族，确保概念一致性，同时精细控制同配性和度分布等结构属性，实现受控分布偏移下的鲁棒性测试。

Result: 对GNN、图变换器和拓扑架构的基准测试表明，强转导性能不能预测归纳泛化能力，且对分布偏移的鲁棒性高度依赖于模型架构选择和初始图机制。
...
<details>
  <summary>Details</summary>
Motivation: 在线强化学习在复杂任务中耗时较长，需要大量交互步骤来学习最优Q函数。现有的视觉语言动作策略在低层控制方面表现有限，且通常需要任务特定的专家演示进行微调。
...
TL;DR: LAVA是一种后处理、模型无关的方法，用于解释无监督黑盒模型的局部嵌入结构，通过分析特征相关性模式来揭示潜在空间的组织。
...
Result: 基于MNIST和单细胞肾脏数据集的UMAP嵌入，LAVA能够捕获相关的特征关联，在看似遥远的潜在空间区域之间发现视觉和生物学上相关的局部模式。
...
TL;DR: EvoMail是一个自进化的认知代理框架，用于检测垃圾邮件和网络钓鱼攻击。它通过构建异构邮件图谱，结合认知图神经网络和大型语言模型进行多模态推理，并采用红蓝对抗自进化机制来持续适应新型攻击策略。


<details>
  <summary>Details</summary>
Motivation: 现代垃圾邮件和网络钓鱼攻击已超越传统关键词黑名单和简单启发式方法，采用多模态策略并快速演变，导致传统检测系统性能迅速下降。

Method: 构建统一的异构邮件图谱融合文本内容、元数据和嵌入资源；使用认知图神经网络和LLM进行上下文感知推理；建立红蓝对抗自进化循环，红队生成新型规避策略，蓝队从失败中学习并压缩经验到记忆模块。

Result: 在真实数据集和合成对抗变体上的实验表明，EvoMail在检测精度、对演化垃圾邮件策略的适应性和推理可解释性方面均优于最先进的基线方法。

Conclusion: EvoMail作为一个弹性和可解释的防御框架，具有应对下一代垃圾邮件和网络钓鱼威胁的潜力。
...
TL;DR: 该论文研究使用稀疏主成分分析(SPCA)作为前端特征提取器来防御对抗性攻击，通过理论和实验证明SPCA比标准PCA在保持准确性的同时具有更好的鲁棒性。
...
Method: 实证比较标准PCA与其稀疏变体SPCA作为下游分类器的前端特征提取器，并进行理论分析。理论方面推导了SPCA特征的线性分类器的精确鲁棒性证书，实验方面使用小型非线性网络评估防御效果。

Result: 实验表明，在强白盒和黑盒攻击下，SPCA比PCA性能下降更平缓，同时保持竞争力的干净准确率。理论分析确认稀疏投影通过减少对抗性杠杆来提升鲁棒性。

Conclusion: 稀疏投影通过降低对抗性杠杆来增强鲁棒性，这一机制在非线性设置中仍然有效，SPCA是一种有效的对抗性防御方法。
...
### [179] [GRPO is Secretly a Process Reward Model](https://arxiv.org/abs/2509.21154)
*Michael Sullivan*

Main category: cs.LG

TL;DR: 本文证明了GRPO强化学习算法在特定假设下会诱导出非平凡的过程奖励模型（PRM），并通过实证验证这些假设在现实条件下成立。作者识别了GRPO目标的一个缺陷，提出了λ-GRPO改进算法，在保持训练成本基本不变的情况下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究GRPO算法中隐含的过程奖励模型特性，探索如何在不增加显式PRM成本的情况下提升模型性能。

Method: 首先理论证明GRPO诱导PRM的条件，然后实证验证这些条件在现实中成立。基于GRPO-as-a-PRM框架识别算法缺陷，提出λ-GRPO改进方法。

Result: λ-GRPO训练的LLM在验证准确率和下游推理任务上表现优于标准GRPO，且达到峰值性能更快。

Conclusion: 研究表明可以利用GRPO算法中隐藏的内置PRM结构来提升性能，而无需依赖成本高昂的显式定义PRM。
...
TL;DR: 本文提出了CAD-Tokenizer框架，通过多模态分词策略解决传统LLM分词器在CAD序列处理中的不足，显著提升了文本引导CAD原型设计的性能。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型的分词器将CAD序列分解为自然语言词片段，无法捕捉CAD的原始语义和几何结构，限制了文本引导CAD原型设计的发展。
...
Conclusion: 多模态分词策略与CAD的原始和结构特性对齐，能够为CAD数据提供更有效的表示，推动文本引导CAD原型设计的发展。
...
### [181] [Towards Foundation Models for Zero-Shot Time Series Anomaly Detection: Leveraging Synthetic Data and Relative Context Discrepancy](https://arxiv.org/abs/2509.21190)
*Tian Lan,Hao Duong Le,Jinbo Li,Wenjun He,Meng Wang,Chenghao Liu,Chen Zhang*

Main category: cs.LG

TL;DR: TimeRCD是一个基于相对上下文差异（RCD）预训练范式的创新时间序列异常检测基础模型，通过检测相邻时间窗口间的显著差异来识别异常，而非传统的重构方法，在零样本检测中表现优异。
...
Method: 提出相对上下文差异（RCD）预训练范式，训练模型检测相邻时间窗口间的显著差异；使用标准Transformer架构；构建大规模多样化合成语料库进行预训练。

Result: 在多个数据集上的广泛实验表明，TimeRCD在零样本时间序列异常检测中显著优于现有的通用和异常专用基础模型。

Conclusion: RCD范式具有优越性，为构建鲁棒且可泛化的时间序列异常检测基础模型提供了新的有效路径。
...
### [182] [Tree Search for LLM Agent Reinforcement Learning](https://arxiv.org/abs/2509.21240)
*Yuxiang Ji,Ziyu Ma,Yong Wang,Guanhua Chen,Xiangxiang Chu,Liaoni Wu*

Main category: cs.LG

TL;DR: Tree-GRPO是一种基于树搜索的分组智能体强化学习方法，通过树结构轨迹和共享前缀解决长周期多轮智能体任务中的稀疏监督问题，在11个数据集和3类QA任务上优于链式RL方法。
...
Method: 提出Tree-GRPO方法，将每个树节点表示为完整的智能体交互步骤，通过共享前缀增加固定预算下的rollout数量，利用树结构轨迹构建步进过程监督信号，并在树内和树间层面估计分组相对优势。

Result: 理论分析表明树内分组相对策略优化目标等价于步级直接偏好学习目标。实验在11个数据集和3类QA任务上验证了树基RL优于链式RL方法。

Conclusion: Tree-GRPO通过树搜索和分组相对优势估计有效解决了长周期智能体任务中的稀疏监督问题，为LLM智能体能力提升提供了新思路。
...
### [183] [DATS: Distance-Aware Temperature Scaling for Calibrated Class-Incremental Learning](https://arxiv.org/abs/2509.21161)
*Giuseppe Serra,Florian Buettner*

Main category: cs.LG

TL;DR: 本文提出了一种名为DATS（Distance-Aware Temperature Scaling）的新方法，用于解决持续学习中的校准问题，通过距离感知的温度缩放来适应不同任务间的差异，从而降低校准误差。


<details>
  <summary>Details</summary>
Motivation: 在持续学习场景中，现有方法主要从数据角度进行校准，使用单一温度参数，忽略了任务间的差异，导致不同任务间的校准误差波动较大。本文旨在通过任务距离感知的方式实现更精确的校准。

Method: 提出DATS方法，结合原型距离估计和距离感知校准，在没有先验任务信息的情况下推断任务接近度并分配自适应温度参数。

Result: 在标准基准和真实世界不平衡生物医学数据集上的广泛实验表明，DATS方法在减少任务间校准误差方面表现出稳定性、可靠性和一致性，优于现有最先进方法。
...
### [184] [Explaining Fine Tuned LLMs via Counterfactuals A Knowledge Graph Driven Framework](https://arxiv.org/abs/2509.21241)
...
Conclusion: 该工作为理解微调LLM的内部机制提供了新见解，并强调反事实图谱作为可解释AI的潜在工具。
...
TL;DR: MoT是一种简单的方法，通过在共享潜在空间中实现异构专家模型的潜在层协作，超越现有的路由和聚合方法，在多个基准测试中表现优异。
...
Method: 使用轻量级路由器选择top-K专家并指定主专家，在共享潜在空间中通过交互层让主专家对活跃同行进行交叉注意力计算。预训练专家保持冻结，仅训练路由器和交互层。

Result: 在5个分布内和3个分布外基准测试中，MoT分别比当前最先进的Avengers方法提升+0.38%和+2.92%，显著优于最佳单模型，且推理时间与路由基线相当。

Conclusion: MoT为组合异构LLMs提供了一种简单的潜在空间机制，是实现更广泛多LLM协作的实用步骤。
...
TL;DR: 本文提出了一个基于f-散度的统一框架，用于扩散模型的机器遗忘，相比传统的MSE方法具有更好的灵活性和性能权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型遗忘方法主要依赖均方误差（MSE），但这种方法只是f-散度的一种特例。作者希望建立一个更通用的框架，利用不同的f-散度来优化遗忘效果。

Method: 提出了基于f-散度的统一框架，将MSE方法扩展为更一般的形式，允许选择不同的f-散度来平衡遗忘强度与概念保留之间的权衡。

Result: 分析表明不同的f-散度主要影响算法的收敛性和遗忘质量，该框架为特定应用选择最优散度提供了灵活性。

Conclusion: 该统一框架为扩散模型遗忘提供了更灵活和优化的方法，能够在激进遗忘和概念保留之间实现更好的平衡。
...
TL;DR: 本文提出了一种新的逆强化学习方法，通过将最大似然解转化为线性固定点方程，将IRL简化为两个监督学习问题：概率分类估计行为策略和迭代回归求解固定点。


<details>
  <summary>Details</summary>
Motivation: 传统的最大熵IRL方法涉及复杂的内部循环优化、重复动态规划或对抗训练，这限制了现代高表达能力函数逼近器（如神经网络）的使用。本文旨在简化IRL算法，使其更易于实现和扩展。

Method: 将softmax IRL的最大似然解重新表述为涉及行为策略的线性固定点方程，然后将其分解为两个监督学习任务：1）使用概率分类估计行为策略；2）使用迭代回归求解固定点方程。

Result: 提出的方法简单模块化，适用于各种函数逼近类，提供了最优解的精确特征描述、通用算法、有限样本误差界限，并在实验中表现出与MaxEnt IRL相当或更优的性能。

Conclusion: 该方法将复杂的IRL问题转化为标准的监督学习问题，大大简化了实现过程，同时保持了理论保证和竞争性性能，为IRL的实际应用提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决过参数化线性回归中ℓp偏置插值下所有ℓr范数（r∈[1,p]）缩放规律这一基本但未解决的问题，为理解不同范数在泛化中的预测能力提供理论依据。

Method: 采用对偶射线分析技术，识别X⊤Y中信号尖峰与零坐标体之间的竞争关系，通过校准DLNs初始化尺度到有效peff(α)来建立与显式偏置的联系。

Result: 发现了数据依赖的过渡点n⋆（肘点）和普适阈值r⋆=2(p-1)，该阈值区分了饱和范数与持续增长的范数，并验证了DLNs继承相同的肘点/阈值规律。

Conclusion: 统一解决了ℓp偏置插值下所有ℓr范数的缩放问题，揭示了不同范数对泛化代理指标预测能力的敏感性差异，为理解显式与隐式偏置提供了理论桥梁。
...
TL;DR: 提出了AirPCM模型，一种集成多区域多污染物动态与显式气象-污染物因果建模的深度时空预测模型，用于解决空气污染预测中的复杂多污染物相互作用和空间异质性挑战。


<details>
  <summary>Details</summary>
Motivation: 空气污染是全球性问题，现有方法局限于单一污染物或局部区域，难以处理复杂的多污染物相互作用、变化的气象条件和区域特定的空间异质性。

Method: AirPCM采用统一架构，联合捕捉跨站点空间相关性、时间自相关性和气象-污染物动态因果关系，实现细粒度、可解释的多污染物预测。

Result: 在多尺度真实数据集上的广泛评估表明，AirPCM在预测准确性和泛化能力方面持续超越现有最先进基线方法。
...
TL;DR: 提出了Differential-Integral Neural Operator (DINO)框架，通过分解局部微分算子和全局积分算子来改进湍流长期预测，在2D Kolmogorov流基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在湍流长期自回归预测中存在灾难性误差累积和物理保真度损失问题，无法同时捕捉湍流动力学的局部耗散效应和全局非局部相互作用。

Method: DINO框架通过并行分支分别学习局部微分算子（使用约束卷积网络）和全局积分算子（使用Transformer架构），实现基于物理原理的算子分解。

Result: 在2D Kolmogorov流基准测试中，DINO显著优于最先进模型，成功抑制了数百个时间步的误差累积，在涡度场和能谱方面保持高保真度。

Conclusion: DINO为物理一致的长期湍流预测建立了新基准，展示了基于物理原理的算子分解方法在湍流建模中的有效性。
...
TL;DR: 本文综述了物理信息机器学习在预测与健康管理中的应用，重点探讨了如何通过物理知识嵌入来解决实际PHM中的挑战，包括数据噪声、标签有限和复杂退化行为等问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的PHM面临传感器数据噪声大、标签有限、退化行为复杂等挑战，需要将物理知识融入数据驱动模型以提高预测的物理一致性和可靠性。

Method: 通过物理信息建模和数据策略引入学习偏差和观测偏差：学习偏差通过物理约束损失函数、控制方程和单调性等属性嵌入模型训练；观测偏差通过虚拟传感、物理仿真数据增强和多传感器融合确保模型捕获真实系统行为。

Result: 这些方法实现了从被动预测到主动决策的转变，通过强化学习学习满足物理约束的维护策略，并探讨了从单设备扩展到全舰队部署的快速适应方法。

Conclusion: 物理信息机器学习能够有效提升PHM系统的预测准确性和决策能力，实现模型预测、仿真和实际系统操作之间的闭环，支持自适应决策制定。
...
TL;DR: GWTF是第一个支持容错的实用去中心化大语言模型训练框架，能够在异构客户端上实现高效协作训练，解决节点流失和网络不稳定问题。
...
Result: 在涉及10个不同地理位置的异构客户端节点和高节点流失率的挑战性场景中，GWTF将训练时间减少高达45%。

Conclusion: GWTF是第一个实用的去中心化LLM训练框架，能够有效应对现实世界中的节点流失和网络不稳定性问题。
...
### [193] [It's Not You, It's Clipping: A Soft Trust-Region via Probability Smoothing for LLM RL](https://arxiv.org/abs/2509.21282)
...
<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法如PPO和GRPO依赖比率裁剪来稳定更新，但裁剪会丢弃信息并引入梯度不连续性。需要一种既能防止不稳定更新又能保留更多信息的方法。

Method: PSPO方法在计算重要性比率之前，将当前策略的概率向旧策略平滑，类似于标签平滑。这种方法保留了梯度信号，同时通过向旧策略插值创建软信任区域来防止大的破坏性更新。

Result: 在Qwen2.5-0.5B和1.5B模型上的实验显示，相比裁剪的GRPO，GR-PSPO在GSM8K上的性能提升超过20%（0.5B：39.7% vs 17.6%；1.5B：59.4% vs 37.8%），并且产生更清晰、简洁和逻辑性更强的推理过程。

Conclusion: PSPO提供了一种有效的替代比率裁剪的方法，既能稳定训练又能提升模型性能，特别是在数学推理任务上表现出显著优势。
...
### [194] [AbideGym: Turning Static RL Worlds into Adaptive Challenges](https://arxiv.org/abs/2509.21234)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习智能体通常训练出脆弱的策略，在环境动态变化时会失效，而静态基准测试加剧了这个问题。需要一种能够测试智能体适应能力的动态评估框架。

Method: 开发AbideGym作为MiniGrid环境的动态包装器，包含智能体感知扰动机制和可扩展的复杂性设置，强制智能体在单个回合内进行实时适应。

Result: AbideGym能够暴露静态策略的弱点，促进智能体的鲁棒性，为课程学习、持续学习和鲁棒泛化研究提供模块化、可复现的评估框架。

Conclusion: AbideGym为解决强化学习智能体在动态环境中的脆弱性问题提供了有效的测试平台，推动了适应性智能体的研究发展。
...
### [195] [No Prior, No Leakage: Revisiting Reconstruction Attacks in Trained Neural Networks](https://arxiv.org/abs/2509.21296)
*Yehonatan Refael,Guy Smorodinsky,Ofir Lindenbaum,Itay Safran*

Main category: cs.LG

TL;DR: 该论文分析了神经网络训练数据重建攻击的固有弱点，证明在没有先验知识的情况下存在无限多远离真实训练集的替代解，使得重建从根本上不可靠。


<details>
  <summary>Details</summary>
Motivation: 当前神经网络训练数据重建攻击虽然在某些条件下有效，但其可靠性和理论基础不足，需要分析这些攻击的局限性以及失败的条件。

Method: 采用理论分析和实证验证相结合的方法，严格证明重建攻击的不可靠性，并通过实验展示精确复制训练样本仅靠偶然发生。

Result: 研究发现，更充分训练的网络（满足隐式偏差条件更强的网络）实际上更不容易受到重建攻击，这在该设置下调和了隐私保护与强泛化需求之间的矛盾。

Conclusion: 研究结果完善了对训练集泄露可能性的理论理解，并为缓解重建攻击提供了新见解，表明更强的泛化能力反而可能增强隐私保护。
...
TL;DR: 提出了联邦流匹配（FFM）框架，用于在隐私约束下训练流匹配模型，包括FFM-vanilla、FFM-LOT和FFM-GOT三种方法，在分布式数据上实现隐私保护的生成模型训练。
...
Method: 开发了三种联邦流匹配方法：FFM-vanilla（本地独立训练）、FFM-LOT（使用本地最优传输耦合）和FFM-GOT（基于半对偶最优传输的全局协调策略）。

Result: 在合成和图像数据集上的实验表明，FFM能够在保护隐私的同时提高流直度和样本质量，性能接近集中式基线。

Conclusion: FFM框架成功解决了分布式数据上的隐私保护生成模型训练问题，通过全局协调机制在保持隐私的同时实现了良好的性能。
...
TL;DR: 本文介绍了humancompatible.train，一个基于PyTorch的Python工具包，用于训练带有随机约束的深度神经网络，实现了多种随机约束随机优化算法，并通过公平性约束的深度学习任务比较了两种算法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络约束训练在公平性和安全性等应用中受到广泛关注，但目前缺乏行业标准工具包。

Method: 开发了一个易于扩展的PyTorch-based Python包，实现了多种之前未实现的随机约束随机优化算法。

Result: 通过公平性约束的深度学习任务比较了两种算法，展示了工具包的使用效果。

Conclusion: humancompatible.train为深度神经网络的约束训练提供了一个实用的工具包解决方案。
...
TL;DR: SuperOffload是一个针对Superchips（GPU-CPU异构架构）的卸载系统，通过自适应权重卸载、桶化重分区等技术，在NVIDIA GH200上实现2.5倍吞吐量提升，支持单芯片训练25B模型。


<details>
  <summary>Details</summary>
Motivation: Superchips采用紧密耦合的异构架构，但现有研究很少探索LLM训练如何受益于此新架构，需要重新审视卸载策略的假设。

Method: 提出SuperOffload系统，结合自适应权重卸载、桶化重分区、Superchip感知类型转换、推测执行等技术，以及针对Grace CPU优化的Adam优化器。

Result: 在NVIDIA GH200上实现2.5倍吞吐量提升，单芯片可训练25B模型；扩展到8个GH200时可训练13B模型（序列长度达100万token），达到55%模型FLOPs利用率。
...
<details>
  <summary>Details</summary>
Motivation: 现有鲁棒补救方法在模型更新时容易失效，且大多数方法缺乏理论最优性保证。虽然已有工作使用L^∞范数提供了可证明最优的算法，但这可能导致补救成本过高。

Method: 提出新算法，考虑使用L^p范数（p≥1但p≠∞）来定义更受限的模型变化，为广义线性模型提供可证明最优的鲁棒补救计算。

Result: 实验表明，该算法在线性和非线性模型上都能显著降低补救成本（可达几个数量级），在补救实施成本与有效性之间表现出更好的权衡，且提供更稀疏的补救方案。

Conclusion: 该方法相比现有工作具有更低的补救成本、更好的权衡性能和稀疏性，同时对保证可行性的后处理方法保持弹性。
...
### [200] [SeHDR: Single-Exposure HDR Novel View Synthesis via 3D Gaussian Bracketing](https://arxiv.org/abs/2509.20400)
*Yiyu Li,Haoyuan Wang,Ke Xu,Gerhard Petrus Hancke,Rynson W. H. Lau*

Main category: cs.GR

TL;DR: SeHDR是一种新颖的高动态范围3D高斯泼溅方法，能够从单曝光的多视角LDR图像生成HDR新视图，无需多曝光图像输入。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要从不同曝光的多视角LDR图像中生成HDR视图，这种数据采集过程繁琐且容易受到物体运动模糊和校准误差的影响。SeHDR旨在解决从单曝光图像学习HDR场景表示的病态问题。

Method: 首先从单曝光LDR输入学习基础3D高斯，然后估计具有相同几何但不同线性颜色的多个3D高斯（基于曝光操作），最后通过可微分神经曝光融合（NeEF）将基础和估计的3D高斯集成到HDR高斯中进行新视图渲染。

Result: 大量实验表明，SeHDR在性能上优于现有方法以及精心设计的基线方法。

Conclusion: SeHDR成功实现了从单曝光多视角LDR图像生成高质量HDR新视图的目标，为解决这一具有挑战性的问题提供了有效解决方案。
...
### [201] [SGAligner++: Cross-Modal Language-Aided 3D Scene Graph Alignment](https://arxiv.org/abs/2509.20401)
*Binod Singh,Sayan Deb Sarkar,Iro Armeni*

Main category: cs.GR

TL;DR: SGAligner++是一个跨模态的语言辅助3D场景图对齐框架，通过统一联合嵌入空间解决部分重叠场景观测对齐问题，在噪声现实重建中比现有方法性能提升40%
...
Conclusion: 该方法通过语言辅助的跨模态框架显著提升了3D场景图对齐的准确性和鲁棒性，适用于视觉定位、3D重建和导航等应用
...
### [202] [SceneWeaver: All-in-One 3D Scene Synthesis with an Extensible and Self-Reflective Agent](https://arxiv.org/abs/2509.20414)
*Yandan Yang,Baoxiong Jia,Shujie Zhang,Siyuan Huang*

Main category: cs.GR

TL;DR: SceneWeaver是一个反射性代理框架，通过工具化迭代优化统一了多种室内场景合成范式，解决了现有方法在物理一致性、视觉真实性和语义对齐方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI的发展，需要既视觉逼真又物理合理、功能多样的3D环境。现有方法虽然视觉保真度有所提升，但仍受限于固定场景类别，缺乏足够的物体级细节和物理一致性，难以与复杂用户指令对齐。

Method: SceneWeaver采用基于语言模型的规划器，从可扩展的场景生成工具套件中选择工具，包括数据驱动的生成模型、视觉和LLM方法，并通过自我评估物理合理性、视觉真实性和语义对齐来指导迭代优化。

Result: 在常见和开放词汇房间类型上的广泛实验表明，SceneWeaver在物理、视觉和语义指标上优于现有方法，并能有效泛化到具有复杂指令的多样化场景。

Conclusion: SceneWeaver通过闭环的推理-行动-反思设计，实现了对语义不一致性的识别、针对性工具的调用和环境迭代更新，标志着向通用3D环境生成迈出了一步。
...
### [203] [ArtUV: Artist-style UV Unwrapping](https://arxiv.org/abs/2509.20710)
*Yuguang Chen,Xinhai Liu,Yang Li,Victor Cheung,Zhuo Chen,Dongyu Zhang,Chunchao Guo*

Main category: cs.GR

TL;DR: ArtUV是一种全自动端到端的艺术家风格UV展开方法，通过两阶段流程（接缝预测和参数化）生成高质量的UV贴图，解决了传统方法在语义性、碎片化和不规则性方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有UV展开方法存在耗时、碎片化、缺乏语义性和不规则UV岛等问题，限制了实际应用。需要满足无重叠映射、最小扭曲等基本标准，同时达到清晰边界、高效空间利用和语义连贯性等高级要求。

Method: 采用两阶段方法：1）使用SeamGPT预测语义切割接缝；2）将优化方法获得的粗糙UV输入Auto-Encoder，精炼成艺术家风格的UV贴图。

Result: ArtUV在多个基准测试中表现优异，确保语义一致性和拓扑结构保持，生成的UV贴图可直接用于2D编辑。
...
### [204] [SeamCrafte: Enhancing Mesh Seam Generation for Artist UV Unwrapping via Reinforcement Learning](https://arxiv.org/abs/2509.20725)
*Duoteng Xu,Yuguang Chen,Jing Li,Xinhai Liu,Xueqi Ma,Zhuo Chen,Dongyu Zhang,Chunchao Guo*

Main category: cs.GR

TL;DR: SeamCrafter是一个基于GPT风格的自回归接缝生成器，通过双分支点云编码器分离拓扑和几何信息，并使用DPO优化方法在偏好数据集上微调，显著降低了UV失真和碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的接缝放置方法往往在高失真和过度碎片化之间权衡，无法同时解决两个问题，这阻碍了纹理合成和艺术家工作流程。

Method: 提出SeamCrafter模型，使用双分支点云编码器分离拓扑和几何特征，通过自回归方式生成接缝，并采用DPO方法在基于UV失真和碎片化评估的偏好数据集上进行微调。

Result: 实验表明SeamCrafter相比现有方法能产生显著降低失真和碎片化的接缝，同时保持拓扑一致性和视觉保真度。

Conclusion: SeamCrafter通过创新的模型架构和优化策略，有效解决了3D表面UV参数化中的接缝放置问题，为纹理映射提供了更优的解决方案。
...
### [205] [ARMesh: Autoregressive Mesh Generation via Next-Level-of-Detail Prediction](https://arxiv.org/abs/2509.20824)
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归网格生成模型按字典序逐面构建网格，无法有效捕捉与人类感知一致的底层几何结构。受2D渐进式图像生成模型启发，希望开发更自然的网格生成方式。

Method: 将网格简化为从细到粗的自然过程，推广网格到单纯复形，开发基于transformer的自回归模型来近似简化过程的逆过程。从单点开始，通过局部重网格化逐步添加几何细节，拓扑结构不预定义且可改变。

Result: 实验表明该渐进式网格生成方法不仅可以通过提前停止自回归过程来直观控制生成质量和时间消耗，还支持网格细化和编辑等应用。

Conclusion: 提出的渐进式自回归网格生成方法比传统方法更自然地捕捉几何结构，提供了更好的控制性和应用灵活性。
...
### [206] [ArchGPT: Understanding the World's Architectures with Large Multimodal Models](https://arxiv.org/abs/2509.20858)
*Yuze Wang,Luo Yang,Junyi Wang,Yue Qi*

Main category: cs.GR

TL;DR: 本文提出了ArchGPT，一个多模态建筑视觉问答模型，以及一个可扩展的数据构建管道Arch-300K，用于解决现有VR/MR/AR系统在建筑探索中缺乏可扩展性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VR/MR/AR系统通常针对特定案例开发，依赖硬编码注释和任务特定交互，无法在不同建筑环境中扩展。需要一种能够理解建筑语义并支持自然交互的通用解决方案。

Method: 1）通过多阶段流程构建Arch-300K数据集：从Wikimedia Commons筛选建筑图像，使用粗到细策略结合3D重建和语义分割；2）提出LLM引导的文本验证和知识蒸馏管道生成可靠问答对；3）在ShareGPT4V-7B上进行监督微调得到ArchGPT。

Result: 成功构建了包含约31.5万图像-问题-答案三元组的Arch-300K数据集，并训练出专门针对建筑领域的多模态VQA模型ArchGPT。

Conclusion: ArchGPT和Arch-300K为建筑领域的沉浸式探索提供了可扩展的解决方案，能够增强建筑教育、遗产保护和专业设计实践中的可访问性和理解。
...
### [207] [Marching Neurons: Accurate Surface Extraction for Neural Implicit Shapes](https://arxiv.org/abs/2509.21007)
...
<details>
  <summary>Details</summary>
Motivation: 传统隐式表示表面提取方法（如Marching Cubes）依赖空间分解和采样，由于固定和有限的分辨率导致精度不足

Method: 利用每个神经元划分域的特性，开发深度优先遍历策略来高效追踪编码表面，支持并行操作并能处理大型神经架构

Result: 生成的网格准确捕捉了网络中的完整几何信息，在不同形状和网络架构上实现了前所未有的精度，同时保持竞争性速度
...
### [208] [CHARM: Control-point-based 3D Anime Hairstyle Auto-Regressive Modeling](https://arxiv.org/abs/2509.21114)
*Yuze He,Yanning Zhou,Wang Zhao,Jingwen Ye,Yushi Bai,Kaiwen Xiao,Yong-Jin Liu,Zhongqian Sun,Wei Yang*

Main category: cs.GR

TL;DR: CHARM提出了一种基于控制点的参数化表示和自回归生成框架，专门用于动漫发型建模，解决了传统方法在处理高度风格化、分段结构几何时的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统头发建模方法主要关注真实头发，使用基于发丝或体积的表示，而动漫发型具有高度风格化、分段结构的几何特征，现有技术难以有效处理。现有方法依赖密集网格建模或手工样条曲线，编辑效率低且不适合可扩展学习。

Method: CHARM引入紧凑、可逆的控制点参数化表示，每个发片用控制点序列表示，每个点仅用五个几何参数编码。基于此表示构建自回归生成框架，将动漫发型视为"头发语言"，使用自回归transformer捕获局部几何和全局发型拓扑。
...
### [209] [An Approach to Checking Correctness for Agentic Systems](https://arxiv.org/abs/2509.20364)
*Thomas J Sheffler*

Main category: cs.AI

TL;DR: 提出一种用于监控AI智能体行为的时间表达式语言，通过监测智能体工具调用和状态转换的执行轨迹来检测与预期行为模式的偏差，解决了传统基于文本匹配的错误检测方法的脆弱性问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体系统由于随机生成过程导致输出可变性，传统错误检测方法主要依赖输入输出的文本匹配，这种方法在面对LLM响应的自然语言变异性时显得脆弱。需要一种能够独立于具体文本输出来验证系统行为的方法。

Method: 借鉴硬件验证中的时序逻辑技术，开发一种时间表达式语言来监控智能体工具调用和状态转换的执行轨迹。该方法关注智能体动作序列（如工具调用和智能体间通信），提供能够捕捉多种执行场景下正确行为模式的断言。

Result: 在三智能体系统的实验中，当使用大型模型时，所有时间断言都得到满足；但当两个智能体使用较小模型时，执行违反了行为断言，主要由于工具序列不当和协调交接失败。时间表达式成功标记了这些异常。
...
### [210] [LATTS: Locally Adaptive Test-Time Scaling](https://arxiv.org/abs/2509.20368)
*Theo Uscidda,Matthew Trager,Michael Kleinman,Aditya Chattopadhyay,Wei Xia,Stefano Soatto*

Main category: cs.AI

TL;DR: LATTS方法通过在每个生成步骤中动态调整计算资源，基于验证器模型的局部难度评估来决定是否重新采样、回溯、重启或停止生成过程，从而在保持准确性的同时优化计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的验证器方法在测试时均匀分配计算资源，不考虑不同样本和生成步骤的复杂性，导致计算资源使用效率低下。

Method: 提出局部自适应测试时缩放（LATTS）方法，在每个生成步骤使用验证器接受标准来动态决定计算资源的分配策略。

Result: 实验结果表明，LATTS相比标准验证器方法在准确性和计算效率之间取得了显著更好的平衡。

Conclusion: LATTS通过局部自适应计算分配策略，有效解决了验证器方法中计算资源浪费的问题，实现了更优的性能-计算权衡。
...
### [211] [Philosophy-informed Machine Learning](https://arxiv.org/abs/2509.20370)
*MZ Naser*

Main category: cs.AI

TL;DR: 哲学启发机器学习（PhIML）将分析哲学的核心思想直接融入机器学习模型架构、目标和评估协议中，旨在设计出尊重哲学概念和价值观的模型。


<details>
  <summary>Details</summary>
Motivation: 通过将哲学思想融入机器学习，使模型能够更好地尊重哲学概念和价值观，从而提升模型的安全性和伦理责任。

Method: 回顾概念基础以展示哲学收益和对齐，并通过案例研究展示如何将PhIML作为后验工具或内建到模型架构中。

Result: 提出了PhIML的概念框架和应用案例，展示了其在哲学对齐和模型设计中的潜力。

Conclusion: 指出了开放的技术障碍以及哲学、实践和治理挑战，并概述了实现安全、哲学意识和伦理责任的PhIML的研究路线图。
...
### [212] [InsightGUIDE: An Opinionated AI Assistant for Guided Critical Reading of Scientific Literature](https://arxiv.org/abs/2509.20493)
...
<details>
  <summary>Details</summary>
Motivation: 科学文献的激增给研究人员带来了挑战，现有工具往往提供冗长的摘要，有替代而非辅助阅读源材料的风险

Method: 系统将专家的阅读方法嵌入核心AI逻辑，采用提示驱动的方法论，提供结构化洞察作为论文关键要素的"地图"

Result: 与通用LLM相比，InsightGUIDE产生更结构化、可操作的指导，作为现代研究人员更有效的工具

Conclusion: InsightGUIDE作为一个阅读助手而非替代品，能够为研究人员提供更好的文献阅读辅助
...
### [213] [Reconstruction-Based Adaptive Scheduling Using AI Inferences in Safety-Critical Systems](https://arxiv.org/abs/2509.20513)
*Samer Alshaer,Ala Khalifeh,Roman Obermaisser*

Main category: cs.AI

TL;DR: 本文提出了一种新颖的重构框架，用于动态验证和组装时间触发系统（TTS）的调度表，解决消息碰撞、锁定循环和不完整调度等问题。


<details>
  <summary>Details</summary>
Motivation: 在动态操作环境中，自适应调度对确保时间触发系统的可靠性和安全性至关重要。现有调度框架面临消息碰撞、错误优先级处理导致的锁定循环以及生成不完整或无效调度等挑战，这些都会影响系统安全性和性能。

Method: 提出重构框架，通过系统地将AI生成或启发式得到的调度优先级转换为完全可执行的调度表，确保遵守关键系统约束（如优先级规则和无碰撞通信）。包含安全检查、高效分配算法和恢复机制来处理意外上下文事件。

Result: 在多个性能配置文件（包括最小化完工时间、工作负载平衡和能效）上的综合实验表明，该框架显著增强了系统适应性、操作完整性和运行时性能，同时保持计算效率。

Conclusion: 这项工作为安全关键TTS中的安全调度生成问题提供了实用且可扩展的解决方案，即使在高度动态和不确定的操作条件下也能实现可靠灵活实时调度。
...
### [214] [Adaptive Approach to Enhance Machine Learning Scheduling Algorithms During Runtime Using Reinforcement Learning in Metascheduling Applications](https://arxiv.org/abs/2509.20520)
*Samer Alshaer,Ala Khalifeh,Roman Obermaisser*

Main category: cs.AI

TL;DR: 提出了一个集成在元调度器中的自适应在线学习单元，通过强化学习实时探索和发现新的调度解决方案，解决传统离线训练AI调度推理时构建全面多调度图的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统离线训练方法在构建考虑所有可能场景的多调度图时面临资源密集和不可行的问题，特别是在处理硬件故障、松弛变化或模式变化等上下文事件时。

Method: 在元调度器中集成自适应在线学习单元，使用强化学习模型实时探索和发现新的调度解决方案，动态扩展多调度图。

Result: 在线学习单元能够有效处理意外事件和复杂调度场景，优化现有调度器，并在引入更严格截止时间或新性能标准时保持系统灵活性。

Conclusion: 通过实时训练持续优化AI推理，系统在大规模安全关键环境中保持灵活性和能力，确保鲁棒性和效率。
...
### [215] [A Compound Classification System Based on Fuzzy Relations Applied to the Noise-Tolerant Control of a Bionic Hand via EMG Signal Recognition](https://arxiv.org/abs/2509.20523)
*Pawel Trajdos,Marek Kurzynski*

Main category: cs.AI

TL;DR: 提出了一种用于肌电假手控制的新型识别系统，通过检测受污染的肌电信号来减轻污染对分类质量的不利影响。系统包含两个集成：用于评估单个通道污染程度的一类分类器集成和用于识别患者意图的K近邻分类器集成。


<details>
  <summary>Details</summary>
Motivation: 现代仿生上肢假肢通常使用肌电信号进行模式识别控制，但肌电信号容易受到污染，这会显著降低识别系统的分类质量。需要开发能够检测和减轻污染影响的识别系统。

Method: 开发了一个包含两个集成的模糊识别系统：一类分类器(OCC)集成用于评估单个通道的污染程度，K近邻(KNN)分类器集成用于识别患者意图。整个识别过程采用统一的模糊决策方案。

Result: 使用公共存储库中的真实肌电信号进行了实验评估，对开发方法的参数和程序进行了比较分析，并与文献中的类似系统进行了比较。

Conclusion: 提出的模糊识别系统能够有效检测肌电信号污染并减轻其对分类质量的不利影响，为肌电假肢控制提供了改进的解决方案。
...
### [216] [SAMULE: Self-Learning Agents Enhanced by Multi-level Reflection](https://arxiv.org/abs/2509.20562)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在生成有意义的反思方面存在挑战，主要由于错误分析不足和依赖罕见成功轨迹，特别是在复杂任务中。

Method: 提出SAMULE框架，通过多级反思合成训练回顾性语言模型：单轨迹学习（微观）进行详细错误修正；任务内学习（中观）构建错误分类；任务间学习（宏观）提取可转移见解。还扩展到交互设置，通过基于前瞻的反思机制实现主动反思。
...
### [217] [Adaptive Cybersecurity Architecture for Digital Product Ecosystems Using Agentic AI](https://arxiv.org/abs/2509.20640)
*Oluwakemi T. Olayinka,Sumeet Jeswani,Divine Iloh*

Main category: cs.AI

TL;DR: 提出基于智能代理AI的自适应网络安全架构，解决传统静态模型在可扩展性、实时检测和上下文响应方面的不足
...
Method: 引入具有动态学习和上下文感知决策能力的自主目标驱动代理，构建基于智能代理AI的自适应网络安全架构，整合行为基线、去中心化风险评分和联邦威胁情报共享

Result: 通过原生云模拟验证系统识别零日攻击和动态修改访问策略的能力，评估结果显示适应性增强、响应延迟降低、检测准确性提高
...
### [218] [Accelerate Creation of Product Claims Using Generative AI](https://arxiv.org/abs/2509.20652)
...
<details>
  <summary>Details</summary>
Motivation: 产品声明对消费者购买行为至关重要，但创建过程耗时耗资。需要一种高效的方法来加速声明创建流程。

Method: 使用大语言模型的上下文学习和微调技术，开发了具有三个主要功能的Claim Advisor应用：语义搜索现有声明、基于产品描述和消费者画像生成优化声明、通过合成消费者模拟对声明进行排名。

Result: 在消费品公司的应用中显示出非常有前景的结果，能够显著提高声明创建的效率和效果。
...
### [219] [An Automated Retrieval-Augmented Generation LLaMA-4 109B-based System for Evaluating Radiotherapy Treatment Plans](https://arxiv.org/abs/2509.20707)
...
<details>
  <summary>Details</summary>
Motivation: 需要开发一个能够自动评估放疗治疗计划的系统，该系统应具备协议感知能力和可解释性，以提高放疗计划评估的效率和透明度

Method: 构建多协议数据集和知识库，集成检索引擎、百分位数预测组件和临床约束检查器，通过多步提示驱动推理管道指导LLM生成评估结果

Result: 优化后的检索配置在5百分位点范围内实现完美最近邻准确率，MAE低于2点；端到端测试显示与独立模块计算结果100%一致

Conclusion: 结合结构化群体评分与模块化工具增强推理的方法可行，系统提供可追溯输出、减少幻觉，并在不同协议间表现稳健，未来需临床验证和改进检索模型
...
### [220] [Fairy: Interactive Mobile Assistant to Real-world Tasks via LMM-based Multi-agent](https://arxiv.org/abs/2509.20729)
*Jiazheng Sun,Te Yang,Jiayang Niu,Mingxuan Li,Yongyong Lu,Ruimeng Yang,Xin Peng*

Main category: cs.AI

TL;DR: Fairy是一个交互式多智能体移动助手，通过跨应用协作、交互执行和持续学习来解决现有方法在多样化应用界面和动态用户需求下的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型多模态模型在移动GUI代理中面临挑战：端到端方法在长尾应用上表现不佳，而无用户交互的代理会单方面行动损害用户体验。
...
Conclusion: Fairy通过交互和自我学习机制，在真实移动场景中显著提升了性能，证明了其设计的有效性。
...
### [221] [Parallel Thinking, Sequential Answering: Bridging NAR and AR for Efficient Reasoning](https://arxiv.org/abs/2509.20744)
...
<details>
  <summary>Details</summary>
Motivation: AR模型在推理密集型领域（如数学和代码）生成连贯输出但推理速度慢，NAR模型支持并行生成但输出质量较低。需要结合两者优势来解决推理任务中的效率和质量平衡问题。

Method: 使用NAR模型（如离散扩散模型）并行生成中间推理轨迹，然后由AR模型基于这些轨迹生成最终答案，形成AR-NAR混合推理框架。

Result: 实验显示该方法相比强基线有26%的性能提升，同时显著降低了推理成本。

Conclusion: AR-NAR混合框架有效平衡了推理质量和效率，为推理密集型任务提供了新的解决方案。
...
### [222] [Meta-Memory: Retrieving and Integrating Semantic-Spatial Memories for Robot Spatial Reasoning](https://arxiv.org/abs/2509.20754)
...
<details>
  <summary>Details</summary>
Motivation: 解决机器人在复杂环境中有效存储观察记忆并利用这些记忆回答人类关于空间位置查询的关键研究挑战，现有工作缺乏有效的记忆检索和整合机制。

Method: 使用大语言模型构建高密度环境记忆表示，通过语义和空间模态的联合推理来检索和整合相关记忆。

Result: 在SpaceLocQA和NaVQA基准测试中显著优于现有方法，并在真实机器人平台上成功部署验证了实用性。
...
### [223] [LogReasoner: Empowering LLMs with Expert-like Coarse-to-Fine Reasoning for Log Analysis Tasks](https://arxiv.org/abs/2509.20798)
*Lipeng Ma,Yixuan Li,Weidong Yang,Mingjie Zhou,Xinyi Liu,Ben Fei,Shuhao Li,Xiaoyan Sun,Sihang Jiang,Yanghua Xiao*

Main category: cs.AI

TL;DR: LogReasoner是一个粗到细的推理增强框架，旨在让LLM能够像专家一样进行日志分析推理。它通过两阶段方法增强LLM的推理能力：粗粒度的专家思维增强和细粒度的具体步骤增强。


<details>
  <summary>Details</summary>
Motivation: 通用LLM在日志分析任务中难以构建与专家认知一致的结构化推理工作流，无法提供精确的推理步骤细节。

Method: LogReasoner框架包含两个阶段：1）粗粒度专家思维增强：从故障排除流程图构建高层专家思维；2）细粒度步骤增强：通过任务特定步骤解决方案微调LLM，并使用偏好学习校准推理细节。

Result: 在四个不同日志分析任务上的实验结果表明，LogReasoner显著优于现有LLM，达到了最先进的性能。
...
### [224] [DeFacto: Counterfactual Thinking with Images for Enforcing Evidence-Grounded and Faithful Reasoning](https://arxiv.org/abs/2509.20912)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态语言模型在视觉语言推理中可能依赖无关区域得出正确答案，表明模型并未真正理解图像，推理忠实性成为关键挑战。

Method: 开发自动定位问题相关证据的流程，构建正向、反事实和随机变体数据集（约10万张图像），基于GRPO强化学习设计三种互补奖励来训练模型。

Result: 在多个基准测试中，DeFacto显著提高了答案准确性和推理忠实性。

Conclusion: DeFacto为可解释的多模态推理建立了更坚实的基础，代码和数据集已开源。
...
### [225] [GALAX: Graph-Augmented Language Model for Explainable Reinforcement-Guided Subgraph Reasoning in Precision Medicine](https://arxiv.org/abs/2509.20935)
*Heming Zhang,Di Huang,Wenyu Li,Michael Province,Yixin Chen,Philip Payne,Fuhai Li*

Main category: cs.AI

TL;DR: GALAX是一个创新框架，将预训练图神经网络集成到大型语言模型中，通过图过程奖励模型进行强化学习，实现疾病相关子图的逐步生成和解释性推理。
...
Method: 提出GALAX框架，通过图过程奖励模型(GPRM)将预训练GNN集成到LLM中，以逐步方式生成疾病相关子图，由LLM启动并由GNN迭代评估。

Result: 开发了Target-QA基准，结合CRISPR识别靶点、多组学谱和生物医学图知识，支持在文本-数值图上进行长上下文推理。
...
### [226] [Beyond Stars: Bridging the Gap Between Ratings and Review Sentiment with LLM](https://arxiv.org/abs/2509.20953)
*Najla Zuhir,Amna Mohammad Salim,Parvathy Premkumar,Moshiur Farazi*

Main category: cs.AI

TL;DR: 提出基于大语言模型的移动应用评论分析框架，解决传统星级评分系统无法捕捉文本反馈细微差别的局限性
...
Method: 采用模块化框架，利用大语言模型结合结构化提示技术，量化数值评分与文本情感之间的差异，提取特征级洞察，并通过检索增强的对话式问答支持交互式评论探索

Result: 在三个多样化数据集上的综合实验表明，该LLM驱动方法显著超越基线方法，在具有挑战性和上下文丰富的评论场景中提高了准确性、鲁棒性和可操作性洞察
...
### [227] [AOT*: Efficient Synthesis Planning via LLM-Empowered AND-OR Tree Search](https://arxiv.org/abs/2509.20988)
*Xiaozhuang Song,Xuanhao Pan,Xinjian Zhao,Hangting Ye,Shufei Zhang,Jian Tang,Tianshu Yu*

Main category: cs.AI

TL;DR: AOT*是一个将大型语言模型生成的化学合成路径与AND-OR树搜索相结合的回溯合成规划框架，显著提高了搜索效率
...
Result: 在多个合成基准测试中达到SOTA性能，比现有LLM方法减少3-5倍迭代次数，对复杂分子目标效率优势更明显

Conclusion: AOT*框架通过整合LLM和系统搜索，有效解决了回溯合成规划中的效率和可扩展性问题
...
### [228] [CORE: Full-Path Evaluation of LLM Agents Beyond Final State](https://arxiv.org/abs/2509.20998)
*Panagiotis Michelakis,Yiannis Hadjiyiannis,Dimitrios Stamoulis*

Main category: cs.AI

TL;DR: 提出了一个基于确定性有限自动机(DFA)的框架来评估AI代理通过函数调用序列解决现实世界任务的表现，并引入CORE评估套件包含五个指标来量化代理行为与预期执行模式的匹配度


<details>
  <summary>Details</summary>
Motivation: 现有代理基准通常将评估简化为对最终状态的二元判断，忽视了安全性、效率和中间正确性等关键方面，需要更全面的评估方法
...
Conclusion: 提出的DFA框架和CORE评估套件能够对AI代理行为进行原则性评估，弥补了传统评估方法的不足
...
### [229] [Who Gets Cited Most? Benchmarking Long-Context Language Models on Scientific Articles](https://arxiv.org/abs/2509.21028)
...
<details>
  <summary>Details</summary>
Motivation: 当前的长上下文基准大多使用非科学文本，专注于简单的信息检索任务或使用人工构造的上下文，无法充分评估模型在真实科学文献上的复杂推理能力。
...
Conclusion: SciTrek揭示了当前LLM在长上下文科学推理方面的局限性，为开发更强大的长上下文模型提供了有价值的基准和分析工具。
...
### [230] [CLAUSE: Agentic Neuro-Symbolic Knowledge Graph Reasoning via Dynamic Learnable Context Engineering](https://arxiv.org/abs/2509.21035)
...
Method: 采用三智能体神经符号框架（Subgraph Architect、Path Navigator、Context Curator），使用LC-MAPPO算法在资源预算下联合优化子图构建、推理路径发现和证据选择。
...
Conclusion: CLAUSE能够生成紧凑、可追溯的上下文，在部署约束下提供可预测的性能，实现了准确性、延迟和成本之间的有效权衡。
...
### [231] [Combinatorial Creativity: A New Frontier in Generalization Abilities](https://arxiv.org/abs/2509.21043)
*Samuel Schapiro,Sumuk Shashidhar,Alexi Gladstone,Jonah Black,Royce Moon,Dilek Hakkani-Tur,Lav R. Varshney*

Main category: cs.AI

TL;DR: 该论文提出了一个评估大语言模型创造力的理论框架，重点研究模型在科学创意生成中的新颖性和实用性权衡，发现即使模型规模扩大，创造力仍存在根本性限制。


<details>
  <summary>Details</summary>
Motivation: 现有框架无法评估AI系统在创造性任务（如科学创意生成）中的表现，需要建立新的评估方法来衡量开放式的组合创造力。

Method: 提出理论框架和算法任务来评估输出的新颖性和实用性程度，研究不同模型规模和架构对创造力的影响。

Result: 发现创造力存在最优模型深度和宽度；LLMs在创意生成和执行可行性之间存在差距；新颖性和实用性之间存在持久性权衡。

Conclusion: 当前形式的LLMs在长期创造力方面存在根本性限制，需要新的方法来提升AI模型的创造性能力。
...
### [232] [Disagreements in Reasoning: How a Model's Thinking Process Dictates Persuasion in Multi-Agent Systems](https://arxiv.org/abs/2509.21054)
*Haodong Zhao,Jidong Li,Zhaomin Wu,Tianjie Ju,Zhuosheng Zhang,Bingsheng He,Gongshen Liu*

Main category: cs.AI

TL;DR: 本文挑战了模型规模决定说服效力的主流假设，提出说服动态主要由模型的认知过程（特别是显式推理能力）决定，揭示了"说服二元性"：推理模型更抗说服但透明化推理过程后说服力更强


<details>
  <summary>Details</summary>
Motivation: 理解多智能体系统中LLM和LRM协作解决复杂问题时的说服动态，挑战模型规模决定说服效力的假设

Method: 通过一系列多智能体说服实验，研究推理过程对说服抵抗力和说服能力的影响，并分析多跳说服网络中的影响传播和衰减动态

Result: 发现LRM的推理过程显著增强了抗说服能力，但透明化推理内容后其说服他人能力大幅提升；在多智能体网络中观察到复杂的影响传播模式
...
### [233] [Recon-Act: A Self-Evolving Multi-Agent Browser-Use System via Web Reconnaissance, Tool Generation, and Task Execution](https://arxiv.org/abs/2509.21072)
*Kaiwen He,Zhiwei Wang,Chenyi Zhuang,Jinjie Gu*

Main category: cs.AI

TL;DR: Recon-Act是一个自进化的多智能体框架，基于侦察-行动行为范式，通过对比错误轨迹与成功轨迹来生成通用工具，提高在真实网页上多轮长视野任务中的执行效率和适应性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型在解决真实网页上的多轮长视野任务时，存在动作序列混乱和执行过程中过多试错的问题，需要一种更有效的框架来提升智能体在复杂网页任务中的表现。

Method: 采用侦察团队和行动团队的双团队架构：侦察团队进行对比分析和工具生成，行动团队负责意图分解、工具编排和执行。通过对比错误与成功轨迹，抽象出通用工具（提示或基于规则的代码）并实时注册到工具库中。

Result: 在VisualWebArena数据集上达到了最先进的性能，显著提高了对未见网站的适应性和长视野任务的可解性，目前实现了6级实施路线图中的第3级（有限的人类干预）。

Conclusion: Recon-Act通过建立数据-工具-行动-反馈的闭环训练管道，有效解决了多轮网页任务中的动作序列问题，为智能浏览器使用代理提供了可行的解决方案。
...
### [234] [TrustJudge: Inconsistencies of LLM-as-a-Judge and How to Alleviate Them](https://arxiv.org/abs/2509.21117)
...
TL;DR: 本文提出了TrustJudge框架，解决LLM作为自动评估器时的评分比较不一致性和成对传递性不一致性问题，通过分布敏感评分和似然感知聚合方法显著降低了不一致性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为评估器的方法存在严重的不一致性问题，包括评分比较不一致（低分回答在成对比较中优于高分回答）和成对传递性不一致（循环偏好链和等价矛盾），这些问题源于离散评分系统的信息丢失和模糊的平局判断。

Method: TrustJudge框架包含两个关键创新：1）分布敏感评分，通过计算离散评分概率的连续期望来保留信息熵；2）似然感知聚合，使用双向偏好概率或困惑度解决传递性违规问题。

Result: 使用Llama-3.1-70B-Instruct作为评估器的实验显示，TrustJudge将评分比较不一致性从23.32%降低到14.89%（减少8.43%），成对传递性不一致性从15.22%降低到4.40%（减少10.82%），同时保持更高的评估准确性。

Conclusion: TrustJudge是首个系统分析LLM-as-a-judge评估框架不一致性的工作，提供了理论洞见和实用解决方案，在不同模型架构和规模上都表现出一致的改进，无需额外训练或人工标注即可实现更可靠的LLM评估。
...
### [235] [Expanding Reasoning Potential in Foundation Model by Learning Diverse Chains of Thought Patterns](https://arxiv.org/abs/2509.21124)
*Xuemiao Zhang,Can Ren,Chengying Tu,Rongxiang Weng,Shuo Wang,Hongfei Yan,Jingang Wang,Xunliang Cai*

Main category: cs.AI

TL;DR: 本文提出了一种基于推理模式的高价值数据选择方法，通过识别和利用核心推理模式来提升大型推理模型的数学推理能力，仅用10B token数据就能显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法对链式思维数据的使用缺乏选择性，不清楚哪些数据类型最能有效增强模型的推理能力。本文旨在解决如何识别和利用高价值推理数据来最大化模型推理潜力的问题。

Method: 首先定义推理潜力为正确回答问题所需独立尝试次数的倒数，然后从CoT序列中抽象出具有共性和归纳能力的原子推理模式，构建核心参考集。提出双粒度算法（推理模式链和token熵）从数据池中高效选择高价值CoT数据。

Result: 仅使用10B token的CoTP数据，85A6B MoE模型在AIME 2024和2025上的性能提升9.58%，下游RL性能上限提高7.81%。

Conclusion: 通过选择性利用富含高价值推理模式的数据，可以显著提升模型的推理能力和最终性能，为高效训练大型推理模型提供了新思路。
...
### [236] [RL Squeezes, SFT Expands: A Comparative Study of Reasoning LLMs](https://arxiv.org/abs/2509.21128)
*Kohsei Matsutani,Shota Takashiro,Gouki Minegishi,Takeshi Kojima,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.AI

TL;DR: 本文提出了一个分析框架来量化推理路径，研究强化学习(RL)和监督微调(SFT)如何塑造大语言模型的推理能力。研究发现RL压缩错误轨迹而SFT扩展正确轨迹，RL集中推理功能而SFT均匀分布推理功能。


<details>
  <summary>Details</summary>
Motivation: 理解RL和SFT如何塑造大语言模型的推理能力，超越简单的准确率分析，深入探究推理过程的质变。

Method: 在数学领域使用1.5B、7B和14B参数模型，从轨迹级和步骤级两个粒度分析推理过程，包括聚类推理轨迹和分析推理图拓扑结构。

Result: RL使错误轨迹压缩约2.5倍，SFT使正确轨迹扩展；RL集中推理功能到少数步骤，SFT将推理功能均匀分布到多个步骤。

Conclusion: 两阶段训练(SFT后接RL)的成功源于互补效应：SFT扩展正确推理模式，RL压缩错误模式，这为数据构建和高效学习方法提供了实践指导。
...
### [237] [ToMPO: Training LLM Strategic Decision Making from a Multi-Agent Perspective](https://arxiv.org/abs/2509.21134)
*Yiwen Zhang,Ziang Chen,Fanqi Kong,Yizhe Huang,Xue Feng*

Main category: cs.AI

TL;DR: 本文提出了ToMPO算法，通过推理他人策略、多层级优势估计和平衡全局局部奖励，显著提升LLM在战略决策任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注社交任务中的多轮对话，忽视了不同类型决策及其相互依赖性。当前强化学习方法在训练时难以考虑他人策略。

Method: 提出ToMPO算法，包括：基于他人策略推理生成rollout、图级和样本级优势估计、平衡全局和局部奖励。

Result: ToMPO相比GRPO算法在模型输出合规性和合作结果上提升35%，相比参数大100倍的模型提升18%。

Conclusion: ToMPO算法有效增强了模型的战略决策能力。
...
### [238] [Embodied Representation Alignment with Mirror Neurons](https://arxiv.org/abs/2509.21136)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法将动作理解和动作执行视为独立任务，忽视了镜像神经元机制揭示的二者之间的内在联系。
...
### [239] [Distributed Specialization: Rare-Token Neurons in Large Language Models](https://arxiv.org/abs/2509.21163)
*Jing Liu,Haozheng Wang,Yueheng Li*

Main category: cs.AI

TL;DR: 论文研究发现大语言模型通过分布式专业化机制处理罕见词汇，而非模块化架构。该机制包含三个组织原则：三区域影响层次结构、协调激活模式、以及通过标准注意力路径的通用访问性。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型如何表示和生成罕见词汇，尽管这些词汇在专业领域中很重要，但模型对此存在困难。

Method: 通过分析多个模型家族中最后一层MLP神经元，系统研究罕见词汇处理机制，包括影响层次分析、激活模式研究和训练动态观察。

Result: 发现罕见词汇处理通过分布式专业化实现，包含三个可重现的组织原则，且功能专业化通过参数分化逐渐形成。

Conclusion: LLMs通过共享架构内的分布式协调而非专家混合式模块化来处理罕见词汇，这对可解释模型编辑、计算效率优化和Transformer网络功能组织理解具有重要意义。
...
### [240] [A Fano-Style Accuracy Upper Bound for LLM Single-Pass Reasoning in Multi-Hop QA](https://arxiv.org/abs/2509.21199)
*Kaiyang Wan,Lang Gao,Honglin Mu,Preslav Nakov,Yuxia Wang,Xiuying Chen*

Main category: cs.AI

TL;DR: 本文分析了单次推理LLM在多跳问答任务中的容量限制问题，提出了基于容量感知任务分解的多调用框架InfoQA来解决性能崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 多跳问答需要整合分散的相互依赖证据，但LLM有限的单次输出容量导致任务复杂度超过模型容量时准确性必然崩溃。

Method: 提出InfoQA框架，通过容量感知任务分解和主动剪枝先前推理痕迹来保持信息负载在单次推理限制内，采用依赖显式工作流实现精确的推理路径控制。

Result: 实验结果表明模型行为与预测的容量曲线一致，InfoQA实现了持续的性能改进。

Conclusion: 研究为LLM多步推理方法提供了理论基础和实践框架，揭示了容量感知表示和结构化的重要性。
...
### [241] [What Do LLM Agents Do When Left Alone? Evidence of Spontaneous Meta-Cognitive Patterns](https://arxiv.org/abs/2509.21224)
*Stefan Szeider*

Main category: cs.AI

TL;DR: 本文介绍了一个研究大型语言模型（LLM）智能体在无外部任务约束下行为的架构，通过持续推理和行动框架发现智能体会自发形成三种行为模式，这些模式具有模型特异性，为预测自主系统的行为提供了基准。


<details>
  <summary>Details</summary>
Motivation: 研究LLM智能体在没有外部任务约束时的自发行为，建立预测智能体在任务模糊、错误恢复或长期自主操作中行为的基准。

Method: 使用持续推理和行动框架，结合持久记忆和自我反馈机制，在6个前沿模型上进行了18次部署实验。

Result: 发现智能体自发形成三种行为模式：多周期项目系统生产、认知过程自我探究、自身本质递归概念化。这些行为模式具有模型特异性，不同模型在评估这些行为时表现出稳定的偏差。

Conclusion: 这是首次系统记录无提示LLM智能体行为的研究，为预测自主系统在不确定环境中的行为建立了重要基准。
...
### [242] [Grounding AI Explanations in Experience: A Reflective Cognitive Architecture for Clinical Decision Support](https://arxiv.org/abs/2509.21266)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习和大语言模型方法难以平衡预测准确性和临床可解释性，要么准确但解释不清，要么解释流畅但统计不支持。需要开发能像人类专家一样深度理解数据的模型。
...
Conclusion: RCA展示了创建真正可信赖的临床决策支持系统的潜力，证明高准确性和高质量解释是相互促进的深度数据理解的结果。
...
### [243] [VC-Agent: An Interactive Agent for Customized Video Dataset Collection](https://arxiv.org/abs/2509.21291)
...
<details>
  <summary>Details</summary>
Motivation: 面对数据规模扩展的需求，从互联网收集符合特定需求的视频数据极其耗时耗力，需要加快这一收集过程。

Method: 利用多模态大语言模型连接用户需求与视频内容，定义基于文本描述和确认的用户友好交互方式，提出两种可随用户交互更新的过滤策略。

Result: 建立了个性化视频数据集收集的新基准，通过用户研究验证了代理在各种真实场景中的有效性，实验证明了其高效性。

Conclusion: VC-Agent为定制化视频数据集收集提供了一种有效的解决方案，显著提高了收集效率。
...
### [244] [SAGE: A Realistic Benchmark for Semantic Understanding](https://arxiv.org/abs/2509.21310)
*Samarth Goel,Reagan J. Lee,Kannan Ramchandran*

Main category: cs.AI

TL;DR: SAGE是一个新的语义理解评估基准，通过5个维度（人类偏好对齐、变换鲁棒性、信息敏感性、聚类性能、检索鲁棒性）在30+数据集上全面评估嵌入模型和相似度指标，揭示了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分评估语义理解的深度方面，需要更挑战性的框架来测试模型在对抗条件、噪声变换和人类判断任务中的表现。

Method: 设计了SAGE基准，包含5个评估类别，在30多个数据集上测试9种嵌入模型和传统相似度指标，涵盖对抗条件、噪声变换等复杂场景。

Result: 研究发现没有单一方法在所有维度都表现优异：OpenAI text-embedding-3-large在人类偏好对齐上最优（0.682），但传统方法如Jaccard相似度在信息敏感性上更好（0.905 vs 0.794）；OpenAI text-embedding-3-small聚类性能最佳（0.483）但鲁棒性最差（0.011）。

Conclusion: SAGE暴露了当前语义理解能力的关键局限性，为现实世界部署提供了更真实的模型鲁棒性评估，揭示了不同方法之间的重要权衡关系。
